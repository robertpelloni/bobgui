...
} BobguiFileChooserNativeAndroidData;

static void
bobgui_file_chooser_native_android_result (GdkAndroidToplevel *toplevel, GAsyncResult *res, BobguiFileChooserNative *self)
{
  ...
  if (!gdk_android_toplevel_launch_activity_for_result_finish (toplevel, res, &ret, &result, &err) ||
    ...
    {
      _bobgui_native_dialog_emit_response (BOBGUI_NATIVE_DIALOG (self), BOBGUI_RESPONSE_CANCEL);
      if (err)
        g_error_free (err);
      goto exit;
    }
  ...
  _bobgui_native_dialog_emit_response (BOBGUI_NATIVE_DIALOG (self), BOBGUI_RESPONSE_ACCEPT);
exit:
  (*env)->PopLocalFrame (env, NULL);

  g_object_unref (((BobguiFileChooserNativeAndroidData *)self->mode_data)->cancellable);
...
gboolean
bobgui_file_chooser_native_android_show (BobguiFileChooserNative *self)
{
  BobguiWindow *transient_for = bobgui_native_dialog_get_transient_for (BOBGUI_NATIVE_DIALOG (self));
  if (!transient_for)
    {
  ...
  BobguiFileChooserNativeAndroidData *data = g_new(BobguiFileChooserNativeAndroidData, 1);
  data->cancellable = g_cancellable_new();

  JNIEnv *env = gdk_android_get_env ();
  (*env)->PushLocalFrame (env, 8);

  BobguiFileChooserAction action = bobgui_file_chooser_get_action ((BobguiFileChooser *)self);
  jstring jaction;
  switch (action)
    {
  ...
  (*env)->CallObjectMethod (env, intent,
                            ...
                            bobgui_file_chooser_get_select_multiple ((BobguiFileChooser *)self->dialog));

  if (action != BOBGUI_FILE_CHOOSER_ACTION_SELECT_FOLDER)
    {
      GListModel *filters = bobgui_file_chooser_get_filters ((BobguiFileChooser *)self);
          ...
          for (guint i = 0; i < n_filters; i++)
            {
              ...
              _bobgui_file_filter_store_types_in_list (filter, list);
  ...
  GdkAndroidToplevel* parent = (GdkAndroidToplevel *)bobgui_native_get_surface (BOBGUI_NATIVE (transient_for));
  gdk_android_toplevel_launch_activity_for_result_async (parent,
                                                         intent,
                                                         data->cancellable,
                                                         (GAsyncReadyCallback)bobgui_file_chooser_native_android_result,
                                                         g_object_ref (self));
  self->mode_data = data;
...
void
bobgui_file_chooser_native_android_hide (BobguiFileChooserNative *self)
{
  BobguiFileChooserNativeAndroidData *data = self->mode_data;
