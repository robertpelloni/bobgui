...
void
bobgui_print_win32_devnames_free (BobguiPrintWin32Devnames *devnames)
{
...
BobguiPrintWin32Devnames *
bobgui_print_win32_devnames_from_win32 (HGLOBAL global)
{
  LPDEVNAMES win = GlobalLock (global);
  gunichar2 *data = (gunichar2 *)win;
  BobguiPrintWin32Devnames *devnames = g_new (BobguiPrintWin32Devnames, 1);
...
HGLOBAL 
bobgui_print_win32_devnames_to_win32 (const BobguiPrintWin32Devnames *devnames)
{
  HGLOBAL global;
...
HGLOBAL 
bobgui_print_win32_devnames_to_win32_from_printer_name (const char *printer_name)
{
  HGLOBAL global;
  BobguiPrintWin32Devnames *devnames;

  devnames = bobgui_print_win32_devnames_from_printer_name(printer_name);
  if (devnames)
    {
      global = bobgui_print_win32_devnames_to_win32 (devnames);
 ...
 */
BobguiPrintWin32Devnames *
bobgui_print_win32_devnames_from_printer_name (const char *printer_name)
{
  HANDLE hprinter;
  ...
  if (OpenPrinterW (win32_printer_name, &hprinter, NULL))
    {
      ...
      devnames = g_new (BobguiPrintWin32Devnames, 1);
