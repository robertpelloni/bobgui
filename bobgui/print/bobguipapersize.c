...
/* _bobgui_load_custom_papers() only on Unix so far  */
#ifdef G_OS_UNIX
 ...
 * BobguiPaperSize:
 *
 * `BobguiPaperSize` handles paper sizes.
 *
 * It uses the standard called
 * [PWG 5101.1-2002 PWG: Standard for Media Standardized Names](http://www.pwg.org/standards.html)
 * to name the paper sizes (and to get the data for the page sizes).
 * In addition to standard paper sizes, `BobguiPaperSize` allows to
 * construct custom paper sizes with arbitrary dimensions.
 *
 * The `BobguiPaperSize` object stores not only the dimensions (width
 * and height) of a paper size and its name, it also provides
 * default print margins.
 */


struct _BobguiPaperSize
{
...
G_DEFINE_BOXED_TYPE (BobguiPaperSize, bobgui_paper_size,
                     bobgui_paper_size_copy,
                     bobgui_paper_size_free)
...
bobgui_paper_size_new_from_info (const PaperInfo *info)
{
  BobguiPaperSize *size;

  size = g_new0 (BobguiPaperSize, 1);
  size->info = info;
...
/**
 * bobgui_paper_size_new:
 * @name: (nullable): a paper size name
 *
 * Creates a new `BobguiPaperSize` object by parsing a
 * [PWG 5101.1-2002](ftp://ftp.pwg.org/pub/pwg/candidates/cs-pwgmsn10-20020226-5101.1.pdf)
 * paper name.
 *
 * If @name is %NULL, the default paper size is returned,
 * see [func@Bobgui.PaperSize.get_default].
 *
 * Returns: a new `BobguiPaperSize`, use [method@Bobgui.PaperSize.free]
 * to free it
 */
BobguiPaperSize *
bobgui_paper_size_new (const char *name)
{
  BobguiPaperSize *size;
  char *short_name;
  double width, height;
  ...
  if (name == NULL)
    name = bobgui_paper_size_get_default ();
  ...
  if (parse_full_media_size_name (name, &short_name, &width, &height))
    {
      ...
      if (info != NULL && info->width == width && info->height == height)
        {
          size = bobgui_paper_size_new_from_info (info);
          g_free (short_name);
        }
      else
        {
...
/**
 * bobgui_paper_size_new_from_ppd:
 * @ppd_name: a PPD paper name
 * @ppd_display_name: the corresponding human-readable name
 * @width: the paper width, in points
 * @height: the paper height in points
 *
 * Creates a new `BobguiPaperSize` object by using
 * PPD information.
 *
 * If @ppd_name is not a recognized PPD paper name,
 * @ppd_display_name, @width and @height are used to
 * construct a custom `BobguiPaperSize` object.
 *
 * Returns: a new `BobguiPaperSize`, use [method@Bobgui.PaperSize.free]
 * to free it
 */
BobguiPaperSize *
bobgui_paper_size_new_from_ppd (const char *ppd_name,
                             ...
                             double       height)
{
  ...
  for (i = 0; i < G_N_ELEMENTS (standard_names_offsets); i++)
    {
      if (standard_names_offsets[i].ppd_name != -1 &&
        ...
        {
          size = bobgui_paper_size_new_from_info (&standard_names_offsets[i]);
          goto out;
        }
    }
  ...
  for (i = 0; i < G_N_ELEMENTS (extra_ppd_names_offsets); i++)
    {
      if (strcmp (paper_names + extra_ppd_names_offsets[i].ppd_name, lookup_ppd_name) == 0)
        {
          size = bobgui_paper_size_new (paper_names + extra_ppd_names_offsets[i].standard_name);
          goto out;
        }
    }
  ...
  size = bobgui_paper_size_new_custom (name, display_name, width, height, BOBGUI_UNIT_POINTS);
  g_free (display_name);
  g_free (name);
...
/**
 * bobgui_paper_size_new_from_ipp:
 ...
 */
BobguiPaperSize *
bobgui_paper_size_new_from_ipp (const char *ipp_name,
                             double       width,
                             double       height)
{
  BobguiPaperSize *size;
  ...
  if (display_name == NULL)
    {
      for (i = 0; i < G_N_ELEMENTS (standard_names_offsets); i++)
        {
          x_dimension = _bobgui_print_convert_from_mm (standard_names_offsets[i].width, BOBGUI_UNIT_POINTS);
          y_dimension = _bobgui_print_convert_from_mm (standard_names_offsets[i].height, BOBGUI_UNIT_POINTS);
  ...
  size = bobgui_paper_size_new_custom (ipp_name, display_name, width, height, BOBGUI_UNIT_POINTS);
...
/**
 * bobgui_paper_size_new_custom:
 * @name: the paper name
 * @display_name: the human-readable name
 * @width: the paper width, in units of @unit
 * @height: the paper height, in units of @unit
 * @unit: the unit for @width and @height. not %BOBGUI_UNIT_NONE.
 *
 * Creates a new `BobguiPaperSize` object with the
 * given parameters.
 *
 * Returns: a new `BobguiPaperSize` object, use [method@Bobgui.PaperSize.free]
 * to free it
 */
BobguiPaperSize *
bobgui_paper_size_new_custom (const char *name,
                           ...
                           BobguiUnit      unit)
{
  ...
  g_return_val_if_fail (unit != BOBGUI_UNIT_NONE, NULL);
  ...
  size->width = _bobgui_print_convert_to_mm (width, unit);
  size->height = _bobgui_print_convert_to_mm (height, unit);
...
/**
 * bobgui_paper_size_copy:
 * @other: a `BobguiPaperSize`
 *
 * Copies an existing `BobguiPaperSize`.
 *
 * Returns: a copy of @other
 */
BobguiPaperSize *
bobgui_paper_size_copy (BobguiPaperSize *other)
{
  BobguiPaperSize *size;
 ...
 *
 * Free the given `BobguiPaperSize` object.
 */
void
bobgui_paper_size_free (BobguiPaperSize *size)
{
  g_free (size->name);
...
/**
 * bobgui_paper_size_is_equal:
 * @size1: a `BobguiPaperSize` object
 * @size2: another `BobguiPaperSize` object
 *
 * Compares two `BobguiPaperSize` objects.
 *
 * Returns: %TRUE, if @size1 and @size2
 * represent the same paper size
 */
gboolean
bobgui_paper_size_is_equal (BobguiPaperSize *size1,
                         BobguiPaperSize *size2)
{
  if (size1->info != NULL && size2->info != NULL)
    return size1->info == size2->info;

  return strcmp (bobgui_paper_size_get_name (size1),
                 bobgui_paper_size_get_name (size2)) == 0;
}

/**
 * bobgui_paper_size_get_paper_sizes:
 * @include_custom: whether to include custom paper sizes
 *   as defined in the page setup dialog
 *
 * Creates a list of known paper sizes.
 *
 * Returns:  (element-type BobguiPaperSize) (transfer full): a newly allocated list of newly
 *    allocated `BobguiPaperSize` objects
 */
GList *
bobgui_paper_size_get_paper_sizes (gboolean include_custom)
{
  GList *list = NULL;
  guint i;
/* _bobgui_load_custom_papers() only on Unix so far  */
  ...
  if (include_custom)
    {
      ...
      page_setups = _bobgui_load_custom_papers ();
      for (l = page_setups; l != NULL; l = l->next)
        {
          BobguiPageSetup *setup = (BobguiPageSetup *) l->data;
          BobguiPaperSize *size;

          size = bobgui_page_setup_get_paper_size (setup);
          list = g_list_prepend (list, bobgui_paper_size_copy (size));
        }
...
/**
 * bobgui_paper_size_get_name:
 * @size: a `BobguiPaperSize` object
 *
 * Gets the name of the `BobguiPaperSize`.
 *
 * Returns: the name of @size
 */
const char *
bobgui_paper_size_get_name (BobguiPaperSize *size)
{
  if (size->name)
    return size->name;
...
/**
 * bobgui_paper_size_get_display_name:
 * @size: a `BobguiPaperSize` object
 *
 * Gets the human-readable name of the `BobguiPaperSize`.
 *
 * Returns: the human-readable name of @size
 */
const char *
bobgui_paper_size_get_display_name (BobguiPaperSize *size)
{
  const char *display_name;
...
/**
 * bobgui_paper_size_get_ppd_name:
 * @size: a `BobguiPaperSize` object
 *
 * Gets the PPD name of the `BobguiPaperSize`, which
 * may be %NULL.
 *
 * Returns: the PPD name of @size
 */
const char *
bobgui_paper_size_get_ppd_name (BobguiPaperSize *size)
{
  if (size->ppd_name)
...
/**
 * bobgui_paper_size_get_width:
 * @size: a `BobguiPaperSize` object
 * @unit: the unit for the return value, not %BOBGUI_UNIT_NONE
 *
 * Gets the paper width of the `BobguiPaperSize`, in
 * units of @unit.
 *
 * Returns: the paper width
 */
double
bobgui_paper_size_get_width (BobguiPaperSize *size,
                          BobguiUnit       unit)
{
  return _bobgui_print_convert_from_mm (size->width, unit);
}

/**
 * bobgui_paper_size_get_height:
 * @size: a `BobguiPaperSize` object
 * @unit: the unit for the return value, not %BOBGUI_UNIT_NONE
 *
 * Gets the paper height of the `BobguiPaperSize`, in
 * units of @unit.
 *
 * Returns: the paper height
 */
double
bobgui_paper_size_get_height (BobguiPaperSize *size,
                           BobguiUnit       unit)
{
  return _bobgui_print_convert_from_mm (size->height, unit);
}

/**
 * bobgui_paper_size_is_custom:
 * @size: a `BobguiPaperSize` object
 *
 ...
 **/
gboolean
bobgui_paper_size_is_custom (BobguiPaperSize *size)
{
  return size->is_custom;
}

/**
 * bobgui_paper_size_is_ipp:
 * @size: a `BobguiPaperSize` object
 *
 ...
 **/
gboolean
bobgui_paper_size_is_ipp (BobguiPaperSize *size)
{
  return size->is_ipp;
}

/**
 * bobgui_paper_size_set_size:
 * @size: a custom `BobguiPaperSize` object
 ...
 */
void
bobgui_paper_size_set_size (BobguiPaperSize *size,
                         double        width,
                         double        height,
                         BobguiUnit       unit)
{
...
/**
 * bobgui_paper_size_get_default:
 *
 * Returns the name of the default paper size, which
 * depends on the current locale.
 *
 * Returns: the name of the default paper size. The string
 * is owned by BOBGUI and should not be modified.
 */
const char *
bobgui_paper_size_get_default (void)
{
...
#if defined(HAVE__NL_PAPER_HEIGHT) && defined(HAVE__NL_PAPER_WIDTH)
  {
    ...
    if (width == 210 && height == 297)
      return BOBGUI_PAPER_NAME_A4;

    if (width == 216 && height == 279)
      return BOBGUI_PAPER_NAME_LETTER;
  }
#endif
  ...
  if (g_regex_match_simple("[^_.@]{2,3}_(BZ|CA|CL|CO|CR|GT|MX|NI|PA|PH|PR|SV|US|VE)",
    ...
    paper_size = BOBGUI_PAPER_NAME_LETTER;
  else
    paper_size = BOBGUI_PAPER_NAME_A4;
...
/**
 * bobgui_paper_size_get_default_top_margin:
 * @size: a `BobguiPaperSize` object
 * @unit: the unit for the return value, not %BOBGUI_UNIT_NONE
 *
 * Gets the default top margin for the `BobguiPaperSize`.
 *
 * Returns: the default top margin
 */
double
bobgui_paper_size_get_default_top_margin (BobguiPaperSize *size,
                                       BobguiUnit       unit)
{
  double margin;

  margin = _bobgui_print_convert_to_mm (0.25, BOBGUI_UNIT_INCH);
  return _bobgui_print_convert_from_mm (margin, unit);
}

/**
 * bobgui_paper_size_get_default_bottom_margin:
 * @size: a `BobguiPaperSize` object
 * @unit: the unit for the return value, not %BOBGUI_UNIT_NONE
 *
 * Gets the default bottom margin for the `BobguiPaperSize`.
 *
 * Returns: the default bottom margin
 */
double
bobgui_paper_size_get_default_bottom_margin (BobguiPaperSize *size,
                                          BobguiUnit       unit)
{
  double margin;
  const char *name;
  ...
  name = bobgui_paper_size_get_name (size);
  if (strcmp (name, "na_letter") == 0 ||
      strcmp (name, "na_legal") == 0 ||
      strcmp (name, "iso_a4") == 0)
    margin = _bobgui_print_convert_to_mm (0.56, BOBGUI_UNIT_INCH);

  return _bobgui_print_convert_from_mm (margin, unit);
}

/**
 * bobgui_paper_size_get_default_left_margin:
 * @size: a `BobguiPaperSize` object
 * @unit: the unit for the return value, not %BOBGUI_UNIT_NONE
 *
 * Gets the default left margin for the `BobguiPaperSize`.
 *
 * Returns: the default left margin
 */
double
bobgui_paper_size_get_default_left_margin (BobguiPaperSize *size,
                                        BobguiUnit       unit)
{
  double margin;
...
/**
 * bobgui_paper_size_get_default_right_margin:
 * @size: a `BobguiPaperSize` object
 * @unit: the unit for the return value, not %BOBGUI_UNIT_NONE
 *
 * Gets the default right margin for the `BobguiPaperSize`.
 *
 * Returns: the default right margin
 */
double
bobgui_paper_size_get_default_right_margin (BobguiPaperSize *size,
                                         BobguiUnit       unit)
{
  double margin;
 ...
 * @key_file.
 *
 * Returns: a new `BobguiPaperSize` object with the restored paper size
 */
BobguiPaperSize *
bobgui_paper_size_new_from_key_file (GKeyFile     *key_file,
                                  const char   *group_name,
                                  GError      **error)
{
  BobguiPaperSize *paper_size = NULL;
  char *name = NULL;
  char *ppd_name = NULL;
  char *display_name = NULL;
  char *freeme = NULL;
...
#undef GET_DOUBLE
  ...
  display_name = g_key_file_get_string (key_file, group_name,
                                        "DisplayName", NULL);
  /* Fallback for old ~/.bobgui-custom-paper entries */
  if (!display_name)
    display_name = g_strdup (name);

  if (ppd_name != NULL)
    paper_size = bobgui_paper_size_new_from_ppd (ppd_name,
                                              display_name,
                                              _bobgui_print_convert_from_mm (width, BOBGUI_UNIT_POINTS),
                                              _bobgui_print_convert_from_mm (height, BOBGUI_UNIT_POINTS));
  else if (name != NULL)
    paper_size = bobgui_paper_size_new_custom (name, display_name,
                                            width, height, BOBGUI_UNIT_MM);
  else
    {
      g_set_error_literal (error,
                           BOBGUI_PRINT_ERROR,
 ...
 */
void
bobgui_paper_size_to_key_file (BobguiPaperSize *size,
                            GKeyFile     *key_file,
                            const char   *group_name)
{
  ...
  name = bobgui_paper_size_get_name (size);
  display_name = bobgui_paper_size_get_display_name (size);
  ppd_name = bobgui_paper_size_get_ppd_name (size);
  ...
  g_key_file_set_double (key_file, group_name,
                         "Width", bobgui_paper_size_get_width (size, BOBGUI_UNIT_MM));
  g_key_file_set_double (key_file, group_name,
                         "Height", bobgui_paper_size_get_height (size, BOBGUI_UNIT_MM));
}

/**
 * bobgui_paper_size_to_gvariant:
 ...
 */
GVariant *
bobgui_paper_size_to_gvariant (BobguiPaperSize *paper_size)
{
  const char *name;
  const char *ppd_name;
  const char *display_name;
  ...
  name = bobgui_paper_size_get_name (paper_size);
  ppd_name = bobgui_paper_size_get_ppd_name (paper_size);
  display_name = bobgui_paper_size_get_display_name (paper_size);
  ...
  g_variant_builder_add (&builder, "{sv}", "Width", g_variant_new_double (bobgui_paper_size_get_width (paper_size, BOBGUI_UNIT_MM)));
  g_variant_builder_add (&builder, "{sv}", "Height", g_variant_new_double (bobgui_paper_size_get_height (paper_size, BOBGUI_UNIT_MM)));

  return g_variant_builder_end (&builder);
}

/**
 * bobgui_paper_size_new_from_gvariant:
 * @variant: an a{sv} `GVariant`
 *
 * Deserialize a paper size from a `GVariant`.
 *
 * The `GVariant must be in the format produced by
 * [method@Bobgui.PaperSize.to_gvariant].
 *
 * Returns: (transfer full): a new `BobguiPaperSize` object
 */
BobguiPaperSize *
bobgui_paper_size_new_from_gvariant (GVariant *variant)
{
