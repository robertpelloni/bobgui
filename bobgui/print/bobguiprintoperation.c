...
/**
 * BobguiPrintOperation:
 *
 * High-level, portable printing API.
 *
 * It looks a bit different than other BOBGUI dialogs such as the
 * `BobguiFileChooser`, since some platforms don’t expose enough
 * infrastructure to implement a good print dialog. On such
 * platforms, `BobguiPrintOperation` uses the native print dialog.
 * On platforms which do not provide a native print dialog, BOBGUI
 * uses its own, see [class@Bobgui.PrintUnixDialog].
 *
 * The typical way to use the high-level printing API is to create
 * a `BobguiPrintOperation` object with [ctor@Bobgui.PrintOperation.new]
 * when the user selects to print. Then you set some properties on it,
 * e.g. the page size, any [class@Bobgui.PrintSettings] from previous print
 * operations, the number of pages, the current page, etc.
 *
 * Then you start the print operation by calling [method@Bobgui.PrintOperation.run].
 * It will then show a dialog, let the user select a printer and options.
 * When the user finished the dialog, various signals will be emitted on
 * the `BobguiPrintOperation`, the main one being
 * [signal@Bobgui.PrintOperation::draw-page], which you are supposed to handle
 * and render the page on the provided [class@Bobgui.PrintContext] using Cairo.
 *
 ...
 *
 *   res = bobgui_print_operation_run (print, BOBGUI_PRINT_OPERATION_ACTION_PRINT_DIALOG,
 *                                  BOBGUI_WINDOW (main_window), NULL);
 *
 *   if (res == BOBGUI_PRINT_OPERATION_RESULT_APPLY)
 *     {
 *       if (settings != NULL)
 *         g_object_unref (settings);
 *       settings = g_object_ref (bobgui_print_operation_get_print_settings (print));
 *     }
 *
 ...
 * ```
 *
 * By default `BobguiPrintOperation` uses an external application to do
 * print preview. To implement a custom print preview, an application
 * must connect to the preview signal. The functions
 * [method@Bobgui.PrintOperationPreview.render_page],
 * [method@Bobgui.PrintOperationPreview.end_preview] and
 * [method@Bobgui.PrintOperationPreview.is_selected]
...
static void          preview_iface_init      (BobguiPrintOperationPreviewIface *iface);
static BobguiPageSetup *create_page_setup       (BobguiPrintOperation             *op);
static void          common_render_page      (BobguiPrintOperation             *op,
					      int                            page_nr);
...
G_DEFINE_TYPE_WITH_CODE (BobguiPrintOperation, bobgui_print_operation, G_TYPE_OBJECT,
                         G_ADD_PRIVATE (BobguiPrintOperation)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_PRINT_OPERATION_PREVIEW,
						preview_iface_init))

/**
 * bobgui_print_error_quark:
 *
 * Registers an error quark for `BobguiPrintOperation` if necessary.
 * 
 * Returns: The error quark used for `BobguiPrintOperation` errors.
 **/
GQuark     
bobgui_print_error_quark (void)
{
  static GQuark quark = 0;
  if (quark == 0)
    quark = g_quark_from_static_string ("bobgui-print-error-quark");
  return quark;
}
     
static void
bobgui_print_operation_finalize (GObject *object)
{
  BobguiPrintOperation *op = BOBGUI_PRINT_OPERATION (object);
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
  ...
  G_OBJECT_CLASS (bobgui_print_operation_parent_class)->finalize (object);
}

static void
bobgui_print_operation_init (BobguiPrintOperation *operation)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (operation);
  const char *appname;

  priv->status = BOBGUI_PRINT_STATUS_INITIAL;
  ...
  priv->page_drawing_state = BOBGUI_PAGE_DRAWING_STATE_READY;

  priv->rloop = NULL;
  priv->unit = BOBGUI_UNIT_NONE;
...
static void
preview_iface_render_page (BobguiPrintOperationPreview *preview,
			   int                       page_nr)
{

  BobguiPrintOperation *op;

  op = BOBGUI_PRINT_OPERATION (preview);
  common_render_page (op, page_nr);
}

static void
preview_iface_end_preview (BobguiPrintOperationPreview *preview)
{
  BobguiPrintOperation *op;
...
static gboolean
preview_iface_is_selected (BobguiPrintOperationPreview *preview,
			   int                       page_nr)
{
  BobguiPrintOperation *op = BOBGUI_PRINT_OPERATION (preview);
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
  int i;
...
static void
preview_iface_init (BobguiPrintOperationPreviewIface *iface)
{
...
static void
preview_start_page (BobguiPrintOperation *op,
...
static void
preview_end_page (BobguiPrintOperation *op,
		  BobguiPrintContext   *print_context)
{
  cairo_t *cr;
...
static void
preview_end_run (BobguiPrintOperation *op,
		 gboolean           wait,
		 gboolean           cancelled)
{
...
static void
bobgui_print_operation_set_property (GObject      *object,
				  guint         prop_id,
				  const GValue *value,
				  GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_DEFAULT_PAGE_SETUP:
      bobgui_print_operation_set_default_page_setup (op, g_value_get_object (value));
      break;
    case PROP_PRINT_SETTINGS:
      bobgui_print_operation_set_print_settings (op, g_value_get_object (value));
      break;
    case PROP_JOB_NAME:
      bobgui_print_operation_set_job_name (op, g_value_get_string (value));
      break;
    case PROP_N_PAGES:
      bobgui_print_operation_set_n_pages (op, g_value_get_int (value));
      break;
    case PROP_CURRENT_PAGE:
      bobgui_print_operation_set_current_page (op, g_value_get_int (value));
      break;
    case PROP_USE_FULL_PAGE:
      bobgui_print_operation_set_use_full_page (op, g_value_get_boolean (value));
      break;
    case PROP_TRACK_PRINT_STATUS:
      bobgui_print_operation_set_track_print_status (op, g_value_get_boolean (value));
      break;
    case PROP_UNIT:
      bobgui_print_operation_set_unit (op, g_value_get_enum (value));
      break;
    case PROP_ALLOW_ASYNC:
      bobgui_print_operation_set_allow_async (op, g_value_get_boolean (value));
      break;
    case PROP_SHOW_PROGRESS:
      bobgui_print_operation_set_show_progress (op, g_value_get_boolean (value));
      break;
    case PROP_EXPORT_FILENAME:
      bobgui_print_operation_set_export_filename (op, g_value_get_string (value));
      break;
    case PROP_CUSTOM_TAB_LABEL:
      bobgui_print_operation_set_custom_tab_label (op, g_value_get_string (value));
      break;
    case PROP_EMBED_PAGE_SETUP:
      bobgui_print_operation_set_embed_page_setup (op, g_value_get_boolean (value));
      break;
    case PROP_HAS_SELECTION:
      bobgui_print_operation_set_has_selection (op, g_value_get_boolean (value));
      break;
    case PROP_SUPPORT_SELECTION:
      bobgui_print_operation_set_support_selection (op, g_value_get_boolean (value));
      break;
    default:
...
static void
bobgui_print_operation_get_property (GObject    *object,
				  guint       prop_id,
				  GValue     *value,
				  GParamSpec *pspec)
{
...
preview_print_idle_done (gpointer data)
{
  ...
  op = BOBGUI_PRINT_OPERATION (pop->preview);

  cairo_surface_finish (pop->surface);

  if (op->priv->status == BOBGUI_PRINT_STATUS_FINISHED_ABORTED)
    {
  ...
  else
    {
      ...
      _bobgui_print_operation_platform_backend_launch_preview (op,
							    pop->surface,
							    pop->parent,
							    pop->filename);
    }
  ...
  bobgui_print_operation_preview_end_preview (pop->preview);
...
{
  PreviewOp *pop = (PreviewOp *) data;
  BobguiPrintOperation *op = BOBGUI_PRINT_OPERATION (pop->preview);
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
  gboolean done = FALSE;

  if (priv->page_drawing_state == BOBGUI_PAGE_DRAWING_STATE_READY)
    {
      if (priv->cancelled)
	{
	  done = TRUE;
          _bobgui_print_operation_set_status (op, BOBGUI_PRINT_STATUS_FINISHED_ABORTED, NULL);
	}
      ...
      else
        {
          ...
          if (!pop->pages_data->done)
            bobgui_print_operation_preview_render_page (pop->preview, pop->pages_data->page);
          else
            done = priv->page_drawing_state == BOBGUI_PAGE_DRAWING_STATE_READY;
        }
    }
...
static void
preview_got_page_size (BobguiPrintOperationPreview *preview, 
		       ...
		       PreviewOp                *pop)
{
  ...
  _bobgui_print_operation_platform_backend_resize_preview_surface (op, page_setup, pop->surface);

  cr = bobgui_print_context_get_cairo_context (pop->print_context);
  _bobgui_print_operation_platform_backend_preview_start_page (op, pop->surface, cr);

}

static void
preview_ready (BobguiPrintOperationPreview *preview,
               BobguiPrintContext          *context,
	       PreviewOp                *pop)
{
  guint id;
  ...
  g_source_set_static_name (g_main_context_find_source_by_id (NULL, id), "[bobgui] preview_print_idle");
}


static gboolean
bobgui_print_operation_preview_handler (BobguiPrintOperation        *op,
                                     BobguiPrintOperationPreview *preview, 
				     BobguiPrintContext          *context,
				     BobguiWindow                *parent)
{
  ...
  pop->pages_data->op = g_object_ref (BOBGUI_PRINT_OPERATION (preview));
  pop->pages_data->is_preview = TRUE;

  page_setup = bobgui_print_context_get_page_setup (context);

  pop->surface =
    _bobgui_print_operation_platform_backend_create_preview_surface (op,
								  page_setup,
								  &dpi_x, &dpi_y,
								  &pop->filename);
  ...
  bobgui_print_context_set_cairo_context (op->priv->print_context, cr,
				       dpi_x, dpi_y);
  cairo_destroy (cr);
...
static BobguiWidget *
bobgui_print_operation_create_custom_widget (BobguiPrintOperation *operation)
{
  return NULL;
}

static gboolean
bobgui_print_operation_paginate (BobguiPrintOperation *operation,
                              BobguiPrintContext   *context)
{
...
static void
bobgui_print_operation_done (BobguiPrintOperation       *operation,
                          BobguiPrintOperationResult  result)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (operation);
...
static void
bobgui_print_operation_class_init (BobguiPrintOperationClass *class)
{
  GObjectClass *gobject_class = (GObjectClass *)class;

  gobject_class->set_property = bobgui_print_operation_set_property;
  gobject_class->get_property = bobgui_print_operation_get_property;
  gobject_class->finalize = bobgui_print_operation_finalize;
 
  class->preview = bobgui_print_operation_preview_handler; 
  class->create_custom_widget = bobgui_print_operation_create_custom_widget;
  class->paginate = bobgui_print_operation_paginate;
  class->done = bobgui_print_operation_done;

  /**
   * BobguiPrintOperation::done:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   ...
   * If @result is %BOBGUI_PRINT_OPERATION_RESULT_ERROR then you can call
   * [method@Bobgui.PrintOperation.get_error] for more information.
   *
   * If you enabled print status tracking then
   * [method@Bobgui.PrintOperation.is_finished] may still return %FALSE
  ...
  signals[DONE] =
    g_signal_new ("done",
		  ...
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, done),
		  NULL, NULL,
		  NULL,
		  G_TYPE_NONE, 1, BOBGUI_TYPE_PRINT_OPERATION_RESULT);

  /**
   * BobguiPrintOperation::begin-print:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   * @context: the `BobguiPrintContext` for the current operation
   *
   * Emitted after the user has finished changing print settings
   * in the dialog, before the actual rendering starts.
   *
   * A typical use for ::begin-print is to use the parameters from the
   * [class@Bobgui.PrintContext] and paginate the document accordingly,
   * and then set the number of pages with
   * [method@Bobgui.PrintOperation.set_n_pages].
   */
  signals[BEGIN_PRINT] =
    g_signal_new ("begin-print",
		  G_TYPE_FROM_CLASS (gobject_class),
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, begin_print),
		  NULL, NULL,
		  NULL,
		  G_TYPE_NONE, 1, BOBGUI_TYPE_PRINT_CONTEXT);

   /**
   * BobguiPrintOperation::paginate:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   * @context: the `BobguiPrintContext` for the current operation
   *
   ...
   * [method@Bobgui.PrintOperation.set_n_pages], and return %TRUE if the document
  ...
  signals[REQUEST_PAGE_SETUP] =
    g_signal_new ("request-page-setup",
                  G_TYPE_FROM_CLASS (gobject_class),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (BobguiPrintOperationClass, request_page_setup),
                  NULL, NULL,
                  _bobgui_marshal_VOID__OBJECT_INT_OBJECT,
                  G_TYPE_NONE, 3,
  ...
  g_signal_set_va_marshaller (signals[REQUEST_PAGE_SETUP],
                              G_TYPE_FROM_CLASS (gobject_class),
                              _bobgui_marshal_VOID__OBJECT_INT_OBJECTv);

  /**
   * BobguiPrintOperation::draw-page:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   * @context: the `BobguiPrintContext` for the current operation
   * @page_nr: the number of the currently printed page (0-based)
   *
   ...
   * [method@Bobgui.PrintContext.get_cairo_context].
   *
   * ```c
   * static void
   ...
   * ```
   *
   * Use [method@Bobgui.PrintOperation.set_use_full_page] and
   * [method@Bobgui.PrintOperation.set_unit] before starting the print
  ...
  signals[DRAW_PAGE] =
    g_signal_new ("draw-page",
		  G_TYPE_FROM_CLASS (gobject_class),
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, draw_page),
		  NULL, NULL,
		  _bobgui_marshal_VOID__OBJECT_INT,
		  G_TYPE_NONE, 2,
		  BOBGUI_TYPE_PRINT_CONTEXT,
		  G_TYPE_INT);
  g_signal_set_va_marshaller (signals[DRAW_PAGE],
                              G_TYPE_FROM_CLASS (gobject_class),
                              _bobgui_marshal_VOID__OBJECT_INTv);

  /**
   * BobguiPrintOperation::end-print:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   * @context: the `BobguiPrintContext` for the current operation
   *
   ...
   * been allocated in the [signal@Bobgui.PrintOperation::begin-print] handler.
   */
  signals[END_PRINT] =
    g_signal_new ("end-print",
		  G_TYPE_FROM_CLASS (gobject_class),
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, end_print),
		  NULL, NULL,
		  NULL,
		  G_TYPE_NONE, 1, BOBGUI_TYPE_PRINT_CONTEXT);

  /**
   * BobguiPrintOperation::status-changed:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   *
   * Emitted at between the various phases of the print operation.
   *
   * See [enum@Bobgui.PrintStatus] for the phases that are being discriminated.
   * Use [method@Bobgui.PrintOperation.get_status] to find out the current
   * status.
   */
  signals[STATUS_CHANGED] =
    g_signal_new ("status-changed",
		  ...
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, status_changed),
		  NULL, NULL,
		  NULL,
		  G_TYPE_NONE, 0);
  ...
  /**
   * BobguiPrintOperation::create-custom-widget:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   *
   ...
   * to stay around until the [signal@Bobgui.PrintOperation::custom-widget-apply]
  ...
  signals[CREATE_CUSTOM_WIDGET] =
    g_signal_new ("create-custom-widget",
		  G_TYPE_FROM_CLASS (class),
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, create_custom_widget),
  ...
  /**
   * BobguiPrintOperation::update-custom-widget:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
  ...
  signals[UPDATE_CUSTOM_WIDGET] =
    g_signal_new ("update-custom-widget",
		  G_TYPE_FROM_CLASS (class),
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, update_custom_widget),
		  NULL, NULL,
		  _bobgui_marshal_VOID__OBJECT_OBJECT_OBJECT,
		  G_TYPE_NONE, 3, BOBGUI_TYPE_WIDGET, BOBGUI_TYPE_PAGE_SETUP, BOBGUI_TYPE_PRINT_SETTINGS);
  g_signal_set_va_marshaller (signals[UPDATE_CUSTOM_WIDGET],
                              G_TYPE_FROM_CLASS (gobject_class),
                              _bobgui_marshal_VOID__OBJECT_OBJECT_OBJECTv);

  /**
   * BobguiPrintOperation::custom-widget-apply:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   * @widget: the custom widget added in ::create-custom-widget
   *
  ...
  signals[CUSTOM_WIDGET_APPLY] =
    g_signal_new ("custom-widget-apply",
		  G_TYPE_FROM_CLASS (class),
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, custom_widget_apply),
		  NULL, NULL,
		  NULL,
		  G_TYPE_NONE, 1, BOBGUI_TYPE_WIDGET);

   /**
   * BobguiPrintOperation::preview:
   * @operation: the `BobguiPrintOperation` on which the signal was emitted
   * @preview: the `BobguiPrintOperationPreview` for the current operation
   * @context: the `BobguiPrintContext` that will be used
   * @parent: (nullable): the `BobguiWindow` to use as window parent
   *
   ...
   * [method@Bobgui.PrintContext.set_cairo_context].
   *
   * The custom preview implementation can use
   * [method@Bobgui.PrintOperationPreview.is_selected] and
   * [method@Bobgui.PrintOperationPreview.render_page] to find pages which
   * are selected for print and render them. The preview must be
   * finished by calling [method@Bobgui.PrintOperationPreview.end_preview]
  ...
  signals[PREVIEW] =
    g_signal_new ("preview",
		  G_TYPE_FROM_CLASS (gobject_class),
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (BobguiPrintOperationClass, preview),
		  _bobgui_boolean_handled_accumulator, NULL,
		  _bobgui_marshal_BOOLEAN__OBJECT_OBJECT_OBJECT,
		  G_TYPE_BOOLEAN, 3,
  ...
  g_signal_set_va_marshaller (signals[PREVIEW],
                              G_TYPE_FROM_CLASS (gobject_class),
                              _bobgui_marshal_BOOLEAN__OBJECT_OBJECT_OBJECTv);

  /**
   * BobguiPrintOperation:default-page-setup:
   *
   * The `BobguiPageSetup` used by default.
   *
   * This page setup will be used by [method@Bobgui.PrintOperation.run],
   * but it can be overridden on a per-page basis by connecting
   * to the [signal@Bobgui.PrintOperation::request-page-setup] signal.
   */
  ...
  /**
   * BobguiPrintOperation:print-settings:
   *
   * The `BobguiPrintSettings` used for initializing the dialog.
   *
   * Setting this property is typically used to re-establish
   * print settings from a previous print operation, see
   * [method@Bobgui.PrintOperation.run].
   */
  g_object_class_install_property (gobject_class,
				   PROP_PRINT_SETTINGS,
  ...
  /**
   * BobguiPrintOperation:job-name:
   *
   * A string used to identify the job (e.g. in monitoring
   * applications like eggcups).
   *
   * If you don't set a job name, BOBGUI picks a default one
  ...
  /**
   * BobguiPrintOperation:n-pages:
   *
   * The number of pages in the document.
   *
   * This must be set to a positive number before the rendering
   * starts. It may be set in a [signal@Bobgui.PrintOperation::begin-print]
   * signal handler.
   *
   * Note that the page numbers passed to the
   * [signal@Bobgui.PrintOperation::request-page-setup] and
   * [signal@Bobgui.PrintOperation::draw-page] signals are 0-based, i.e.
  ...
  /**
   * BobguiPrintOperation:current-page:
   *
   * The current page in the document.
   *
   * If this is set before [method@Bobgui.PrintOperation.run],
  ...
  /**
   * BobguiPrintOperation:use-full-page:
   *
   * If %TRUE, the transformation for the cairo context obtained
   * from `BobguiPrintContext` puts the origin at the top left corner
   * of the page.
   *
  ...
  /**
   * BobguiPrintOperation:track-print-status:
   *
  ...
  /**
   * BobguiPrintOperation:unit:
   *
   * The transformation for the cairo context obtained from
   * `BobguiPrintContext` is set up in such a way that distances
  ...
  /**
   * BobguiPrintOperation:show-progress:
   *
  ...
  /**
   * BobguiPrintOperation:allow-async:
   *
   * Determines whether the print operation may run asynchronously or not.
   *
   * Some systems don't support asynchronous printing, but those that do
   * will return %BOBGUI_PRINT_OPERATION_RESULT_IN_PROGRESS as the status, and
   * emit the [signal@Bobgui.PrintOperation::done] signal when the operation
   * is actually done.
   *
   * The Windows port does not support asynchronous operation at all (this
   * is unlikely to change). On other platforms, all actions except for
   * %BOBGUI_PRINT_OPERATION_ACTION_EXPORT support asynchronous operation.
   */
  g_object_class_install_property (gobject_class,
				   PROP_ALLOW_ASYNC,
  ...
  /**
   * BobguiPrintOperation:export-filename:
   *
  ...
  g_object_class_install_property (gobject_class,
				   ...
				   g_param_spec_enum ("status", NULL, NULL,
						      BOBGUI_TYPE_PRINT_STATUS,
						      BOBGUI_PRINT_STATUS_INITIAL,
						      G_PARAM_READABLE|G_PARAM_EXPLICIT_NOTIFY));

  /**
   * BobguiPrintOperation:status-string:
   *
   * A string representation of the status of the print operation.
   *
   * The string is translated and suitable for displaying the print
   * status e.g. in a `BobguiStatusbar`.
   *
   * See the [property@Bobgui.PrintOperation:status] property for a status
  ...
  /**
   * BobguiPrintOperation:custom-tab-label:
   *
   * Used as the label of the tab containing custom widgets.
   *
   * Note that this property may be ignored on some platforms.
   *
   * If this is %NULL, BOBGUI uses a default label.
   */
  g_object_class_install_property (gobject_class,
				   PROP_CUSTOM_TAB_LABEL,
  ...
  /**
   * BobguiPrintOperation:support-selection:
   *
  ...
  /**
   * BobguiPrintOperation:has-selection:
   *
  ...
  /**
   * BobguiPrintOperation:embed-page-setup:
   *
  ...
  /**
   * BobguiPrintOperation:n-pages-to-print:
   *
   * The number of pages that will be printed.
   *
   * Note that this value is set during print preparation phase
   * (%BOBGUI_PRINT_STATUS_PREPARING), so this value should never be
   * get before the data generation phase (%BOBGUI_PRINT_STATUS_GENERATING_DATA).
   * You can connect to the [signal@Bobgui.PrintOperation::status-changed] signal
   * and call [method@Bobgui.PrintOperation.get_n_pages_to_print] when
   * print status is %BOBGUI_PRINT_STATUS_GENERATING_DATA.
   *
...
/**
 * bobgui_print_operation_new:
 *
 * Creates a new `BobguiPrintOperation`.
 *
 * Returns: a new `BobguiPrintOperation`
 */
BobguiPrintOperation *
bobgui_print_operation_new (void)
{
  BobguiPrintOperation *print_operation;

  print_operation = g_object_new (BOBGUI_TYPE_PRINT_OPERATION, NULL);
  
  return print_operation;
}

/**
 * bobgui_print_operation_set_default_page_setup:
 * @op: a `BobguiPrintOperation`
 * @default_page_setup: (nullable): a `BobguiPageSetup`
 *
 ...
 **/
void
bobgui_print_operation_set_default_page_setup (BobguiPrintOperation *op,
					    BobguiPageSetup      *default_page_setup)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);

  g_return_if_fail (BOBGUI_IS_PRINT_OPERATION (op));
  g_return_if_fail (default_page_setup == NULL ||
                    BOBGUI_IS_PAGE_SETUP (default_page_setup));
...
/**
 * bobgui_print_operation_get_default_page_setup:
 * @op: a `BobguiPrintOperation`
 *
 ...
 */
BobguiPageSetup *
bobgui_print_operation_get_default_page_setup (BobguiPrintOperation *op)
{
  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), NULL);
...
/**
 * bobgui_print_operation_set_print_settings:
 * @op: a `BobguiPrintOperation`
 * @print_settings: (nullable): `BobguiPrintSettings`
 *
 * Sets the print settings for @op.
 *
 * This is typically used to re-establish print settings
 * from a previous print operation, see [method@Bobgui.PrintOperation.run].
 */
void
bobgui_print_operation_set_print_settings (BobguiPrintOperation *op,
					BobguiPrintSettings  *print_settings)
{
  ...
  g_return_if_fail (print_settings == NULL || 
                    BOBGUI_IS_PRINT_SETTINGS (print_settings));
...
/**
 * bobgui_print_operation_get_print_settings:
 * @op: a `BobguiPrintOperation`
 *
 * Returns the current print settings.
 *
 * Note that the return value is %NULL until either
 * [method@Bobgui.PrintOperation.set_print_settings] or
 * [method@Bobgui.PrintOperation.run] have been called.
 *
 * Returns: (transfer none) (nullable): the current print settings of @op.
 **/
BobguiPrintSettings *
bobgui_print_operation_get_print_settings (BobguiPrintOperation *op)
{
  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), NULL);
...
/**
 * bobgui_print_operation_set_job_name:
 * @op: a `BobguiPrintOperation`
 ...
 * applications like eggcups).
 *
 * If you don’t set a job name, BOBGUI picks a default one by
 * numbering successive print jobs.
 */
void
bobgui_print_operation_set_job_name (BobguiPrintOperation *op,
				  const char        *job_name)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_set_n_pages:
 * @op: a `BobguiPrintOperation`
 ...
 *
 * Note that the page numbers passed to the
 * [signal@Bobgui.PrintOperation::request-page-setup]
 * and [signal@Bobgui.PrintOperation::draw-page] signals are 0-based, i.e.
 * if the user chooses to print all pages, the last ::draw-page signal
 * will be for page @n_pages - 1.
 */
void
bobgui_print_operation_set_n_pages (BobguiPrintOperation *op,
				 int                n_pages)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_set_current_page:
 * @op: a `BobguiPrintOperation`
 * @current_page: the current page, 0-based
 *
 * Sets the current page.
 *
 * If this is called before [method@Bobgui.PrintOperation.run],
 * the user will be able to select to print only the current page.
 *
 * Note that this only makes sense for pre-paginated documents.
 */
void
bobgui_print_operation_set_current_page (BobguiPrintOperation *op,
				      int                current_page)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_set_use_full_page:
 * @op: a `BobguiPrintOperation`
 * @full_page: %TRUE to set up the `BobguiPrintContext` for the full page
 *
 * If @full_page is %TRUE, the transformation for the cairo context
 * obtained from `BobguiPrintContext` puts the origin at the top left
 * corner of the page.
 *
 ...
 */
void
bobgui_print_operation_set_use_full_page (BobguiPrintOperation *op,
				       gboolean           full_page)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_set_unit:
 * @op: a `BobguiPrintOperation`
 * @unit: the unit to use
 *
 * Sets up the transformation for the cairo context obtained from
 * `BobguiPrintContext` in such a way that distances are measured in
 * units of @unit.
 */
void
bobgui_print_operation_set_unit (BobguiPrintOperation *op,
			      BobguiUnit            unit)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_set_track_print_status:
 * @op: a `BobguiPrintOperation`
 ...
 */
void
bobgui_print_operation_set_track_print_status (BobguiPrintOperation  *op,
...
void
_bobgui_print_operation_set_status (BobguiPrintOperation *op,
				 ...
				 const char        *string)
{
  ...
  if (status > BOBGUI_PRINT_STATUS_FINISHED_ABORTED)
...
/**
 * bobgui_print_operation_get_status:
 * @op: a `BobguiPrintOperation`
 *
 * Returns the status of the print operation.
 *
 * Also see [method@Bobgui.PrintOperation.get_status_string].
 *
 * Returns: the status of the print operation
 */
BobguiPrintStatus
bobgui_print_operation_get_status (BobguiPrintOperation *op)
{
  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), 
...
/**
 * bobgui_print_operation_get_status_string:
 * @op: a `BobguiPrintOperation`
 *
 ...
 * the print status e.g. in a `BobguiStatusbar`.
 *
 * Use [method@Bobgui.PrintOperation.get_status] to obtain
 ...
 */
const char *
bobgui_print_operation_get_status_string (BobguiPrintOperation *op)
{
  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), "");
...
/**
 * bobgui_print_operation_is_finished:
 * @op: a `BobguiPrintOperation`
 *
 ...
 * %BOBGUI_PRINT_STATUS_FINISHED or %BOBGUI_PRINT_STATUS_FINISHED_ABORTED.
 *
 ...
 */
gboolean
bobgui_print_operation_is_finished (BobguiPrintOperation *op)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);

  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), TRUE);

  return
    priv->status == BOBGUI_PRINT_STATUS_FINISHED_ABORTED ||
    priv->status == BOBGUI_PRINT_STATUS_FINISHED;
}

/**
 * bobgui_print_operation_set_show_progress:
 * @op: a `BobguiPrintOperation`
 ...
 */
void
bobgui_print_operation_set_show_progress (BobguiPrintOperation  *op,
...
/**
 * bobgui_print_operation_set_allow_async:
 * @op: a `BobguiPrintOperation`
 * @allow_async: %TRUE to allow asynchronous operation
 *
 * Sets whether bobgui_print_operation_run() may return
 ...
 */
void
bobgui_print_operation_set_allow_async (BobguiPrintOperation  *op,
...
/**
 * bobgui_print_operation_set_custom_tab_label:
 * @op: a `BobguiPrintOperation`
 ...
 */
void
bobgui_print_operation_set_custom_tab_label (BobguiPrintOperation  *op,
					  const char         *label)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_set_export_filename:
 * @op: a `BobguiPrintOperation`
 * @filename: (type filename): the filename for the exported file
 *
 * Sets up the `BobguiPrintOperation` to generate a file instead
 ...
 */
void
bobgui_print_operation_set_export_filename (BobguiPrintOperation *op,
					 const char        *filename)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
#ifdef CAIRO_HAS_PDF_SURFACE
static void 
pdf_start_page (BobguiPrintOperation *op,
		BobguiPrintContext   *print_context,
		BobguiPageSetup      *page_setup)
{
...
static void
pdf_end_page (BobguiPrintOperation *op,
	      BobguiPrintContext   *print_context)
{
  cairo_t *cr;
...
static void
pdf_end_run (BobguiPrintOperation *op,
	     gboolean           wait,
	     gboolean           cancelled)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
run_pdf (BobguiPrintOperation  *op,
	 BobguiWindow          *parent,
	 gboolean           *do_print)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
  ...
  width = bobgui_page_setup_get_paper_width (page_setup, BOBGUI_UNIT_POINTS);
  height = bobgui_page_setup_get_paper_height (page_setup, BOBGUI_UNIT_POINTS);
  ...
  bobgui_print_context_set_cairo_context (op->priv->print_context,
				       cr, 72, 72);
  cairo_destroy (cr);

  
  priv->print_pages = BOBGUI_PRINT_PAGES_ALL;
  ...
  priv->manual_page_set = BOBGUI_PAGE_SET_ALL;
  priv->manual_scale = 1.0;
  priv->manual_orientation = FALSE;
  priv->manual_number_up = 1;
  priv->manual_number_up_layout = BOBGUI_NUMBER_UP_LAYOUT_LEFT_TO_RIGHT_TOP_TO_BOTTOM;
  ...
  *do_print = TRUE;
  ...
  return BOBGUI_PRINT_OPERATION_RESULT_APPLY; 
}
#endif
...
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (data->op);
  ...
  else
    {
      ...
      if (priv->manual_number_up < 2 ||
          ...
          priv->page_position == priv->nr_of_pages_to_print - 1)
        {
          ...
          if (priv->manual_page_set == BOBGUI_PAGE_SET_ODD ||
              priv->manual_page_set == BOBGUI_PAGE_SET_EVEN)
            data->sheet += 2 * inc;
          else
            data->sheet += inc;
...
print_pages_idle_done (gpointer user_data)
{
  ...
  if (data->progress)
    bobgui_window_destroy (BOBGUI_WINDOW (data->progress));
...
update_progress (PrintPagesData *data)
{
  ...
  if (data->progress)
    {
      if (priv->status == BOBGUI_PRINT_STATUS_PREPARING)
	{
      ...
      else if (priv->status == BOBGUI_PRINT_STATUS_GENERATING_DATA)
...
/**
 * bobgui_print_operation_set_defer_drawing:
 * @op: a `BobguiPrintOperation`
 *
 * Sets up the `BobguiPrintOperation` to wait for calling of
 * [method@Bobgui.PrintOperation.draw_page_finish from application.
 *
 * This can be used for drawing page in another thread.
 *
 * This function must be called in the callback of the
 * [signal@Bobgui.PrintOperation::draw-page] signal.
 */
void
bobgui_print_operation_set_defer_drawing (BobguiPrintOperation *op)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);

  g_return_if_fail (priv->page_drawing_state == BOBGUI_PAGE_DRAWING_STATE_DRAWING);

  priv->page_drawing_state = BOBGUI_PAGE_DRAWING_STATE_DEFERRED_DRAWING;
}

/**
 * bobgui_print_operation_set_embed_page_setup:
 * @op: a `BobguiPrintOperation`
 * @embed: %TRUE to embed page setup selection in the `BobguiPrintUnixDialog`
 *
 * Embed page size combo box and orientation combo box into page setup page.
 *
 * Selected page setup is stored as default page setup in `BobguiPrintOperation`.
 */
void
bobgui_print_operation_set_embed_page_setup (BobguiPrintOperation  *op,
                                          gboolean            embed)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_get_embed_page_setup:
 * @op: a `BobguiPrintOperation`
 *
 ...
 */
gboolean
bobgui_print_operation_get_embed_page_setup (BobguiPrintOperation *op)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);

  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), FALSE);

  return priv->embed_page_setup;
}

/**
 * bobgui_print_operation_draw_page_finish:
 * @op: a `BobguiPrintOperation`
 *
 * Signal that drawing of particular page is complete.
 *
 * It is called after completion of page drawing (e.g. drawing
 * in another thread). If [method@Bobgui.PrintOperation.set_defer_drawing]
 * was called before, then this function has to be called by application.
 * Otherwise it is called by BOBGUI itself.
 */
void
bobgui_print_operation_draw_page_finish (BobguiPrintOperation *op)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
  BobguiPageSetup *page_setup;
  BobguiPrintContext *print_context;
  ...
  page_setup = bobgui_print_context_get_page_setup (print_context);
...
static void
common_render_page (BobguiPrintOperation *op,
		    int                page_nr)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
  BobguiPageSetup *page_setup;
  ...
  _bobgui_print_context_set_page_setup (print_context, page_setup);
  ...
  if (priv->manual_orientation)
    _bobgui_print_context_rotate_according_to_orientation (print_context);
  else
    _bobgui_print_context_reverse_according_to_orientation (print_context);

  if (priv->manual_number_up <= 1)
    {
      if (!priv->use_full_page)
        _bobgui_print_context_translate_into_margin (print_context);
  ...
  else
    {
      ...
      top_margin = bobgui_page_setup_get_top_margin (page_setup, BOBGUI_UNIT_POINTS);
      bottom_margin = bobgui_page_setup_get_bottom_margin (page_setup, BOBGUI_UNIT_POINTS);
      left_margin = bobgui_page_setup_get_left_margin (page_setup, BOBGUI_UNIT_POINTS);
      right_margin = bobgui_page_setup_get_right_margin (page_setup, BOBGUI_UNIT_POINTS);

      paper_width = bobgui_page_setup_get_paper_width (page_setup, BOBGUI_UNIT_POINTS);
      paper_height = bobgui_page_setup_get_paper_height (page_setup, BOBGUI_UNIT_POINTS);

      context_width = bobgui_print_context_get_width (print_context);
      context_height = bobgui_print_context_get_height (print_context);

      if (orientation == BOBGUI_PAGE_ORIENTATION_PORTRAIT ||
          orientation == BOBGUI_PAGE_ORIENTATION_REVERSE_PORTRAIT)
        {
  ...
  priv->page_drawing_state = BOBGUI_PAGE_DRAWING_STATE_DRAWING;

  g_signal_emit (op, signals[DRAW_PAGE], 0, 
		 print_context, page_nr);

  if (priv->page_drawing_state == BOBGUI_PAGE_DRAWING_STATE_DRAWING)
    bobgui_print_operation_draw_page_finish (op);
}
...
prepare_data (PrintPagesData *data)
{
  ...
  if (!data->initialized)
    {
      ...
      _bobgui_print_context_set_page_setup (priv->print_context,
                                         page_setup);
      g_object_unref (page_setup);
   ...
   */
  if (priv->print_pages == BOBGUI_PRINT_PAGES_RANGES)
    {
  ...
  else if (priv->print_pages == BOBGUI_PRINT_PAGES_CURRENT &&
   priv->current_page != -1)
    {
  ...
  if (priv->manual_reverse)
    {
      ...
      else if (priv->manual_page_set == BOBGUI_PAGE_SET_EVEN)
  ...
  _bobgui_print_operation_set_status (data->op, 
...
print_pages_idle (gpointer user_data)
{
 ...
 out:
      ...
      if (priv->cancelled)
        {
          _bobgui_print_operation_set_status (data->op, BOBGUI_PRINT_STATUS_FINISHED_ABORTED, NULL);
...
static void
handle_progress_response (BobguiWidget *dialog, 
			  int        response,
			  gpointer   data)
{
  BobguiPrintOperation *op = (BobguiPrintOperation *)data;

  bobgui_widget_set_visible (dialog, FALSE);
  bobgui_print_operation_cancel (op);
}

static gboolean
show_progress_timeout (PrintPagesData *data)
{
  bobgui_window_present (BOBGUI_WINDOW (data->progress));
...
static void
print_pages (BobguiPrintOperation       *op,
	     BobguiWindow               *parent,
	     gboolean                 do_print,
	     BobguiPrintOperationResult  result)
{
  ...
  if (!do_print) 
    {
      BobguiPrintOperationResult tmp_result;

      _bobgui_print_operation_set_status (op, BOBGUI_PRINT_STATUS_FINISHED_ABORTED, NULL);

      if (priv->error)
        tmp_result = BOBGUI_PRINT_OPERATION_RESULT_ERROR;
      else if (priv->cancelled)
        tmp_result = BOBGUI_PRINT_OPERATION_RESULT_CANCEL;
      else
        tmp_result = result;
  ...
  _bobgui_print_operation_set_status (op, BOBGUI_PRINT_STATUS_PREPARING, NULL);  

  data = g_new0 (PrintPagesData, 1);
  data->op = g_object_ref (op);
  data->is_preview = (priv->action == BOBGUI_PRINT_OPERATION_ACTION_PREVIEW);
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
      progress = bobgui_message_dialog_new (parent, 0,
...
G_GNUC_END_IGNORE_DEPRECATIONS
      ...
      priv->show_progress_timeout_id =
        g_timeout_add (SHOW_PROGRESS_TIME,
                       ...
                       data);
      g_source_set_static_name (g_main_context_find_source_by_id (NULL, priv->show_progress_timeout_id), "[bobgui] show_progress_timeout");
  ...
  if (data->is_preview)
    {
      ...
      g_signal_emit_by_name (op, "preview",
			     BOBGUI_PRINT_OPERATION_PREVIEW (op),
			     priv->print_context,
			     parent,
			     &handled);
      ...
      if (!handled)
        {
          BobguiWidget *error_dialog;

G_GNUC_BEGIN_IGNORE_DEPRECATIONS
          error_dialog = bobgui_message_dialog_new (parent,
                                                 BOBGUI_DIALOG_MODAL | BOBGUI_DIALOG_DESTROY_WITH_PARENT,
                                                 BOBGUI_MESSAGE_ERROR,
                                                 BOBGUI_BUTTONS_OK,
                                                 _("Error creating print preview"));

          bobgui_message_dialog_format_secondary_text (BOBGUI_MESSAGE_DIALOG (error_dialog),
                                                    _("The most probable reason is that a temporary file could not be created."));
G_GNUC_END_IGNORE_DEPRECATIONS

          if (parent && bobgui_window_has_group (parent))
            bobgui_window_group_add_window (bobgui_window_get_group (parent),
                                         BOBGUI_WINDOW (error_dialog));

          g_signal_connect (error_dialog, "response",
                            G_CALLBACK (bobgui_window_destroy), NULL);

          bobgui_window_present (BOBGUI_WINDOW (error_dialog));
      ...
      if (bobgui_print_context_get_cairo_context (priv->print_context) == NULL)
        {
          /* Programmer error */
      ...
      priv->print_pages = bobgui_print_settings_get_print_pages (priv->print_settings);
      priv->page_ranges = bobgui_print_settings_get_page_ranges (priv->print_settings,
							      &priv->num_page_ranges);
      priv->manual_num_copies = 1;
      priv->manual_collation = FALSE;
      priv->manual_reverse = bobgui_print_settings_get_reverse (priv->print_settings);
      priv->manual_page_set = bobgui_print_settings_get_page_set (priv->print_settings);
      priv->manual_scale = bobgui_print_settings_get_scale (priv->print_settings) / 100.0;
      priv->manual_orientation = FALSE;
      priv->manual_number_up = bobgui_print_settings_get_number_up (priv->print_settings);
      priv->manual_number_up_layout = bobgui_print_settings_get_number_up_layout (priv->print_settings);
    }
  ...
  g_source_set_static_name (g_main_context_find_source_by_id (NULL, priv->print_pages_idle_id), "[bobgui] print_pages_idle");
...
/**
 * bobgui_print_operation_get_error:
 * @op: a `BobguiPrintOperation`
 ...
 *
 * It can be called either after [method@Bobgui.PrintOperation.run]
 * returns, or in the [signal@Bobgui.PrintOperation::done] signal
 * handler.
 *
 * The returned `GError` will contain more details on what went wrong.
 */
void
bobgui_print_operation_get_error (BobguiPrintOperation  *op,
			       GError            **error)
{
  g_return_if_fail (BOBGUI_IS_PRINT_OPERATION (op));
 ...
 * [signal@Bobgui.PrintOperation::status-changed] signal on @op to obtain
 * some information about the progress of the print operation.
 *
 * Furthermore, it may use a recursive mainloop to show the print dialog.
 *
 * If you set the [Bobgui.PrintOperation:allow-async] property, the operation
 * will run asynchronously if this is supported on the platform. The
 * [signal@Bobgui.PrintOperation::done] signal will be emitted with the result
 ...
 *
 * res = bobgui_print_operation_run (print,
 ...
 *  {
 *    error_dialog = bobgui_message_dialog_new (BOBGUI_WINDOW (parent),
 ...
 * ```
 *
 * Note that bobgui_print_operation_run() can only be called once on a
 * given `BobguiPrintOperation`.
 *
 * Returns: the result of the print operation. A return value of
 *   %BOBGUI_PRINT_OPERATION_RESULT_APPLY indicates that the printing was
 *   completed successfully. In this case, it is a good idea to obtain
 *   the used print settings with
 *   [method@Bobgui.PrintOperation.get_print_settings]
 *   and store them for reuse with the next print operation. A value of
 *   %BOBGUI_PRINT_OPERATION_RESULT_IN_PROGRESS means the operation is running
 *   asynchronously, and will emit the [signal@Bobgui.PrintOperation::done]
 *   signal when done.
 */
BobguiPrintOperationResult
bobgui_print_operation_run (BobguiPrintOperation        *op,
			 ...
			 GError                  **error)
{
  ...
  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), 
                        BOBGUI_PRINT_OPERATION_RESULT_ERROR);
  g_return_val_if_fail (op->priv->status == BOBGUI_PRINT_STATUS_INITIAL,
                        BOBGUI_PRINT_OPERATION_RESULT_ERROR);
  ...
  if (priv->print_settings == NULL)
    priv->print_settings = bobgui_print_settings_new ();
  
  if (action == BOBGUI_PRINT_OPERATION_ACTION_EXPORT)
    {
      /* note: if you implement async EXPORT, update the docs
       * docs for the allow-async property.
       */
      priv->is_sync = TRUE;
      g_return_val_if_fail (priv->export_filename != NULL, BOBGUI_PRINT_OPERATION_RESULT_ERROR);
#ifdef CAIRO_HAS_PDF_SURFACE
  ...
  else if (action == BOBGUI_PRINT_OPERATION_ACTION_PREVIEW)
    {
      ...
      g_object_unref (page_setup);
      do_print = TRUE;
      result = priv->is_sync ? BOBGUI_PRINT_OPERATION_RESULT_APPLY : BOBGUI_PRINT_OPERATION_RESULT_IN_PROGRESS;
    }
#ifndef G_OS_WIN32
  else if (priv->allow_async)
    {
      priv->is_sync = FALSE;
      _bobgui_print_operation_platform_backend_run_dialog_async (op,
							      action == BOBGUI_PRINT_OPERATION_ACTION_PRINT_DIALOG,
							      parent,
							      print_pages);
      result = BOBGUI_PRINT_OPERATION_RESULT_IN_PROGRESS;
    ...
    {
      priv->is_sync = TRUE;
      result = _bobgui_print_operation_platform_backend_run_dialog (op, 
								 action == BOBGUI_PRINT_OPERATION_ACTION_PRINT_DIALOG,
								 parent,
								 &do_print);
    }
...
/**
 * bobgui_print_operation_cancel:
 * @op: a `BobguiPrintOperation`
 *
 * Cancels a running print operation.
 *
 * This function may be called from a [signal@Bobgui.PrintOperation::begin-print],
 * [signal@Bobgui.PrintOperation::paginate] or [signal@Bobgui.PrintOperation::draw-page]
 * signal handler to stop the currently running print operation.
 */
void
bobgui_print_operation_cancel (BobguiPrintOperation *op)
{
  g_return_if_fail (BOBGUI_IS_PRINT_OPERATION (op));
...
/**
 * bobgui_print_operation_set_support_selection:
 * @op: a `BobguiPrintOperation`
 * @support_selection: %TRUE to support selection
 *
 * Sets whether selection is supported by `BobguiPrintOperation`.
 */
void
bobgui_print_operation_set_support_selection (BobguiPrintOperation  *op,
...
/**
 * bobgui_print_operation_get_support_selection:
 * @op: a `BobguiPrintOperation`
 *
 ...
 */
gboolean
bobgui_print_operation_get_support_selection (BobguiPrintOperation *op)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_set_has_selection:
 * @op: a `BobguiPrintOperation`
 ...
 * will draw by [method@Bobgui.PrintOperation.set_n_pages] in a handler
 * for the [signal@Bobgui.PrintOperation::begin-print] signal.
 */
void
bobgui_print_operation_set_has_selection (BobguiPrintOperation  *op,
...
/**
 * bobgui_print_operation_get_has_selection:
 * @op: a `BobguiPrintOperation`
 *
 ...
 */
gboolean
bobgui_print_operation_get_has_selection (BobguiPrintOperation *op)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);
...
/**
 * bobgui_print_operation_get_n_pages_to_print:
 * @op: a `BobguiPrintOperation`
 *
 * Returns the number of pages that will be printed.
 *
 * Note that this value is set during print preparation phase
 * (%BOBGUI_PRINT_STATUS_PREPARING), so this function should never be
 * called before the data generation phase (%BOBGUI_PRINT_STATUS_GENERATING_DATA).
 * You can connect to the [signal@Bobgui.PrintOperation::status-changed]
 * signal and call bobgui_print_operation_get_n_pages_to_print() when
 * print status is %BOBGUI_PRINT_STATUS_GENERATING_DATA.
 *
 * This is typically used to track the progress of print operation.
 *
 ...
 */
int
bobgui_print_operation_get_n_pages_to_print (BobguiPrintOperation *op)
{
  BobguiPrintOperationPrivate *priv = bobgui_print_operation_get_instance_private (op);

  g_return_val_if_fail (BOBGUI_IS_PRINT_OPERATION (op), -1);
