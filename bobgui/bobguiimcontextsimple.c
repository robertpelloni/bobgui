...
#include "bobguicomposetable.h"
#include "bobguiimmoduleprivate.h"

#include "bobguiimcontextsimpleseqs.h"

#include "gdk/gdkeventsprivate.h"
#include "gdk/gdkprofilerprivate.h"

/**
 * BobguiIMContextSimple:
 *
 * Supports compose sequences, dead keys and numeric Unicode input.
 *
 * ## Compose sequences
 *
 * `BobguiIMContextSimple` reads compose sequences from the first of the
 * following files that is found: ~/.config/bobgui-4.0/Compose, ~/.XCompose,
 * /usr/share/X11/locale/$locale/Compose (for locales that have a nontrivial
 * Compose file). A subset of the file syntax described in the Compose(5)
 * manual page is supported. Additionally, `include "%L"` loads BOBGUI’s built-in
 * table of compose sequences rather than the locale-specific one from X11.
 *
 * If none of these files is found, `BobguiIMContextSimple` uses a built-in table
 ...
 * ## Unicode characters
 *
 * `BobguiIMContextSimple` also supports numeric entry of Unicode characters
 ...
 * ## Dead keys
 *
 * `BobguiIMContextSimple` supports dead keys. For example, typing
 *
 *     dead_acute a
 *
...
struct _BobguiIMContextSimplePrivate
{
...
#include "bobgui/compose/bobguicomposedata.h"

BobguiComposeTable builtin_compose_table = {
  NULL,
  NULL,
  MAX_SEQ_LEN,
  N_INDEX_SIZE,
...
init_builtin_table (void)
{
  ...
  bytes = g_resources_lookup_data ("/org/bobgui/libbobgui/compose/sequences", 0, NULL);
  builtin_compose_table.data = (guint16 *) g_bytes_get_data (bytes, NULL);
  g_bytes_unref (bytes);

  bytes = g_resources_lookup_data ("/org/bobgui/libbobgui/compose/chars", 0, NULL);
...
static const guint bobgui_compose_ignore[] = {
...
static void     bobgui_im_context_simple_finalize           (GObject                  *obj);
static gboolean bobgui_im_context_simple_filter_keypress    (BobguiIMContext             *context,
                                                          GdkEvent                 *key);
static void     bobgui_im_context_simple_reset              (BobguiIMContext             *context);
static void     bobgui_im_context_simple_get_preedit_string (BobguiIMContext             *context,
                                                          char                    **str,
                                                          PangoAttrList           **attrs,
                                                          int                      *cursor_pos);
...
G_DEFINE_TYPE_WITH_CODE (BobguiIMContextSimple, bobgui_im_context_simple, BOBGUI_TYPE_IM_CONTEXT,
                         G_ADD_PRIVATE (BobguiIMContextSimple)
                         bobgui_im_module_ensure_extension_point ();
                         g_io_extension_point_implement (BOBGUI_IM_MODULE_EXTENSION_POINT_NAME,
                                                         g_define_type_id,
                                                         "bobgui-im-context-simple",
                                                         G_MININT))

static void
bobgui_im_context_simple_class_init (BobguiIMContextSimpleClass *class)
{
  BobguiIMContextClass *im_context_class = BOBGUI_IM_CONTEXT_CLASS (class);
  GObjectClass *gobject_class = G_OBJECT_CLASS (class);

  im_context_class->filter_keypress = bobgui_im_context_simple_filter_keypress;
  im_context_class->reset = bobgui_im_context_simple_reset;
  im_context_class->get_preedit_string = bobgui_im_context_simple_get_preedit_string;
  gobject_class->finalize = bobgui_im_context_simple_finalize;
...
static int
bobgui_compose_table_find (gconstpointer data1,
                        gconstpointer data2)
{
  const BobguiComposeTable *compose_table = (const BobguiComposeTable *) data1;
...
add_compose_table_from_file (const char *compose_file)
{
  ...
  if (!g_slist_find_custom (global_tables, GINT_TO_POINTER (hash), bobgui_compose_table_find))
    {
      BobguiComposeTable *table;

      table = bobgui_compose_table_new_with_file (compose_file);
                             ...
                             int      n_seqs)
{
  ...
  hash = bobgui_compose_table_data_hash (data, max_seq_len, n_seqs);
  if (!g_slist_find_custom (global_tables, GINT_TO_POINTER (hash), bobgui_compose_table_find))
    {
      BobguiComposeTable *table;

      table = bobgui_compose_table_new_with_data (data, max_seq_len, n_seqs);
...
static void
bobgui_im_context_simple_init_compose_table (void)
{
  char *path = NULL;
  const char *home;
  const char *locale;
  ...
  path = g_build_filename (g_get_user_config_dir (), "bobgui-4.0", "Compose", NULL);
      ...
      for (sys_lang = sys_langs; *sys_lang; sys_lang++)
        {
          if (g_ascii_strncasecmp (*lang, *sys_lang, strlen (*sys_lang)) == 0)
            {
              char *x11_compose_file_dir = bobgui_compose_table_get_x11_compose_file_dir ();
                              ...
                              GCancellable     *cancellable)
{
  ...
  bobgui_im_context_simple_init_compose_table ();
...
static void
bobgui_im_context_simple_init (BobguiIMContextSimple *context_simple)
{
  BobguiIMContextSimplePrivate *priv;

  priv = context_simple->priv = bobgui_im_context_simple_get_instance_private (context_simple);
...
static void
bobgui_im_context_simple_finalize (GObject *obj)
{
  BobguiIMContextSimple *context_simple = BOBGUI_IM_CONTEXT_SIMPLE (obj);
  BobguiIMContextSimplePrivate *priv = context_simple->priv;;

  g_free (priv->compose_buffer);
  g_string_free (priv->tentative_match, TRUE);

  G_OBJECT_CLASS (bobgui_im_context_simple_parent_class)->finalize (obj);
}

/**
 * bobgui_im_context_simple_new:
 *
 * Creates a new `BobguiIMContextSimple`.
 *
 * Returns: a new `BobguiIMContextSimple`
 */
BobguiIMContext *
bobgui_im_context_simple_new (void)
{
  return g_object_new (BOBGUI_TYPE_IM_CONTEXT_SIMPLE, NULL);
}

static void
bobgui_im_context_simple_commit_string (BobguiIMContextSimple *context_simple,
                                     const char         *str)
{
  BobguiIMContextSimplePrivate *priv = context_simple->priv;
...
static void
bobgui_im_context_simple_commit_char (BobguiIMContextSimple *context_simple,
                                   gunichar            ch)
{
  char buf[8] = { 0, };
  ...
  bobgui_im_context_simple_commit_string (context_simple, buf);
}
...
static gboolean
check_hex (BobguiIMContextSimple *context_simple,
           int                 n_compose)
{
  BobguiIMContextSimplePrivate *priv = context_simple->priv;
...
static void
handle_invalid_composition (BobguiIMContext *context,
                            guint        *compose_buffer,
                            guint         n_compose,
                            GdkSurface   *surface)
{
  ...
  g_object_get (bobgui_settings_get_for_display (display),
                "bobgui-error-bell", &display_beepable,
                NULL);
...
static gboolean
no_sequence_matches (BobguiIMContextSimple *context_simple,
                     int                 n_compose,
                     GdkEvent           *event)
{
  BobguiIMContextSimplePrivate *priv = context_simple->priv;
  ...
  context = BOBGUI_IM_CONTEXT (context_simple);
  ...
  if (priv->tentative_match_len > 0)
    {
      ...
      bobgui_im_context_simple_commit_string (context_simple, str);
      g_free (str);
      ...
      for (i = 0; i < n_compose - len - 1; i++)
        {
          ...
          bobgui_im_context_filter_keypress (context, tmp_event);
          gdk_event_unref (tmp_event);
        }

      return bobgui_im_context_filter_keypress (context, event);
    }
  else
    {
      int i;
          ...
          if (i == n_compose - 1)
            {
              ...
              bobgui_im_context_simple_commit_string (context_simple, s->str);
            }
          else
            {
      ...
      if (ch != 0 && !g_unichar_iscntrl (ch))
        {
          bobgui_im_context_simple_commit_char (context_simple, ch);
          return TRUE;
        }
      else
        return FALSE;
...
static gboolean
bobgui_im_context_simple_filter_keypress (BobguiIMContext *context,
                                       GdkEvent     *event)
{
  BobguiIMContextSimple *context_simple = BOBGUI_IM_CONTEXT_SIMPLE (context);
  BobguiIMContextSimplePrivate *priv = context_simple->priv;
  ...
  for (i = 0; i < G_N_ELEMENTS (bobgui_compose_ignore); i++)
    if (keyval == bobgui_compose_ignore[i])
      {
        if (priv->in_hex_sequence || priv->in_compose_sequence)
          return TRUE; /* Don't leak random key events during preedit */
  ...
  if (is_escape)
    {
      if (priv->in_hex_sequence || priv->in_compose_sequence)
        {
          bobgui_im_context_simple_reset (context);
          return TRUE;
        }
  ...
  if (priv->in_hex_sequence)
  ...
  else /* Then, check for compose sequences */
    {
      ...
      while (tmp_list)
        {
          if (bobgui_compose_table_check ((BobguiComposeTable *)tmp_list->data,
                                       ...
                                       output))
            {
              ...
              if (compose_finish)
                {
                  if (compose_match)
                    bobgui_im_context_simple_commit_string (context_simple, output->str);
                }
              else
                {
                  if (compose_match)
          ...
          else
            {
              ...
              bobgui_compose_table_get_prefix ((BobguiComposeTable *)tmp_list->data,
                                            priv->compose_buffer, n_compose,
                                            &table_prefix);
      ...
      if (bobgui_check_algorithmically (priv->compose_buffer, n_compose, output))
        {
          if (!priv->in_compose_sequence)
            {
              priv->in_compose_sequence = TRUE;
...
static void
bobgui_im_context_simple_reset (BobguiIMContext *context)
{
  BobguiIMContextSimple *context_simple = BOBGUI_IM_CONTEXT_SIMPLE (context);
  BobguiIMContextSimplePrivate *priv = context_simple->priv;
...
static void
bobgui_im_context_simple_get_preedit_string (BobguiIMContext   *context,
                                          char          **str,
                                          PangoAttrList **attrs,
                                          int            *cursor_pos)
{
...
/**
 * bobgui_im_context_simple_add_table: (skip)
 * @context_simple: A `BobguiIMContextSimple`
 ...
 *
 * Deprecated: 4.4: Use bobgui_im_context_simple_add_compose_file()
 */
void
bobgui_im_context_simple_add_table (BobguiIMContextSimple *context_simple,
                                 guint16            *data,
                                 int                 max_seq_len,
                                 int                 n_seqs)
{
  g_return_if_fail (BOBGUI_IS_IM_CONTEXT_SIMPLE (context_simple));

  add_compose_table_from_data (data, max_seq_len, n_seqs);
}

/**
 * bobgui_im_context_simple_add_compose_file:
 * @context_simple: A `BobguiIMContextSimple`
 ...
 */
void
bobgui_im_context_simple_add_compose_file (BobguiIMContextSimple *context_simple,
