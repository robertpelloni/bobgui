 ...
 * bobguifilesystemmodel.c: BobguiTreeModel wrapping a BobguiFileSystem
...
struct _BobguiFileSystemModel
{
  ...
  BobguiFileFilter *       filter;         /* filter to use for deciding which nodes are visible */
...
static void freeze_updates (BobguiFileSystemModel *model);
static void thaw_updates (BobguiFileSystemModel *model);
 ...
 */
static void
node_validate_rows (BobguiFileSystemModel *model,
                    guint               up_to_index,
                    guint               up_to_row)
{
  guint i, row;
...
node_get_tree_row (BobguiFileSystemModel *model,
                   guint               index)
{
...
static void
node_invalidate_index (BobguiFileSystemModel *model,
                       guint               id)
{
...
static void
node_set_visible_and_filtered_out (BobguiFileSystemModel *model,
                                   guint               id,
                                   gboolean            visible,
...
static gboolean
node_should_be_filtered_out (BobguiFileSystemModel *model,
                             guint               id)
{
  FileModelNode *node = get_node (model, id);
  ...
  return !bobgui_filter_match (BOBGUI_FILTER (model->filter), node->info);
}

static gboolean
node_should_be_visible (BobguiFileSystemModel *model,
                        guint               id,
                        gboolean            filtered_out)
{
  FileModelNode *node = get_node (model, id);
  ...
  if (_bobgui_file_info_consider_as_directory (node->info))
    {
...
static gboolean
node_should_be_selectable (BobguiFileSystemModel *model,
                          guint                id)
{
  FileModelNode *node = get_node (model, id);
...
static void
node_compute_visibility_and_filters (BobguiFileSystemModel *model,
                                     guint               id)
{
  gboolean filtered_out;
  gboolean visible;
...
static guint
node_get_for_file (BobguiFileSystemModel *model,
                   GFile              *file)
{
  gpointer position;
  int i;
...
{
  BobguiFileSystemModel *model = BOBGUI_FILE_SYSTEM_MODEL (list_model);
...
G_DEFINE_TYPE_WITH_CODE (BobguiFileSystemModel, bobgui_file_system_model, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (G_TYPE_LIST_MODEL,
                                                g_list_model_iface_init))

static void
freeze_updates (BobguiFileSystemModel *model)
{
  g_return_if_fail (BOBGUI_IS_FILE_SYSTEM_MODEL (model));

  model->frozen++;
}

static void
bobgui_file_system_model_refilter_all (BobguiFileSystemModel *model)
{
  guint i;
...
static void
thaw_updates (BobguiFileSystemModel *model)
{
  ...
  if (model->filter_on_thaw)
    bobgui_file_system_model_refilter_all (model);
  if (stuff_added)
    {
      guint i;
...
static void
add_file (BobguiFileSystemModel *model,
          GFile              *file,
          GFileInfo          *info)
{
  FileModelNode *node;
...
static void
adjust_file_lookup (BobguiFileSystemModel *model, guint id, int increment)
{
  GHashTableIter iter;
  gpointer key;
  gpointer value;
...
static void
remove_file (BobguiFileSystemModel *model,
             GFile              *file)
{
  FileModelNode *node;
  guint id;
  ...
  if (id == BOBGUI_INVALID_LIST_POSITION)
    return;
...
static void
bobgui_file_system_model_dispose (GObject *object)
{
  BobguiFileSystemModel *model = BOBGUI_FILE_SYSTEM_MODEL (object);
  ...
  G_OBJECT_CLASS (bobgui_file_system_model_parent_class)->dispose (object);
}


static void
bobgui_file_system_model_finalize (GObject *object)
{
  BobguiFileSystemModel *model = BOBGUI_FILE_SYSTEM_MODEL (object);
  guint i;
  ...
  G_OBJECT_CLASS (bobgui_file_system_model_parent_class)->finalize (object);
}

static void
bobgui_file_system_model_class_init (BobguiFileSystemModelClass *class)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (class);

  gobject_class->finalize = bobgui_file_system_model_finalize;
  gobject_class->dispose = bobgui_file_system_model_dispose;
...
static void
bobgui_file_system_model_init (BobguiFileSystemModel *model)
{
...
static void
bobgui_file_system_model_closed_enumerator (GObject      *object,
                                         GAsyncResult *res,
                                         gpointer      data)
{
...
{
  BobguiFileSystemModel *model = data;
...
static void
bobgui_file_system_model_got_files (GObject      *object,
                                 GAsyncResult *res,
                                 gpointer      data)
{
  ...
  if (files)
    {
      if (model->dir_thaw_source == 0)
        {
          ...
          model->dir_thaw_source = g_timeout_add_full (IO_PRIORITY + 1, 50,
                                                       ...
                                                       model,
                                                       NULL);
          gdk_source_set_static_name_by_id (model->dir_thaw_source, "[bobgui] thaw_func");
        }
      ...
      g_file_enumerator_next_files_async (enumerator,
                                          ...
                                          IO_PRIORITY,
                                          model->cancellable,
                                          bobgui_file_system_model_got_files,
                                          model);
    }
  else
    {
      if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
        {
          g_file_enumerator_close_async (enumerator,
                                         IO_PRIORITY,
                                         model->cancellable,
                                         bobgui_file_system_model_closed_enumerator,
                                         NULL);
          if (model->dir_thaw_source != 0)
            {
              g_clear_handle_id (&model->dir_thaw_source, g_source_remove);
...
static void
bobgui_file_system_model_update_file (BobguiFileSystemModel *model,
                                   GFile              *file,
                                   GFileInfo          *info)
{
  FileModelNode *node;
...
/* Helper for bobgui_file_system_model_query_done and
 * bobgui_file_system_model_one_query_done */
static void
...
{
  BobguiFileSystemModel *model = BOBGUI_FILE_SYSTEM_MODEL (data);
  ...
  if (info)
    {
      bobgui_file_system_model_update_file (model, file, info);
      g_object_unref (info);
    }
...
static void
bobgui_file_system_model_query_done (GObject *     object,
                                  GAsyncResult *res,
                                  gpointer      data)
{
...
static void
bobgui_file_system_model_monitor_change (GFileMonitor *      monitor,
                                      GFile *             file,
                                      GFile *             other_file,
  ...
  switch (type)
    {
      ...
      case G_FILE_MONITOR_EVENT_ATTRIBUTE_CHANGED:
        ...
        g_file_query_info_async (file,
                                 ...
                                 IO_PRIORITY,
                                 model->cancellable,
                                 bobgui_file_system_model_query_done,
                                 model);
        break;
...
static void
bobgui_file_system_model_got_enumerator (GObject *dir, GAsyncResult *res, gpointer data)
{
  BobguiFileSystemModel *model = data;
  ...
  else
    {
      ...
      if (model->dir_monitor)
        g_signal_connect (model->dir_monitor,
                          "changed",
                          G_CALLBACK (bobgui_file_system_model_monitor_change),
                          model);
    }
}

static void
bobgui_file_system_model_set_directory (BobguiFileSystemModel *model,
                                     GFile *             dir,
                                     const char *       attributes)
{
  ...
  g_file_enumerate_children_async (model->dir,
                                   ...
                                   IO_PRIORITY,
                                   model->cancellable,
                                   bobgui_file_system_model_got_enumerator,
                                   model);

}

/**
 * _bobgui_file_system_model_new:
 *
 * Creates a new `BobguiFileSystemModel` object. You need to add files
 * to the list using _bobgui_file_system_model_add_and_query_file()
 * or _bobgui_file_system_model_update_file().
 *
 * Returns: the newly created `BobguiFileSystemModel`
 **/
BobguiFileSystemModel *
_bobgui_file_system_model_new (void)
{
  BobguiFileSystemModel *model;

  model = g_object_new (BOBGUI_TYPE_FILE_SYSTEM_MODEL, NULL);
...
/**
 * _bobgui_file_system_model_new_for_directory:
 * @directory: the directory to show.
 * @attributes: (nullable): attributes to immediately load or %NULL for all
 *
 * Creates a new `BobguiFileSystemModel` object.
 *
 * The `BobguiFileSystemModel` object wraps the given @directory as a
 * `BobguiTreeModel`. The model will query the given directory with the
 ...
 **/
BobguiFileSystemModel *
_bobgui_file_system_model_new_for_directory (GFile      *dir,
                                          const char *attributes)
{
  BobguiFileSystemModel *model;
  ...
  model = _bobgui_file_system_model_new ();
  bobgui_file_system_model_set_directory (model, dir, attributes);

  return model;
}

/**
 * _bobgui_file_system_model_set_show_hidden:
 * @model: a `BobguiFileSystemModel`
 * @show_hidden: whether hidden files should be displayed
 *
 * Sets whether hidden files should be included in the `BobguiTreeModel`
 * for display.
 **/
void
_bobgui_file_system_model_set_show_hidden (BobguiFileSystemModel *model,
...
/**
 * _bobgui_file_system_model_set_show_folders:
 * @model: a `BobguiFileSystemModel`
 * @show_folders: whether folders should be displayed
 *
 * Sets whether folders should be included in the `BobguiTreeModel`
 * for display.
 */
void
_bobgui_file_system_model_set_show_folders (BobguiFileSystemModel *model,
...
/**
 * _bobgui_file_system_model_set_show_files:
 * @model: a `BobguiFileSystemModel`
 ...
 * in the `BobguiTreeModel` for display.
 */
void
_bobgui_file_system_model_set_show_files (BobguiFileSystemModel *model,
                                       gboolean            show_files)
{
  g_return_if_fail (BOBGUI_IS_FILE_SYSTEM_MODEL (model));
...
/**
 * _bobgui_file_system_model_set_filter_folders:
 * @model: a `BobguiFileSystemModel`
 * @filter_folders: whether the filter applies to folders
 *
 * Sets whether the filter set by _bobgui_file_system_model_set_filter()
 * applies to folders. By default, it does not and folders are always
 * visible.
 */
void
_bobgui_file_system_model_set_filter_folders (BobguiFileSystemModel *model,
...
/**
 * _bobgui_file_system_model_get_cancellable:
 * @model: the model
 *
 ...
 **/
GCancellable *
_bobgui_file_system_model_get_cancellable (BobguiFileSystemModel *model)
{
  g_return_val_if_fail (BOBGUI_IS_FILE_SYSTEM_MODEL (model), NULL);

  return model->cancellable;
}

GFileInfo *
_bobgui_file_system_model_get_info_for_file (BobguiFileSystemModel *model,
                                          GFile              *file)
{
  FileModelNode *node;
  guint i;

  g_return_val_if_fail (BOBGUI_IS_FILE_SYSTEM_MODEL (model), FALSE);
  g_return_val_if_fail (G_IS_FILE (file), FALSE);

  i = node_get_for_file (model, file);

  if (i == BOBGUI_INVALID_LIST_POSITION)
    return NULL;
...
/**
 * _bobgui_file_system_model_update_files:
 * @model: the model
 * @files: the files
 ...
 **/
void
_bobgui_file_system_model_update_files (BobguiFileSystemModel *model,
                                     GList              *files,
                                     GList              *infos)
{
  GList *l, *i;
  ...
  for (l = files, i = infos; l; l = l->next, i = i->next)
    bobgui_file_system_model_update_file (model, (GFile *)l->data, (GFileInfo *)i->data);

  thaw_updates (model);
}

/**
 * _bobgui_file_system_model_set_filter:
 * @mode: a `BobguiFileSystemModel`
 ...
 **/
void
_bobgui_file_system_model_set_filter (BobguiFileSystemModel      *model,
...
/**
 * _bobgui_file_system_model_add_and_query_file:
 * @model: a `BobguiFileSystemModel`
 ...
 **/
void
_bobgui_file_system_model_add_and_query_file (BobguiFileSystemModel *model,
                                           GFile *             file,
                                           const char *        attributes)
{
  g_return_if_fail (BOBGUI_IS_FILE_SYSTEM_MODEL (model));
...
static void
bobgui_file_system_model_one_query_done (GObject *     object,
                                      GAsyncResult *res,
                                      gpointer      data)
{
...
void
_bobgui_file_system_model_add_and_query_files (BobguiFileSystemModel *model,
                                            GList              *list,
                                            const char         *attributes)
{
  GList *l;
...
GFile *
_bobgui_file_system_model_get_directory (BobguiFileSystemModel *model)
{
  g_return_val_if_fail (BOBGUI_IS_FILE_SYSTEM_MODEL (model), NULL);
...
void
_bobgui_file_system_model_set_can_select_files (BobguiFileSystemModel *model,
                                             gboolean            can_select)
{
  g_return_if_fail (BOBGUI_IS_FILE_SYSTEM_MODEL (model));
