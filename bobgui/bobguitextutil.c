...
/**
 * bobgui_text_util_create_drag_icon:
 * @widget: `BobguiWidget` to extract the pango context
 ...
 */
GdkPaintable *
bobgui_text_util_create_drag_icon (BobguiWidget *widget,
                                char      *text,
                                gssize     len)
{
  ...
  if (!gdk_display_is_rgba (display) ||
    ...
    {
      BobguiWidget *bg_widget;
      BobguiCssBoxes boxes;

      if (BOBGUI_IS_TEXT (widget))
        bg_widget = bobgui_widget_get_parent (widget);
      else
        bg_widget = widget;

      pango_layout_get_size (layout, &layout_width, &layout_height);

      style = bobgui_css_node_get_style (bobgui_widget_get_css_node (bg_widget));
...
static void
set_attributes_from_style (BobguiWidget         *widget,
...
static int
get_border_window_size (BobguiTextView       *text_view,
                        BobguiTextWindowType  window_type)
{
  BobguiWidget *gutter;

  gutter = bobgui_text_view_get_gutter (text_view, window_type);
  if (gutter != NULL)
    return bobgui_widget_get_width (gutter);

  return 0;
}

GdkPaintable *
bobgui_text_util_create_rich_drag_icon (BobguiWidget     *widget,
                                     ...
                                     BobguiTextIter   *end)
{
  ...
  new_buffer = bobgui_text_buffer_new (bobgui_text_buffer_get_tag_table (buffer));
  bobgui_text_buffer_get_start_iter (new_buffer, &iter);

  bobgui_text_buffer_insert_range (new_buffer, &iter, start, end);

  bobgui_text_buffer_get_start_iter (new_buffer, &iter);

  layout = bobgui_text_layout_new ();

  ltr_context = bobgui_widget_create_pango_context (widget);
  pango_context_set_base_dir (ltr_context, PANGO_DIRECTION_LTR);
  rtl_context = bobgui_widget_create_pango_context (widget);
  pango_context_set_base_dir (rtl_context, PANGO_DIRECTION_RTL);

  bobgui_text_layout_set_contexts (layout, ltr_context, rtl_context);
  ...
  layout_width = bobgui_widget_get_width (widget);

  set_attributes_from_style (widget, style);

  if (BOBGUI_IS_TEXT_VIEW (widget))
    {
      layout_width = layout_width
        - get_border_window_size (BOBGUI_TEXT_VIEW (widget), BOBGUI_TEXT_WINDOW_LEFT)
        - get_border_window_size (BOBGUI_TEXT_VIEW (widget), BOBGUI_TEXT_WINDOW_RIGHT);
    }

  style->direction = bobgui_widget_get_direction (widget);
  style->wrap_mode = BOBGUI_WRAP_WORD_CHAR;

  bobgui_text_layout_set_default_style (layout, style);
  bobgui_text_attributes_unref (style);

  bobgui_text_layout_set_buffer (layout, new_buffer);
  bobgui_text_layout_set_cursor_visible (layout, FALSE);
  bobgui_text_layout_set_screen_width (layout, layout_width);

  bobgui_text_layout_validate (layout, DRAG_ICON_MAX_HEIGHT);
  bobgui_text_layout_get_size (layout, &layout_width, &layout_height);
  ...
  if (!gdk_display_is_rgba (display) ||
    ...
    {
      BobguiCssBoxes boxes;
      BobguiCssStyle *css_style;

      css_style = bobgui_css_node_get_style (bobgui_widget_get_css_node (widget));
      bobgui_css_boxes_init_border_box (&boxes, css_style,
                                     0, 0, layout_width, layout_height);
      bobgui_css_style_snapshot_background (&boxes, snapshot);
    }

  bobgui_text_layout_snapshot (layout,
                            widget,
                            snapshot,
  ...
  paintable = bobgui_snapshot_free_to_paintable (snapshot, &(graphene_size_t) { layout_width, layout_height });
...
/*
 * _bobgui_text_util_get_block_cursor_location
 ...
 */
gboolean
_bobgui_text_util_get_block_cursor_location (PangoLayout    *layout,
					  int             index,
					  PangoRectangle *pos,
