...
typedef struct _BobguiIMContextWaylandGlobal BobguiIMContextWaylandGlobal;
typedef struct _BobguiIMContextWayland BobguiIMContextWayland;
typedef struct _BobguiIMContextWaylandClass BobguiIMContextWaylandClass;

struct _BobguiIMContextWaylandGlobal
{
  ...
  BobguiIMContext *current;

  /* The input-method.enter event may happen before or after BOBGUI focus-in,
...
struct _BobguiIMContextWaylandClass
{
...
struct _BobguiIMContextWayland
{
  BobguiIMContext parent_instance;
  BobguiWidget *widget;
...
static void bobgui_im_context_wayland_focus_out (BobguiIMContext *context);

static void commit_state (BobguiIMContextWayland *context);
static void notify_surrounding_text (BobguiIMContextWayland *context);
static void notify_cursor_location (BobguiIMContextWayland *context);
static void notify_content_type (BobguiIMContextWayland *context);

G_DEFINE_TYPE_WITH_CODE (BobguiIMContextWayland, bobgui_im_context_wayland, BOBGUI_TYPE_IM_CONTEXT,
...
#define BOBGUI_IM_CONTEXT_WAYLAND(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), bobgui_im_context_wayland_get_type (), BobguiIMContextWayland))

static BobguiIMContextWaylandGlobal *
bobgui_im_context_wayland_global_get (GdkDisplay *display);

static BobguiIMContextWaylandGlobal *
bobgui_im_context_wayland_get_global (BobguiIMContextWayland *self)
{
  BobguiIMContextWaylandGlobal *global;

  if (self->widget == NULL)
    return NULL;

  global = bobgui_im_context_wayland_global_get (bobgui_widget_get_display (self->widget));
  if (global->current != BOBGUI_IM_CONTEXT (self))
    return NULL;
...
struct wl_proxy *
bobgui_im_context_wayland_get_text_protocol (GdkDisplay *display)
{
  BobguiIMContextWaylandGlobal *global;

  global = bobgui_im_context_wayland_global_get (display);
...
static void
notify_im_change (BobguiIMContextWayland                 *context,
                  enum zwp_text_input_v3_change_cause  cause)
{
  BobguiIMContextWaylandGlobal *global;
  gboolean result;

  global = bobgui_im_context_wayland_get_global (context);
  if (global == NULL)
    return;
...
text_input_preedit (void                     *data,
                    ...
                    int                       cursor_end)
{
  BobguiIMContextWayland *context;
  BobguiIMContextWaylandGlobal *global = data;

  if (!global->current)
    return;

  context = BOBGUI_IM_CONTEXT_WAYLAND (global->current);
...
static void
text_input_preedit_apply (BobguiIMContextWaylandGlobal *global)
{
  BobguiIMContextWayland *context;
...
static void
text_input_commit_apply (BobguiIMContextWaylandGlobal *global)
{
  BobguiIMContextWayland *context;
                                    ...
                                    uint32_t                  after_length)
{
  ...
  /* We already got byte lengths from text_input_v3, but BOBGUI uses char lengths
...
static void
text_input_delete_surrounding_text_apply (BobguiIMContextWaylandGlobal *global)
{
  BobguiIMContextWayland *context;
  gboolean retval;
  int len;
  ...
  if (len > 0)
    {
      g_signal_emit_by_name (global->current, "delete-surrounding",
                             -context->pending_surrounding_delete.before_length,
                             len, &retval);
      notify_im_change (BOBGUI_IM_CONTEXT_WAYLAND (context),
...
static void
notify_surrounding_text (BobguiIMContextWayland *context)
{
#define MAX_LEN 4000
  BobguiIMContextWaylandGlobal *global;
...
static void
notify_cursor_location (BobguiIMContextWayland *context)
{
  BobguiIMContextWaylandGlobal *global;
  ...
  if (!bobgui_widget_compute_point (context->widget,
                                 BOBGUI_WIDGET (bobgui_widget_get_native (context->widget)),
                                 &GRAPHENE_POINT_INIT (rect.x, rect.y),
                                 &p))
    graphene_point_init (&p, rect.x, rect.y);

  bobgui_native_get_surface_transform (bobgui_widget_get_native (context->widget), &nx, &ny);
...
static uint32_t
translate_hints (BobguiInputHints   input_hints,
                 BobguiInputPurpose purpose)
{
  uint32_t hints = 0;

  if (input_hints & BOBGUI_INPUT_HINT_SPELLCHECK)
    hints |= ZWP_TEXT_INPUT_V3_CONTENT_HINT_SPELLCHECK;
  if (input_hints & BOBGUI_INPUT_HINT_WORD_COMPLETION)
    hints |= ZWP_TEXT_INPUT_V3_CONTENT_HINT_COMPLETION;
  if (input_hints & BOBGUI_INPUT_HINT_LOWERCASE)
    hints |= ZWP_TEXT_INPUT_V3_CONTENT_HINT_LOWERCASE;
  if (input_hints & BOBGUI_INPUT_HINT_UPPERCASE_CHARS)
    hints |= ZWP_TEXT_INPUT_V3_CONTENT_HINT_UPPERCASE;
  if (input_hints & BOBGUI_INPUT_HINT_UPPERCASE_WORDS)
    hints |= ZWP_TEXT_INPUT_V3_CONTENT_HINT_TITLECASE;
  if (input_hints & BOBGUI_INPUT_HINT_UPPERCASE_SENTENCES)
    hints |= ZWP_TEXT_INPUT_V3_CONTENT_HINT_AUTO_CAPITALIZATION;

  if (purpose == BOBGUI_INPUT_PURPOSE_PIN ||
      purpose == BOBGUI_INPUT_PURPOSE_PASSWORD)
    {
...
static void
notify_content_type (BobguiIMContextWayland *context)
{
  BobguiIMContextWaylandGlobal *global;
...
static void
commit_state (BobguiIMContextWayland *context)
{
  BobguiIMContextWaylandGlobal *global;
...
static void
bobgui_im_context_wayland_finalize (GObject *object)
{
  BobguiIMContextWayland *context = BOBGUI_IM_CONTEXT_WAYLAND (object);

  bobgui_im_context_wayland_focus_out (BOBGUI_IM_CONTEXT (context));
  ...
  G_OBJECT_CLASS (bobgui_im_context_wayland_parent_class)->finalize (object);
}

static void
bobgui_im_context_wayland_set_client_widget (BobguiIMContext *context,
                                          BobguiWidget    *widget)
{
  BobguiIMContextWayland *context_wayland = BOBGUI_IM_CONTEXT_WAYLAND (context);

  if (widget == context_wayland->widget)
    return;

  if (context_wayland->widget)
    bobgui_im_context_wayland_focus_out (context);

  g_set_object (&context_wayland->widget, widget);
}

static void
bobgui_im_context_wayland_get_preedit_string (BobguiIMContext   *context,
                                           char          **str,
                                           PangoAttrList **attrs,
                                           int            *cursor_pos)
{
...
static void
bobgui_im_context_commit_char (BobguiIMContext *context,
                            gunichar      ch)
{
  char buf[8] = { 0, };
...
static gboolean
bobgui_im_context_wayland_filter_keypress (BobguiIMContext *context,
                                        GdkEvent     *event)
{
  guint keyval, state;
  gunichar ch;
  ...
  if (ch != 0 && !g_unichar_iscntrl (ch))
    {
      bobgui_im_context_commit_char (context, ch);
      return TRUE;
    }
...
static void
enable (BobguiIMContextWayland       *context_wayland,
        BobguiIMContextWaylandGlobal *global)
{
...
static void
disable (BobguiIMContextWayland       *context_wayland,
         BobguiIMContextWaylandGlobal *global)
{
                  ...
                  struct wl_surface        *surface)
{
  ...
  if (global->current)
    enable (BOBGUI_IM_CONTEXT_WAYLAND (global->current), global);
}
                  ...
                  struct wl_surface        *surface)
{
  ...
  if (global->current)
    disable (BOBGUI_IM_CONTEXT_WAYLAND (global->current), global);
}
...
static void
bobgui_im_context_wayland_global_free (gpointer data)
{
  BobguiIMContextWaylandGlobal *global = data;
...
BobguiIMContextWaylandGlobal *
bobgui_im_context_wayland_global_get (GdkDisplay *display)
{
  BobguiIMContextWaylandGlobal *global;

  global = g_object_get_data (G_OBJECT (display), "bobgui-im-context-wayland-global");
  if (global != NULL)
    return global;

  global = g_new0 (BobguiIMContextWaylandGlobal, 1);
  ...
  g_object_set_data_full (G_OBJECT (display),
                          "bobgui-im-context-wayland-global",
                          global,
                          bobgui_im_context_wayland_global_free);

  return global;
}

static void
bobgui_im_context_wayland_focus_in (BobguiIMContext *context)
{
  BobguiIMContextWayland *self = BOBGUI_IM_CONTEXT_WAYLAND (context);
  BobguiIMContextWaylandGlobal *global;
...
static void
bobgui_im_context_wayland_focus_out (BobguiIMContext *context)
{
  BobguiIMContextWayland *self = BOBGUI_IM_CONTEXT_WAYLAND (context);
  BobguiIMContextWaylandGlobal *global;

  global = bobgui_im_context_wayland_get_global (self);
  if (global == NULL)
    return;
...
static void
bobgui_im_context_wayland_reset (BobguiIMContext *context)
{
  notify_im_change (BOBGUI_IM_CONTEXT_WAYLAND (context),
                    ZWP_TEXT_INPUT_V3_CHANGE_CAUSE_OTHER);
}
...
static void
bobgui_im_context_wayland_set_cursor_location (BobguiIMContext *context,
                                            GdkRectangle *rect)
{
  BobguiIMContextWayland *context_wayland;

  context_wayland = BOBGUI_IM_CONTEXT_WAYLAND (context);
...
static void
bobgui_im_context_wayland_set_use_preedit (BobguiIMContext *context,
...
static void
bobgui_im_context_wayland_set_surrounding (BobguiIMContext *context,
                                        const char   *text,
                                        int           len,
                                        int           cursor_index,
                                        int           selection_bound)
...
static gboolean
bobgui_im_context_wayland_get_surrounding (BobguiIMContext  *context,
                                        char         **text,
                                        int           *cursor_index,
                                        int           *selection_bound)
{
...
static void
bobgui_im_context_wayland_commit (BobguiIMContext *context,
                               const gchar  *str)
{
  notify_im_change (BOBGUI_IM_CONTEXT_WAYLAND (context),
                    ZWP_TEXT_INPUT_V3_CHANGE_CAUSE_INPUT_METHOD);
...
static gboolean
bobgui_im_context_wayland_activate_osk_with_event (BobguiIMContext *context,
                                                GdkEvent     *event)
{
  BobguiIMContextWaylandGlobal *global;

  global = bobgui_im_context_wayland_get_global (BOBGUI_IM_CONTEXT_WAYLAND (context));
  if (global == NULL)
    return FALSE;
...
static void
bobgui_im_context_wayland_class_init (BobguiIMContextWaylandClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  BobguiIMContextClass *im_context_class = BOBGUI_IM_CONTEXT_CLASS (klass);

  object_class->finalize = bobgui_im_context_wayland_finalize;

  im_context_class->set_client_widget = bobgui_im_context_wayland_set_client_widget;
  im_context_class->get_preedit_string = bobgui_im_context_wayland_get_preedit_string;
  im_context_class->filter_keypress = bobgui_im_context_wayland_filter_keypress;
  im_context_class->focus_in = bobgui_im_context_wayland_focus_in;
  im_context_class->focus_out = bobgui_im_context_wayland_focus_out;
  im_context_class->reset = bobgui_im_context_wayland_reset;
  im_context_class->set_cursor_location = bobgui_im_context_wayland_set_cursor_location;
  im_context_class->set_use_preedit = bobgui_im_context_wayland_set_use_preedit;
  im_context_class->set_surrounding_with_selection = bobgui_im_context_wayland_set_surrounding;
  im_context_class->get_surrounding_with_selection = bobgui_im_context_wayland_get_surrounding;
  im_context_class->commit = bobgui_im_context_wayland_commit;
  im_context_class->activate_osk_with_event = bobgui_im_context_wayland_activate_osk_with_event;
}

static void
on_content_type_changed (BobguiIMContextWayland *context)
{
  notify_content_type (context);
  commit_state (context);
}
...
static void
bobgui_im_context_wayland_init (BobguiIMContextWayland *context)
{
