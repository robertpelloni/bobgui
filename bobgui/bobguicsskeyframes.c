...
struct _BobguiCssKeyframes {
  ...
  BobguiCssValue **values;          /* 2D array: n_keyframes * n_properties of (value or NULL) for all the keyframes */
  BobguiCssVariableSet **variables; /* array of variable sets for each keyframe */
...
BobguiCssKeyframes *
_bobgui_css_keyframes_ref (BobguiCssKeyframes *keyframes)
{
...
void
_bobgui_css_keyframes_unref (BobguiCssKeyframes *keyframes)
{
  guint k, p;
  ...
  for (k = 0; k < keyframes->n_keyframes; k++)
    {
      for (p = 0; p < keyframes->n_properties; p++)
        {
          bobgui_css_value_unref (KEYFRAMES_VALUE (keyframes, k, p));
          KEYFRAMES_VALUE (keyframes, k, p) = NULL;
        }

      if (keyframes->variables && keyframes->variables[k])
        bobgui_css_variable_set_unref (keyframes->variables[k]);
    }
...
static guint
bobgui_css_keyframes_add_keyframe (BobguiCssKeyframes *keyframes,
                                double           progress)
{
  guint k, p;
  ...
  if (keyframes->n_properties)
    {
      gsize size = sizeof (BobguiCssValue *) * keyframes->n_properties;
      
      keyframes->values = g_realloc (keyframes->values, sizeof (BobguiCssValue *) * keyframes->n_keyframes * keyframes->n_properties);
  ...
  if (keyframes->variables)
    {
      keyframes->variables = g_realloc (keyframes->variables, sizeof (BobguiCssVariableSet *) * keyframes->n_keyframes);
...
static guint
bobgui_css_keyframes_lookup_property (BobguiCssKeyframes *keyframes,
                                   guint            property_id)
{
  guint p;
      ...
      if (p + 1 < keyframes->n_properties)
        {
          memmove (&KEYFRAMES_VALUE (keyframes, keyframes->n_keyframes - 1, p + 1),
                   ...
                   sizeof (BobguiCssValue *) * (keyframes->n_properties - p - 1));
        }
      ...
      for (k = keyframes->n_keyframes - 2; k >= 0; k--)
        {
          memmove (&KEYFRAMES_VALUE (keyframes, k, p + 1),
                   ...
                   sizeof (BobguiCssValue *) * old_n_properties);
          KEYFRAMES_VALUE (keyframes, k, p) = NULL;
        }
    }
  else
    {
      keyframes->values = g_new0 (BobguiCssValue *, keyframes->n_keyframes);
    }
...
static void
bobgui_css_keyframes_register_variable (BobguiCssKeyframes *keyframes,
                                     int              variable_id)
{
  guint p;
...
bobgui_css_keyframes_alloc (void)
{
  BobguiCssKeyframes *keyframes;

  keyframes = g_new0 (BobguiCssKeyframes, 1);
...
static BobguiCssKeyframes *
bobgui_css_keyframes_new (void)
{
  BobguiCssKeyframes *keyframes;

  keyframes = bobgui_css_keyframes_alloc ();

  bobgui_css_keyframes_add_keyframe (keyframes, 0);
  bobgui_css_keyframes_add_keyframe (keyframes, 1);

  return keyframes;
}

static gboolean
keyframes_set_value (BobguiCssKeyframes     *keyframes,
                     guint                k,
                     ...
                     BobguiCssValue         *value)
{
  ...
  if (!_bobgui_css_style_property_is_animated (property))
    return FALSE;

  p = bobgui_css_keyframes_lookup_property (keyframes, _bobgui_css_style_property_get_id (property));
  
  if (KEYFRAMES_VALUE (keyframes, k, p))
    bobgui_css_value_unref (KEYFRAMES_VALUE (keyframes, k, p));

  KEYFRAMES_VALUE (keyframes, k, p) = bobgui_css_value_ref (value);

  return TRUE;
}

static gboolean
bobgui_css_keyframes_parse_declaration (BobguiCssKeyframes *keyframes,
                                     guint            k,
                                     BobguiCssParser    *parser)
{
  ...
  if (name == NULL)
    {
      if (!bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_EOF))
        bobgui_css_parser_error_syntax (parser, "Expected a property name");
      return FALSE;
    }
  ...
  if (name[0] == '-' && name[1] == '-')
    {
      ...
      if (!bobgui_css_parser_try_token (parser, BOBGUI_CSS_TOKEN_COLON))
        {
          bobgui_css_parser_error_syntax (parser, "Expected a ':'");
          g_free (name);
          return FALSE;
        }

      var_value = bobgui_css_parser_parse_value_into_token_stream (parser);
      if (var_value == NULL)
        {
          g_free (name);
          return FALSE;
      ...
      if (!keyframes->variables)
        keyframes->variables = g_new0 (BobguiCssVariableSet *, keyframes->n_keyframes);

      if (!keyframes->variables[k])
        keyframes->variables[k] = bobgui_css_variable_set_new ();

      pool = bobgui_css_custom_property_pool_get ();
      id = bobgui_css_custom_property_pool_add (pool, name);
      bobgui_css_keyframes_register_variable (keyframes, id);

      bobgui_css_variable_value_taint (var_value);

      bobgui_css_variable_set_add (keyframes->variables[k], id, var_value);

      bobgui_css_custom_property_pool_unref (pool, id);
  ...
  if (property == NULL)
    {
      bobgui_css_parser_error_value (parser, "No property named '%s'", name);
      g_free (name);
      return FALSE;
    }
  ...
  if (bobgui_css_parser_has_references (parser))
    {
      BobguiCssVariableValue *var_value;
          ...
          for (i = 0; i < n; i++)
            {
              BobguiCssValue *child =
                _bobgui_css_reference_value_new (property,
                                              var_value,
                                              bobgui_css_parser_get_file (parser));
    ...
    {
      value = _bobgui_style_property_parse_value (property, parser);
      if (value == NULL)
        return FALSE;
    }
  ...
  if (BOBGUI_IS_CSS_SHORTHAND_PROPERTY (property))
    {
      ...
      if (!animatable)
        bobgui_css_parser_error_value (parser, "shorthand '%s' cannot be animated", _bobgui_style_property_get_name (property));
    }
  else if (BOBGUI_IS_CSS_STYLE_PROPERTY (property))
    {
      if (!keyframes_set_value (keyframes, k, BOBGUI_CSS_STYLE_PROPERTY (property), value))
        bobgui_css_parser_error_value (parser, "Cannot animate property '%s'", _bobgui_style_property_get_name (property));
    }
  else
    {
...
static guint
bobgui_css_keyframes_copy_keyframe (BobguiCssKeyframes *keyframes,
                                 guint            k,
                                 double           progress)
{
  guint l = bobgui_css_keyframes_add_keyframe (keyframes, progress);

  if (l <= k)
    k++;

  if (keyframes->variables && keyframes->variables[k])
    keyframes->variables[l] = bobgui_css_variable_set_copy (keyframes->variables[k]);

  for (guint p = 0; p < keyframes->n_properties; p++)
    {
      BobguiCssValue *value = KEYFRAMES_VALUE (keyframes, k, p);
      if (value)
        KEYFRAMES_VALUE (keyframes, l, p) = bobgui_css_value_ref (value);
    }
...
static gboolean
bobgui_css_keyframes_parse_block (BobguiCssKeyframes *keyframes,
                               guint            k,
                               BobguiCssParser    *parser)
{
  if (!bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_OPEN_CURLY))
    {
      bobgui_css_parser_error_syntax (parser, "Expected '{'");
      return FALSE;
    }
  ...
  while (!bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_EOF))
    {
      ...
      bobgui_css_keyframes_parse_declaration (keyframes, k, parser);
      bobgui_css_parser_end_block (parser);
    }

  if (keyframes->variables && keyframes->variables[k])
    bobgui_css_variable_set_resolve_cycles (keyframes->variables[k]);
...
BobguiCssKeyframes *
_bobgui_css_keyframes_parse (BobguiCssParser *parser)
{
  BobguiCssKeyframes *keyframes;
  double progress;
  guint k;
  ...
  keyframes = bobgui_css_keyframes_new ();
  ...
  while (!bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_EOF))
    {
      ...
      while (TRUE)
        {
          if (bobgui_css_parser_try_ident (parser, "from"))
            progress = 0;
          else if (bobgui_css_parser_try_ident (parser, "to"))
            progress = 1;
          else if (bobgui_css_parser_consume_percentage (parser, &progress))
            {
              if (progress < 0 || progress > 100)
                {
                  /* XXX: should we skip over the block here? */
                  bobgui_css_parser_error_value (parser, "percentages must be between 0%% and 100%%");
                  g_clear_pointer (&keyframes, _bobgui_css_keyframes_unref);
                  goto done;
                }
              progress /= 100;
            }
            ...
            {
              bobgui_css_parser_error_value (parser, "failed to parse keyframe selector");
              g_clear_pointer (&keyframes, _bobgui_css_keyframes_unref);
              goto done;
            }
      ...
      k = bobgui_css_keyframes_add_keyframe (keyframes, g_array_index (selectors, double, 0));

      if (!bobgui_css_keyframes_parse_block (keyframes, k, parser))
        {
          g_clear_pointer (&keyframes, _bobgui_css_keyframes_unref);
          goto done;
        }
      ...
      for (guint i = 1; i < selectors->len; i++)
        k = bobgui_css_keyframes_copy_keyframe (keyframes, k, g_array_index (selectors, double, i));
    }
...
compare_property_by_name (gconstpointer a,
                          ...
                          gpointer      data)
{
  BobguiCssKeyframes *keyframes = data;

  return strcmp (_bobgui_style_property_get_name (BOBGUI_STYLE_PROPERTY (
                    _bobgui_css_style_property_lookup_by_id (keyframes->property_ids[*(const guint *) a]))),
                 _bobgui_style_property_get_name (BOBGUI_STYLE_PROPERTY (
                    _bobgui_css_style_property_lookup_by_id (keyframes->property_ids[*(const guint *) b]))));
}
...
compare_custom_property_ids (gconstpointer a, gconstpointer b, gpointer user_data)
{
  ...
  name1 = bobgui_css_custom_property_pool_get_name (pool, id1);
  name2 = bobgui_css_custom_property_pool_get_name (pool, id2);

  return strcmp (name1, name2);
}

void
_bobgui_css_keyframes_print (BobguiCssKeyframes *keyframes,
                          GString         *string)
{
      ...
      for (p = 0; p < keyframes->n_properties; p++)
        {
          ...
          g_string_append_printf (string, "    %s: ", _bobgui_style_property_get_name (
                                                        BOBGUI_STYLE_PROPERTY (
                                                          _bobgui_css_style_property_lookup_by_id (
                                                            keyframes->property_ids[sorted[p]]))));
          bobgui_css_value_print (KEYFRAMES_VALUE (keyframes, k, sorted[p]), string);
          ...
          for (p = 0; p < keyframes->n_variables; p++)
            {
              ...
              BobguiCssVariableValue *value =
                bobgui_css_variable_set_lookup (keyframes->variables[k], variable_id, NULL);
              const char *name;
              ...
              name = bobgui_css_custom_property_pool_get_name (pool, variable_id);
...
BobguiCssKeyframes *
_bobgui_css_keyframes_compute (BobguiCssKeyframes  *keyframes,
                            BobguiStyleProvider *provider,
                            BobguiCssStyle      *style,
                            BobguiCssStyle      *parent_style)
{
  BobguiCssComputeContext context = { NULL, };
  BobguiCssKeyframes *resolved;
  guint k, p;

  g_return_val_if_fail (keyframes != NULL, NULL);
  g_return_val_if_fail (BOBGUI_IS_STYLE_PROVIDER (provider), NULL);
  g_return_val_if_fail (BOBGUI_IS_CSS_STYLE (style), NULL);
  g_return_val_if_fail (parent_style == NULL || BOBGUI_IS_CSS_STYLE (parent_style), NULL);

  resolved = bobgui_css_keyframes_alloc ();
  ...
  resolved->values = g_new0 (BobguiCssValue *, resolved->n_keyframes * resolved->n_properties);
      ...
      for (k = 0; k < resolved->n_keyframes; k++)
        {
          ...
          KEYFRAMES_VALUE (resolved, k, p) =  bobgui_css_value_compute (KEYFRAMES_VALUE (keyframes, k, p),
  ...
  if (keyframes->variables)
    {
      resolved->variables = g_new0 (BobguiCssVariableSet *, resolved->n_keyframes);

      for (k = 0; k < resolved->n_keyframes; k++)
        {
          if (keyframes->variables[k])
            resolved->variables[k] = bobgui_css_variable_set_ref (keyframes->variables[k]);
        }
    }
  else
...
guint
_bobgui_css_keyframes_get_n_properties (BobguiCssKeyframes *keyframes)
{
...
guint
_bobgui_css_keyframes_get_property_id (BobguiCssKeyframes *keyframes,
                                    guint            id)
{
  g_return_val_if_fail (keyframes != NULL, 0);
...
BobguiCssValue *
_bobgui_css_keyframes_get_value (BobguiCssKeyframes *keyframes,
                              guint            id,
                              double           progress,
                              BobguiCssValue     *default_value)
{
  BobguiCssValue *start_value, *end_value, *result;
  ...
  for (k = 0; k < keyframes->n_keyframes; k++)
    {
      ...
      if (keyframes->keyframe_progress[k] == progress)
        {
          return bobgui_css_value_ref (KEYFRAMES_VALUE (keyframes, k, id));
        }
      else if (keyframes->keyframe_progress[k] < progress)
        {
  ...
  result = bobgui_css_value_transition (start_value,
                                     end_value,
  ...
  if (result == NULL)
    return bobgui_css_value_ref (start_value);

  return result;
}

guint
_bobgui_css_keyframes_get_n_variables (BobguiCssKeyframes *keyframes)
{
  g_return_val_if_fail (keyframes != NULL, 0);
...
int
_bobgui_css_keyframes_get_variable_id (BobguiCssKeyframes *keyframes,
                                    guint            id)
{
  g_return_val_if_fail (keyframes != NULL, 0);
...
_bobgui_css_keyframes_get_variable (BobguiCssKeyframes     *keyframes,
                                 guint                id,
                                 double               progress,
                                 BobguiCssVariableValue *default_value)
{
  BobguiCssVariableValue *start_value, *end_value, *result;
  double start_progress, end_progress;
  int variable_id;
  guint k;
  ...
  for (k = 0; k < keyframes->n_keyframes; k++)
    {
      ...
      value = bobgui_css_variable_set_lookup (keyframes->variables[k], variable_id, NULL);
      ...
      if (keyframes->keyframe_progress[k] == progress)
        {
          return bobgui_css_variable_value_ref (value);
        }
      else if (keyframes->keyframe_progress[k] < progress)
        {
          start_value = value;
  ...
  result = bobgui_css_variable_value_transition (start_value,
                                              end_value,
                                              progress);

  /* XXX: Dear spec, what's the correct thing to do here? */
  if (result == NULL)
    return start_value ? bobgui_css_variable_value_ref (start_value) : NULL;
