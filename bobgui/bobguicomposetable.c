...
#define BOBGUI_COMPOSE_TABLE_MAGIC "BobguiComposeTable"
#define BOBGUI_COMPOSE_TABLE_VERSION (4)

extern const BobguiComposeTable builtin_compose_table;
...
static BobguiComposeParser *
parser_new (void)
{
  BobguiComposeParser *parser;

  parser = g_new (BobguiComposeParser, 1);
...
static void
parser_free (BobguiComposeParser *parser)
{
...
static void parser_parse_file (BobguiComposeParser *parser,
                               const char       *path);

char *
bobgui_compose_table_get_x11_compose_file_dir (void)
{
...
#else
  compose_file_dir = g_build_filename (_bobgui_get_datadir (), "X11", "locale", NULL);
#endif
      ...
      if (*p != '%')
      ...
      else
        {
          ...
          switch (p[1])
            ...
            case 'L':
              p++;
              x11_compose_file_dir = bobgui_compose_table_get_x11_compose_file_dir ();
...
add_sequence (gunichar   *sequence,
              ...
              gpointer    data)
{
  BobguiComposeParser *parser = data;
  gunichar *seq;
...
static void
parser_add_default_sequences (BobguiComposeParser *parser)
{
  const BobguiComposeTable *table = &builtin_compose_table;

  bobgui_compose_table_foreach (table, add_sequence, parser);
}

static void
parser_handle_include (BobguiComposeParser *parser,
                       const char       *line)
{
  const char *p;
...
static void
parser_parse_line (BobguiComposeParser *parser,
                   const char       *line)
{
...
static void
parser_read_file (BobguiComposeParser *parser,
                  const char       *compose_file)
{
  char *contents = NULL;
  char **lines = NULL;
 ...
 */
static void
parser_remove_duplicates (BobguiComposeParser *parser)
{
  GHashTableIter iter;
  gunichar *sequence;
  char *value;
  ...
  while (g_hash_table_iter_next (&iter, (gpointer *)&sequence, (gpointer *)&value))
    {
      ...
      if (bobgui_check_algorithmically (keysyms, n_compose, output))
        {
...
static void
parser_compute_max_compose_len (BobguiComposeParser *parser,
                                int              *max_compose_len,
                                int              *n_first,
                                int              *size)
{
...
guint32
bobgui_compose_table_data_hash (const guint16 *data,
                             int            max_seq_len,
                             int            n_seqs)
{
...
static char *
bobgui_compose_hash_get_cache_path (guint32 hash)
{
  char *basename = NULL;
  char *dir = NULL;
  char *path = NULL;
  ...
  dir = g_build_filename (g_get_user_cache_dir (), "bobgui-4.0", "compose", NULL);
...
static char *
bobgui_compose_table_serialize (BobguiComposeTable *compose_table,
                             gsize           *count)
{
  char *p, *contents;
  gsize header_length, total_length;
  guint16 bytes;
  const char *header = BOBGUI_COMPOSE_TABLE_MAGIC;
  const guint16 version = BOBGUI_COMPOSE_TABLE_VERSION;
...
static BobguiComposeTable *
bobgui_compose_table_load_cache (const char *compose_file,
                              gboolean   *found_old_cache)
{
  ...
  guint16 *data = NULL;
  char *char_data = NULL;
  BobguiComposeTable *retval;

  *found_old_cache = FALSE;

  hash = g_str_hash (compose_file);
  if ((path = bobgui_compose_hash_get_cache_path (hash)) == NULL)
    return NULL;
...
#define GET_GUINT16(elt) \
  ...
  p = contents;
  if (g_ascii_strncasecmp (p, BOBGUI_COMPOSE_TABLE_MAGIC,
                           strlen (BOBGUI_COMPOSE_TABLE_MAGIC)) != 0)
    {
      g_warning ("The file is not a BobguiComposeTable cache file %s", path);
      goto out_load_cache;
    }

  p += strlen (BOBGUI_COMPOSE_TABLE_MAGIC);
  ...
  GET_GUINT16 (version);
  if (version != BOBGUI_COMPOSE_TABLE_VERSION)
    {
      if (version < BOBGUI_COMPOSE_TABLE_VERSION)
  ...
  retval = g_new0 (BobguiComposeTable, 1);
  retval->data = data;
...
static void
bobgui_compose_table_save_cache (BobguiComposeTable *compose_table)
{
  char *path = NULL;
  char *contents = NULL;
  GError *error = NULL;
  ...
  if ((path = bobgui_compose_hash_get_cache_path (compose_table->id)) == NULL)
    return;

  contents = bobgui_compose_table_serialize (compose_table, &length);
  if (contents == NULL)
    {
...
static BobguiComposeTable *
parser_get_compose_table (BobguiComposeParser *parser)
{
  guint16 *data;
      ...
      if (g_utf8_strlen (value, -1) == 1 &&
      ...
      else
        {
          ...
          if (char_data->len >= 0x8000)
            {
              g_warning ("BOBGUI can't handle compose tables this large (%s)", parser->compose_file ? parser->compose_file : "");
              g_free (data);
  ...
  table = g_new0 (BobguiComposeTable, 1);
  table->data = data;
...
static void
parser_parse_file (BobguiComposeParser *parser,
                   const char       *compose_file)
{
  char *path;
...
BobguiComposeTable *
bobgui_compose_table_parse (const char *compose_file,
                         gboolean   *found_include)
{
  BobguiComposeParser *parser;
  BobguiComposeTable *compose_table;
...
rewrite_compose_file (const char *compose_file)
{
  static const char *prefix =
    "# BOBGUI has rewritten this file to add the line:\n"
    "\n"
    "include \"%L\"\n"
    "\n"
    "# This is necessary to add your own Compose sequences\n"
    "# in addition to the builtin sequences of BOBGUI. If this\n"
...
BobguiComposeTable *
bobgui_compose_table_new_with_file (const char *compose_file)
{
  BobguiComposeTable *compose_table;
  ...
  compose_table = bobgui_compose_table_load_cache (compose_file, &found_old_cache);
  if (compose_table != NULL)
    return compose_table;

parse:
  compose_table = bobgui_compose_table_parse (compose_file, &found_include);
  ...
  if (found_old_cache && !found_include &&
      compose_table != NULL && compose_table->n_sequences < 100)
    {
      if (rewrite_compose_file (compose_file))
        {
          g_warning ("\nSince BOBGUI 4.4, Compose files replace the builtin\n"
  ...
  if (compose_table != NULL)
    bobgui_compose_table_save_cache (compose_table);

  return compose_table;
}

BobguiComposeTable *
bobgui_compose_table_new_with_data (const guint16 *data,
                                 int            max_seq_len,
                                 int            n_seqs)
{
  BobguiComposeParser *parser;
...
static guint
bobgui_compose_key_flag (guint key)
{
...
compare_seq (const void *key, const void *value)
{
  ...
  while (keysyms[i])
    {
      guint keysym = seq[i];
      guint flag = bobgui_compose_key_flag (keysym);
 ...
 */
gboolean
bobgui_compose_table_check (const BobguiComposeTable *table,
...
void
bobgui_compose_table_get_prefix (const BobguiComposeTable *table,
                              const guint           *compose_buffer,
                              int                    n_compose,
                              int                   *prefix)
{
...
void
bobgui_compose_table_foreach (const BobguiComposeTable      *table,
                           BobguiComposeSequenceCallback  callback,
                           gpointer                    data)
{
  int index_stride = table->max_seq_len + 2;
  gunichar *sequence;
...
gboolean
bobgui_check_algorithmically (const guint *compose_buffer,
                           int          n_compose,
                           GString     *output)
