...
static gboolean
bobgui_fnmatch_intern (const char *pattern,
		    const char *string,
	...
	case '*':
...

	  {
	    ...
	    for (p = last_p; nc != '\0';)
	      {
		if ((c == '[' || nc == c) &&
		    bobgui_fnmatch_intern (p, last_n, component_start, no_leading_period, casefold))
		  return TRUE;
 ...
 *  it matches, nonzero if not.
 *
 * BOBGUI+ used to use a old version of GNU fnmatch() that was buggy
 ...
 */
gboolean
_bobgui_fnmatch (const char *pattern,
	      const char *string,
	      gboolean no_leading_period,
              gboolean casefold)
{
  return bobgui_fnmatch_intern (pattern, string, TRUE, no_leading_period, casefold);
}
 ...
 */
char *
_bobgui_make_ci_glob_pattern (const char *pattern)
{
  GString *s;
