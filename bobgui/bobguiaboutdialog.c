 ...
 *   <img alt="An example BobguiAboutDialog" src="aboutdialog.png">
 * </picture>
 *
 * About dialogs often contain links and email addresses. `BobguiAboutDialog`
 * displays these as clickable links. By default, it calls [method@Bobgui.FileLauncher.launch]
 * when a user clicks one. The behaviour can be overridden with the
 * [signal@Bobgui.AboutDialog::activate-link] signal.
 *
 * To specify a person with an email address, use a string like
 * `Edgar Allan Poe <edgar@poe.com>`. To specify a website with a title,
 * use a string like `BOBGUI team https://www.bobgui.org`.
 *
 * To make constructing an about dialog as convenient as possible, you can
 * use the function [func@Bobgui.show_about_dialog] which constructs and shows
 * a dialog and keeps it around so that it can be shown again.
 *
 * Note that BOBGUI sets a default title of `_("About %s")` on the dialog
 ...
 *
 * `BobguiAboutDialog` supports the following keyboard shortcuts:
 *
 * - <kbd>Escape</kbd> closes the window.
 *
 * ## CSS nodes
 *
 * `BobguiAboutDialog` has a single CSS node with the name `window` and style
 * class `.aboutdialog`.
...
/* LicenseInfo for each BobguiLicense type; keep in the same order as the enumeration */
static const LicenseInfo bobgui_license_info [] = {
  { N_("License"), NULL },
...
G_STATIC_ASSERT (G_N_ELEMENTS (bobgui_license_info) - 1 == BOBGUI_LICENSE_0BSD);
...
typedef struct _BobguiAboutDialogClass BobguiAboutDialogClass;

struct _BobguiAboutDialog
{
  ...
  BobguiWidget *stack;
  BobguiWidget *stack_switcher;

  BobguiWidget *logo_image;
  BobguiWidget *name_label;
  BobguiWidget *version_label;
  BobguiWidget *comments_label;
  BobguiWidget *copyright_label;
  BobguiWidget *license_label;
  BobguiWidget *website_label;

  BobguiWidget *credits_page;
  BobguiWidget *license_page;
  BobguiWidget *system_page;

  BobguiWidget *credits_grid;
  BobguiWidget *license_view;
  BobguiWidget *system_view;

  GPtrArray *visited_links;

  BobguiCssNode *link_node;
  BobguiCssNode *visited_link_node;
...
struct _BobguiAboutDialogClass
{
  BobguiWindowClass parent_class;

  gboolean (*activate_link) (BobguiAboutDialog *dialog,
                             const char     *uri);
};
...
static void                 bobgui_about_dialog_finalize       (GObject            *object);
static void                 bobgui_about_dialog_get_property   (GObject            *object,
                                                             guint               prop_id,
                                                             GValue             *value,
                                                             GParamSpec         *pspec);
static void                 bobgui_about_dialog_set_property   (GObject            *object,
                                                             guint               prop_id,
                                                             const GValue       *value,
                                                             GParamSpec         *pspec);
static void                 update_name_version             (BobguiAboutDialog     *about);
static void                 follow_if_link                  (BobguiAboutDialog     *about,
                                                             BobguiTextView        *text_view,
                                                             BobguiTextIter        *iter);
static void                 set_cursor_if_appropriate       (BobguiAboutDialog     *about,
                                                             BobguiTextView        *text_view,
                                                             int                 x,
                                                             int                 y);
static void                 populate_credits_page           (BobguiAboutDialog     *about);
static void                 populate_license_page           (BobguiAboutDialog     *about);
static void                 populate_system_page            (BobguiAboutDialog     *about);
static gboolean             bobgui_about_dialog_activate_link  (BobguiAboutDialog     *about,
                                                             const char         *uri);
static gboolean             emit_activate_link              (BobguiAboutDialog     *about,
							     const char         *uri);
static gboolean             text_view_key_pressed           (BobguiEventController *controller,
                                                             guint               keyval,
                                                             guint               keycode,
                                                             GdkModifierType     state,
							     BobguiAboutDialog     *about);
static void                 text_view_released              (BobguiGestureClick *press,
                                                             int                   n,
                                                             double                x,
                                                             double                y,
							     BobguiAboutDialog       *about);
static void                 text_view_motion                (BobguiEventControllerMotion *motion,
                                                             double                    x,
                                                             double                    y,
							     BobguiAboutDialog           *about);
...
G_DEFINE_TYPE (BobguiAboutDialog, bobgui_about_dialog, BOBGUI_TYPE_WINDOW)

static gboolean
stack_visible_child_notify (BobguiStack       *stack,
                            GParamSpec     *pspec,
                            BobguiAboutDialog *about)
{
  BobguiWidget *child;

  child = bobgui_stack_get_visible_child (stack);
...
static void
bobgui_about_dialog_map (BobguiWidget *widget)
{
  BobguiAboutDialog *about = BOBGUI_ABOUT_DIALOG (widget);

  if (bobgui_widget_get_visible (about->stack_switcher))
    bobgui_widget_grab_focus (bobgui_widget_get_first_child (about->stack_switcher));

  BOBGUI_WIDGET_CLASS (bobgui_about_dialog_parent_class)->map (widget);
}

static void
bobgui_about_dialog_class_init (BobguiAboutDialogClass *klass)
{
  ...
  object_class->set_property = bobgui_about_dialog_set_property;
  object_class->get_property = bobgui_about_dialog_get_property;

  object_class->finalize = bobgui_about_dialog_finalize;

  widget_class->map = bobgui_about_dialog_map;

  klass->activate_link = bobgui_about_dialog_activate_link;

  /**
   * BobguiAboutDialog::activate-link:
  ...
  signals[ACTIVATE_LINK] =
    g_signal_new (I_("activate-link"),
                  ...
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (BobguiAboutDialogClass, activate_link),
  ...
  /**
   * BobguiAboutDialog:program-name:
   *
   ...
   * [property@Bobgui.AboutDialog:license-type] property is set to
   * [enum@Bobgui.License.custom] as a side effect.
   *
  ...
  /**
   * BobguiAboutDialog:system-information:
   *
  ...
  /**
   * BobguiAboutDialog:license-type:
   *
   * The license of the program.
   *
   * The `BobguiAboutDialog` will automatically fill out a standard disclaimer
   * and link the user to the appropriate online resource for the license
   * text.
   *
   * If [enum@Bobgui.License.unknown] is used, the link used will be the same
   * specified in the [property@Bobgui.AboutDialog:website] property.
   *
   * If [enum@Bobgui.License.custom] is used, the current contents of the
   * [property@Bobgui.AboutDialog:license] property are used.
   *
   * For any other [enum@Bobgui.License] value, the contents of the
   * [property@Bobgui.AboutDialog:license] property are also set by
  ...
  /**
   * BobguiAboutDialog:website-label:
   *
  ...
  /**
   * BobguiAboutDialog:translator-credits:
   *
  ...
  /**
   * BobguiAboutDialog:logo:
   *
   * A logo for the about box.
   *
   * If it is `NULL`, the default window icon set with
   * [func@Bobgui.Window.set_default_icon_name] will be used.
   */
  props[PROP_LOGO] =
  ...
  /**
   * BobguiAboutDialog:logo-icon-name:
   *
   * A named icon to use as the logo for the about box.
   *
   * This property overrides the [property@Bobgui.AboutDialog:logo] property.
   */
  ...
  /**
   * BobguiAboutDialog:wrap-license:
   *
  ...
  bobgui_widget_class_add_binding_action (widget_class, GDK_KEY_Escape, 0, "window.close", NULL);

  /* Bind class to template
   */
  bobgui_widget_class_set_template_from_resource (widget_class,
					       "/org/bobgui/libbobgui/ui/bobguiaboutdialog.ui");

  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, stack);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, stack_switcher);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, logo_image);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, name_label);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, version_label);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, comments_label);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, copyright_label);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, license_label);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, website_label);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, credits_page);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, license_page);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, system_page);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, credits_grid);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, license_view);
  bobgui_widget_class_bind_template_child (widget_class, BobguiAboutDialog, system_view);

  bobgui_widget_class_bind_template_callback (widget_class, emit_activate_link);
  bobgui_widget_class_bind_template_callback (widget_class, text_view_released);
  bobgui_widget_class_bind_template_callback (widget_class, text_view_motion);
  bobgui_widget_class_bind_template_callback (widget_class, text_view_key_pressed);
  bobgui_widget_class_bind_template_callback (widget_class, stack_visible_child_notify);
}

static gboolean
emit_activate_link (BobguiAboutDialog *about,
                    const char     *uri)
{
...
static void
update_stack_switcher_visibility (BobguiAboutDialog *about)
{
  BobguiStackPage *page;
  gboolean any_visible = FALSE;

  page = bobgui_stack_get_page (BOBGUI_STACK (about->stack), about->credits_page);
  any_visible |= bobgui_stack_page_get_visible (page);
  page = bobgui_stack_get_page (BOBGUI_STACK (about->stack), about->license_page);
  any_visible |= bobgui_stack_page_get_visible (page);
  page = bobgui_stack_get_page (BOBGUI_STACK (about->stack), about->system_page);
  any_visible |= bobgui_stack_page_get_visible (page);

  bobgui_widget_set_visible (about->stack_switcher, any_visible);
}

static void
update_license_button_visibility (BobguiAboutDialog *about)
{
  BobguiStackPage *page;

  page = bobgui_stack_get_page (BOBGUI_STACK (about->stack), about->license_page);
  bobgui_stack_page_set_visible (page,
                              about->license_type == BOBGUI_LICENSE_CUSTOM &&
                              about->license != NULL &&
                              about->license[0] != '\0');
...
static void
update_system_button_visibility (BobguiAboutDialog *about)
{
  BobguiStackPage *page;
...
static void
update_credits_button_visibility (BobguiAboutDialog *about)
{
  gboolean show;
  BobguiStackPage *page;
  ...
  bobgui_stack_page_set_visible (page, show);

  update_stack_switcher_visibility (about);
}

static void
update_links_cb (BobguiAboutDialog *about)
{
  BobguiCssStyle *style;
  GdkRGBA link_color, visited_link_color;
  GSList *l;

  style = bobgui_css_node_get_style (about->link_node);
  link_color = *bobgui_css_color_value_get_rgba (style->used->color);

  style = bobgui_css_node_get_style (about->visited_link_node);
  visited_link_color = *bobgui_css_color_value_get_rgba (style->used->color);

  for (l = about->link_tags; l != NULL; l = l->next)
    {
      BobguiTextTag *tag = l->data;
      GdkRGBA color;
...
static void
bobgui_about_dialog_init (BobguiAboutDialog *about)
{
  BobguiCssNode *node;
  ...
  about->license_type = BOBGUI_LICENSE_UNKNOWN;

  about->visited_links = g_ptr_array_new_with_free_func (g_free);

  bobgui_widget_init_template (BOBGUI_WIDGET (about));

  bobgui_stack_set_visible_child_name (BOBGUI_STACK (about->stack), "main");
  update_stack_switcher_visibility (about);

  /* force defaults */
  bobgui_about_dialog_set_program_name (about, NULL);
  bobgui_about_dialog_set_logo (about, NULL);

  node = bobgui_widget_get_css_node (BOBGUI_WIDGET (about));
  state = bobgui_css_node_get_state (node);

  about->link_node = bobgui_css_node_new ();
  bobgui_css_node_set_name (about->link_node, g_quark_from_static_string ("link"));
  bobgui_css_node_set_parent (about->link_node, node);
  bobgui_css_node_set_state (about->link_node, state | BOBGUI_STATE_FLAG_LINK);
  g_signal_connect (about->link_node, "style-changed",
                    G_CALLBACK (link_style_changed_cb), about);
  g_object_unref (about->link_node);

  about->visited_link_node = bobgui_css_node_new ();
  bobgui_css_node_set_name (about->visited_link_node, g_quark_from_static_string ("link"));
  bobgui_css_node_set_parent (about->visited_link_node, node);
  bobgui_css_node_set_state (about->visited_link_node, state | BOBGUI_STATE_FLAG_LINK);
...
static void
bobgui_about_dialog_finalize (GObject *object)
{
  BobguiAboutDialog *about = BOBGUI_ABOUT_DIALOG (object);
  ...
  G_OBJECT_CLASS (bobgui_about_dialog_parent_class)->finalize (object);
}

static void
bobgui_about_dialog_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_NAME:
      bobgui_about_dialog_set_program_name (about, g_value_get_string (value));
      break;
    case PROP_VERSION:
      bobgui_about_dialog_set_version (about, g_value_get_string (value));
      break;
    case PROP_COMMENTS:
      bobgui_about_dialog_set_comments (about, g_value_get_string (value));
      break;
    case PROP_WEBSITE:
      bobgui_about_dialog_set_website (about, g_value_get_string (value));
      break;
    case PROP_WEBSITE_LABEL:
      bobgui_about_dialog_set_website_label (about, g_value_get_string (value));
      break;
    case PROP_LICENSE:
      bobgui_about_dialog_set_license (about, g_value_get_string (value));
      break;
    case PROP_SYSTEM_INFORMATION:
      bobgui_about_dialog_set_system_information (about, g_value_get_string (value));
      break;
    case PROP_LICENSE_TYPE:
      bobgui_about_dialog_set_license_type (about, g_value_get_enum (value));
      break;
    case PROP_COPYRIGHT:
      bobgui_about_dialog_set_copyright (about, g_value_get_string (value));
      break;
    case PROP_LOGO:
      bobgui_about_dialog_set_logo (about, g_value_get_object (value));
      break;
    case PROP_AUTHORS:
      bobgui_about_dialog_set_authors (about, (const char **)g_value_get_boxed (value));
      break;
    case PROP_DOCUMENTERS:
      bobgui_about_dialog_set_documenters (about, (const char **)g_value_get_boxed (value));
      break;
    case PROP_ARTISTS:
      bobgui_about_dialog_set_artists (about, (const char **)g_value_get_boxed (value));
      break;
    case PROP_TRANSLATOR_CREDITS:
      bobgui_about_dialog_set_translator_credits (about, g_value_get_string (value));
      break;
    case PROP_LOGO_ICON_NAME:
      bobgui_about_dialog_set_logo_icon_name (about, g_value_get_string (value));
      break;
    case PROP_WRAP_LICENSE:
      bobgui_about_dialog_set_wrap_license (about, g_value_get_boolean (value));
      break;
    default:
...
static void
bobgui_about_dialog_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ARTISTS:
      ...
      break;
    case PROP_LOGO:
      if (bobgui_image_get_storage_type (BOBGUI_IMAGE (about->logo_image)) == BOBGUI_IMAGE_PAINTABLE)
        g_value_set_object (value, bobgui_image_get_paintable (BOBGUI_IMAGE (about->logo_image)));
      else
        g_value_set_object (value, NULL);
      break;
    case PROP_LOGO_ICON_NAME:
      if (bobgui_image_get_storage_type (BOBGUI_IMAGE (about->logo_image)) == BOBGUI_IMAGE_ICON_NAME)
        g_value_set_string (value, bobgui_image_get_icon_name (BOBGUI_IMAGE (about->logo_image)));
      else
...
static gboolean
bobgui_about_dialog_activate_link (BobguiAboutDialog *about,
                                const char     *uri)
{
  ...
  bobgui_uri_launcher_launch (launcher, BOBGUI_WINDOW (about), NULL, NULL, NULL);
...
static void
update_website (BobguiAboutDialog *about)
{
  bobgui_widget_set_visible (about->website_label, TRUE);
  ...
  if (about->website_url)
    {
      ...
      bobgui_label_set_markup (BOBGUI_LABEL (about->website_label), markup);
      g_free (markup);

      tooltip = g_uri_unescape_string (about->website_url, NULL);
      bobgui_widget_set_tooltip_text (about->website_label, tooltip);
      g_free (tooltip);
    }
  else
    {
      if (about->website_text)
        bobgui_label_set_text (BOBGUI_LABEL (about->website_label), about->website_text);
      else
        bobgui_widget_set_visible (about->website_label, FALSE);

      bobgui_widget_set_tooltip_text (about->website_label, NULL);
    }
}

/**
 * bobgui_about_dialog_get_program_name:
 ...
 */
const char *
bobgui_about_dialog_get_program_name (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);

  return about->name;
}

static void
update_name_version (BobguiAboutDialog *about)
{
  char *title_string, *name_string;

  title_string = g_strdup_printf (_("About %s"), about->name);
  bobgui_window_set_title (BOBGUI_WINDOW (about), title_string);
  g_free (title_string);

  bobgui_widget_set_visible (about->version_label, about->version != NULL);
  if (about->version != NULL)
    bobgui_label_set_markup (BOBGUI_LABEL (about->version_label), about->version);

  name_string = g_markup_printf_escaped ("<span weight=\"bold\">%s</span>",
                                         about->name);
  bobgui_label_set_markup (BOBGUI_LABEL (about->name_label), name_string);
  g_free (name_string);
}

/**
 * bobgui_about_dialog_set_program_name:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_program_name (BobguiAboutDialog *about,
                                   const char     *name)
{
  char *tmp;

  g_return_if_fail (BOBGUI_IS_ABOUT_DIALOG (about));
...
/**
 * bobgui_about_dialog_get_version:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_version (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_version:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_version (BobguiAboutDialog *about,
                              const char     *version)
{
  char *tmp;
...
/**
 * bobgui_about_dialog_get_copyright:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_copyright (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_copyright:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_copyright (BobguiAboutDialog *about,
                                const char     *copyright)
{
  ...
  bobgui_widget_set_visible (about->copyright_label, about->copyright != NULL);
  if (about->copyright != NULL)
    {
      copyright_string = g_markup_printf_escaped ("<span size=\"small\">%s</span>",
                                                  about->copyright);
      bobgui_label_set_markup (BOBGUI_LABEL (about->copyright_label), copyright_string);
...
/**
 * bobgui_about_dialog_get_comments:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_comments (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_comments:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_comments (BobguiAboutDialog *about,
                               const char     *comments)
{
  char *tmp;
  ...
  if (comments)
    {
      ...
      bobgui_label_set_text (BOBGUI_LABEL (about->comments_label), about->comments);
    }
  else
    {
      about->comments = NULL;
  ...
  bobgui_widget_set_visible (about->comments_label, about->comments != NULL);
  g_free (tmp);
...
/**
 * bobgui_about_dialog_get_license:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_license (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_license:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_license (BobguiAboutDialog *about,
                              const char     *license)
{
  char *tmp;
  ...
  bobgui_widget_set_visible (about->license_label, FALSE);
...
/**
 * bobgui_about_dialog_get_system_information:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_system_information (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_system_information:
 * @about: an about dialog
 ...
 * page is hidden.
 *
 * See [property@Bobgui.AboutDialog:system-information].
 */
void
bobgui_about_dialog_set_system_information (BobguiAboutDialog *about,
...
/**
 * bobgui_about_dialog_get_wrap_license:
 * @about: an about dialog
 *
 ...
 */
gboolean
bobgui_about_dialog_get_wrap_license (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), FALSE);

  return about->wrap_license;
}

/**
 * bobgui_about_dialog_set_wrap_license:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_wrap_license (BobguiAboutDialog *about,
...
/**
 * bobgui_about_dialog_get_website:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_website (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_website:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_website (BobguiAboutDialog *about,
                              const char     *website)
{
  char *tmp;
...
/**
 * bobgui_about_dialog_get_website_label:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_website_label (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_website_label:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_website_label (BobguiAboutDialog *about,
...
/**
 * bobgui_about_dialog_get_authors:
 * @about: an about dialog
 *
 ...
 */
const char * const *
bobgui_about_dialog_get_authors (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_authors:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_authors (BobguiAboutDialog  *about,
                              const char     **authors)
{
  char **tmp;
...
/**
 * bobgui_about_dialog_get_documenters:
 * @about: an about dialog
 *
 ...
 */
const char * const *
bobgui_about_dialog_get_documenters (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_documenters:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_documenters (BobguiAboutDialog *about,
...
/**
 * bobgui_about_dialog_get_artists:
 * @about: an about dialog
 *
 ...
 */
const char * const *
bobgui_about_dialog_get_artists (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_artists:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_artists (BobguiAboutDialog *about,
                              const char    **artists)
{
  char **tmp;
...
/**
 * bobgui_about_dialog_get_translator_credits:
 * @about: an about dialog
 *
 ...
 */
const char *
bobgui_about_dialog_get_translator_credits (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
...
/**
 * bobgui_about_dialog_set_translator_credits:
 * @about: an about dialog
 ...
 *
 * ```c
 * BobguiWidget *about = bobgui_about_dialog_new ();
 *  bobgui_about_dialog_set_translator_credits (BOBGUI_ABOUT_DIALOG (about),
 *                                           _("translator-credits"));
 * ```
 *
 * It is a good idea to use the customary `msgid` “translator-credits”
 * for this purpose, since translators will already know the purpose of
 * that `msgid`, and since `BobguiAboutDialog` will detect if “translator-credits”
 * is untranslated and omit translator credits.
 */
void
bobgui_about_dialog_set_translator_credits (BobguiAboutDialog *about,
 ...
 *   [method@Bobgui.AboutDialog.set_logo_icon_name]
 */
GdkPaintable *
bobgui_about_dialog_get_logo (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);

  if (bobgui_image_get_storage_type (BOBGUI_IMAGE (about->logo_image)) == BOBGUI_IMAGE_PAINTABLE)
    return bobgui_image_get_paintable (BOBGUI_IMAGE (about->logo_image));
  else
    return NULL;
}

/**
 * bobgui_about_dialog_set_logo:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_logo (BobguiAboutDialog *about,
                           GdkPaintable   *logo)
{
  g_return_if_fail (BOBGUI_IS_ABOUT_DIALOG (about));
  ...
  bobgui_image_set_from_paintable (BOBGUI_IMAGE (about->logo_image), logo);
...
/**
 * bobgui_about_dialog_get_logo_icon_name:
 * @about: an about dialog
 *
 * Returns the icon name displayed as logo in the about dialog.
 *
 * Returns: (transfer none) (nullable): the icon name displayed as logo,
 *   or `NULL` if the logo has been set via [method@Bobgui.AboutDialog.set_logo]
 */
const char *
bobgui_about_dialog_get_logo_icon_name (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), NULL);
  ...
  return bobgui_image_get_icon_name (BOBGUI_IMAGE (about->logo_image));
}

/**
 * bobgui_about_dialog_set_logo_icon_name:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_set_logo_icon_name (BobguiAboutDialog *about,
                                     const char     *icon_name)
{
  ...
  bobgui_image_set_from_icon_name (BOBGUI_IMAGE (about->logo_image), icon_name);
...
static void
follow_if_link (BobguiAboutDialog *about,
                BobguiTextView    *text_view,
                BobguiTextIter    *iter)
{
...
static gboolean
text_view_key_pressed (BobguiEventController *controller,
                       guint               keyval,
                       guint               keycode,
                       GdkModifierType     state,
                       BobguiAboutDialog     *about)
{
  ...
  text_view = bobgui_event_controller_get_widget (controller);
  ...
  switch (keyval)
    {
      ...
      case GDK_KEY_KP_Enter:
        ...
        follow_if_link (about, BOBGUI_TEXT_VIEW (text_view), &iter);
        break;
...
static void
text_view_released (BobguiGestureClick *gesture,
                    int                   n_press,
                    double                x,
                    double                y,
                    BobguiAboutDialog       *about)
{
  ...
  text_view = bobgui_event_controller_get_widget (BOBGUI_EVENT_CONTROLLER (gesture));
  buffer = bobgui_text_view_get_buffer (BOBGUI_TEXT_VIEW (text_view));
  ...
  bobgui_text_view_get_iter_at_location (BOBGUI_TEXT_VIEW (text_view), &iter, tx, ty);

  follow_if_link (about, BOBGUI_TEXT_VIEW (text_view), &iter);
}

static void
set_cursor_if_appropriate (BobguiAboutDialog *about,
                           BobguiTextView    *text_view,
                           int             x,
                           int             y)
{
  ...
  bobgui_text_view_get_iter_at_location (text_view, &iter, x, y);
...
static void
text_view_motion (BobguiEventControllerMotion *motion,
                  double                    x,
                  double                    y,
                  BobguiAboutDialog           *about)
{
  ...
  widget = bobgui_event_controller_get_widget (BOBGUI_EVENT_CONTROLLER (motion));

  bobgui_text_view_window_to_buffer_coords (BOBGUI_TEXT_VIEW (widget),
                                         BOBGUI_TEXT_WINDOW_WIDGET,
                                         x, y, &tx, &ty);

  set_cursor_if_appropriate (about, BOBGUI_TEXT_VIEW (widget), tx, ty);
}

static BobguiTextBuffer *
text_buffer_new (BobguiAboutDialog  *about,
		 char           **strings)
{
  char **p;
  char *q0, *q1, *q2, *r1, *r2;
          ...
          if (q1 && q2)
            {
              ...
              if (*q1 == '<')
                {
                  bobgui_text_buffer_insert_at_cursor (buffer, q0, q1 - q0 + 1);
                ...
                {
                  bobgui_text_buffer_insert_at_cursor (buffer, q0, q1 - q0);
                  bobgui_text_buffer_get_end_iter (buffer, &end);
                  link_type = "uri";
                }
              ...
              bobgui_text_buffer_insert_with_tags (buffer, &end, link, -1, tag, NULL);

              g_free (link);
            }
          else
            {
              bobgui_text_buffer_insert_at_cursor (buffer, q0, -1);
              break;
            }
        }

      if (p[1])
        bobgui_text_buffer_insert_at_cursor (buffer, "\n", 1);
    }
  ...
  bobgui_text_buffer_set_enable_undo (buffer, FALSE);

  return buffer;
}

static void
add_credits_section (BobguiAboutDialog  *about,
                     BobguiGrid         *grid,
                     int             *row,
                     char            *title,
                     char           **people)
{
  ...
  bobgui_grid_attach (grid, label, 0, *row, 1, 1);
  ...
  for (p = people; *p; p++)
    {
      ...
      label = bobgui_label_new (str->str);
      bobgui_label_set_use_markup (BOBGUI_LABEL (label), TRUE);
      ...
      bobgui_widget_set_valign (label, BOBGUI_ALIGN_CENTER);
      bobgui_grid_attach (grid, label, 1, *row, 1, 1);
...
static void
populate_credits_page (BobguiAboutDialog *about)
{
  int row;
  ...
  if (about->authors != NULL)
    add_credits_section (about, BOBGUI_GRID (about->credits_grid), &row, _("Created by"), about->authors);

  if (about->documenters != NULL)
    add_credits_section (about, BOBGUI_GRID (about->credits_grid), &row, _("Documented by"), about->documenters);
  ...
  if (about->translator_credits != NULL &&
      ...
      strcmp (about->translator_credits, "translator-credits") != 0)
    {
      ...
      add_credits_section (about, BOBGUI_GRID (about->credits_grid), &row, _("Translated by"), translators);
      g_strfreev (translators);
    }

  if (about->artists != NULL)
    add_credits_section (about, BOBGUI_GRID (about->credits_grid), &row, _("Design by"), about->artists);
  ...
  if (about->credit_sections != NULL)
      ...
      for (cs = about->credit_sections; cs != NULL; cs = cs->next)
	{
	  ...
	  add_credits_section (about, BOBGUI_GRID (about->credits_grid), &row, section->heading, section->people);
	}
    }
}

static void
populate_license_page (BobguiAboutDialog *about)
{
  BobguiTextBuffer *buffer;
  char *strings[2];

  bobgui_text_view_set_wrap_mode (BOBGUI_TEXT_VIEW (about->license_view), about->wrap_license ? BOBGUI_WRAP_WORD : BOBGUI_WRAP_NONE);

  strings[0] = about->license;
  strings[1] = NULL;
  buffer = text_buffer_new (about, strings);
  bobgui_text_view_set_buffer (BOBGUI_TEXT_VIEW (about->license_view), buffer);
  g_object_unref (buffer);
}

static void
populate_system_page (BobguiAboutDialog *about)
{
  BobguiTextBuffer *buffer;
  char *strings[2];
  ...
  strings[1] = NULL;
  buffer = text_buffer_new (about, strings);
  bobgui_text_view_set_buffer (BOBGUI_TEXT_VIEW (about->system_view), buffer);
  g_object_unref (buffer);
}

/**
 * bobgui_about_dialog_new:
 *
 * Creates a new `BobguiAboutDialog`.
 *
 * Returns: a newly created `BobguiAboutDialog`
 */
BobguiWidget *
bobgui_about_dialog_new (void)
{
  return g_object_new (BOBGUI_TYPE_ABOUT_DIALOG, NULL);
}

static gboolean
close_cb (BobguiAboutDialog *about,
          gpointer        user_data)
{
  bobgui_stack_set_visible_child_name (BOBGUI_STACK (about->stack), "main");
  bobgui_widget_set_visible (BOBGUI_WIDGET (about), FALSE);

  return TRUE;
}

/**
 * bobgui_show_about_dialog:
 ...
 */
void
bobgui_show_about_dialog (BobguiWindow   *parent,
                       const char *first_property_name,
                       ...)
{
  static BobguiWidget *global_about_dialog = NULL;
  BobguiWidget *dialog = NULL;
  va_list var_args;

  if (parent)
    dialog = g_object_get_data (G_OBJECT (parent), "bobgui-about-dialog");
  else
...
/**
 * bobgui_about_dialog_set_license_type:
 * @about: an about dialog
 ...
 * [method@Bobgui.AboutDialog.set_license].
 */
void
bobgui_about_dialog_set_license_type (BobguiAboutDialog *about,
                                   BobguiLicense      license_type)
{
  g_return_if_fail (BOBGUI_IS_ABOUT_DIALOG (about));
  g_return_if_fail (license_type >= BOBGUI_LICENSE_UNKNOWN &&
                    license_type < G_N_ELEMENTS (bobgui_license_info));
  ...
  if (about->license_type != license_type)
    {
      ...
      bobgui_widget_set_visible (about->license_label, TRUE);
      /* custom licenses use the contents of the :license property */
      if (about->license_type != BOBGUI_LICENSE_CUSTOM)
        {
          const char *name;
          const char *url;
          char *license_string;
          ...
          name = _(bobgui_license_info[about->license_type].name);
          url = bobgui_license_info[about->license_type].url;
          if (url == NULL)
          ...
          license_string = g_strdup_printf ("<span size=\"small\">%s</span>",
                                            about->license);
          bobgui_label_set_markup (BOBGUI_LABEL (about->license_label), license_string);
...
/**
 * bobgui_about_dialog_get_license_type:
 * @about: an about dialog
 *
 * Retrieves the license type.
 *
 * Returns: a [enum@Bobgui.License] value
 */
BobguiLicense
bobgui_about_dialog_get_license_type (BobguiAboutDialog *about)
{
  g_return_val_if_fail (BOBGUI_IS_ABOUT_DIALOG (about), BOBGUI_LICENSE_UNKNOWN);

  return about->license_type;
}

/**
 * bobgui_about_dialog_add_credit_section:
 * @about: an about dialog
 ...
 */
void
bobgui_about_dialog_add_credit_section (BobguiAboutDialog  *about,
