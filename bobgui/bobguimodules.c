...
static char *
bobgui_trim_string (const char *str)
{
  int len;
...
split_file_list (const char *str)
{
  ...
  while (files[i])
    {
      char *file = bobgui_trim_string (files[i]);
...
get_module_path (void)
{
  ...
  module_path_env = g_getenv ("BOBGUI_PATH");
  exe_prefix = g_getenv ("BOBGUI_EXE_PREFIX");

  if (exe_prefix)
    default_dir = g_build_filename (exe_prefix, "lib", "bobgui-4.0", NULL);
  else
    default_dir = g_build_filename (_bobgui_get_libdir (), "bobgui-4.0", NULL);
 ...
 **/
char **
_bobgui_get_module_path (const char *type)
{
  ...
  for (path = paths; *path; path++)
    {
      result[count++] = g_build_filename (*path, BOBGUI_BINARY_VERSION, BOBGUI_HOST, type, NULL);
      result[count++] = g_build_filename (*path, BOBGUI_BINARY_VERSION, type, NULL);
      result[count++] = g_build_filename (*path, BOBGUI_HOST, type, NULL);
