 ...
 *
 * To create a `BobguiBuilder` from a user interface description, call
 * [ctor@Bobgui.Builder.new_from_file], [ctor@Bobgui.Builder.new_from_resource]
 * or [ctor@Bobgui.Builder.new_from_string].
 *
 * In the (unusual) case that you want to add user interface
 * descriptions from multiple sources to the same `BobguiBuilder` you can
 * call [ctor@Bobgui.Builder.new] to get an empty builder and populate it by
 * (multiple) calls to [method@Bobgui.Builder.add_from_file],
 * [method@Bobgui.Builder.add_from_resource] or
 * [method@Bobgui.Builder.add_from_string].
 *
 * A `BobguiBuilder` holds a reference to all objects that it has constructed
 * and drops these references when it is finalized. This finalization can
 * cause the destruction of non-widget objects or widgets which are not
 * contained in a toplevel window. For toplevel windows constructed by a
 * builder, it is the responsibility of the user to call
 * [method@Bobgui.Window.destroy] to get rid of them and all the widgets
 * they contain.
 *
 * The functions [method@Bobgui.Builder.get_object] and
 * [method@Bobgui.Builder.get_objects] can be used to access the widgets in
 * the interface by the names assigned to them inside the UI description.
 * Toplevel windows returned by these functions will stay around until the
 * user explicitly destroys them with [method@Bobgui.Window.destroy]. Other
 ...
 *
 * ## BobguiBuilder UI Definitions
 *
 * `BobguiBuilder` parses textual descriptions of user interfaces which are
 * specified in XML format. We refer to these descriptions as “BobguiBuilder
 ...
 * calling [method@Bobgui.Builder.set_translation_domain] on the builder.
 * For example:
 *
 * ```xml
 ...
 * the only supported value is “bobgui”) and the “version” attribute specifies
 * the target version in the form “`<major>`.`<minor>`”. `BobguiBuilder` will
 ...
 * been loaded yet, BOBGUI tries to find the `get_type()` function from the
 ...
 * [method@Bobgui.Builder.get_object]. An id is also necessary to use the
 * object as property value in other parts of the UI definition. BOBGUI
 ...
 * ```
 *
 * If the “translatable” attribute is set to a true value, BOBGUI uses
 ...
 *
 * `BobguiBuilder` can parse textual representations for the most common
 * property types:
 *
 * - characters
 * - strings
 ...
 *   e.g., “BOBGUI_INPUT_HINT_EMOJI|BOBGUI_INPUT_HINT_LOWERCASE”, or “emoji|lowercase” or 520).
 ...
 * - textures (can be specified as an object id, a resource path or a filename of an image file to load relative to the Builder file or the CWD if [method@Bobgui.Builder.add_from_string] was used)
 * - GFile (like textures, can be specified as an object id, a URI or a filename of a file to load relative to the Builder file or the CWD if [method@Bobgui.Builder.add_from_string] was used)
 *
 * Objects can be referred to by their name and by default refer to
 * objects declared in the local XML fragment and objects exposed via
 * [method@Bobgui.Builder.expose_object]. In general, `BobguiBuilder` allows
 ...
 * [property@Bobgui.Expander:child]. In this case, the preferred way to
 ...
 * such as [class@Bobgui.Box] instead use the `<child>` element. A `<child>`
 ...
 *
 * Any object type that implements the [iface@Bobgui.Buildable] interface can
 * specify how children may be added to it. Since many objects and widgets that
 * are included with BOBGUI already implement the `BobguiBuildable` interface,
 ...
 *
 * See the [`BobguiWidget` documentation](class.Widget.html#bobguiwidget-as-bobguibuildable)
 * for many examples of using `BobguiBuilder` with widgets, including setting
 * child objects using the `<child>` element.
 *
 * A noteworthy special case to the general rule that only objects implementing
 * `BobguiBuildable` may specify how to handle the `<child>` element is that
 * `BobguiBuilder` provides special support for adding objects to a
 ...
 * respectively. Internally, `BobguiBuilder` implements this using
 ...
 *   <child>
 *     <object class="BobguiButton" id="top_button">
 *       <property name="label">Hello, world</property>
 *     </object>
 *   </child>
 *   <child>
 *     <object class="BobguiLabel" id="bottom_label">
 *       <property name="label"
 ...
 * is to use the `BobguiExpression` methodology. See the
 * [`BobguiExpression` documentation](class.Expression.html#bobguiexpression-in-ui-files)
 * for more information.
 *
 * ### Internal children
 *
 * Sometimes it is necessary to refer to widgets which have implicitly
 * been constructed by BOBGUI as part of a composite widget, to set
 * properties on them or to add further children (e.g. the content area
 * of a `BobguiDialog`). This can be achieved by setting the “internal-child”
 * property of the `<child>` element to a true value. Note that `BobguiBuilder`
 ...
 * hello_button__clicked (BobguiButton *button,
 *                        gpointer data)
 * {
 *   // ...
 * }
 ...
 *           <object class="BobguiBox">
 *             <child>
 *               <object class="BobguiButton" id="ok_button">
 ...
 * ```
 *
 * ## Using BobguiBuildable for extending UI definitions
 *
 * Objects can implement the [iface@Bobgui.Buildable] interface to add custom
 ...
 * `<property>` elements, `BobguiBuilder` also allows to parse XML menu definitions
 * as used by [class@Gio.Menu] when exporting menu models over D-Bus, and as
 * described in the [class@Bobgui.PopoverMenu] documentation. Menus can be defined
 * as toplevel elements, or as property values for properties of type `GMenuModel`.
 *
 * ## Templates
 *
 * When describing a [class@Bobgui.Widget], you can use the `<template>` tag to
 * describe a UI bound to a specific widget type. BOBGUI will automatically load
 ...
 *
 * For more information, see the [`BobguiWidget` documentation](class.Widget.html#building-composite-widgets-from-template-xml)
 * for details.
 */
...
#include "bobguibuilderscopeprivate.h"
...
static void bobgui_builder_finalize       (GObject         *object);
static void bobgui_builder_set_property   (GObject         *object,
                                        guint            prop_id,
                                        const GValue    *value,
                                        GParamSpec      *pspec);
static void bobgui_builder_get_property   (GObject         *object,
                                        guint            prop_id,
                                        GValue          *value,
                                        GParamSpec      *pspec);
...
struct _BobguiBuilder
{
  GObject parent_instance;
};

struct _BobguiBuilderClass
{
...
} BobguiBuilderPrivate;

G_DEFINE_TYPE_WITH_PRIVATE (BobguiBuilder, bobgui_builder, G_TYPE_OBJECT)

static void
bobgui_builder_dispose (GObject *object)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (BOBGUI_BUILDER (object));

  g_clear_object (&priv->current_object);
  g_clear_object (&priv->scope);

  G_OBJECT_CLASS (bobgui_builder_parent_class)->dispose (object);
}

static void
bobgui_builder_class_init (BobguiBuilderClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->dispose = bobgui_builder_dispose;
  gobject_class->finalize = bobgui_builder_finalize;
  gobject_class->set_property = bobgui_builder_set_property;
  gobject_class->get_property = bobgui_builder_get_property;

 /**
  * BobguiBuilder:translation-domain:
  *
  * The translation domain used when translating property values that
  * have been marked as translatable.
  *
  * If the translation domain is %NULL, `BobguiBuilder` uses gettext(),
...
static void
bobgui_builder_init (BobguiBuilder *builder)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
static void
bobgui_builder_finalize (GObject *object)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (BOBGUI_BUILDER (object));
  ...
  if (BOBGUI_DEBUG_CHECK (BUILDER_OBJECTS))
    {
      GHashTableIter iter;
      gpointer key, value;
  ...
  G_OBJECT_CLASS (bobgui_builder_parent_class)->finalize (object);
}

static void
bobgui_builder_set_property (GObject      *object,
                          guint         prop_id,
                          const GValue *value,
                          GParamSpec   *pspec)
{
  BobguiBuilder *builder = BOBGUI_BUILDER (object);

  switch (prop_id)
    {
    case PROP_CURRENT_OBJECT:
      bobgui_builder_set_current_object (builder, g_value_get_object (value));
      break;

    case PROP_SCOPE:
      bobgui_builder_set_scope (builder, g_value_get_object (value));
      break;

    case PROP_TRANSLATION_DOMAIN:
      bobgui_builder_set_translation_domain (builder, g_value_get_string (value));
      break;
...
static void
bobgui_builder_get_property (GObject    *object,
                          guint       prop_id,
                          GValue     *value,
                          GParamSpec *pspec)
{
...
static void
bobgui_builder_get_parameters (BobguiBuilder         *builder,
                            GType               object_type,
      ...
      if (prop->value)
        {
          ...
          if (G_PARAM_SPEC_VALUE_TYPE (prop->pspec) == BOBGUI_TYPE_EXPRESSION)
            bobgui_value_set_expression (&property_value, prop->value);
          else
      ...
      else if (prop->bound && (!prop->text || prop->text->len == 0))
        {
          ...
          continue;
               ...
               (G_PARAM_SPEC_VALUE_TYPE (prop->pspec) != BOBGUI_TYPE_SHORTCUT_TRIGGER) &&
               (G_PARAM_SPEC_VALUE_TYPE (prop->pspec) != BOBGUI_TYPE_SHORTCUT_ACTION) &&
               (G_PARAM_SPEC_VALUE_TYPE (prop->pspec) != G_TYPE_FILE))
        {
          ...
          else if (!bobgui_builder_value_from_string (builder, prop->pspec,
                                                   prop->text->str,
                                                   &property_value,
                                                   NULL))
            {
...
object_get_id (GObject *object)
{
  if (BOBGUI_IS_BUILDABLE (object))
    return bobgui_buildable_get_buildable_id (BOBGUI_BUILDABLE (object));
  else
    return g_object_get_data (object, "bobgui-builder-id");
}

static GObject *
bobgui_builder_get_internal_child (BobguiBuilder   *builder,
                                ObjectInfo   *info,
  ...
  while (!obj)
    {
      ...
      BOBGUI_DEBUG (BUILDER_TRACE, "Trying to get internal child %s from %s",
                                childname, object_get_id (info->object));

      if (BOBGUI_IS_BUILDABLE (info->object))
          obj = bobgui_buildable_get_internal_child (BOBGUI_BUILDABLE (info->object),
                                                  builder,
                                                  childname);
    };
...
{
  if (BOBGUI_IS_BUILDABLE (object))
    bobgui_buildable_set_buildable_id (BOBGUI_BUILDABLE (object), id);
  else
    g_object_set_data_full (object, "bobgui-builder-id", g_strdup (id), g_free);
}

void
_bobgui_builder_add_object (BobguiBuilder  *builder,
                         const char *id,
                         GObject     *object)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
void
bobgui_builder_take_bindings (BobguiBuilder *builder,
                           GObject    *target,
                           GSList     *bindings)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
static void
ensure_special_construct_parameters (BobguiBuilder       *builder,
                                     GType             object_type,
                                     ObjectProperties *construct_parameters)
{
  ...
  if (g_type_is_a (object_type, BOBGUI_TYPE_BUILDER_LIST_ITEM_FACTORY))
    {
      g_value_init (&value, BOBGUI_TYPE_BUILDER_SCOPE);
      g_value_set_object (&value, priv->scope);
...
GObject *
_bobgui_builder_construct (BobguiBuilder  *builder,
                        ObjectInfo  *info,
                        GError     **error)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  ...
  bobgui_builder_get_parameters (builder, info->type,
                              info->id,
                              info->properties,
                              param_filter_flags,
                              &parameters,
  ...
  if (info->constructor)
    {
      ...
      obj = bobgui_buildable_construct_child (BOBGUI_BUILDABLE (constructor),
                                           builder,
                                           info->id);
      g_assert (obj != NULL);
           ...
           ((ChildInfo*)info->parent)->internal_child != NULL)
    {
      ...
      obj = bobgui_builder_get_internal_child (builder, info, childname, error);
      if (!obj)
        {
          object_properties_destroy (&parameters);
          object_properties_destroy (&construct_parameters);
  ...
  else
    {
      ...
      BOBGUI_DEBUG (BUILDER_TRACE, "created %s of type %s", info->id, g_type_name (info->type));
    }
  object_properties_destroy (&construct_parameters);

  if (parameters.names)
    {
      BobguiBuildableIface *iface = NULL;
      gboolean custom_set_property = FALSE;
      BobguiBuildable *buildable = NULL;

      if (BOBGUI_IS_BUILDABLE (obj))
        {
          buildable = BOBGUI_BUILDABLE (obj);
          iface = BOBGUI_BUILDABLE_GET_IFACE (obj);
          ...
          for (i = 0; i < parameters.names->len; i++)
            {
              ...
              if (BOBGUI_DEBUG_CHECK (BUILDER_TRACE))
                {
  ...
  _bobgui_builder_add_object (builder, info->id, obj);
...
void
_bobgui_builder_apply_properties (BobguiBuilder  *builder,
                               ObjectInfo  *info,
                               GError     **error)
{
  ObjectProperties parameters;
  ...
  if (parameters.names)
    {
      ...
      if (BOBGUI_IS_BUILDABLE (info->object))
        {
          buildable = BOBGUI_BUILDABLE (info->object);
          iface = BOBGUI_BUILDABLE_GET_IFACE (info->object);
          if (iface->set_buildable_property)
            custom_set_property = TRUE;
        }
...
void
_bobgui_builder_add (BobguiBuilder *builder,
                  ChildInfo  *child_info)
{
  ...
  BOBGUI_DEBUG (BUILDER_TRACE, "adding %s to %s", object_get_id (object), object_get_id (parent));

  if (G_IS_LIST_STORE (parent))
    {
      if (child_info->type != NULL)
        {
          BOBGUI_BUILDER_WARN_INVALID_CHILD_TYPE (parent, child_info->type);
        }
      else
        {
    ...
    {
      g_assert (BOBGUI_IS_BUILDABLE (parent));
      bobgui_buildable_add_child (BOBGUI_BUILDABLE (parent), builder, object,
                               child_info->type);
    }
...
void
_bobgui_builder_add_signals (BobguiBuilder *builder,
                          GPtrArray  *signals)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
static gboolean
bobgui_builder_apply_delayed_properties (BobguiBuilder  *builder,
                                      GError     **error)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  GSList *l, *props;
      ...
      if (result)
        {
          ...
          obj = bobgui_builder_lookup_object (builder, property->value, property->line, property->col, error);
          if (obj)
...
bobgui_builder_create_bindings (BobguiBuilder  *builder,
                             GError     **error)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  GSList *l;
      ...
      if (common_info->tag_type == TAG_BINDING)
        {
          ...
          source = bobgui_builder_lookup_object (builder, info->source, info->line, info->col, error);
          if (source)
      ...
      else if (common_info->tag_type == TAG_BINDING_EXPRESSION)
        {
          ...
          if (info->object_name)
            {
              object = bobgui_builder_lookup_object (builder, info->object_name, info->line, info->col, error);
              if (object == NULL)
                {
                  error = NULL;
                  result = FALSE;
          ...
          if (object)
            {
              ...
              if (expression == NULL)
                ...
                {
                  bobgui_expression_bind (expression, info->target, info->target_pspec->name, object);
                }
            }
 ...
 * to [method@Bobgui.Builder.add_from_file], [method@Bobgui.Builder.add_from_resource]
 * or [method@Bobgui.Builder.add_from_string] in order to merge multiple UI
 * descriptions into a single builder.
 *
 * Returns: a new (empty) `BobguiBuilder` object
 */
BobguiBuilder *
bobgui_builder_new (void)
{
  return g_object_new (BOBGUI_TYPE_BUILDER, NULL);
}

/**
 * bobgui_builder_add_from_file:
 ...
 * [method@Bobgui.Builder.set_current_object]) to add user data to
 * callbacks before loading BobguiBuilder UI. Otherwise, you probably
 * want [ctor@Bobgui.Builder.new_from_file] instead.
 *
 * If an error occurs, 0 will be returned and @error will be assigned a
 * `GError` from the `BOBGUI_BUILDER_ERROR`, `G_MARKUP_ERROR` or `G_FILE_ERROR`
 * domains.
 *
 * It’s not really reasonable to attempt to handle failures of this
 * call. You should not use this function with untrusted files (ie:
 * files that are not part of your application). Broken `BobguiBuilder`
 ...
 */
gboolean
bobgui_builder_add_from_file (BobguiBuilder   *builder,
                           const char   *filename,
                           GError      **error)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  ...
  g_return_val_if_fail (BOBGUI_IS_BUILDER (builder), 0);
  ...
  _bobgui_builder_parser_parse_buffer (builder, filename,
...
/**
 * bobgui_builder_add_objects_from_file:
 * @builder: a `BobguiBuilder`
 * @filename: (type filename): the name of the file to parse
 ...
 * its child (for instance a `BobguiTreeView` that depends on its
 * `BobguiTreeModel`), you have to explicitly list all of them in @object_ids.
 *
 * Returns: %TRUE on success, %FALSE if an error occurred
 */
gboolean
bobgui_builder_add_objects_from_file (BobguiBuilder   *builder,
                                   const char   *filename,
...
void
bobgui_builder_set_allow_template_parents (BobguiBuilder *builder,
...
/**
 * bobgui_builder_extend_with_template:
 * @builder: a `BobguiBuilder`
 ...
 * templates are handled by `BobguiWidget`.
 *
 * Returns: A positive value on success, 0 if an error occurred
 */
gboolean
bobgui_builder_extend_with_template (BobguiBuilder   *builder,
                                  GObject      *object,
                                  GType         template_type,
                                  const char   *buffer,
                                  gssize        length,
                                  GError      **error)
{
  ...
  if (bobgui_builder_get_object (builder, name) != object)
    bobgui_builder_expose_object (builder, name, object);
...
/**
 * bobgui_builder_add_from_resource:
 * @builder: a `BobguiBuilder`
 ...
 * [method@Bobgui.Builder.set_current_object] to add user data to
 * callbacks before loading BobguiBuilder UI. Otherwise, you probably
 * want [ctor@Bobgui.Builder.new_from_resource] instead.
 *
 * If an error occurs, 0 will be returned and @error will be assigned a
 * `GError` from the %BOBGUI_BUILDER_ERROR, %G_MARKUP_ERROR or %G_RESOURCE_ERROR
 * domain.
 *
 * It’s not really reasonable to attempt to handle failures of this
 ...
 */
gboolean
bobgui_builder_add_from_resource (BobguiBuilder   *builder,
                               ...
                               GError      **error)
{
  ...
  _bobgui_builder_parser_parse_buffer (builder, filename_for_errors,
...
/**
 * bobgui_builder_add_objects_from_resource:
 * @builder: a `BobguiBuilder`
 * @resource_path: the path of the resource file to parse
 * @object_ids: (array zero-terminated=1) (element-type utf8): nul-terminated array of objects to build
 * @error: (nullable): return location for an error
 ...
 */
gboolean
bobgui_builder_add_objects_from_resource (BobguiBuilder   *builder,
                                       const char   *resource_path,
                                       const char  **object_ids,
                                       GError      **error)
{
...
/**
 * bobgui_builder_add_from_string:
 * @builder: a `BobguiBuilder`
 * @buffer: the string to parse
 * @length: the length of @buffer (may be -1 if @buffer is nul-terminated)
 * @error: (nullable): return location for an error
 ...
 * [method@Bobgui.Builder.set_current_object] to add user data to
 * callbacks before loading `BobguiBuilder` UI. Otherwise, you probably
 * want [ctor@Bobgui.Builder.new_from_string] instead.
 *
 * Upon errors %FALSE will be returned and @error will be assigned a
 * `GError` from the %BOBGUI_BUILDER_ERROR, %G_MARKUP_ERROR or
 ...
 */
gboolean
bobgui_builder_add_from_string (BobguiBuilder   *builder,
                             const char   *buffer,
                             gssize        length,
                             GError      **error)
{
  ...
  _bobgui_builder_parser_parse_buffer (builder, "<input>",
                                    buffer, length,
                                    NULL,
                                    &tmp_error);
  if (tmp_error != NULL)
...
/**
 * bobgui_builder_add_objects_from_string:
 * @builder: a `BobguiBuilder`
 * @buffer: the string to parse
 * @length: the length of @buffer (may be -1 if @buffer is nul-terminated)
 * @object_ids: (array zero-terminated=1) (element-type utf8): nul-terminated array of objects to build
 ...
 *
 * Upon errors %FALSE will be returned and @error will be assigned a
 * `GError` from the %BOBGUI_BUILDER_ERROR or %G_MARKUP_ERROR domain.
 *
 * If you are adding an object that depends on an object that is not
 * its child (for instance a `BobguiTreeView` that depends on its
 * `BobguiTreeModel`), you have to explicitly list all of them in @object_ids.
 ...
 */
gboolean
bobgui_builder_add_objects_from_string (BobguiBuilder   *builder,
                                     const char   *buffer,
                                     gssize        length,
                                     const char  **object_ids,
                                     GError      **error)
 ...
 */
GObject *
bobgui_builder_get_object (BobguiBuilder  *builder,
                        const char *name)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
 ...
 *   constructed by the `BobguiBuilder instance`. It should be
 *   freed by g_slist_free()
 */
GSList *
bobgui_builder_get_objects (BobguiBuilder *builder)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  GSList *objects = NULL;
  GObject *object;
...
/**
 * bobgui_builder_set_translation_domain:
 * @builder: a `BobguiBuilder`
 ...
 */
void
bobgui_builder_set_translation_domain (BobguiBuilder  *builder,
                                    const char *domain)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  char *new_domain;
...
/**
 * bobgui_builder_get_translation_domain:
 * @builder: a `BobguiBuilder`
 *
 ...
 */
const char *
bobgui_builder_get_translation_domain (BobguiBuilder *builder)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
 ...
 * names. `bobgui_builder_get_object()` may be used to determine
 * if an object has already been added with @name.
 */
void
bobgui_builder_expose_object (BobguiBuilder    *builder,
                           const char    *name,
                           GObject       *object)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
/**
 * bobgui_builder_get_current_object:
 * @builder: a `BobguiBuilder`
 *
 * Gets the current object set via bobgui_builder_set_current_object().
 *
 * Returns: (nullable) (transfer none): the current object
 */
GObject *
bobgui_builder_get_current_object (BobguiBuilder *builder)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
/**
 * bobgui_builder_set_current_object:
 * @builder: a `BobguiBuilder`
 ...
 *
 * [method@Bobgui.Widget.init_template] for example will set the current
 * object to the widget the template is inited for. For functions like
 * [ctor@Bobgui.Builder.new_from_resource], the current object will be %NULL.
 */
void
bobgui_builder_set_current_object (BobguiBuilder *builder,
                                GObject    *current_object)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
/**
 * bobgui_builder_get_scope:
 * @builder: a `BobguiBuilder`
 *
 * Gets the scope in use that was set via bobgui_builder_set_scope().
 *
 * Returns: (transfer none): the current scope
 */
BobguiBuilderScope *
bobgui_builder_get_scope (BobguiBuilder *builder)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
/**
 * bobgui_builder_set_scope:
 * @builder: a `BobguiBuilder`
 * @scope: (nullable) (transfer none): the scope to use
 *
 * Sets the scope the builder should operate in.
 *
 * If @scope is %NULL, a new [class@Bobgui.BuilderCScope] will be created.
 */
void
bobgui_builder_set_scope (BobguiBuilder      *builder,
                       BobguiBuilderScope *scope)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);

  g_return_if_fail (BOBGUI_IS_BUILDER (builder));
  g_return_if_fail (scope == NULL || BOBGUI_IS_BUILDER_SCOPE (scope));
  ...
  if (scope)
    ...
    priv->scope = bobgui_builder_cscope_new ();

  g_object_notify_by_pspec (G_OBJECT (builder), builder_props[PROP_SCOPE]);
}

static gboolean
bobgui_builder_connect_signals (BobguiBuilder  *builder,
                             GError     **error)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  GObject *object;
  ...
  for (guint i = 0; i < priv->signals->len; i++)
    {
      ...
      if (signal->connect_object_name)
        {
          ...
          if (!connect_object)
            {
              g_set_error (error,
                           BOBGUI_BUILDER_ERROR, BOBGUI_BUILDER_ERROR_INVALID_ID,
      ...
      closure = bobgui_builder_create_closure (builder,
                                            signal->handler,
...
gboolean
_bobgui_builder_finish (BobguiBuilder  *builder,
                     GError     **error)
{
  return bobgui_builder_apply_delayed_properties (builder, error) &&
         bobgui_builder_create_bindings (builder, error) &&
         bobgui_builder_connect_signals (builder, error);
}

/**
 * bobgui_builder_value_from_string:
 * @builder: a `BobguiBuilder`
 ...
 * `BobguiAdjustment` type values.
 *
 * Upon errors %FALSE will be returned and @error will be
 * assigned a `GError` from the %BOBGUI_BUILDER_ERROR domain.
 *
 * Returns: %TRUE on success
 */
gboolean
bobgui_builder_value_from_string (BobguiBuilder   *builder,
                               GParamSpec   *pspec,
                               const char   *string,
                               GValue       *value,
                               GError      **error)
{
  ...
  return bobgui_builder_value_from_string_type (builder,
                                             G_PARAM_SPEC_VALUE_TYPE (pspec),
                                             string, value, error);
}

gboolean
_bobgui_builder_boolean_from_string (const char   *string,
                                  gboolean     *value,
                                  GError      **error)
{
  if (string[0] == '\0')
...
gboolean
bobgui_builder_parse_translatable (const char  *string,
                                gboolean    *value,
                                GError     **error)
{
  if (!_bobgui_builder_boolean_from_string (string, value, error))
    return FALSE;

  if (*value && strcmp (string, "yes") != 0)
    BOBGUI_DEBUG (BUILDER, "Useless translatable attribute: '%s' (xgettext only recognizes 'yes')", string);

  return TRUE;
}

/**
 * bobgui_builder_value_from_string_type:
 * @builder: a `BobguiBuilder`
 ...
 * Demarshals a value from a string.
 *
 * Unlike [method@Bobgui.Builder.value_from_string], this function
 ...
 */
gboolean
bobgui_builder_value_from_string_type (BobguiBuilder   *builder,
                                    GType         type,
                                    const char   *string,
                                    GValue       *value,
                                    GError      **error)
{
  ...
  switch (G_TYPE_FUNDAMENTAL (type))
    ...
    case G_TYPE_BOOLEAN:
      {
        ...
        if (!_bobgui_builder_boolean_from_string (string, &b, error))
          {
            ret = FALSE;
            break;
          }
    ...
    case G_TYPE_ENUM:
      {
        int enum_value;
        if (!_bobgui_builder_enum_from_string (type, string, &enum_value, error))
          {
            ret = FALSE;
            break;
          }
    ...
    case G_TYPE_FLAGS:
      {
        ...
        if (!_bobgui_builder_flags_from_string (type, string, &flags_value, error))
          {
            ret = FALSE;
            break;
          }
    ...
    case G_TYPE_INTERFACE:
      if (G_VALUE_HOLDS (value, GDK_TYPE_PAINTABLE) ||
          G_VALUE_HOLDS (value, GDK_TYPE_TEXTURE))
        {
          ...
          filename = _bobgui_builder_get_resource_path (builder, string);
          if (filename != NULL)
            {
          ...
          else
            {
              ...
              filename = _bobgui_builder_get_absolute_filename (builder, string);
      ...
      else if (G_VALUE_HOLDS (value, G_TYPE_FILE))
        {
          ...
          if (!g_uri_is_valid (string, G_URI_FLAGS_NONE, NULL))
            {
              gchar *fullpath = _bobgui_builder_get_absolute_filename (builder, string);
          ...
          else if (g_str_has_prefix (string, "file://"))
            {
              ...
              gchar *fullpath = _bobgui_builder_get_absolute_filename (builder, path);
      ...
      else if (G_VALUE_HOLDS (value, BOBGUI_TYPE_SHORTCUT_TRIGGER))
        {
          BobguiShortcutTrigger *trigger = bobgui_shortcut_trigger_parse_string (string);
      ...
      else if (G_VALUE_HOLDS (value, BOBGUI_TYPE_SHORTCUT_ACTION))
        {
          BobguiShortcutAction *action = bobgui_shortcut_action_parse_builder (builder, string, error);
      ...
      if (G_VALUE_HOLDS (value, G_TYPE_GTYPE))
        {
          ...
          resolved_type = bobgui_builder_get_type_from_name (builder, string);
   ...
   * While parsing BobguiBuilder; object types are deserialized
   * without calling bobgui_builder_value_from_string_type().
   */
...
gboolean
_bobgui_builder_enum_from_string (GType         type,
                               const char   *string,
                               int          *enum_value,
                               GError      **error)
{
...
gboolean
_bobgui_builder_flags_from_string (GType         type,
                                const char   *string,
                                guint        *flags_value,
                                GError      **error)
{
...
/**
 * bobgui_builder_get_type_from_name:
 * @builder: a `BobguiBuilder`
 * @type_name: type name to lookup
 *
 * Looks up a type by name.
 *
 * This is using the virtual function that `BobguiBuilder` has
 * for that purpose. This is mainly used when implementing
 * the `BobguiBuildable` interface on a type.
 *
 * Returns: the `GType` found for @type_name or %G_TYPE_INVALID
 *   if no type was found
 */
GType
bobgui_builder_get_type_from_name (BobguiBuilder  *builder,
                                const char *type_name)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  GType type;

  g_return_val_if_fail (BOBGUI_IS_BUILDER (builder), G_TYPE_INVALID);
  g_return_val_if_fail (type_name != NULL, G_TYPE_INVALID);

  type = bobgui_builder_scope_get_type_from_name (priv->scope, builder, type_name);
...
/**
 * bobgui_builder_error_quark:
 *
 * Registers an error quark for [class@Bobgui.Builder] errors.
 *
 * Returns: the error quark
 **/
GQuark
bobgui_builder_error_quark (void)
{
  return g_quark_from_static_string ("bobgui-builder-error-quark");
}

char *
_bobgui_builder_get_resource_path (BobguiBuilder *builder, const char *string)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
...
char *
_bobgui_builder_get_absolute_filename (BobguiBuilder  *builder,
                                    const char *string)
{
  BobguiBuilderPrivate *priv = bobgui_builder_get_instance_private (builder);
  char *filename;
...
GType
bobgui_builder_get_template_type (BobguiBuilder *builder,
 ...
 */
GClosure *
bobgui_builder_create_closure (BobguiBuilder             *builder,
                            ...
                            GError                **error)
{
  ...
  return bobgui_builder_scope_create_closure (priv->scope, builder, function_name, flags, object, error);
}

/**
 * bobgui_builder_new_from_file: (constructor)
 ...
 *
 * Returns: (transfer full): a `BobguiBuilder` containing the described interface
 */
BobguiBuilder *
bobgui_builder_new_from_file (const char *filename)
{
  GError *error = NULL;
...
/**
 * bobgui_builder_new_from_resource: (constructor)
 ...
 */
BobguiBuilder *
bobgui_builder_new_from_resource (const char *resource_path)
{
  GError *error = NULL;
  BobguiBuilder *builder;

  builder = bobgui_builder_new ();
  if (!bobgui_builder_add_from_resource (builder, resource_path, &error))
...
/**
 * bobgui_builder_new_from_string: (constructor)
 ...
 *
 * Returns: (transfer full): a `BobguiBuilder` containing the interface described by @string
 */
BobguiBuilder *
bobgui_builder_new_from_string (const char *string,
                             gssize       length)
{
  GError *error = NULL;
  BobguiBuilder *builder;
  ...
  builder = bobgui_builder_new ();
  if (!bobgui_builder_add_from_string (builder, string, length, &error))
...
/*< private >
 * _bobgui_builder_prefix_error:
 * @builder: a `BobguiBuilder`
 * @context: the `BobguiBuildableParseContext`
 * @error: an error
 *
 ...
 */
void
_bobgui_builder_prefix_error (BobguiBuilder                *builder,
                           ...
                           GError                   **error)
{
  ...
  bobgui_buildable_parse_context_get_position (context, &line, &col);
  g_prefix_error (error, "%s:%d:%d ", priv->filename, line, col);
}

/*< private >
 * _bobgui_builder_error_unhandled_tag:
 * @builder: a `BobguiBuilder`
 * @context: the `BobguiBuildableParseContext`
 ...
 */
void
_bobgui_builder_error_unhandled_tag (BobguiBuilder                *builder,
                                  BobguiBuildableParseContext  *context,
                                  const char                *object,
 ...
 */
gboolean
_bobgui_builder_check_parent (BobguiBuilder                *builder,
                           BobguiBuildableParseContext  *context,
                           const char                *parent_name,
                           GError                   **error)
{
  return _bobgui_builder_check_parents (builder, context, error, parent_name, NULL);
}

gboolean
_bobgui_builder_check_parents (BobguiBuilder                *builder,
                            BobguiBuildableParseContext  *context,
                            GError                   **error,
                            ...)
{
  ...
  stack = bobgui_buildable_parse_context_get_element_stack (context);
...
GObject *
bobgui_builder_lookup_object (BobguiBuilder   *builder,
                           const char   *name,
                           int           line,
                           int           col,
                           GError      **error)
 ...
 *
 * Looks up an object by name. Similar to bobgui_builder_get_object(),
 ...
 */
GObject *
_bobgui_builder_lookup_object (BobguiBuilder  *builder,
                            const char *name,
                            int          line,
                            int          col)
{
...
/*< private >
 * _bobgui_builder_lookup_failed:
 * @BobguiBuilder: a `BobguiBuilder`
 ...
 */
gboolean
_bobgui_builder_lookup_failed (BobguiBuilder  *builder,
                            GError     **error)
{
  GError *lookup_error;
...
void
bobgui_buildable_child_deprecation_warning (BobguiBuildable *buildable,
                                         BobguiBuilder   *builder,
                                         const char   *type,
                                         const char   *prop)
{
  if (type)
    BOBGUI_DEBUG (BUILDER, "<child type=\"%s\"> in %s is deprecated, just set the %s property",
               type, G_OBJECT_TYPE_NAME (buildable), prop);
  else
    BOBGUI_DEBUG (BUILDER, "<child> in %s is deprecated, just set the %s property",
               G_OBJECT_TYPE_NAME (buildable), prop);
}

void
bobgui_buildable_tag_deprecation_warning (BobguiBuildable *buildable,
                                       BobguiBuilder   *builder,
                                       const char   *tag,
                                       const char   *prop)
{
  BOBGUI_DEBUG (BUILDER, "<%s> in %s is deprecated, just set the %s property",
