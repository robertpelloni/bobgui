/* bobguitextmark.c - mark segments
 *
 ...
 * BobguiTextMark:
 *
 * Marks a position in a `BobguiTextbuffer` that is preserved
 ...
 *
 * A `BobguiTextMark` is like a bookmark in a text buffer; it preserves
 * a position in the text. You can convert the mark to an iterator using
 * [method@Bobgui.TextBuffer.get_iter_at_mark]. Unlike iterators, marks remain
 ...
 * any time with [method@Bobgui.TextBuffer.delete_mark]. Once deleted from
 * the buffer, a mark is essentially useless.
 *
 * Marks optionally have names; these can be convenient to avoid passing
 * the `BobguiTextMark` object around.
 *
 * Marks are typically created using the [method@Bobgui.TextBuffer.create_mark]
 * function.
 */
 ...
 */
#define MSEG_SIZE ((unsigned) (G_STRUCT_OFFSET (BobguiTextLineSegment, body) \
        + sizeof (BobguiTextMarkBody)))

static void bobgui_text_mark_set_property (GObject         *object,
				        guint            prop_id,
					const GValue    *value,
					GParamSpec      *pspec);
static void bobgui_text_mark_get_property (GObject         *object,
					guint            prop_id,
					GValue          *value,
					GParamSpec      *pspec);
static void bobgui_text_mark_finalize     (GObject         *object);

static BobguiTextLineSegment *bobgui_mark_segment_new (BobguiTextMark *mark_obj);

G_DEFINE_TYPE (BobguiTextMark, bobgui_text_mark, G_TYPE_OBJECT)
...
static void
bobgui_text_mark_class_init (BobguiTextMarkClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = bobgui_text_mark_finalize;
  object_class->set_property = bobgui_text_mark_set_property;
  object_class->get_property = bobgui_text_mark_get_property;
...
static void
bobgui_text_mark_init (BobguiTextMark *mark)
{
  mark->segment = bobgui_mark_segment_new (mark);
}

static void
bobgui_text_mark_finalize (GObject *obj)
{
  BobguiTextMark *mark;
  BobguiTextLineSegment *seg;

  mark = BOBGUI_TEXT_MARK (obj);
  ...
  if (seg)
    {
      if (seg->body.mark.tree != NULL)
        g_warning ("BobguiTextMark being finalized while still in the buffer; "
  ...
  G_OBJECT_CLASS (bobgui_text_mark_parent_class)->finalize (obj);
}

static void
bobgui_text_mark_set_property (GObject      *object,
			    guint         prop_id,
			    const GValue *value,
			    GParamSpec   *pspec)
{
  char *tmp;
  BobguiTextMark *mark = BOBGUI_TEXT_MARK (object);
  BobguiTextLineSegment *seg = mark->segment;
...
static void
bobgui_text_mark_get_property (GObject    *object,
			    guint       prop_id,
			    GValue     *value,
			    GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_NAME:
      g_value_set_string (value, bobgui_text_mark_get_name (mark));
      break;

    case PROP_LEFT_GRAVITY:
      g_value_set_boolean (value, bobgui_text_mark_get_left_gravity (mark));
      break;
...
/**
 * bobgui_text_mark_new:
 * @name: (nullable): mark name
 * @left_gravity: whether the mark should have left gravity
 *
 * Creates a text mark.
 *
 * Add it to a buffer using [method@Bobgui.TextBuffer.add_mark].
 * If @name is %NULL, the mark is anonymous; otherwise, the mark can be
 * retrieved by name using [method@Bobgui.TextBuffer.get_mark]. If a mark
 ...
 *
 * Returns: new `BobguiTextMark`
 */
BobguiTextMark *
bobgui_text_mark_new (const char *name,
		   gboolean     left_gravity)
{
  return g_object_new (BOBGUI_TYPE_TEXT_MARK,
		       "name", name,
 ...
 */
gboolean
bobgui_text_mark_get_visible (BobguiTextMark *mark)
{
  BobguiTextLineSegment *seg;
...
/**
 * bobgui_text_mark_get_deleted:
 * @mark: a `BobguiTextMark`
 *
 * Returns %TRUE if the mark has been removed from its buffer.
 *
 * See [method@Bobgui.TextBuffer.add_mark] for a way to add it
 * to a buffer again.
 *
 * Returns: whether the mark is deleted
 */
gboolean
bobgui_text_mark_get_deleted (BobguiTextMark *mark)
{
  BobguiTextLineSegment *seg;

  g_return_val_if_fail (BOBGUI_IS_TEXT_MARK (mark), FALSE);
 ...
 *
 * Returns: (transfer none) (nullable): the mark’s `BobguiTextBuffer`
 */
BobguiTextBuffer*
bobgui_text_mark_get_buffer (BobguiTextMark *mark)
{
  BobguiTextLineSegment *seg;

  g_return_val_if_fail (BOBGUI_IS_TEXT_MARK (mark), NULL);
  ...
  if (seg->body.mark.tree == NULL)
    ...
    return _bobgui_text_btree_get_buffer (seg->body.mark.tree);
}

/**
 * bobgui_text_mark_get_left_gravity:
 * @mark: a `BobguiTextMark`
 *
 ...
 */
gboolean
bobgui_text_mark_get_left_gravity (BobguiTextMark *mark)
{
  BobguiTextLineSegment *seg;
  ...
  return seg->type == &bobgui_text_left_mark_type;
}

static BobguiTextLineSegment *
bobgui_mark_segment_new (BobguiTextMark *mark_obj)
{
  BobguiTextLineSegment *mark;

  mark = g_malloc0 (MSEG_SIZE);
  mark->body.mark.name = NULL;
  mark->type = &bobgui_text_right_mark_type;
...
void
_bobgui_mark_segment_set_tree (BobguiTextLineSegment *mark,
...
static int                 mark_segment_delete_func  (BobguiTextLineSegment *segPtr,
                                                      BobguiTextLine        *line,
                                                      int                 treeGone);
static BobguiTextLineSegment *mark_segment_cleanup_func (BobguiTextLineSegment *segPtr,
                                                      BobguiTextLine        *line);
static void                mark_segment_check_func   (BobguiTextLineSegment *segPtr,
                                                      BobguiTextLine        *line);
...
const BobguiTextLineSegmentClass bobgui_text_right_mark_type = {
  "mark",                                               /* name */
  FALSE,                                                /* leftGravity */
  NULL,                                         /* splitFunc */
...
const BobguiTextLineSegmentClass bobgui_text_left_mark_type = {
  "mark",                                               /* name */
  TRUE,                                         /* leftGravity */
  NULL,                                         /* splitFunc */
  mark_segment_delete_func,                             /* deleteFunc */
...
static gboolean
mark_segment_delete_func (BobguiTextLineSegment *seg,
                          BobguiTextLine        *line,
                          gboolean            tree_gone)
{
  if (tree_gone)
    {
      _bobgui_text_btree_release_mark_segment (seg->body.mark.tree, seg);
      return FALSE;
    }
  else
    return TRUE;
...
static BobguiTextLineSegment *
mark_segment_cleanup_func (BobguiTextLineSegment *seg,
                           BobguiTextLine        *line)
{
...
static void
mark_segment_check_func (BobguiTextLineSegment *seg,
                         BobguiTextLine        *line)
{
