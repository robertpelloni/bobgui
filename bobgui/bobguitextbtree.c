...
#include "bobguitextmarkprivate.h"
 ...
 * _bobgui_text_btree_get_tags and inc_count:
 */
...
typedef struct Summary {
  BobguiTextTagInfo *info;                     /* Handle for tag. */
...
struct _BobguiTextBTreeNode {
  BobguiTextBTreeNode *parent;         /* Pointer to parent node, or NULL if
                                     * this is the root. */
  BobguiTextBTreeNode *next;           /* Next in list of siblings with the
  ...
  union {                               /* First in linked list of children. */
    struct _BobguiTextBTreeNode *node;         /* Used if level > 0. */
    BobguiTextLine *line;         /* Used if level == 0. */
  } children;
...
struct _BobguiTextBTree {
  BobguiTextBTreeNode *root_node;          /* Pointer to root of B-tree. */
  BobguiTextTagTable *table;
  GHashTable *mark_table;
  guint refcount;
  BobguiTextMark *insert_mark;
  BobguiTextMark *selection_bound_mark;
   ...
   */
  BobguiTextLine *end_iter_line;
  BobguiTextLineSegment *end_iter_segment;
...
static BTreeView        *bobgui_text_btree_get_view                 (BobguiTextBTree     *tree,
                                                                  gpointer          view_id);
static void              bobgui_text_btree_rebalance                (BobguiTextBTree     *tree,
                                                                  BobguiTextBTreeNode *node);
static BobguiTextLine     * get_last_line                           (BobguiTextBTree     *tree);
static void              post_insert_fixup                       (BobguiTextBTree     *tree,
                                                                  BobguiTextLine      *insert_line,
                                                                  int               line_count_delta,
                                                                  int               char_count_delta);
static void              bobgui_text_btree_node_adjust_toggle_count (BobguiTextBTreeNode *node,
                                                                  BobguiTextTagInfo   *info,
                                                                  int               adjust);
static gboolean          bobgui_text_btree_node_has_tag             (BobguiTextBTreeNode *node,
                                                                  BobguiTextTag       *tag);

static void             segments_changed                (BobguiTextBTree     *tree);
static void             chars_changed                   (BobguiTextBTree     *tree);
static void             summary_list_destroy            (Summary          *summary);
static BobguiTextLine     *bobgui_text_line_new               (void);
static void             bobgui_text_line_destroy           (BobguiTextBTree     *tree,
                                                         BobguiTextLine      *line);
static void             bobgui_text_line_set_parent        (BobguiTextLine      *line,
                                                         BobguiTextBTreeNode *node);
static void             bobgui_text_btree_node_remove_data (BobguiTextBTreeNode *node,
                                                         gpointer          view_id);

static BobguiTextBTreeNode     *bobgui_text_btree_node_new                  (void);
#if 0
static void                  bobgui_text_btree_node_invalidate_downward  (BobguiTextBTreeNode *node);
#endif
static void                  bobgui_text_btree_node_invalidate_upward    (BobguiTextBTreeNode *node,
                                                                       gpointer          view_id);
static NodeData *            bobgui_text_btree_node_check_valid          (BobguiTextBTreeNode *node,
                                                                       gpointer          view_id);
static NodeData *            bobgui_text_btree_node_check_valid_downward (BobguiTextBTreeNode *node,
                                                                       gpointer          view_id);
static void                  bobgui_text_btree_node_check_valid_upward   (BobguiTextBTreeNode *node,
                                                                       gpointer          view_id);

static void                  bobgui_text_btree_node_remove_view         (BTreeView        *view,
                                                                      BobguiTextBTreeNode *node,
                                                                      gpointer          view_id);
static void                  bobgui_text_btree_node_destroy             (BobguiTextBTree     *tree,
                                                                      BobguiTextBTreeNode *node);
static void                  bobgui_text_btree_node_free_empty          (BobguiTextBTree *tree,
                                                                      BobguiTextBTreeNode *node);
static NodeData         *    bobgui_text_btree_node_ensure_data         (BobguiTextBTreeNode *node,
                                                                      gpointer          view_id);
static void                  bobgui_text_btree_node_get_size            (BobguiTextBTreeNode *node,
                                                                      gpointer          view_id,
                                                                      int              *width,
                                                                      int              *height);
static BobguiTextBTreeNode *    bobgui_text_btree_node_common_parent       (BobguiTextBTreeNode *node1,
                                                                      BobguiTextBTreeNode *node2);
static void get_tree_bounds       (BobguiTextBTree     *tree,
                                   BobguiTextIter      *start,
                                   BobguiTextIter      *end);
static void tag_changed_cb        (BobguiTextTagTable  *table,
                                   BobguiTextTag       *tag,
                                   gboolean          size_changed,
                                   BobguiTextBTree     *tree);
static void cleanup_line          (BobguiTextLine      *line);
static void recompute_node_counts (BobguiTextBTree     *tree,
                                   BobguiTextBTreeNode *node);
static void inc_count             (BobguiTextTag       *tag,
                                   int               inc,
                                   TagInfo          *tagInfoPtr);

static void summary_destroy       (Summary          *summary);

static void bobgui_text_btree_link_segment   (BobguiTextLineSegment *seg,
                                           const BobguiTextIter  *iter);
static void bobgui_text_btree_unlink_segment (BobguiTextBTree       *tree,
                                           BobguiTextLineSegment *seg,
                                           BobguiTextLine        *line);


static BobguiTextTagInfo *bobgui_text_btree_get_tag_info          (BobguiTextBTree   *tree,
                                                             BobguiTextTag     *tag);
static BobguiTextTagInfo *bobgui_text_btree_get_existing_tag_info (BobguiTextBTree   *tree,
                                                             BobguiTextTag     *tag);
static void            bobgui_text_btree_remove_tag_info       (BobguiTextBTree   *tree,
                                                             BobguiTextTag     *tag);

static void redisplay_region (BobguiTextBTree      *tree,
...
static inline void
segments_changed (BobguiTextBTree *tree)
{
  tree->segments_changed_stamp += 1;
}

static inline void
chars_changed (BobguiTextBTree *tree)
{
...
BobguiTextBTree*
_bobgui_text_btree_new (BobguiTextTagTable *table,
                     BobguiTextBuffer *buffer)
{
  BobguiTextBTree *tree;
  BobguiTextBTreeNode *root_node;
  BobguiTextLine *line, *line2;
   ...
   * makes several operations easier.  The tree will have one BobguiTextBTreeNode,
   * which is also the root of the tree.
   */

  /* Create the root node. */

  root_node = bobgui_text_btree_node_new ();

  line = bobgui_text_line_new ();
  line2 = bobgui_text_line_new ();
  ...
  line->segments = _bobgui_char_segment_new ("\n", 1);

  line2->parent = root_node;
  line2->next = NULL;
  line2->segments = _bobgui_char_segment_new ("\n", 1);

  /* Create the tree itself */

  tree = g_new0 (BobguiTextBTree, 1);
...

  {
    ...
    _bobgui_text_btree_get_iter_at_line_char (tree, &start, 0, 0);


    tree->insert_mark = _bobgui_text_btree_set_mark (tree,
                                                 NULL,
                                                 "insert",
                                                 FALSE,
                                                 &start,
    ...
    tree->selection_bound_mark = _bobgui_text_btree_set_mark (tree,
                                                          NULL,
                                                          "selection_bound",
                                                          FALSE,
                                                          &start,
...
void
_bobgui_text_btree_ref (BobguiTextBTree *tree)
{
...
void
_bobgui_text_btree_unref (BobguiTextBTree *tree)
{
  g_return_if_fail (tree != NULL);
  g_return_if_fail (tree->refcount > 0);
  ...
  if (tree->refcount == 0)
    {
      ...
      bobgui_text_btree_node_destroy (tree, tree->root_node);
      tree->root_node = NULL;
...
BobguiTextBuffer*
_bobgui_text_btree_get_buffer (BobguiTextBTree *tree)
{
  return tree->buffer;
}

guint
_bobgui_text_btree_get_chars_changed_stamp (BobguiTextBTree *tree)
{
  return tree->chars_changed_stamp;
}

guint
_bobgui_text_btree_get_segments_changed_stamp (BobguiTextBTree *tree)
{
  return tree->segments_changed_stamp;
}

void
_bobgui_text_btree_segments_changed (BobguiTextBTree *tree)
{
  g_return_if_fail (tree != NULL);
 ...
 */
static void
bobgui_text_btree_resolve_bidi (BobguiTextIter *start,
			     BobguiTextIter *end)
{
  BobguiTextBTree *tree = _bobgui_text_iter_get_btree (start);
  BobguiTextLine *start_line, *end_line, *start_line_prev, *end_line_next, *line;
   ...
   * start and end.
  */
  start_line = _bobgui_text_iter_get_text_line (start);
  start_line_prev = _bobgui_text_line_previous (start_line);
  end_line = _bobgui_text_iter_get_text_line (end);
  end_line_next = _bobgui_text_line_next (end_line);
  ...
  while (line && line != end_line_next)
    {
       ...
       */
      BobguiTextLineSegment *seg = line->segments;
      line->dir_strong = PANGO_DIRECTION_NEUTRAL;

      while (seg)
        {
          if (seg->type == &bobgui_text_char_type && seg->byte_count > 0)
            {
...
>>>>>>> origin/4627-printing-Unref-old-spool_io-before-setting-new-one-gtk3:gtk/gtktextbtree.c
      ...
      line = _bobgui_text_line_next (line);
    }
   ...
   */
  {
    BobguiTextIter end_propagate;

    while (line &&
	   line->dir_strong == PANGO_DIRECTION_NEUTRAL &&
	   line->dir_propagated_forward != last_strong)
      {
        BobguiTextLine *prev = line;
        line->dir_propagated_forward = last_strong;
     ...
     * calling _bobgui_text_btree_get_iter_at_line (causes crash). Thus the
     * _bobgui_text_line_previous is ok in that case as well.)
     */
    line = _bobgui_text_line_previous (line);
    _bobgui_text_btree_get_iter_at_line (tree, &end_propagate, line, 0);
    _bobgui_text_btree_invalidate_region (tree, end, &end_propagate, FALSE);
  }
  ...
  */
   ...
   */
  {
    BobguiTextIter start_propagate;
    ...
    if (line && line->dir_propagated_forward == PANGO_DIRECTION_NEUTRAL)
      {
        _bobgui_text_btree_get_iter_at_line (tree, &start_propagate, line, 0);
        _bobgui_text_btree_invalidate_region (tree, &start_propagate, start, FALSE);
      }
  }
}

void
_bobgui_text_btree_delete (BobguiTextIter *start,
                        BobguiTextIter *end)
{
  BobguiTextLineSegment *prev_seg;             /* The segment just before the start
                                             * of the deletion range. */
  BobguiTextLineSegment *last_seg;             /* The segment just after the end
                                             * of the deletion range. */
  BobguiTextLineSegment *seg, *next, *next2;
  BobguiTextLine *curline;
  BobguiTextBTreeNode *curnode, *node;
  BobguiTextBTree *tree;
  BobguiTextLine *start_line;
  BobguiTextLine *end_line;
  BobguiTextLine *line;
  BobguiTextLine *deleted_lines = NULL;        /* List of lines we've deleted */
  int start_byte_offset;

  g_return_if_fail (start != NULL);
  g_return_if_fail (end != NULL);
  g_return_if_fail (_bobgui_text_iter_get_btree (start) ==
                    _bobgui_text_iter_get_btree (end));

  bobgui_text_iter_order (start, end);

  tree = _bobgui_text_iter_get_btree (start);

  if (BOBGUI_DEBUG_CHECK (TEXT))
    _bobgui_text_btree_check (tree);

  /* Broadcast the need for redisplay before we break the iterators */
  DV (g_print ("invalidating due to deleting some text (%s)\n", G_STRLOC));
  _bobgui_text_btree_invalidate_region (tree, start, end, FALSE);

  /* Save the byte offset so we can reset the iterators */
  start_byte_offset = bobgui_text_iter_get_line_index (start);
  ...
  last_seg = bobgui_text_line_segment_split (end);
  if (last_seg != NULL)
    last_seg = last_seg->next;
  else
    last_seg = end_line->segments;

  prev_seg = bobgui_text_line_segment_split (start);
  if (prev_seg != NULL)
    {
      seg = prev_seg->next;
      ...
      if (seg == NULL)
        {
          ...
          nextline = _bobgui_text_line_next (curline);
          ...
          /*
           * If the BobguiTextBTreeNode is empty then delete it and its parents,
           * recursively upwards until a non-empty BobguiTextBTreeNode is found.
           */

          while (curnode->num_children == 0)
            {
              BobguiTextBTreeNode *parent;
              ...
              if (parent->children.node == curnode)
                ...
                {
                  BobguiTextBTreeNode *prevnode = parent->children.node;
              ...
              parent->num_children--;
              bobgui_text_btree_node_free_empty (tree, curnode);
              curnode = parent;
            }
          curnode = curline->parent;
          continue;
      ...
      if ((*seg->type->deleteFunc)(seg, curline, FALSE) != 0)
        {
          ...
          if (prev_seg == NULL)
		   ...
		   prev_seg->next->type == &bobgui_text_toggle_on_type &&
		   seg->body.toggle.info == prev_seg->next->body.toggle.info)
	    {
	      ...
	      next2 = prev_seg->next->next;
              _bobgui_toggle_segment_free (prev_seg->next);
	      prev_seg->next = next2;
              _bobgui_toggle_segment_free (seg);
	      seg = NULL;
	    }
	  else
	    {
  ...
  if (start_line != end_line)
    {
      BTreeView *view;
      BobguiTextBTreeNode *ancestor_node;
      BobguiTextLine *prevline;
      int chars_moved;
       ...
       */
      ancestor_node = bobgui_text_btree_node_common_parent (curnode, start_line->parent);
      view = tree->views;
      while (view)
        {
          ...
          while (line)
            {
              BobguiTextLine *next_line = line->next;
              ld = _bobgui_text_line_get_data (line, view->view_id);
          ...
          if (deleted_width > 0 || deleted_height > 0)
            {
              ld = _bobgui_text_line_get_data (start_line, view->view_id);
              ...
              if (ld == NULL)
                {
                   ...
                   */
                  ld = _bobgui_text_line_data_new (view->layout, start_line);
                  _bobgui_text_line_add_data (start_line, ld);
                  ld->width = 0;
                  ld->height = 0;
                  ld->valid = FALSE;
                }
          ...
          bobgui_text_btree_node_check_valid_downward (ancestor_node, view->view_id);
          if (ancestor_node->parent)
            bobgui_text_btree_node_check_valid_upward (ancestor_node->parent, view->view_id);
      ...
      while (line)
        {
          ...
          bobgui_text_line_destroy (tree, line);
      ...
      bobgui_text_btree_rebalance (tree, curnode);
    }
  ...
  /*
   * Lastly, rebalance the first BobguiTextBTreeNode of the range.
   */

  bobgui_text_btree_rebalance (tree, start_line->parent);
  ...
  _bobgui_text_btree_get_iter_at_line (tree, start, start_line, start_byte_offset);
  *end = *start;

  bobgui_text_btree_resolve_bidi (start, end);
}

void
_bobgui_text_btree_insert (BobguiTextIter *iter,
                        const char *text,
                        int          len)
{
  BobguiTextLineSegment *prev_seg;     /* The segment just before the first
                                     * new segment (NULL means new segment
                                     * is at beginning of line). */
  BobguiTextLineSegment *cur_seg;              /* Current segment;  new characters
                                             * are inserted just after this one.
                                             * NULL means insert at beginning of
                                             * line. */
  BobguiTextLine *line;           /* Current line (new segments are
                                * added to this line). */
  BobguiTextLineSegment *seg;
  BobguiTextLine *newline;
  ...
  start_line = line;
  start_byte_index = bobgui_text_iter_get_line_index (iter);
   ...
   */
  g_assert (!_bobgui_text_line_is_last (line, tree));
  prev_seg = bobgui_text_line_segment_split (iter);
  cur_seg = prev_seg;
  ...
  while (eol < len)
    {
      ...
      seg = _bobgui_char_segment_new (&text[sol], chunk_len);
      ...
      /*
       * The chunk ended with a newline, so create a new BobguiTextLine
       * and move the remainder of the old line to it.
       */

      newline = bobgui_text_line_new ();
      bobgui_text_line_set_parent (newline, line->parent);
     ...
     passed in to point to the end of the inserted text. */
  {
    ...
    _bobgui_text_btree_get_iter_at_line (tree,
                                      &start,
                                      start_line,
                                      start_byte_index);
    end = start;
    ...
    /* We could almost certainly be more efficient here
       ...
       above. FIXME */
    bobgui_text_iter_forward_chars (&end, char_count_delta);
...
static void
insert_paintable_or_widget_segment (BobguiTextIter        *iter,
                                    BobguiTextLineSegment *seg)
{
  BobguiTextIter start;
  BobguiTextLineSegment *prevPtr;
  BobguiTextLine *line;
  BobguiTextBTree *tree;
  int start_byte_offset;
  ...
  line = _bobgui_text_iter_get_text_line (iter);
  tree = _bobgui_text_iter_get_btree (iter);
  start_byte_offset = bobgui_text_iter_get_line_index (iter);

  prevPtr = bobgui_text_line_segment_split (iter);
  if (prevPtr == NULL)
    {
      seg->next = line->segments;
      line->segments = seg;
  ...
  _bobgui_text_btree_get_iter_at_line (tree, &start, line, start_byte_offset);

  *iter = start;
  bobgui_text_iter_forward_char (iter); /* skip forward past the segment */

  DV (g_print ("invalidating due to inserting paintable/widget (%s)\n", G_STRLOC));
  _bobgui_text_btree_invalidate_region (tree, &start, iter, FALSE);
}

void
_bobgui_text_btree_insert_paintable (BobguiTextIter  *iter,
                                  GdkPaintable *paintable)
{
  BobguiTextLineSegment *seg;

  seg = _bobgui_paintable_segment_new (paintable);
  seg->body.paintable.tree = _bobgui_text_iter_get_btree (iter);
  seg->body.paintable.line = _bobgui_text_iter_get_text_line (iter);

  insert_paintable_or_widget_segment (iter, seg);
}

void
_bobgui_text_btree_insert_child_anchor (BobguiTextIter        *iter,
                                     BobguiTextChildAnchor *anchor)
{
  ...
  seg = _bobgui_widget_segment_new (anchor);

  tree = seg->body.child.tree = _bobgui_text_iter_get_btree (iter);
  seg->body.child.line = _bobgui_text_iter_get_text_line (iter);
...
void
_bobgui_text_btree_unregister_child_anchor (BobguiTextChildAnchor *anchor)
{
  BobguiTextLineSegment *seg;
...
static BobguiTextLine*
find_line_by_y (BobguiTextBTree *tree, BTreeView *view,
                BobguiTextBTreeNode *node, int y, int *line_top,
                BobguiTextLine *last_line)
{
  int current_y = 0;
      ...
      while (child != NULL)
        {
          ...
          bobgui_text_btree_node_get_size (child, view->view_id,
                                        &width, &height);
...
BobguiTextLine *
_bobgui_text_btree_find_line_by_y (BobguiTextBTree *tree,
                                gpointer      view_id,
                                int           ypixel,
                                int          *line_top_out)
{
  ...
  view = bobgui_text_btree_get_view (tree, view_id);
...
static int
find_line_top_in_line_list (BobguiTextBTree *tree,
                            BTreeView *view,
                            BobguiTextLine *line,
                            BobguiTextLine *target_line,
                            int y)
{
  while (line != NULL)
    {
...
int
_bobgui_text_btree_find_line_top (BobguiTextBTree *tree,
                              BobguiTextLine *target_line,
                              gpointer view_id)
{
  int y = 0;
  ...
  while (tos >= 0)
    {
      ...
      if (node->level == 0)
        ...
        {
          BobguiTextBTreeNode *child;
          BobguiTextBTreeNode *target_node;
...
void
_bobgui_text_btree_add_view (BobguiTextBTree *tree,
                         BobguiTextLayout *layout)
{
  BTreeView *view;
  BobguiTextLine *last_line;
  ...
  line_data = g_new (BobguiTextLineData, 1);
  ...
  _bobgui_text_line_add_data (last_line, line_data);
}

void
_bobgui_text_btree_remove_view (BobguiTextBTree *tree,
                             gpointer      view_id)
{
  BTreeView *view;
  BobguiTextLine *last_line;
   ...
   */
  last_line = get_last_line (tree);
  line_data = _bobgui_text_line_remove_data (last_line, view_id);
  g_free (line_data);

  bobgui_text_btree_node_remove_view (view, tree->root_node, view_id);

  g_free (view);
}

void
_bobgui_text_btree_invalidate_region (BobguiTextBTree      *tree,
                                   const BobguiTextIter *start,
                                   const BobguiTextIter *end,
                                   gboolean           cursors_only)
{
  ...
  while (view != NULL)
    {
      if (cursors_only)
	bobgui_text_layout_invalidate_cursors (view->layout, start, end);
      else
	bobgui_text_layout_invalidate (view->layout, start, end);
...
void
_bobgui_text_btree_get_view_size (BobguiTextBTree *tree,
                              gpointer view_id,
                              int *width,
                              int *height)
{
  ...
  bobgui_text_btree_node_get_size (tree->root_node, view_id,
                                width, height);
}
  ...
  if (stack->count > stack->allocated)
    {
      ...
      stack->iters = g_realloc (stack->iters,
                                stack->allocated * sizeof (BobguiTextIter));
    }
...
static void
queue_tag_redisplay (BobguiTextBTree      *tree,
                     BobguiTextTag        *tag,
                     const BobguiTextIter *start,
                     const BobguiTextIter *end)
{
  if (_bobgui_text_tag_affects_size (tag))
    {
      DV (g_print ("invalidating due to size-affecting tag (%s)\n", G_STRLOC));
      _bobgui_text_btree_invalidate_region (tree, start, end, FALSE);
    }
  else if (_bobgui_text_tag_affects_nonsize_appearance (tag))
    {
...
void
_bobgui_text_btree_tag (const BobguiTextIter *start_orig,
                     const BobguiTextIter *end_orig,
                     BobguiTextTag        *tag,
                     gboolean           add)
{
  BobguiTextLineSegment *seg, *prev;
  BobguiTextLine *cleanupline;
  gboolean toggled_on;
  BobguiTextLine *start_line;
  BobguiTextLine *end_line;
  BobguiTextIter iter;
  ...
  g_return_if_fail (_bobgui_text_iter_get_btree (start_orig) ==
                    _bobgui_text_iter_get_btree (end_orig));
  g_return_if_fail (tag->priv->table == _bobgui_text_iter_get_btree (start_orig)->table);

#if 0
  printf ("%s tag %s from %d to %d\n",
          add ? "Adding" : "Removing",
          tag->name,
          bobgui_text_buffer_get_offset (start_orig),
          bobgui_text_buffer_get_offset (end_orig));
#endif

  if (bobgui_text_iter_equal (start_orig, end_orig))
    return;
  ...
  info = bobgui_text_btree_get_tag_info (tree, tag);
   ...
   * start.
   */
  while (bobgui_text_iter_forward_to_tag_toggle (&iter, tag))
    {
  ...
  toggled_on = bobgui_text_iter_has_tag (&start, tag);
  if ( (add && !toggled_on) ||
       (!add && toggled_on) )
    {
      /* This could create a second toggle at the start position;
         cleanup_line () will remove it if so. */
      seg = _bobgui_toggle_segment_new (info, add);

      prev = bobgui_text_line_segment_split (&start);
      if (prev == NULL)
        {
          seg->next = start_line->segments;
          start_line->segments = seg;
  ...
  while (iter_stack_pop (stack, &iter))
    {
      BobguiTextLineSegment *indexable_seg;
      BobguiTextLine *line;

      line = _bobgui_text_iter_get_text_line (&iter);
      seg = _bobgui_text_iter_get_any_segment (&iter);
      indexable_seg = _bobgui_text_iter_get_indexable_segment (&iter);
      ...
      while (seg != indexable_seg)
        {
          ...
          if ( (seg->type == &bobgui_text_toggle_on_type ||
                seg->type == &bobgui_text_toggle_off_type) &&
      ...
      /* If this happens, when previously tagging we didn't merge
         overlapping tags. */
      g_assert ( (toggled_on && seg->type == &bobgui_text_toggle_off_type) ||
                 (!toggled_on && seg->type == &bobgui_text_toggle_on_type) );

      toggled_on = !toggled_on;

#if 0
      printf ("deleting %s toggle\n",
              seg->type == &bobgui_text_toggle_on_type ? "on" : "off");
#endif
      ...
      if (seg->body.toggle.inNodeCounts)
        {
          _bobgui_change_node_toggle_count (line->parent,
                                         info, -1);
  ...
  if ( (add && !toggled_on) ||
       (!add && toggled_on) )
    {
      ...
      prev = bobgui_text_line_segment_split (&end);
      if (prev == NULL)
        {
...
static BobguiTextLine*
get_line_internal (BobguiTextBTree *tree,
                   int           line_number,
                   ...
                   gboolean      include_last)
{
  ...
  line_count = _bobgui_text_btree_line_count (tree);
  if (!include_last)
    line_count -= 1;
  ...
  /*
   * Work down through levels of the tree until a BobguiTextBTreeNode is found at
   * level 0.
   */
  ...
  while (node->level != 0)
    {
      for (node = node->children.node;
           ...
           node = node->next)
        {
          ...
          if (node == NULL)
            {
              g_error ("bobgui_text_btree_find_line ran out of BobguiTextBTreeNodes");
            }
#endif
  ...
  /*
   * Work through the lines attached to the level-0 BobguiTextBTreeNode.
   */
  ...
  for (line = node->children.line; lines_left > 0;
       line = line->next)
    {
      ...
      if (line == NULL)
        {
          g_error ("bobgui_text_btree_find_line ran out of lines");
        }
#endif
      lines_left -= 1;
    }
...
BobguiTextLine*
_bobgui_text_btree_get_end_iter_line (BobguiTextBTree *tree)
{
  return
    _bobgui_text_btree_get_line (tree,
                              _bobgui_text_btree_line_count (tree) - 1,
                              NULL);
}

BobguiTextLine*
_bobgui_text_btree_get_line (BobguiTextBTree *tree,
                          int           line_number,
                          int          *real_line_number)
{
...
BobguiTextLine*
_bobgui_text_btree_get_line_no_last (BobguiTextBTree      *tree,
                                  int                line_number,
                                  int               *real_line_number)
{
...
BobguiTextLine*
_bobgui_text_btree_get_line_at_char (BobguiTextBTree      *tree,
                                  int                char_index,
 ...
 * _bobgui_text_attributes_fill_from_tags() */
GPtrArray *
_bobgui_text_btree_get_tags (const BobguiTextIter *iter)
{
  BobguiTextBTreeNode *node;
  BobguiTextLine *siblingline;
  BobguiTextLineSegment *seg;
  int src, dst, index;
  TagInfo tagInfo;
  BobguiTextLine *line;
...
#define NUM_TAG_INFOS 10
   ...
   * line but under the same level-0 BobguiTextBTreeNode.
   */
  ...
  /*
   * For each BobguiTextBTreeNode in the ancestry of this line, record tag
   * toggles for all siblings that precede that BobguiTextBTreeNode.
   */

  for (node = line->parent; node->parent != NULL;
       node = node->parent)
    {
      BobguiTextBTreeNode *siblingPtr;
      Summary *summary;
  ...
  for (src = 0, dst = 0; src < tagInfo.tags->len; src++)
    {
      if (g_array_index (tagInfo.counts, int, src) & 1)
        {
          g_assert (BOBGUI_IS_TEXT_TAG (g_ptr_array_index (tagInfo.tags, src)));
  ...
  _bobgui_text_tag_array_sort (tagInfo.tags);
...
copy_segment (GString *string,
              ...
              const BobguiTextIter *end)
{
  BobguiTextLineSegment *end_seg;
  BobguiTextLineSegment *seg;

  if (bobgui_text_iter_equal (start, end))
    return;

  seg = _bobgui_text_iter_get_indexable_segment (start);
  end_seg = _bobgui_text_iter_get_indexable_segment (end);
  ...
  if (seg->type == &bobgui_text_char_type)
    {
      ...
      if (!include_hidden &&
          _bobgui_text_btree_char_is_invisible (start))
        {
          copy = FALSE;
          /* printf (" <invisible>\n"); */
        }

      copy_start = _bobgui_text_iter_get_segment_byte (start);

      if (seg == end_seg)
        {
          /* End is in the same segment; need to copy fewer bytes. */
          int end_byte = _bobgui_text_iter_get_segment_byte (end);
  ...
  else if (seg->type == &bobgui_text_paintable_type)
    {
      ...
      if (copy)
        {
          g_string_append_len (string,
                               _bobgui_text_unknown_char_utf8,
                               BOBGUI_TEXT_UNKNOWN_CHAR_UTF8_LEN);
        }
    }
  else if (seg->type == &bobgui_text_child_type)
    {
      gboolean copy = TRUE;
      if (!include_nonchars &&
          g_strcmp0 (_bobgui_text_unknown_char_utf8, bobgui_text_child_anchor_get_replacement (seg->body.child.obj)) == 0)
        {
          copy = FALSE;
        }
      else if (!include_hidden &&
      ...
      if (copy)
        {
          g_string_append_len (string,
                               bobgui_text_child_anchor_get_replacement (seg->body.child.obj),
                               seg->byte_count);
        }
    }
}
...
char *
_bobgui_text_btree_get_text (const BobguiTextIter *start_orig,
                         const BobguiTextIter *end_orig,
                         gboolean include_hidden,
                         gboolean include_nonchars)
{
  ...
  g_return_val_if_fail (_bobgui_text_iter_get_btree (start_orig) ==
                        _bobgui_text_iter_get_btree (end_orig), NULL);
  ...
  end_seg = _bobgui_text_iter_get_indexable_segment (&end);
  iter = start;
  seg = _bobgui_text_iter_get_indexable_segment (&iter);
  while (seg != end_seg)
    {
      copy_segment (retval, include_hidden, include_nonchars,
                    &iter, &end);

      _bobgui_text_iter_forward_indexable_segment (&iter);
...
int
_bobgui_text_btree_line_count (BobguiTextBTree *tree)
{
...
int
_bobgui_text_btree_char_count (BobguiTextBTree *tree)
{
...
gboolean
_bobgui_text_btree_char_is_invisible (const BobguiTextIter *iter)
{
...
>>>>>>> origin/4627-printing-Unref-old-spool_io-before-setting-new-one-gtk3:gtk/gtktextbtree.c
   ...
   * tag table (meaning everything must be visible).
   */
  if G_LIKELY (!_bobgui_text_tag_table_affects_visibility (tree->table))
    return FALSE;
  ...
  numTags = bobgui_text_tag_table_get_size (tree->table);

  tagCnts = g_alloca (sizeof (int) * numTags);
  tags = g_alloca (sizeof (BobguiTextTag *) * numTags);
  ...
  /*
   * For each BobguiTextBTreeNode in the ancestry of this line, record tag toggles
   * for all siblings that precede that BobguiTextBTreeNode.
   */
...
static void
redisplay_region (BobguiTextBTree      *tree,
                  const BobguiTextIter *start,
                  const BobguiTextIter *end,
                  gboolean           cursors_only)
{
  BTreeView *view;
  BobguiTextLine *start_line, *end_line;

  if (bobgui_text_iter_compare (start, end) > 0)
    {
      const BobguiTextIter *tmp = start;
      start = end;
      end = tmp;
    }
  ...
  while (view != NULL)
    {
      ...
      start_y = _bobgui_text_btree_find_line_top (tree, start_line, view->view_id);

      if (end_line == start_line)
        end_y = start_y;
      else
        end_y = _bobgui_text_btree_find_line_top (tree, end_line, view->view_id);

      ld = _bobgui_text_line_get_data (start_line, view->view_id);
      if (ld)
        start_y -= ld->top_ink;

      ld = _bobgui_text_line_get_data (end_line, view->view_id);
      if (ld)
        end_y += ld->height + ld->bottom_ink;

      if (cursors_only)
	bobgui_text_layout_cursors_changed (view->layout, start_y,
					 end_y - start_y,
					 end_y - start_y);
      else
	bobgui_text_layout_changed (view->layout, start_y,
				 end_y - start_y,
				 end_y - start_y);
...
static void
redisplay_mark (BobguiTextLineSegment *mark)
{
  BobguiTextIter iter;
  BobguiTextIter end;
  gboolean cursor_only;

  _bobgui_text_btree_get_iter_at_mark (mark->body.mark.tree,
                                   &iter,
                                   mark->body.mark.obj);
  ...
  _bobgui_text_btree_invalidate_region (mark->body.mark.tree, &iter, &end, cursor_only);
}

static void
redisplay_mark_if_visible (BobguiTextLineSegment *mark)
{
...
static void
ensure_not_off_end (BobguiTextBTree *tree,
                    BobguiTextLineSegment *mark,
                    BobguiTextIter *iter)
{
  if (bobgui_text_iter_get_line (iter) == _bobgui_text_btree_line_count (tree))
    bobgui_text_iter_backward_char (iter);
}

static BobguiTextLineSegment*
real_set_mark (BobguiTextBTree      *tree,
               ...
               gboolean           redraw_selections)
{
  ...
  g_return_val_if_fail (_bobgui_text_iter_get_btree (where) == tree, NULL);

  if (existing_mark)
    {
      if (bobgui_text_mark_get_buffer (existing_mark) != NULL)
  ...
  if (mark != NULL)
    {
      if (redraw_selections &&
        ...
        {
          BobguiTextIter old_pos;

          _bobgui_text_btree_get_iter_at_mark (tree, &old_pos,
                                           mark->body.mark.obj);
      ...
      bobgui_text_btree_unlink_segment (tree, mark,
                                     mark->body.mark.line);
      mark->body.mark.line = _bobgui_text_iter_get_text_line (&iter);
      g_assert (mark->body.mark.line == _bobgui_text_iter_get_text_line (&iter));
	...
	existing_mark = bobgui_text_mark_new (name, left_gravity);

      mark = existing_mark->segment;
      _bobgui_mark_segment_set_tree (mark, tree);
  ...
  bobgui_text_btree_link_segment (mark, &iter);
...
BobguiTextMark*
_bobgui_text_btree_set_mark (BobguiTextBTree *tree,
                         BobguiTextMark  *existing_mark,
                         const char *name,
                         gboolean left_gravity,
                         const BobguiTextIter *iter,
...
gboolean
_bobgui_text_btree_get_selection_bounds (BobguiTextBTree *tree,
                                     BobguiTextIter  *start,
                                     BobguiTextIter  *end)
{
  BobguiTextIter tmp_start, tmp_end;

  _bobgui_text_btree_get_iter_at_mark (tree, &tmp_start,
                                   tree->insert_mark);
  _bobgui_text_btree_get_iter_at_mark (tree, &tmp_end,
                                   tree->selection_bound_mark);

  if (bobgui_text_iter_equal (&tmp_start, &tmp_end))
    {
      if (start)
        *start = tmp_start;
    ...
    {
      bobgui_text_iter_order (&tmp_start, &tmp_end);
...
void
_bobgui_text_btree_place_cursor (BobguiTextBTree      *tree,
                             const BobguiTextIter *iter)
{
  _bobgui_text_btree_select_range (tree, iter, iter);
}

void
_bobgui_text_btree_select_range (BobguiTextBTree      *tree,
			      const BobguiTextIter *ins,
                              const BobguiTextIter *bound)
{
  BobguiTextIter old_ins, old_bound;

  _bobgui_text_btree_get_iter_at_mark (tree, &old_ins,
                                    tree->insert_mark);
  _bobgui_text_btree_get_iter_at_mark (tree, &old_bound,
                                    tree->selection_bound_mark);

  /* Check if it's no-op since bobgui_text_buffer_place_cursor()
   * also calls this, and this will redraw the cursor line. */
  if (!bobgui_text_iter_equal (&old_ins, ins) ||
      !bobgui_text_iter_equal (&old_bound, bound))
    {
...
void
_bobgui_text_btree_remove_mark_by_name (BobguiTextBTree *tree,
                                    const char *name)
{
  BobguiTextMark *mark;
  ...
  _bobgui_text_btree_remove_mark (tree, mark);
}

void
_bobgui_text_btree_release_mark_segment (BobguiTextBTree       *tree,
...
void
_bobgui_text_btree_remove_mark (BobguiTextBTree *tree,
                             BobguiTextMark *mark)
{
  BobguiTextLineSegment *segment;
  ...
  bobgui_text_btree_unlink_segment (tree, segment, segment->body.mark.line);

  _bobgui_text_btree_release_mark_segment (tree, segment);
}

gboolean
_bobgui_text_btree_mark_is_insert (BobguiTextBTree *tree,
...
gboolean
_bobgui_text_btree_mark_is_selection_bound (BobguiTextBTree *tree,
                                         BobguiTextMark *segment)
{
...
BobguiTextMark *
_bobgui_text_btree_get_insert (BobguiTextBTree *tree)
{
  return tree->insert_mark;
}

BobguiTextMark *
_bobgui_text_btree_get_selection_bound (BobguiTextBTree *tree)
{
  return tree->selection_bound_mark;
}

BobguiTextMark*
_bobgui_text_btree_get_mark_by_name (BobguiTextBTree *tree,
                                  const char *name)
{
  BobguiTextLineSegment *seg;
 ...
 */
void
bobgui_text_mark_set_visible (BobguiTextMark *mark,
                           gboolean     setting)
{
  BobguiTextLineSegment *seg;
...
BobguiTextLine*
_bobgui_text_btree_first_could_contain_tag (BobguiTextBTree *tree,
                                        BobguiTextTag *tag)
{
  BobguiTextBTreeNode *node;
  BobguiTextTagInfo *info;
  ...
  if (tag != NULL)
    {
      info = bobgui_text_btree_get_existing_tag_info (tree, tag);
      ...
      while (node->level > 0)
        {
          ...
          while (node != NULL)
            {
              if (bobgui_text_btree_node_has_tag (node, tag))
  ...
  else
    {
      ...
      return _bobgui_text_btree_get_line (tree, 0, NULL);
    }
}

BobguiTextLine*
_bobgui_text_btree_last_could_contain_tag (BobguiTextBTree *tree,
                                       BobguiTextTag *tag)
{
  BobguiTextBTreeNode *node;
  BobguiTextBTreeNode *last_node;
  BobguiTextLine *line;
  BobguiTextTagInfo *info;
  ...
  if (tag != NULL)
  ...
  else
    {
      ...
      */
      return _bobgui_text_btree_get_end_iter_line (tree);
    }
}
...
int
_bobgui_text_line_get_number (BobguiTextLine *line)
{
  BobguiTextLine *line2;
  BobguiTextBTreeNode *node, *parent, *node2;
  int index;
  ...
  for (line2 = node->children.line; line2 != line;
       line2 = line2->next)
    {
      if (line2 == NULL)
        {
          g_error ("bobgui_text_btree_line_number couldn't find line");
        }
      index += 1;
    }
   ...
   * counting how many lines are in BobguiTextBTreeNodes preceding the current
   * BobguiTextBTreeNode.
   */
       ...
       node = parent, parent = parent->parent)
    {
      for (node2 = parent->children.node; node2 != node;
           node2 = node2->next)
        {
          if (node2 == NULL)
            {
              g_error ("bobgui_text_btree_line_number couldn't find BobguiTextBTreeNode");
            }
...
static BobguiTextLineSegment*
find_toggle_segment_before_char (BobguiTextLine *line,
                                 int char_in_line,
                                 BobguiTextTag *tag)
{
  BobguiTextLineSegment *seg;
  BobguiTextLineSegment *toggle_seg;
  int index;
...
static BobguiTextLineSegment*
find_toggle_segment_before_byte (BobguiTextLine *line,
                                 int byte_in_line,
                                 BobguiTextTag *tag)
{
  BobguiTextLineSegment *seg;
...
static gboolean
find_toggle_outside_current_line (BobguiTextLine *line,
                                  BobguiTextBTree *tree,
                                  BobguiTextTag *tag)
{
  BobguiTextBTreeNode *node;
  BobguiTextLine *sibling_line;
  BobguiTextLineSegment *seg;
  BobguiTextLineSegment *toggle_seg;
  int toggles;
  BobguiTextTagInfo *info = NULL;

  /*
   * No toggle in this line.  Look for toggles for the tag in lines
   * that are predecessors of line but under the same
   * level-0 BobguiTextBTreeNode.
   */
  toggle_seg = NULL;
  ...
  if (toggle_seg != NULL)
    return (toggle_seg->type == &bobgui_text_toggle_on_type);

  /*
   * No toggle in this BobguiTextBTreeNode.  Scan upwards through the ancestors of
   * this BobguiTextBTreeNode, counting the number of toggles of the given tag in
   * siblings that precede that BobguiTextBTreeNode.
   */
  ...
  while (node->parent != NULL)
    {
      BobguiTextBTreeNode *sibling_node;
...
gboolean
_bobgui_text_line_char_has_tag (BobguiTextLine *line,
                             BobguiTextBTree *tree,
                             int char_in_line,
                             BobguiTextTag *tag)
{
...
gboolean
_bobgui_text_line_byte_has_tag (BobguiTextLine *line,
                             BobguiTextBTree *tree,
                             int byte_in_line,
                             BobguiTextTag *tag)
{
...
gboolean
_bobgui_text_line_is_last (BobguiTextLine *line,
                        BobguiTextBTree *tree)
{
...
static void
ensure_end_iter_line (BobguiTextBTree *tree)
{
  if (tree->end_iter_line_stamp != tree->chars_changed_stamp)
    {
      ...
      g_assert (_bobgui_text_btree_line_count (tree) >= 1);

      tree->end_iter_line = _bobgui_text_btree_get_line_no_last (tree, -1, &real_line);
...
static void
ensure_end_iter_segment (BobguiTextBTree *tree)
{
  if (tree->end_iter_segment_stamp != tree->segments_changed_stamp)
    {
      BobguiTextLineSegment *seg;
      BobguiTextLineSegment *last_with_chars;
      ...
      g_assert (tree->end_iter_segment->type == &bobgui_text_char_type);
      g_assert (tree->end_iter_segment->body.chars[tree->end_iter_segment_byte_index] == '\n');
    }
}

gboolean
_bobgui_text_line_contains_end_iter (BobguiTextLine  *line,
                                  BobguiTextBTree *tree)
{
  ensure_end_iter_line (tree);
...
gboolean
_bobgui_text_btree_is_end (BobguiTextBTree       *tree,
                        BobguiTextLine        *line,
                        BobguiTextLineSegment *seg,
                        int                 byte_index,
                        int                 char_offset)
{
  ...
  if (!_bobgui_text_line_contains_end_iter (line, tree))
    return FALSE;
...
BobguiTextLine*
_bobgui_text_line_next (BobguiTextLine *line)
{
  BobguiTextBTreeNode *node;
  ...
  if (line->next != NULL)
  ...
  else
    {
       ...
       * BobguiTextBTreeNode.  Search up the tree for the next BobguiTextBTreeNode,
       * then search down from that BobguiTextBTreeNode to find the first
       * line.
       */
...
BobguiTextLine*
_bobgui_text_line_next_excluding_last (BobguiTextLine *line)
{
  BobguiTextLine *next;

  next = _bobgui_text_line_next (line);

  /* If we were on the end iter line, we can't go to
   * the last line
   */
  if (next && next->next == NULL && /* these checks are optimization only */
      _bobgui_text_line_next (next) == NULL)
    return NULL;
...
BobguiTextLine*
_bobgui_text_line_previous (BobguiTextLine *line)
{
  BobguiTextBTreeNode *node;
  BobguiTextBTreeNode *node2;
  BobguiTextLine *prev;
  ...
  /*
   * Find the line under this BobguiTextBTreeNode just before the starting line.
   */
  ...
  while (prev != line)
    {
      ...
      if (prev == NULL)
        g_error ("bobgui_text_btree_previous_line ran out of lines");
    }

  /*
   * This was the first line associated with the particular parent
   * BobguiTextBTreeNode.  Search up the tree for the previous BobguiTextBTreeNode,
   * then search down from that BobguiTextBTreeNode to find its last line.
   */
...
BobguiTextLineData*
_bobgui_text_line_data_new (BobguiTextLayout *layout,
                         BobguiTextLine   *line)
{
  BobguiTextLineData *line_data;
...
void
_bobgui_text_line_add_data (BobguiTextLine     *line,
...
gpointer
_bobgui_text_line_remove_data (BobguiTextLine *line,
                           gpointer view_id)
{
  BobguiTextLineData *prev;
  BobguiTextLineData *iter;
...
gpointer
_bobgui_text_line_get_data (BobguiTextLine *line,
                         gpointer view_id)
{
  BobguiTextLineData *iter;
...
void
_bobgui_text_line_invalidate_wrap (BobguiTextLine *line,
                                BobguiTextLineData *ld)
{
  ...
  */
  ...
  ld->valid = FALSE;
  bobgui_text_btree_node_invalidate_upward (line->parent, ld->view_id);
}

int
_bobgui_text_line_char_count (BobguiTextLine *line)
{
  BobguiTextLineSegment *seg;
  int size;
...
int
_bobgui_text_line_byte_count (BobguiTextLine *line)
{
  BobguiTextLineSegment *seg;
  int size;
...
int
_bobgui_text_line_char_index (BobguiTextLine *target_line)
{
  BobguiTextBTreeNode *node_stack[64];
  BobguiTextBTreeNode *iter;
  BobguiTextLine *line;
  int num_chars;
  int tos = 0;
  ...
  while (tos >= 0)
    {
      BobguiTextBTreeNode *child_iter;
  ...
  while (line != target_line)
    {
      ...
      num_chars += _bobgui_text_line_char_count (line);
      line = line->next;
    }
...
_bobgui_text_line_byte_to_segment (BobguiTextLine *line,
                               int byte_offset,
                               int *seg_offset)
{
  BobguiTextLineSegment *seg;
...
BobguiTextLineSegment*
_bobgui_text_line_char_to_segment (BobguiTextLine *line,
                               int char_offset,
                               int *seg_offset)
{
  BobguiTextLineSegment *seg;
...
BobguiTextLineSegment*
_bobgui_text_line_byte_to_any_segment (BobguiTextLine *line,
                                   int byte_offset,
                                   int *seg_offset)
{
  BobguiTextLineSegment *seg;
...
BobguiTextLineSegment*
_bobgui_text_line_char_to_any_segment (BobguiTextLine *line,
                                   int char_offset,
                                   int *seg_offset)
{
  BobguiTextLineSegment *seg;
...
int
_bobgui_text_line_byte_to_char (BobguiTextLine *line,
                            int byte_offset)
{
  int char_offset;
  BobguiTextLineSegment *seg;
...
int
_bobgui_text_line_char_to_byte (BobguiTextLine *line,
                            int          char_offset)
{
...
gboolean
_bobgui_text_line_byte_locate (BobguiTextLine *line,
                            int byte_offset,
                            BobguiTextLineSegment **segment,
...
{
  BobguiTextLineSegment *seg;
  BobguiTextLineSegment *after_last_indexable;
  BobguiTextLineSegment *last_indexable;
  int offset;
  int bytes_in_line;
...
/* FIXME sync with byte_locate (or figure out a clean
   way to merge the two functions) */
gboolean
_bobgui_text_line_char_locate     (BobguiTextLine     *line,
                                int               char_offset,
                                BobguiTextLineSegment **segment,
                                BobguiTextLineSegment **any_segment,
                                int              *seg_char_offset,
...
void
_bobgui_text_line_byte_to_char_offsets (BobguiTextLine *line,
                                    int byte_offset,
                                    int *line_char_offset,
                                    int *seg_char_offset)
{
...
void
_bobgui_text_line_char_to_byte_offsets (BobguiTextLine *line,
                                    int char_offset,
                                    int *line_byte_offset,
                                    int *seg_byte_offset)
{
...
static int
node_compare (BobguiTextBTreeNode *lhs,
              BobguiTextBTreeNode *rhs)
{
  BobguiTextBTreeNode *iter;
  BobguiTextBTreeNode *node;
  BobguiTextBTreeNode *common_parent;
  BobguiTextBTreeNode *parent_of_lower;
  BobguiTextBTreeNode *parent_of_higher;
  gboolean lhs_is_lower;
  BobguiTextBTreeNode *lower;
  BobguiTextBTreeNode *higher;
...
BobguiTextLine*
_bobgui_text_line_next_could_contain_tag (BobguiTextLine  *line,
                                       BobguiTextBTree *tree,
                                       BobguiTextTag   *tag)
{
  BobguiTextBTreeNode *node;
  ...
  if (tag == NULL)
    {
       ...
       */
      return _bobgui_text_line_next_excluding_last (line);
    }
...
static BobguiTextLine*
prev_line_under_node (BobguiTextBTreeNode *node,
                      BobguiTextLine      *line)
{
  BobguiTextLine *prev;
...
BobguiTextLine*
_bobgui_text_line_previous_could_contain_tag (BobguiTextLine  *line,
                                          BobguiTextBTree *tree,
                                          BobguiTextTag   *tag)
{
  BobguiTextBTreeNode *node;
  BobguiTextBTreeNode *found_node = NULL;
  BobguiTextTagInfo *info;
  gboolean below_tag_root;
  BobguiTextLine *prev;
  BobguiTextBTreeNode *line_ancestor;
  BobguiTextBTreeNode *line_ancestor_parent;
  ...
  if (tag == NULL)
    {
       ...
       * to know about any tag toggle at all.
       */
      return _bobgui_text_line_previous (line);
    }
  ...
  if (below_tag_root)
    {
      ...
      while (line_ancestor != info->tag_root)
	    ...
	    node = line_ancestor;
          ...
          while (tmp != NULL)
            {
              BobguiTextBTreeNode *this_node = tmp->data;

              g_assert (this_node != line_ancestor);

              if (bobgui_text_btree_node_has_tag (this_node, tag))
                {
  ...
  while (node->level > 0)
    {
      ...
      while (iter != NULL)
        {
          if (bobgui_text_btree_node_has_tag (iter->data, tag))
            {
...
static BobguiTextLine*
get_last_line (BobguiTextBTree *tree)
{
  if (tree->last_line_stamp != tree->chars_changed_stamp)
    {
      ...
      n_lines = _bobgui_text_btree_line_count (tree);

      g_assert (n_lines >= 1); /* num_lines doesn't return bogus last line. */

      line = _bobgui_text_btree_get_line (tree, n_lines, &real_line);
...
static BobguiTextLine*
bobgui_text_line_new (void)
{
  BobguiTextLine *line;

  line = g_new0 (BobguiTextLine, 1);
  line->dir_strong = PANGO_DIRECTION_NEUTRAL;
...
static void
bobgui_text_line_destroy (BobguiTextBTree *tree, BobguiTextLine *line)
{
  BobguiTextLineData *ld;
  ...
  while (ld != NULL)
    {
      ...
      view = bobgui_text_btree_get_view (tree, ld->view_id);

      g_assert (view != NULL);

      next = ld->next;
      bobgui_text_layout_free_line_data (view->layout, line, ld);
...
static void
bobgui_text_line_set_parent (BobguiTextLine *line,
                          BobguiTextBTreeNode *node)
{
  if (line->parent == node)
    return;
  line->parent = node;
  bobgui_text_btree_node_invalidate_upward (node, NULL);
}

static void
cleanup_line (BobguiTextLine *line)
{
  BobguiTextLineSegment *seg, **prev_p;
  gboolean changed;
...
static BobguiTextBTreeNode*
bobgui_text_btree_node_new (void)
{
  BobguiTextBTreeNode *node;

  node = g_new (BobguiTextBTreeNode, 1);
...
static void
bobgui_text_btree_node_adjust_toggle_count (BobguiTextBTreeNode  *node,
                                         BobguiTextTagInfo  *info,
                                         int adjust)
{
  Summary *summary;
...
/* Note that the tag root and above do not have summaries
   ...
   the summaries. */
static gboolean
bobgui_text_btree_node_has_tag (BobguiTextBTreeNode *node, BobguiTextTag *tag)
{
  Summary *summary;
...
#if 0
static void
bobgui_text_btree_node_invalidate_downward (BobguiTextBTreeNode *node)
{
  NodeData *nd;
  ...
  if (node->level == 0)
  ...
  else
    {
      ...
      while (child != NULL)
        {
          bobgui_text_btree_node_invalidate_downward (child);
...
static void
bobgui_text_btree_node_invalidate_upward (BobguiTextBTreeNode *node, gpointer view_id)
{
  BobguiTextBTreeNode *iter;
...
/**
 * _bobgui_text_btree_is_valid:
 * @tree: a BobguiTextBTree
 * @view_id: ID for the view
 *
 * Check to see if the entire BobguiTextBTree is valid or not for
 * the given view.
 *
 * Returns: %TRUE if the entire BobguiTextBTree is valid
 **/
gboolean
_bobgui_text_btree_is_valid (BobguiTextBTree *tree,
                         gpointer      view_id)
{
  NodeData *nd;
...
static void
bobgui_text_btree_node_validate (BTreeView         *view,
                              BobguiTextBTreeNode  *node,
                              gpointer           view_id,
                              ValidateState     *state)
{
  ...
  NodeData *nd = bobgui_text_btree_node_ensure_data (node, view_id);
  g_return_if_fail (!nd->valid);

  if (node->level == 0)
    {
      BobguiTextLine *line = node->children.line;
      BobguiTextLineData *ld;

      /* Iterate over leading valid lines */
      while (line != NULL)
        {
          ld = _bobgui_text_line_get_data (line, view_id);
      ...
      while (line != NULL)
        {
          ...
          if (ld && ld->valid)
          ...
          else
            {
              ...
              ld = bobgui_text_layout_wrap (view->layout, line, ld);
  ...
  else
    {
      ...
      while (child)
        {
          child_nd = bobgui_text_btree_node_ensure_data (child, view_id);
      ...
      while (child)
        {
          ...
          if (child_nd->valid)
            ...
            {
              bobgui_text_btree_node_validate (view, child, view_id, state);
 ...
 *
 * Validate a single contiguous invalid region of a BobguiTextBTree for
 * a given view.
 *
 ...
 **/
gboolean
_bobgui_text_btree_validate (BobguiTextBTree *tree,
                         gpointer      view_id,
                         int           max_pixels,
                         int          *y,
                         int          *old_height,
                         int          *new_height)
{
  ...
  if (!_bobgui_text_btree_is_valid (tree, view_id))
    {
      ValidateState state;
      ...
      bobgui_text_btree_node_validate (view,
                                    tree->root_node,
                                    view_id, &state);
...
static void
bobgui_text_btree_node_compute_view_aggregates (BobguiTextBTreeNode *node,
                                             gpointer          view_id,
                                             int              *width_out,
                                             int              *height_out,
                                             gboolean         *valid_out)
  ...
  if (node->level == 0)
    {
      ...
      while (line != NULL)
        {
          BobguiTextLineData *ld = _bobgui_text_line_get_data (line, view_id);
    ...
    {
      BobguiTextBTreeNode *child = node->children.node;
 ...
 */
static NodeData *
bobgui_text_btree_node_check_valid (BobguiTextBTreeNode *node,
                                 gpointer          view_id)
{
  NodeData *nd = bobgui_text_btree_node_ensure_data (node, view_id);
  gboolean valid;
  ...
  bobgui_text_btree_node_compute_view_aggregates (node, view_id,
                                               &width, &height, &valid);
  nd->width = width;
  nd->height = height;
  nd->valid = valid;
...
static void
bobgui_text_btree_node_check_valid_upward (BobguiTextBTreeNode *node,
                                        gpointer          view_id)
{
  while (node)
    {
      bobgui_text_btree_node_check_valid (node, view_id);
      node = node->parent;
    }
}

static NodeData *
bobgui_text_btree_node_check_valid_downward (BobguiTextBTreeNode *node,
                                          gpointer          view_id)
{
  if (node->level == 0)
    {
      return bobgui_text_btree_node_check_valid (node, view_id);
    }
  else
    {
      BobguiTextBTreeNode *child = node->children.node;
      ...
      while (child)
        {
          NodeData *child_nd = bobgui_text_btree_node_check_valid_downward (child, view_id);
...
/**
 * _bobgui_text_btree_validate_line:
 * @tree: a BobguiTextBTree
 ...
 **/
void
_bobgui_text_btree_validate_line (BobguiTextBTree     *tree,
                               BobguiTextLine      *line,
                               gpointer          view_id)
{
  BobguiTextLineData *ld;
  ...
  if (!ld || !ld->valid)
    {
      bobgui_text_layout_wrap (view->layout, line, ld);
      bobgui_text_btree_node_check_valid_upward (line->parent, view_id);
    }
}

static void
bobgui_text_btree_node_remove_view (BTreeView *view, BobguiTextBTreeNode *node, gpointer view_id)
{
  if (node->level == 0)
    {
      BobguiTextLine *line;
      ...
      while (line != NULL)
        {
          ...
          ld = _bobgui_text_line_remove_data (line, view_id);
  ...
  else
    {
      ...
      while (child != NULL)
        {
          /* recurse */
          bobgui_text_btree_node_remove_view (view, child, view_id);

          child = child->next;
        }
    }

  bobgui_text_btree_node_remove_data (node, view_id);
}

static void
bobgui_text_btree_node_destroy (BobguiTextBTree *tree, BobguiTextBTreeNode *node)
{
  if (node->level == 0)
    {
      BobguiTextLine *line;
    ...
    {
      BobguiTextBTreeNode *childPtr;

      while (node->children.node != NULL)
        {
          childPtr = node->children.node;
          node->children.node = childPtr->next;
          bobgui_text_btree_node_destroy (tree, childPtr);
        }
    }

  bobgui_text_btree_node_free_empty (tree, node);
}

static void
bobgui_text_btree_node_free_empty (BobguiTextBTree *tree,
                                BobguiTextBTreeNode *node)
{
...
static NodeData*
bobgui_text_btree_node_ensure_data (BobguiTextBTreeNode *node, gpointer view_id)
{
  NodeData *nd;
...
static void
bobgui_text_btree_node_remove_data (BobguiTextBTreeNode *node, gpointer view_id)
{
  NodeData *nd;
  NodeData *prev;
...
static void
bobgui_text_btree_node_get_size (BobguiTextBTreeNode *node, gpointer view_id,
                              int *width, int *height)
{
  ...
  nd = bobgui_text_btree_node_ensure_data (node, view_id);
 ...
 */
static BobguiTextBTreeNode *
bobgui_text_btree_node_common_parent (BobguiTextBTreeNode *node1,
                                   BobguiTextBTreeNode *node2)
{
...
static BTreeView*
bobgui_text_btree_get_view (BobguiTextBTree *tree, gpointer view_id)
{
  BTreeView *view;
...
static void
get_tree_bounds (BobguiTextBTree *tree,
                 BobguiTextIter *start,
                 BobguiTextIter *end)
{
  _bobgui_text_btree_get_iter_at_line_char (tree, start, 0, 0);
  _bobgui_text_btree_get_end_iter (tree, end);
}

static void
tag_changed_cb (BobguiTextTagTable *table,
                BobguiTextTag      *tag,
                gboolean         size_changed,
                BobguiTextBTree    *tree)
{
  if (size_changed)
    {
      ...
      if (_bobgui_text_btree_get_iter_at_first_toggle (tree, &start, tag))
        {
          /* Must be a last toggle if there was a first one. */
          _bobgui_text_btree_get_iter_at_last_toggle (tree, &end, tag);
      ...
      while (view != NULL)
        {
          ...
          _bobgui_text_btree_get_view_size (tree, view->view_id, &width, &height);
          bobgui_text_layout_changed (view->layout, 0, height, height);
...
void
_bobgui_text_btree_notify_will_remove_tag (BobguiTextBTree    *tree,
                                        BobguiTextTag      *tag)
{
  ...
  bobgui_text_btree_remove_tag_info (tree, tag);
}


/* Rebalance the out-of-whack node "node" */
static void
bobgui_text_btree_rebalance (BobguiTextBTree *tree,
                          BobguiTextBTreeNode *node)
{
  /*
   * Loop over the entire ancestral chain of the BobguiTextBTreeNode, working
   * up through the tree one BobguiTextBTreeNode at a time until the root
   * BobguiTextBTreeNode has been processed.
   */

  while (node != NULL)
    {
      BobguiTextBTreeNode *new_node, *child;
      BobguiTextLine *line;
      int i;

      /*
       * Check to see if the BobguiTextBTreeNode has too many children.  If it does,
       * then split off all but the first MIN_CHILDREN into a separate
       * BobguiTextBTreeNode following the original one.  Then repeat until the
       * BobguiTextBTreeNode has a decent size.
       */
      ...
      if (node->num_children > MAX_CHILDREN)
        {
          while (1)
            {
              /*
               * If the BobguiTextBTreeNode being split is the root
               * BobguiTextBTreeNode, then make a new root BobguiTextBTreeNode above
               * it first.
               */

              if (node->parent == NULL)
                {
                  new_node = bobgui_text_btree_node_new ();
      ...
      while (node->num_children < MIN_CHILDREN)
        {
          BobguiTextBTreeNode *other;
          BobguiTextBTreeNode *halfwaynode = NULL; /* Initialization needed only */
          BobguiTextLine *halfwayline = NULL; /* to prevent cc warnings. */
          int total_children, first_children;

          /*
           * Too few children for this BobguiTextBTreeNode.  If this is the root then,
           * it's OK for it to have less than MIN_CHILDREN children
           * as long as it's got at least two.  If it has only one
           * (and isn't at level 0), then chop the root BobguiTextBTreeNode out of
          ...
          if (node->parent->num_children < 2)
            {
              bobgui_text_btree_rebalance (tree, node->parent);
              continue;
            }
           ...
           * into one BobguiTextBTreeNode or redivide the children among them to
          ...
          if (total_children <= MAX_CHILDREN)
            {
              ...
              other->children.node = NULL;
              other->children.line = NULL;
              bobgui_text_btree_node_free_empty (tree, other);
              continue;
            }
...
static void
post_insert_fixup (BobguiTextBTree *tree,
                   BobguiTextLine *line,
                   int line_count_delta,
                   int char_count_delta)

{
  ...
  /*
   * Increment the line counts in all the parent BobguiTextBTreeNodes of the insertion
  ...
  if (node->num_children > MAX_CHILDREN)
    {
      bobgui_text_btree_rebalance (tree, node);
    }
...
static BobguiTextTagInfo*
bobgui_text_btree_get_existing_tag_info (BobguiTextBTree *tree,
                                      BobguiTextTag   *tag)
{
  BobguiTextTagInfo *info;
  GSList *list;
...
static BobguiTextTagInfo*
bobgui_text_btree_get_tag_info (BobguiTextBTree *tree,
                             BobguiTextTag   *tag)
{
  BobguiTextTagInfo *info;
  ...
  if (info == NULL)
    {
      ...
      info = g_new (BobguiTextTagInfo, 1);
...
static void
bobgui_text_btree_remove_tag_info (BobguiTextBTree *tree,
                                BobguiTextTag   *tag)
{
  BobguiTextTagInfo *info;
  GSList *list;
...
static void
recompute_level_zero_counts (BobguiTextBTreeNode *node)
{
  BobguiTextLine *line;
  BobguiTextLineSegment *seg;
  ...
  while (line != NULL)
    {
      ...
      if (line->parent != node)
        bobgui_text_line_set_parent (line, node);
      ...
      while (seg != NULL)
        {
          ...
          if (((seg->type != &bobgui_text_toggle_on_type)
          ...
          else
            {
              ...
              bobgui_text_btree_node_adjust_toggle_count (node, info, 1);
            }
...
static void
recompute_level_nonzero_counts (BobguiTextBTreeNode *node)
{
  Summary *summary;
  BobguiTextBTreeNode *child;
  ...
  while (child != NULL)
    {
      ...
      while (summary != NULL)
        {
          bobgui_text_btree_node_adjust_toggle_count (node,
                                                   summary->info,
 ...
 *
 *      This procedure is called to recompute all the counts in a BobguiTextBTreeNode
 *      (tags, child information, etc.) by scanning the information in
 *      its descendants.  This procedure is called during rebalancing
 *      when a BobguiTextBTreeNode’s child structure has changed.
 *
 * Results:
 *      None.
 *
...
static void
recompute_node_counts (BobguiTextBTree *tree, BobguiTextBTreeNode *node)
{
  BTreeView *view;
  Summary *summary, *summary2;

  /*
   * Zero out all the existing counts for the BobguiTextBTreeNode, but don’t delete
   ...
   * the BobguiTextBTreeNode’s tag counts and adding new Summary structures if
   * necessary.
   */
  ...
  while (view)
    {
      bobgui_text_btree_node_check_valid (node, view->view_id);
      view = view->next;
    }

  /*
   * Scan through the BobguiTextBTreeNode’s tag records again and delete any Summary
   * records that still have a zero count, or that have all the toggles.
   * The BobguiTextBTreeNode with the children that account for all the tags toggles
  ...
  for (summary = node->summary; summary != NULL; )
    {
      if (summary->toggle_count > 0 &&
          summary->toggle_count < summary->info->toggle_count)
        {
          if (node->level == summary->info->tag_root->level)
            {
              /*
               * The tag’s root BobguiTextBTreeNode split and some toggles left.
      ...
      if (summary->toggle_count == summary->info->toggle_count)
        {
          /*
           * A BobguiTextBTreeNode merge has collected all the toggles under
           * one BobguiTextBTreeNode.  Push the root down to this level.
           */
...
void
_bobgui_change_node_toggle_count (BobguiTextBTreeNode *node,
                               BobguiTextTagInfo   *info,
                               int               delta) /* may be negative */
{
  Summary *summary, *prevPtr;
  BobguiTextBTreeNode *node2Ptr;
  int rootLevel;                        /* Level of original tag root */

  info->toggle_count += delta;

  if (info->tag_root == (BobguiTextBTreeNode *) NULL)
    {
      info->tag_root = node;
      return;
    }
  ...
  /*
   * Iterate over the BobguiTextBTreeNode and its ancestors up to the tag root, adjusting
   * summary counts at each BobguiTextBTreeNode and moving the tag’s root upwards if
   * necessary.
   */
  ...
  for ( ; node != info->tag_root; node = node->parent)
    {
      /*
       * See if there’s already an entry for this tag for this BobguiTextBTreeNode.  If so,
      ...
      if (summary != NULL)
        {
          ...
          if (summary->toggle_count != 0)
            {
              /*
               * Should never find a BobguiTextBTreeNode with max toggle count at this
          ...
          if (rootLevel == node->level)
            {
               ...
               * BobguiTextBTreeNode, but it isn’t at this BobguiTextBTreeNode.  Move the tag root up
               * a level, in the hopes that it will now cover this BobguiTextBTreeNode
               ...
               * parent BobguiTextBTreeNode.
               */

              BobguiTextBTreeNode *rootnode = info->tag_root;
              summary = g_new (Summary, 1);
              summary->info = info;
  ...
  if (info->toggle_count == 0)
    {
      info->tag_root = (BobguiTextBTreeNode *) NULL;
      return;
    }
  node = info->tag_root;
  while (node->level > 0)
    {
      /*
       * See if a single child BobguiTextBTreeNode accounts for all of the tag’s
       * toggles.  If so, push the root down one level.
       */

      for (node2Ptr = node->children.node;
           node2Ptr != (BobguiTextBTreeNode *)NULL ;
           node2Ptr = node2Ptr->next)
        {
          ...
          if (summary->toggle_count != info->toggle_count)
            {
              /*
               * No BobguiTextBTreeNode has all toggles, so the root is still valid.
               */
          ...
          /*
           * This BobguiTextBTreeNode has all the toggles, so push down the root.
           */
 ...
 * inc_count --
 *
 *      This is a utility procedure used by _bobgui_text_btree_get_tags.  It
...
static void
inc_count (BobguiTextTag *tag, int inc, TagInfo *tagInfoPtr)
{
  for (int i = 0; i < tagInfoPtr->tags->len; i++)
    {
      BobguiTextTag *t = g_ptr_array_index (tagInfoPtr->tags, i);
      if (t == tag)
        {
...
static void
bobgui_text_btree_link_segment (BobguiTextLineSegment *seg,
                             const BobguiTextIter *iter)
{
  BobguiTextLineSegment *prev;
  BobguiTextLine *line;
  BobguiTextBTree *tree;
  ...
  prev = bobgui_text_line_segment_split (iter);
  if (prev == NULL)
    {
      seg->next = line->segments;
      line->segments = seg;
...
static void
bobgui_text_btree_unlink_segment (BobguiTextBTree *tree,
                               BobguiTextLineSegment *seg,
                               BobguiTextLine *line)
{
  BobguiTextLineSegment *prev;
 ...
 * belongs in bobguitextsegment.c
 */
 ...
 *--------------------------------------------------------------
 *
 * _bobgui_toggle_segment_check_func --
 *
...
void
_bobgui_toggle_segment_check_func (BobguiTextLineSegment *segPtr,
                                BobguiTextLine *line)
{
  Summary *summary;
  int needSummary;
  ...
  if (!segPtr->body.toggle.inNodeCounts)
    {
      g_error ("toggle_segment_check_func: toggle counts not updated in BobguiTextBTreeNodes");
    }
      ...
      if (summary == NULL)
        {
          if (needSummary)
            {
              g_error ("toggle_segment_check_func: tag not present in BobguiTextBTreeNode");
            }
          else
            {
              break;
      ...
      if (summary->info == segPtr->body.toggle.info)
        {
          if (!needSummary)
            {
              g_error ("toggle_segment_check_func: tag present in root BobguiTextBTreeNode summary");
            }
          break;
        }
    }
 ...
 */
static void
bobgui_text_btree_node_view_check_consistency (BobguiTextBTree     *tree,
                                            BobguiTextBTreeNode *node,
                                            NodeData         *nd)
{
  int width;
  ...
  bobgui_text_btree_node_compute_view_aggregates (node, nd->view_id,
                                               &width, &height, &valid);
...
static void
bobgui_text_btree_node_check_consistency (BobguiTextBTree     *tree,
                                       BobguiTextBTreeNode *node)
{
  BobguiTextBTreeNode *childnode;
  Summary *summary, *summary2;
  BobguiTextLine *line;
  BobguiTextLineSegment *segPtr;
  ...
  if ((node->num_children < min_children)
    ...
    {
      g_error ("bobgui_text_btree_node_check_consistency: bad child count (%d)",
               node->num_children);
    }
  ...
  while (nd != NULL)
    {
      bobgui_text_btree_node_view_check_consistency (tree, node, nd);
      nd = nd->next;
    }
  ...
  if (node->level == 0)
    {
      for (line = node->children.line; line != NULL;
           line = line->next)
        {
          if (line->parent != node)
            {
              g_error ("bobgui_text_btree_node_check_consistency: line doesn't point to parent");
            }
          if (line->segments == NULL)
            {
              g_error ("bobgui_text_btree_node_check_consistency: line has no segments");
            }
          ...
          for (segPtr = line->segments; segPtr != NULL; segPtr = segPtr->next)
            {
              ...
              if ((segPtr->byte_count == 0) && (!segPtr->type->leftGravity)
                ...
                {
                  g_error ("bobgui_text_btree_node_check_consistency: wrong segment order for gravity");
                }
              if ((segPtr->next == NULL)
                  && (segPtr->type != &bobgui_text_char_type))
                {
                  g_error ("bobgui_text_btree_node_check_consistency: line ended with wrong type");
                }
      ...
      for (childnode = node->children.node; childnode != NULL;
           childnode = childnode->next)
        {
          if (childnode->parent != node)
            {
              g_error ("bobgui_text_btree_node_check_consistency: BobguiTextBTreeNode doesn't point to parent");
            }
          if (childnode->level != (node->level-1))
            {
              g_error ("bobgui_text_btree_node_check_consistency: level mismatch (%d %d)",
                       node->level, childnode->level);
            }
          bobgui_text_btree_node_check_consistency (tree, childnode);
                  ...
                  if (summary2 == NULL)
                    {
                      ...
                      g_error ("bobgui_text_btree_node_check_consistency: BobguiTextBTreeNode tag \"%s\" not %s",
  ...
  if (num_children != node->num_children)
    {
      g_error ("bobgui_text_btree_node_check_consistency: mismatch in num_children (%d %d)",
               num_children, node->num_children);
    }
  if (num_lines != node->num_lines)
    {
      g_error ("bobgui_text_btree_node_check_consistency: mismatch in num_lines (%d %d)",
               num_lines, node->num_lines);
    }
  if (num_chars != node->num_chars)
    {
      g_error ("bobgui_text_btree_node_check_consistency: mismatch in num_chars (%d %d)",
  ...
  for (summary = node->summary; summary != NULL;
       summary = summary->next)
    {
      if (summary->info->toggle_count == summary->toggle_count)
        {
          g_error ("bobgui_text_btree_node_check_consistency: found unpruned root for \"%s\"",
                   summary->info->tag->priv->name);
        }
      toggle_count = 0;
      if (node->level == 0)
          ...
          for (line = node->children.line; line != NULL;
               line = line->next)
            {
              for (segPtr = line->segments; segPtr != NULL;
                   segPtr = segPtr->next)
                {
                  if ((segPtr->type != &bobgui_text_toggle_on_type)
                      && (segPtr->type != &bobgui_text_toggle_off_type))
                    {
                      continue;
                    }
      ...
      if (toggle_count != summary->toggle_count)
        {
          g_error ("bobgui_text_btree_node_check_consistency: mismatch in toggle_count (%d %d)",
      ...
      for (summary2 = summary->next; summary2 != NULL;
           summary2 = summary2->next)
        {
          if (summary2->info == summary->info)
            {
              g_error ("bobgui_text_btree_node_check_consistency: duplicated BobguiTextBTreeNode tag: %s",
                       summary->info->tag->priv->name);
            }
        }
    }
...
static void
listify_foreach (BobguiTextTag *tag, gpointer user_data)
{
...
static GSList*
list_of_tags (BobguiTextTagTable *table)
{
  GSList *list = NULL;

  bobgui_text_tag_table_foreach (table, listify_foreach, &list);

  return list;
}

void
_bobgui_text_btree_check (BobguiTextBTree *tree)
{
  Summary *summary;
  BobguiTextBTreeNode *node;
  BobguiTextLine *line;
      ...
      if (info != NULL)
        {
          ...
          if (node == NULL)
            {
              if (info->toggle_count != 0)
                {
                  g_error ("_bobgui_text_btree_check found \"%s\" with toggles (%d) but no root",
            ...
            {
              g_error ("_bobgui_text_btree_check found root for \"%s\" with no toggles",
                       tag->priv->name);
            }
          else if (info->toggle_count & 1)
            {
              g_error ("_bobgui_text_btree_check found odd toggle count for \"%s\" (%d)",
                       tag->priv->name, info->toggle_count);
            }
          for (summary = node->summary; summary != NULL;
               summary = summary->next)
            {
              if (summary->info->tag == tag)
                {
                  g_error ("_bobgui_text_btree_check found root BobguiTextBTreeNode with summary info");
                }
            }
          count = 0;
          if (node->level > 0)
            ...
            {
              const BobguiTextLineSegmentClass *last = NULL;
          ...
          if (count != info->toggle_count)
            {
              g_error ("_bobgui_text_btree_check toggle_count (%d) wrong for \"%s\" should be (%d)",
  ...
  node = tree->root_node;
  bobgui_text_btree_node_check_consistency (tree, tree->root_node);
  ...
  if (node->num_lines < 2)
    {
      g_error ("_bobgui_text_btree_check: less than 2 lines in tree");
    }
  if (node->num_chars < 2)
    {
      g_error ("_bobgui_text_btree_check: less than 2 chars in tree");
    }
  while (node->level > 0)
    {
      node = node->children.node;
  ...
  seg = line->segments;
  while ((seg->type == &bobgui_text_toggle_off_type)
         || (seg->type == &bobgui_text_right_mark_type)
  ...
  if (seg->type != &bobgui_text_char_type)
    {
      g_error ("_bobgui_text_btree_check: last line has bogus segment type");
    }
  if (seg->next != NULL)
    {
      g_error ("_bobgui_text_btree_check: last line has too many segments");
    }
  if (seg->byte_count != 1)
    {
      g_error ("_bobgui_text_btree_check: last line has wrong # characters: %d",
               seg->byte_count);
    }
  if ((seg->body.chars[0] != '\n') || (seg->body.chars[1] != 0))
    {
      g_error ("_bobgui_text_btree_check: last line had bad value: %s",
               seg->body.chars);
    }
}

void _bobgui_text_btree_spew_line (BobguiTextBTree* tree, BobguiTextLine* line);
void _bobgui_text_btree_spew_segment (BobguiTextBTree* tree, BobguiTextLineSegment* seg);
void _bobgui_text_btree_spew_node (BobguiTextBTreeNode *node, int indent);
void _bobgui_text_btree_spew_line_short (BobguiTextLine *line, int indent);

void
_bobgui_text_btree_spew (BobguiTextBTree *tree)
{
  BobguiTextLine * line;
  int real_line;

  printf ("%d lines in tree %p\n",
          _bobgui_text_btree_line_count (tree), tree);

  line = _bobgui_text_btree_get_line (tree, 0, &real_line);

  while (line != NULL)
    {
      _bobgui_text_btree_spew_line (tree, line);
      line = _bobgui_text_line_next (line);
    }
  ...
  {
    _bobgui_text_btree_spew_node (tree->root_node, 0);
  }
}

void
_bobgui_text_btree_spew_line_short (BobguiTextLine *line, int indent)
{
  char * spaces;
  BobguiTextLineSegment *seg;
  ...
  printf ("%sline %p chars %d bytes %d\n",
          spaces, line,
          _bobgui_text_line_char_count (line),
          _bobgui_text_line_byte_count (line));
  ...
  while (seg != NULL)
    {
      if (seg->type == &bobgui_text_char_type)
        ...
        {
          char *str = g_strndup (bobgui_text_child_anchor_get_replacement (seg->body.child.obj), seg->byte_count);
          printf ("%s child '%s'...\n", spaces, str);
          g_free (str);
        }
      ...
      else if (seg->type == &bobgui_text_left_mark_type)
        {
               ...
               seg->type == &bobgui_text_toggle_off_type)
        {
          printf ("%s tag '%s' %s\n",
                  ...
                  seg->type == &bobgui_text_toggle_off_type ? "off" : "on");
        }
...
void
_bobgui_text_btree_spew_node (BobguiTextBTreeNode *node, int indent)
{
  char * spaces;
  BobguiTextBTreeNode *iter;
  Summary *s;
  ...
  if (node->level > 0)
    {
      ...
      while (iter != NULL)
        {
          _bobgui_text_btree_spew_node (iter, indent + 2);
  ...
  else
    {
      ...
      while (line != NULL)
        {
          _bobgui_text_btree_spew_line_short (line, indent + 2);
...
void
_bobgui_text_btree_spew_line (BobguiTextBTree* tree, BobguiTextLine* line)
{
  BobguiTextLineSegment * seg;

  printf ("%4d| line: %p parent: %p next: %p\n",
          _bobgui_text_line_get_number (line), line, line->parent, line->next);

  seg = line->segments;

  while (seg != NULL)
    {
      _bobgui_text_btree_spew_segment (tree, seg);
      seg = seg->next;
    }
}

void
_bobgui_text_btree_spew_segment (BobguiTextBTree* tree, BobguiTextLineSegment * seg)
{
