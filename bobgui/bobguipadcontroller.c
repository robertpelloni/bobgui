 ...
 * perform no action. `BobguiPadController` is provided to map those to
 ...
 * is defined by a [struct@Bobgui.PadActionEntry]. It contains an action name that
 ...
 *
 * A simple example of `BobguiPadController` usage: Assigning button 1 in all
 * modes and pad devices to an "invert-selection" action:
 *
 * ```c
 * BobguiPadActionEntry *pad_actions[] = {
 *   { BOBGUI_PAD_ACTION_BUTTON, 1, -1, "Invert selection", "pad-actions.invert-selection" },
 *   …
 * };
 *
 * …
 ...
 * …
 * pad_controller = bobgui_pad_controller_new (action_group, NULL);
 * ```
 *
...
struct _BobguiPadControllerClass {
...
G_DEFINE_TYPE (BobguiPadController, bobgui_pad_controller, BOBGUI_TYPE_EVENT_CONTROLLER)

static const ActionEntryData *
bobgui_pad_action_find_match (BobguiPadController *controller,
                           BobguiPadActionType  type,
                           int               index,
                           int               mode)
{
...
static void
bobgui_pad_controller_activate_action (BobguiPadController        *controller,
...
static void
bobgui_pad_controller_activate_action_with_axis (BobguiPadController        *controller,
                                              const ActionEntryData   *entry,
                                              double                   value)
{
  g_action_group_activate_action (controller->action_group,
...
static void
bobgui_pad_controller_handle_mode_switch (BobguiPadController *controller,
                                       GdkDevice        *pad,
                                       guint             group,
                                       guint             mode)
{
  ...
  if (GDK_IS_WAYLAND_DISPLAY (gdk_device_get_display (pad)))
    {
      ...
      for (elem = BOBGUI_PAD_ACTION_BUTTON; elem <= BOBGUI_PAD_ACTION_STRIP; elem++)
        {
          ...
          for (idx = 0; idx < n_features; idx++)
            {
              ...
              entry = bobgui_pad_action_find_match (controller, elem, idx, mode);
              if (!entry)
                continue;
...
static gboolean
bobgui_pad_controller_filter_event (BobguiEventController *controller,
                                 GdkEvent           *event)
{
  BobguiPadController *pad_controller = BOBGUI_PAD_CONTROLLER (controller);
...
static gboolean
bobgui_pad_controller_handle_event (BobguiEventController *controller,
                                 GdkEvent           *event,
                                 double              x,
                                 double              y)
{
  ...
  if (event_type == GDK_PAD_GROUP_MODE)
    {
      bobgui_pad_controller_handle_mode_switch (pad_controller,
  ...
  switch ((guint) event_type)
    ...
    case GDK_PAD_BUTTON_PRESS:
      type = BOBGUI_PAD_ACTION_BUTTON;
      index = gdk_pad_event_get_button (event);
      break;
    case GDK_PAD_RING:
      type = BOBGUI_PAD_ACTION_RING;
      gdk_pad_event_get_axis_value (event, &index, &value);
      break;
    case GDK_PAD_STRIP:
      type = BOBGUI_PAD_ACTION_STRIP;
      gdk_pad_event_get_axis_value (event, &index, &value);
      break;
    case GDK_PAD_DIAL:
      type = BOBGUI_PAD_ACTION_DIAL;
      gdk_pad_event_get_axis_value (event, &index, &value);
      break;
    default:
      return GDK_EVENT_PROPAGATE;
  ...
  entry = bobgui_pad_action_find_match (pad_controller,
                                     type, index, mode);
  if (!entry)
    return GDK_EVENT_PROPAGATE;

  if (event_type == GDK_PAD_RING || event_type == GDK_PAD_STRIP || event_type == GDK_PAD_DIAL)
    bobgui_pad_controller_activate_action_with_axis (pad_controller, entry, value);
  else
    bobgui_pad_controller_activate_action (pad_controller, entry);

  return GDK_EVENT_STOP;
}

static void
bobgui_pad_controller_set_pad (BobguiPadController *controller,
                            GdkDevice        *pad)
{
...
static void
bobgui_pad_controller_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  BobguiPadController *controller = BOBGUI_PAD_CONTROLLER (object);
  ...
  switch (prop_id)
    ...
    case PROP_ACTION_GROUP:
      ...
      break;
    case PROP_PAD:
      bobgui_pad_controller_set_pad (controller, g_value_get_object (value));
      break;
    default:
...
static void
bobgui_pad_controller_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
...
static void
bobgui_pad_controller_dispose (GObject *object)
{
  BobguiPadController *controller = BOBGUI_PAD_CONTROLLER (object);
  ...
  G_OBJECT_CLASS (bobgui_pad_controller_parent_class)->dispose (object);
}

static void
bobgui_pad_controller_finalize (GObject *object)
{
  BobguiPadController *controller = BOBGUI_PAD_CONTROLLER (object);
  guint i;
  ...
  G_OBJECT_CLASS (bobgui_pad_controller_parent_class)->finalize (object);
}

static void
bobgui_pad_controller_class_init (BobguiPadControllerClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  BobguiEventControllerClass *controller_class = BOBGUI_EVENT_CONTROLLER_CLASS (klass);

  controller_class->filter_event = bobgui_pad_controller_filter_event;
  controller_class->handle_event = bobgui_pad_controller_handle_event;

  object_class->set_property = bobgui_pad_controller_set_property;
  object_class->get_property = bobgui_pad_controller_get_property;
  object_class->dispose = bobgui_pad_controller_dispose;
  object_class->finalize = bobgui_pad_controller_finalize;

  /**
   * BobguiPadController:action-group:
   *
...
static void
bobgui_pad_controller_init (BobguiPadController *controller)
{
  controller->action_entries = g_array_new (FALSE, TRUE, sizeof (ActionEntryData));
}

/**
 * bobgui_pad_controller_new:
 * @group: `GActionGroup` to trigger actions from
 * @pad: (nullable): A %GDK_SOURCE_TABLET_PAD device, or %NULL to handle all pads
 *
 * Creates a new `BobguiPadController` that will associate events from @pad to
 * actions.
 *
 * A %NULL pad may be provided so the controller manages all pad devices
 * generically, it is discouraged to mix `BobguiPadController` objects with
 * %NULL and non-%NULL @pad argument on the same toplevel window, as execution
 * order is not guaranteed.
 *
 * The `BobguiPadController` is created with no mapped actions. In order to
 * map pad events to actions, use [method@Bobgui.PadController.set_action_entries]
 * or [method@Bobgui.PadController.set_action].
 *
 * Be aware that pad events will only be delivered to `BobguiWindow`s, so adding
 * a pad controller to any other type of widget will not have an effect.
 *
 * Returns: A newly created `BobguiPadController`
 */
BobguiPadController *
bobgui_pad_controller_new (GActionGroup *group,
                        GdkDevice    *pad)
{
  ...
  return g_object_new (BOBGUI_TYPE_PAD_CONTROLLER,
                       "propagation-phase", BOBGUI_PHASE_CAPTURE,
                       "action-group", group,
                       "pad", pad,
                       NULL);
}
...
entry_compare_func (gconstpointer a,
                    gconstpointer b)
{
  const BobguiPadActionEntry *entry1 = a, *entry2 = b;
...
static void
bobgui_pad_controller_add_entry (BobguiPadController        *controller,
                              const BobguiPadActionEntry *entry)
{
  guint i;
...
/**
 * bobgui_pad_controller_set_action_entries:
 * @controller: a `BobguiPadController`
 ...
 * @controller.
 *
 * See [struct@Bobgui.PadActionEntry] and [method@Bobgui.PadController.set_action].
 */
void
bobgui_pad_controller_set_action_entries (BobguiPadController        *controller,
                                       const BobguiPadActionEntry *entries,
                                       int                      n_entries)
{
  int i;

  g_return_if_fail (BOBGUI_IS_PAD_CONTROLLER (controller));
  g_return_if_fail (entries != NULL);

  for (i = 0; i < n_entries; i++)
    bobgui_pad_controller_add_entry (controller, &entries[i]);
}

/**
 * bobgui_pad_controller_set_action:
 * @controller: a `BobguiPadController`
 ...
 */
void
bobgui_pad_controller_set_action (BobguiPadController *controller,
                               BobguiPadActionType  type,
                               int               index,
                               int               mode,
                               const char       *label,
...
{
  const BobguiPadActionEntry entry = { type, index, mode, label, action_name };

  g_return_if_fail (BOBGUI_IS_PAD_CONTROLLER (controller));
  g_return_if_fail (type <= BOBGUI_PAD_ACTION_STRIP);

  bobgui_pad_controller_add_entry (controller, &entry);
}
