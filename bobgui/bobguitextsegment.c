...
bobgui_text_line_segment_split (const BobguiTextIter *iter)
{
  BobguiTextLineSegment *prev, *seg;
  ...
  count = bobgui_text_iter_get_line_index (iter);
      ...
      if (seg->byte_count > count)
        {
          if (count == 0)
          ...
          else
            {
              ...
              _bobgui_text_btree_segments_changed (tree);
...
#define CSEG_SIZE(chars) ((unsigned) (G_STRUCT_OFFSET (BobguiTextLineSegment, body) \
        + 1 + (chars)))
#define TSEG_SIZE ((unsigned) (G_STRUCT_OFFSET (BobguiTextLineSegment, body) \
        + sizeof (BobguiTextToggleBody)))
...
static void
char_segment_self_check (BobguiTextLineSegment *seg)
{
...
BobguiTextLineSegment*
_bobgui_char_segment_new (const char *text, guint len)
{
  BobguiTextLineSegment *seg;

  g_assert (bobgui_text_byte_begins_utf8_char (text));

  seg = g_malloc (CSEG_SIZE (len));
  seg->type = (BobguiTextLineSegmentClass *)&bobgui_text_char_type;
  seg->next = NULL;
  seg->byte_count = len;
...
BobguiTextLineSegment*
_bobgui_char_segment_new_from_two_strings (const char *text1, 
					guint        len1, 
					guint        chars1,
                                        const char *text2, 
					guint        len2, 
					guint        chars2)
{
  ...
  g_assert (bobgui_text_byte_begins_utf8_char (text1));
  g_assert (bobgui_text_byte_begins_utf8_char (text2));

  seg = g_malloc (CSEG_SIZE (len1+len2));
  seg->type = &bobgui_text_char_type;
  seg->next = NULL;
...
static void
_bobgui_char_segment_free (BobguiTextLineSegment *seg)
{
  if (seg == NULL)
    return;

  g_assert (seg->type == &bobgui_text_char_type);
...
char_segment_split_func (BobguiTextLineSegment *seg, int index)
{
  BobguiTextLineSegment *new1, *new2;
  ...
  new1 = _bobgui_char_segment_new (seg->body.chars, index);
  new2 = _bobgui_char_segment_new (seg->body.chars + index, seg->byte_count - index);

  g_assert (bobgui_text_byte_begins_utf8_char (new1->body.chars));
  g_assert (bobgui_text_byte_begins_utf8_char (new2->body.chars));
 ...
 */
        ...
        /* ARGSUSED */
static BobguiTextLineSegment *
char_segment_cleanup_func (BobguiTextLineSegment *segPtr, BobguiTextLine *line)
{
  BobguiTextLineSegment *segPtr2, *newPtr;

  if (BOBGUI_DEBUG_CHECK (TEXT))
    char_segment_self_check (segPtr);

  segPtr2 = segPtr->next;
  if ((segPtr2 == NULL) || (segPtr2->type != &bobgui_text_char_type))
    {
      return segPtr;
    }

  newPtr =
    _bobgui_char_segment_new_from_two_strings (segPtr->body.chars, 
					    segPtr->byte_count,
					    segPtr->char_count,
                                            segPtr2->body.chars, 
					    segPtr2->byte_count,
  ...
  _bobgui_char_segment_free (segPtr);
  _bobgui_char_segment_free (segPtr2);
  return newPtr;
}
 ...
 */
        ...
        /* ARGSUSED */
static int
char_segment_delete_func (BobguiTextLineSegment *segPtr, BobguiTextLine *line, int treeGone)
{
  _bobgui_char_segment_free (segPtr);
  return 0;
}
 ...
 */
        ...
        /* ARGSUSED */
static void
char_segment_check_func (BobguiTextLineSegment *segPtr, BobguiTextLine *line)
{
  char_segment_self_check (segPtr);

  if (segPtr->next != NULL)
    {
      if (segPtr->next->type == &bobgui_text_char_type)
        {
...
>>>>>>> origin/4627-printing-Unref-old-spool_io-before-setting-new-one-gtk3:gtk/gtktextsegment.c
  ...
  seg->type = on ? &bobgui_text_toggle_on_type : &bobgui_text_toggle_off_type;
...
void
_bobgui_toggle_segment_free (BobguiTextLineSegment *seg)
{
  if (seg == NULL)
    return;

  g_assert (seg->type == &bobgui_text_toggle_on_type ||
 ...
 *      freed;  otherwise the toggle counts in BobguiTextBTreeNodes above the
 *      segment get updated.
 *
 *--------------------------------------------------------------
 */
...
static int
toggle_segment_delete_func (BobguiTextLineSegment *segPtr, BobguiTextLine *line, int treeGone)
{
  if (treeGone)
    {
      _bobgui_toggle_segment_free (segPtr);
      return 0;
    }
   ...
   * later.  Decrement BobguiTextBTreeNode toggle counts here, and set a flag
 ...
 *
 * Side effects:
 *      Toggle counts in the BobguiTextBTreeNodes above the new line will be
...
static BobguiTextLineSegment *
toggle_segment_cleanup_func (BobguiTextLineSegment *segPtr, BobguiTextLine *line)
{
  BobguiTextLineSegment *segPtr2, *prevPtr;
  int counts;
  ...
  if (segPtr->type == &bobgui_text_toggle_off_type)
    {
      for (prevPtr = segPtr, segPtr2 = prevPtr->next;
           ...
           prevPtr = segPtr2, segPtr2 = prevPtr->next)
        {
          ...
          _bobgui_toggle_segment_free (segPtr2);
          segPtr2 = segPtr->next;
          _bobgui_toggle_segment_free (segPtr);
          return segPtr2;
        }
 ...
 *
 * Side effects:
 *      Toggle counts are decremented in the BobguiTextBTreeNodes above the line.
 *
 *--------------------------------------------------------------
 */

static void
toggle_segment_line_change_func (BobguiTextLineSegment *segPtr, BobguiTextLine *line)
{
  if (segPtr->body.toggle.inNodeCounts)
    {
      _bobgui_change_node_toggle_count (line->parent,
...
const BobguiTextLineSegmentClass bobgui_text_char_type = {
  "character",                          /* name */
  0,                                            /* leftGravity */
...
const BobguiTextLineSegmentClass bobgui_text_toggle_on_type = {
  "toggleOn",                                   /* name */
  0,                                            /* leftGravity */
  NULL,                 /* splitFunc */
  ...
  _bobgui_toggle_segment_check_func                        /* checkFunc */
};
...
const BobguiTextLineSegmentClass bobgui_text_toggle_off_type = {
  "toggleOff",                          /* name */
  1,                                            /* leftGravity */
  NULL,                 /* splitFunc */
  toggle_segment_delete_func,                           /* deleteFunc */
