...
typedef enum {
  BOBGUI_KINETIC_SCROLLING_PHASE_DECELERATING,
  BOBGUI_KINETIC_SCROLLING_PHASE_OVERSHOOTING,
  BOBGUI_KINETIC_SCROLLING_PHASE_FINISHED,
} BobguiKineticScrollingPhase;

struct _BobguiKineticScrolling
{
  BobguiKineticScrollingPhase phase;
  double lower;
  double upper;
...
static void bobgui_kinetic_scrolling_init_overshoot (BobguiKineticScrolling *data,
                                                  gint64               frame_time,
                                                  double               equilibrium_position,
                                                  double               initial_position,
                                                  double               initial_velocity);

BobguiKineticScrolling *
bobgui_kinetic_scrolling_new (gint64 frame_time,
                           double lower,
                           double upper,
                           double overshoot_width,
                           double decel_friction,
                           ...
                           double initial_velocity)
{
  BobguiKineticScrolling *data;

  data = g_new0 (BobguiKineticScrolling, 1);
  data->lower = lower;
  data->upper = upper;
  ...
  if (initial_position < lower)
    {
      bobgui_kinetic_scrolling_init_overshoot (data,
                                            frame_time,
                                            lower,
                                            initial_position,
                                            initial_velocity);
    ...
    {
      data->phase = BOBGUI_KINETIC_SCROLLING_PHASE_DECELERATING;
...
BobguiKineticScrollingChange
bobgui_kinetic_scrolling_update_size (BobguiKineticScrolling *data,
                                   double               lower,
                                   double               upper)
{
  BobguiKineticScrollingChange change = BOBGUI_KINETIC_SCROLLING_CHANGE_NONE;

  if (lower != data->lower)
    {
      if (data->position <= lower)
        change |= BOBGUI_KINETIC_SCROLLING_CHANGE_LOWER;
  ...
  if (upper != data->upper)
    {
      if (data->position >= data->upper)
        change |= BOBGUI_KINETIC_SCROLLING_CHANGE_UPPER;

      data->upper = upper;
    }

  if (data->phase == BOBGUI_KINETIC_SCROLLING_PHASE_OVERSHOOTING)
    change |= BOBGUI_KINETIC_SCROLLING_CHANGE_IN_OVERSHOOT;

  return change;
}

void
bobgui_kinetic_scrolling_free (BobguiKineticScrolling *kinetic)
{
  g_free (kinetic);
}

static void
bobgui_kinetic_scrolling_init_overshoot (BobguiKineticScrolling *data,
                                      gint64               frame_time,
                                      double               equilibrium_position,
                                      double               initial_position,
                                      double               initial_velocity)
{
  data->phase = BOBGUI_KINETIC_SCROLLING_PHASE_OVERSHOOTING;
...
gboolean
bobgui_kinetic_scrolling_tick (BobguiKineticScrolling *data,
                            gint64               frame_time,
                            double              *position,
                            double              *velocity)
{
  ...
  switch (data->phase)
    {
    case BOBGUI_KINETIC_SCROLLING_PHASE_DECELERATING:
      {
        double exp_part;
        ...
        if (data->position < data->lower)
          {
            bobgui_kinetic_scrolling_init_overshoot (data, frame_time, data->lower, data->position, data->velocity);
          }
        else if (data->position > data->upper)
          {
            bobgui_kinetic_scrolling_init_overshoot (data, frame_time, data->upper, data->position, data->velocity);
          }
        else if (fabs (data->velocity) < 0.1)
          {
            bobgui_kinetic_scrolling_stop (data);
          }
        break;
      }

    case BOBGUI_KINETIC_SCROLLING_PHASE_OVERSHOOTING:
      {
        ...
        if (pos < data->lower - half_overshoot_width || pos > data->upper + half_overshoot_width)
          {
            ...
            bobgui_kinetic_scrolling_init_overshoot (data, frame_time, data->equilibrium_position, pos, 0);
          }
        else
        ...
        if (fabs (pos) < 0.1)
          {
            data->phase = BOBGUI_KINETIC_SCROLLING_PHASE_FINISHED;
    ...
    case BOBGUI_KINETIC_SCROLLING_PHASE_FINISHED:
    default:
      break;
    }
  ...
  return data->phase != BOBGUI_KINETIC_SCROLLING_PHASE_FINISHED;
}

void
bobgui_kinetic_scrolling_stop (BobguiKineticScrolling *data)
{
  if (data->phase == BOBGUI_KINETIC_SCROLLING_PHASE_DECELERATING)
    {
      data->phase = BOBGUI_KINETIC_SCROLLING_PHASE_FINISHED;
