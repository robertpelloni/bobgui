 ...
 * bobguifilechoosernativeportal.c: Portal File selector dialog
  ...
  if (data->exported_window)
    {
      if (data->exported_handle)
        {
          bobgui_window_unexport_handle (data->exported_window, data->exported_handle);
...
response_cb (GDBusConnection  *connection,
             ...
             gpointer          user_data)
{
  BobguiFileChooserNative *self = user_data;
      ...
      for (i = 0; i < g_variant_n_children (choices); i++)
        {
          ...
          bobgui_file_chooser_set_choice (BOBGUI_FILE_CHOOSER (self), id, selected);
        }
  ...
  if (current_filter)
    {
      BobguiFileFilter *filter = bobgui_file_filter_new_from_gvariant (current_filter);
      const char *current_filter_name = bobgui_file_filter_get_name (filter);
       ...
       */
      BobguiFileFilter *filter_to_select = filter;
      GListModel *filters;
      guint j, n;
      ...
      for (j = 0; j < n; j++)
        {
          BobguiFileFilter *f = g_list_model_get_item (filters, j);
          if (g_strcmp0 (bobgui_file_filter_get_name (f), current_filter_name) == 0)
            {
      ...
      bobgui_file_chooser_set_filter (BOBGUI_FILE_CHOOSER (self), filter_to_select);
  ...
  switch (portal_response)
    {
    case 0:
      bobgui_response = BOBGUI_RESPONSE_ACCEPT;
      break;
    case 1:
      bobgui_response = BOBGUI_RESPONSE_CANCEL;
      break;
    case 2:
    default:
      bobgui_response = BOBGUI_RESPONSE_DELETE_EVENT;
      break;
    }
  ...
  _bobgui_native_dialog_emit_response (BOBGUI_NATIVE_DIALOG (self), bobgui_response);
...
static GVariant *
get_filters (BobguiFileChooser *self)
{
  GListModel *filters;
  guint n, i;
  ...
  filters = bobgui_file_chooser_get_filters (self);
  n = g_list_model_get_n_items (filters);
  for (i = 0; i < n; i++)
    {
      BobguiFileFilter *filter = g_list_model_get_item (filters, i);
      g_variant_builder_add_value (&builder, bobgui_file_filter_to_gvariant (filter));
...
static GVariant *
bobgui_file_chooser_native_choice_to_variant (BobguiFileChooserNativeChoice *choice)
{
...
static GVariant *
serialize_choices (BobguiFileChooserNative *self)
{
  GVariantBuilder builder;
  GSList *l;
  ...
  for (l = self->choices; l; l = l->next)
    {
      ...
      g_variant_builder_add_value (&builder, bobgui_file_chooser_native_choice_to_variant (choice));
    }
...
static void
show_portal_file_chooser (BobguiFileChooserNative *self,
                          const char           *parent_window_str)
{
  ...
  data->portal_handle = bobgui_get_portal_request_path (data->connection, &token);
  data->portal_response_signal_id =
    g_dbus_connection_signal_subscribe (data->connection,
                                        PORTAL_BUS_NAME,
                                        PORTAL_REQUEST_INTERFACE,
  ...
  multiple = bobgui_file_chooser_get_select_multiple (BOBGUI_FILE_CHOOSER (self));
  directory = bobgui_file_chooser_get_action (BOBGUI_FILE_CHOOSER (self)) == BOBGUI_FILE_CHOOSER_ACTION_SELECT_FOLDER;
  ...
  g_variant_builder_add (&opt_builder, "{sv}", "filters", get_filters (BOBGUI_FILE_CHOOSER (self)));
  if (self->current_filter)
    g_variant_builder_add (&opt_builder, "{sv}", "current_filter",
                           bobgui_file_filter_to_gvariant (self->current_filter));
  if (self->current_name)
    g_variant_builder_add (&opt_builder, "{sv}", "current_name",
                           g_variant_new_string (BOBGUI_FILE_CHOOSER_NATIVE (self)->current_name));
  if (self->current_folder)
    {
      char *path;

      path = g_file_get_path (BOBGUI_FILE_CHOOSER_NATIVE (self)->current_folder);
  ...
  if (self->current_file)
    {
      ...
      path = g_file_peek_path (BOBGUI_FILE_CHOOSER_NATIVE (self)->current_file);
  ...
  if (self->choices)
    g_variant_builder_add (&opt_builder, "{sv}", "choices",
                           serialize_choices (BOBGUI_FILE_CHOOSER_NATIVE (self)));

  title = bobgui_native_dialog_get_title (BOBGUI_NATIVE_DIALOG (self));
...
gboolean
bobgui_file_chooser_native_portal_show (BobguiFileChooserNative *self)
{
  ...
  if (transient_for)
    display = bobgui_widget_get_display (BOBGUI_WIDGET (transient_for));
  else
  ...
  if (connection == NULL)
    {
      ...
      alert = bobgui_alert_dialog_new (_("The session bus is not available"));
      bobgui_alert_dialog_show (alert, transient_for);
      g_object_unref (alert);

      return TRUE;
    }

  switch (bobgui_file_chooser_get_action (BOBGUI_FILE_CHOOSER (self)))
    {
  ...
  if (transient_for != NULL && bobgui_widget_is_visible (BOBGUI_WIDGET (transient_for)))
    {
      if (!bobgui_window_export_handle (transient_for,
...
void
bobgui_file_chooser_native_portal_hide (BobguiFileChooserNative *self)
{
  FilechooserPortalData *data = self->mode_data;
