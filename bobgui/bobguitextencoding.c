...
typedef struct {
  const char *charset;
  const char *label;
} BobguiTextEncodingItem;

/* Note: the charsets here must be valid iconv charset names */
static BobguiTextEncodingItem bobgui_text_encodings[] = {
...
char **
bobgui_text_encoding_get_names (void)
{
  GStrvBuilder *builder = g_strv_builder_new ();
  char **res;

  for (guint i = 0; i < G_N_ELEMENTS (bobgui_text_encodings); i++)
    {
      g_strv_builder_add (builder, bobgui_text_encodings[i].charset);
    }
...
char **
bobgui_text_encoding_get_labels (void)
{
  GStrvBuilder *builder = g_strv_builder_new ();
  char **res;

  g_strv_builder_add (builder,
                      g_dpgettext2 (NULL, "Encoding name", bobgui_text_encodings[0].label));

  for (guint i = 1; i < G_N_ELEMENTS (bobgui_text_encodings); i++)
    {
      char *text;

      text = g_strdup_printf ("%s (%s)",
                              g_dpgettext2 (NULL, "Encoding name", bobgui_text_encodings[i].label),
                              bobgui_text_encodings[i].charset);
...
const char *
bobgui_text_encoding_from_name (const char *name)
{
  for (guint i = 0; i < G_N_ELEMENTS (bobgui_text_encodings); i++)
    {
      if (strcmp (name, bobgui_text_encodings[i].charset) == 0)
        return bobgui_text_encodings[i].charset;
    }
...
typedef struct {
  ...
  const char *name;
  const char *label;
} BobguiLineEndingItem;

static BobguiLineEndingItem bobgui_line_endings[] = {
...
char **
bobgui_line_ending_get_names (void)
{
  GStrvBuilder *builder = g_strv_builder_new ();
  char **res;

  for (guint i = 0; i < G_N_ELEMENTS (bobgui_line_endings); i++)
    {
      g_strv_builder_add (builder, bobgui_line_endings[i].name);
    }
...
char **
bobgui_line_ending_get_labels (void)
{
  GStrvBuilder *builder = g_strv_builder_new ();
  char **res;
  ...
  for (guint i = 0; i < G_N_ELEMENTS (bobgui_line_endings); i++)
    {
      g_strv_builder_add (builder,
                          g_dpgettext2 (NULL, "Line ending name", bobgui_line_endings[i].label));
    }
...
const char *
bobgui_line_ending_from_name (const char *name)
{
  for (guint i = 0; i < G_N_ELEMENTS (bobgui_line_endings); i++)
    {
      if (strcmp (name, bobgui_line_endings[i].name) == 0)
        return bobgui_line_endings[i].line_ending;
    }
