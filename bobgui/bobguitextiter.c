 ...
 * bobguitextiter.c Copyright (C) 2000 Red Hat, Inc.
 *
 ...
 * BobguiTextIter:
 *
 * Iterates over the contents of a `BobguiTextBuffer`.
 *
...
typedef struct _BobguiTextRealIter BobguiTextRealIter;

struct G_GNUC_MAY_ALIAS _BobguiTextRealIter
{
  ...
  BobguiTextLineSegment *segment;     /* indexable segment we index */
  BobguiTextLineSegment *any_segment; /* first segment in our location,
...
*/
static void
iter_set_common (BobguiTextRealIter *iter,
                 BobguiTextLine *line)
{
  /* Update segments stamp */
  iter->segments_changed_stamp =
    _bobgui_text_btree_get_segments_changed_stamp (iter->tree);
...
static void
iter_set_from_byte_offset (BobguiTextRealIter *iter,
                           BobguiTextLine *line,
                           int byte_offset)
{
  iter_set_common (iter, line);

  if (!_bobgui_text_line_byte_locate (iter->line,
                                   byte_offset,
                                   &iter->segment,
                                   &iter->any_segment,
...
static void
iter_set_from_char_offset (BobguiTextRealIter *iter,
                           BobguiTextLine *line,
                           int char_offset)
{
  iter_set_common (iter, line);

  if (!_bobgui_text_line_char_locate (iter->line,
                                   char_offset,
                                   &iter->segment,
                                   &iter->any_segment,
...
static void
iter_set_from_segment (BobguiTextRealIter *iter,
                       BobguiTextLine *line,
...
/* This function ensures that the segment-dependent information is
   ...
   update the segments. */
static BobguiTextRealIter*
bobgui_text_iter_make_surreal (const BobguiTextIter *_iter)
{
  BobguiTextRealIter *iter = (BobguiTextRealIter*)_iter;

  if (iter->chars_changed_stamp !=
      _bobgui_text_btree_get_chars_changed_stamp (iter->tree))
    {
...
static BobguiTextRealIter*
bobgui_text_iter_make_real (const BobguiTextIter *_iter)
{
  BobguiTextRealIter *iter;

  iter = bobgui_text_iter_make_surreal (_iter);
  if (iter == NULL)
    return NULL;
...
static BobguiTextRealIter*
iter_init_common (BobguiTextIter *_iter,
                  BobguiTextBTree *tree)
{
  BobguiTextRealIter *iter = (BobguiTextRealIter*)_iter;
...
static BobguiTextRealIter*
iter_init_from_segment (BobguiTextIter *iter,
                        BobguiTextBTree *tree,
                        BobguiTextLine *line,
                        BobguiTextLineSegment *segment)
{
  BobguiTextRealIter *real;
...
static BobguiTextRealIter*
iter_init_from_byte_offset (BobguiTextIter *iter,
                            BobguiTextBTree *tree,
                            BobguiTextLine *line,
                            int line_byte_offset)
{
  ...
  if (real->segment->type == &bobgui_text_char_type &&
...
static BobguiTextRealIter*
iter_init_from_char_offset (BobguiTextIter *iter,
                            BobguiTextBTree *tree,
                            BobguiTextLine *line,
                            int line_char_offset)
{
...
static inline void
invalidate_char_index (BobguiTextRealIter *iter)
{
  iter->cached_char_index = -1;
}

static inline void
adjust_char_index (BobguiTextRealIter *iter, int count)
{
...
static inline void
adjust_line_number (BobguiTextRealIter *iter, int count)
{
...
static inline void
ensure_char_offsets (BobguiTextRealIter *iter)
{
  if (iter->line_char_offset < 0)
    {
      g_assert (iter->line_byte_offset >= 0);

      _bobgui_text_line_byte_to_char_offsets (iter->line,
                                          iter->line_byte_offset,
                                          &iter->line_char_offset,
                                          &iter->segment_char_offset);
    }
...
static inline void
ensure_byte_offsets (BobguiTextRealIter *iter)
{
  if (iter->line_byte_offset < 0)
    {
      g_assert (iter->line_char_offset >= 0);

      _bobgui_text_line_char_to_byte_offsets (iter->line,
                                          iter->line_char_offset,
                                          &iter->line_byte_offset,
                                          &iter->segment_byte_offset);
    }
...
is_segment_start (BobguiTextRealIter *real)
{
  return real->segment_byte_offset == 0 || real->segment_char_offset == 0;
}

static void
check_invariants (const BobguiTextIter *iter)
{
...
/**
 * bobgui_text_iter_get_buffer:
 * @iter: an iterator
 *
 * Returns the `BobguiTextBuffer` this iterator is associated with.
 *
 * Returns: (transfer none): the buffer
 **/
BobguiTextBuffer*
bobgui_text_iter_get_buffer (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;

  g_return_val_if_fail (iter != NULL, NULL);

  real = bobgui_text_iter_make_surreal (iter);
  ...
  return _bobgui_text_btree_get_buffer (real->tree);
}

/**
 * bobgui_text_iter_copy:
 * @iter: an iterator
 *
 ...
 *
 * Returns: a copy of the @iter, free with [method@Bobgui.TextIter.free]
 */
BobguiTextIter*
bobgui_text_iter_copy (const BobguiTextIter *iter)
{
  BobguiTextIter *new_iter;

  g_return_val_if_fail (iter != NULL, NULL);

  new_iter = g_new (BobguiTextIter, 1);
 ...
 */
void
bobgui_text_iter_free (BobguiTextIter *iter)
{
...
/**
 * bobgui_text_iter_assign:
 * @iter: a `BobguiTextIter`
 * @other: another `BobguiTextIter`
 *
 * Assigns the value of @other to @iter.
 *
 * This function is not useful in applications, because
 * iterators can be assigned with `BobguiTextIter i = j;`.
 *
 * The function is used by language bindings.
 */
void
bobgui_text_iter_assign (BobguiTextIter       *iter,
...
G_DEFINE_BOXED_TYPE (BobguiTextIter, bobgui_text_iter,
                     bobgui_text_iter_copy,
                     bobgui_text_iter_free)

BobguiTextLineSegment*
_bobgui_text_iter_get_indexable_segment (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;

  g_return_val_if_fail (iter != NULL, NULL);

  real = bobgui_text_iter_make_real (iter);
...
BobguiTextLineSegment*
_bobgui_text_iter_get_any_segment (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
...
int
_bobgui_text_iter_get_segment_byte (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
...
int
_bobgui_text_iter_get_segment_char (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
...
/* This function does not require a still-valid
   iterator */
BobguiTextLine*
_bobgui_text_iter_get_text_line (const BobguiTextIter *iter)
{
  const BobguiTextRealIter *real;

  g_return_val_if_fail (iter != NULL, NULL);

  real = (const BobguiTextRealIter*)iter;
...
/* This function does not require a still-valid
   iterator */
BobguiTextBTree*
_bobgui_text_iter_get_btree (const BobguiTextIter *iter)
{
  const BobguiTextRealIter *real;
...
/**
 * bobgui_text_iter_get_offset:
 * @iter: an iterator
 *
 * Returns the character offset of an iterator.
 *
 * Each character in a `BobguiTextBuffer` has an offset,
 * starting with 0 for the first character in the buffer.
 * Use [method@Bobgui.TextBuffer.get_iter_at_offset] to convert
 * an offset back into an iterator.
 *
 * Returns: a character offset
 */
int
bobgui_text_iter_get_offset (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  ...
  if (real->cached_char_index < 0)
    {
      ...
      real->cached_char_index =
        _bobgui_text_line_char_index (real->line);
...
/**
 * bobgui_text_iter_get_line:
 * @iter: an iterator
 *
 * Returns the line number containing the iterator.
 *
 * Lines in a `BobguiTextBuffer` are numbered beginning
 * with 0 for the first line in the buffer.
 *
 * Returns: a line number
 */
int
bobgui_text_iter_get_line (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  ...
  if (real->cached_line_number < 0)
    real->cached_line_number =
      _bobgui_text_line_get_number (real->line);
...
/**
 * bobgui_text_iter_get_line_offset:
 * @iter: an iterator
 *
 ...
 */
int
bobgui_text_iter_get_line_offset (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
...
/**
 * bobgui_text_iter_get_line_index:
 * @iter: an iterator
 *
 * Returns the byte index of the iterator, counting
 * from the start of a newline-terminated line.
 *
 * Remember that `BobguiTextBuffer` encodes text in
 ...
 */
int
bobgui_text_iter_get_line_index (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
...
/**
 * bobgui_text_iter_get_visible_line_offset:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
int
bobgui_text_iter_get_visible_line_offset (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  int vis_offset;
  BobguiTextLineSegment *seg;
  ...
  _bobgui_text_btree_get_iter_at_line (real->tree,
                                    &pos,
                                    real->line,
                                    0);

  seg = _bobgui_text_iter_get_indexable_segment (&pos);
  ...
  while (seg != real->segment)
    {
       ...
       * quite a lot.
       */
      if (_bobgui_text_btree_char_is_invisible (&pos))
        vis_offset -= seg->char_count;

      _bobgui_text_iter_forward_indexable_segment (&pos);
...
/**
 * bobgui_text_iter_get_visible_line_index:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
int
bobgui_text_iter_get_visible_line_index (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  int vis_offset;
  BobguiTextLineSegment *seg;
 ...
 */
gunichar
bobgui_text_iter_get_char (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  ...
  if (bobgui_text_iter_is_end (iter))
    return 0;
  else if (real->segment->type == &bobgui_text_char_type)
    {
      ensure_byte_offsets (real);
  ...
  else if (real->segment->type == &bobgui_text_child_type)
    {
      return g_utf8_get_char (bobgui_text_child_anchor_get_replacement (real->segment->body.child.obj));
    }
  else
    {
      /* Unicode "unknown character" 0xFFFC */
      return BOBGUI_TEXT_UNKNOWN_CHAR;
    }
}

/**
 * bobgui_text_iter_get_slice:
 ...
 */
char *
bobgui_text_iter_get_slice       (const BobguiTextIter *start,
                               const BobguiTextIter *end)
{
  ...
  return _bobgui_text_btree_get_text (start, end, TRUE, TRUE);
}

/**
 * bobgui_text_iter_get_text:
 * @start: iterator at start of a range
 * @end: iterator at end of a range
 *
 ...
 * [method@Bobgui.TextIter.get_slice].
 *
 * Returns: (transfer full): array of characters from the buffer
 */
char *
bobgui_text_iter_get_text       (const BobguiTextIter *start,
                              const BobguiTextIter *end)
{
  g_return_val_if_fail (start != NULL, NULL);
  g_return_val_if_fail (end != NULL, NULL);
  ...
  return _bobgui_text_btree_get_text (start, end, TRUE, FALSE);
}

/**
 * bobgui_text_iter_get_visible_slice:
 * @start: iterator at start of range
 * @end: iterator at end of range
 *
 * Returns visible text in the given range.
 *
 * Like [method@Bobgui.TextIter.get_slice], but invisible text
 * is not included. Invisible text is usually invisible because
 * a `BobguiTextTag` with the “invisible” attribute turned on has
 * been applied to it.
 *
 * Returns: (transfer full): slice of text from the buffer
 */
char *
bobgui_text_iter_get_visible_slice (const BobguiTextIter  *start,
                                 const BobguiTextIter  *end)
{
  g_return_val_if_fail (start != NULL, NULL);
  g_return_val_if_fail (end != NULL, NULL);
  ...
  return _bobgui_text_btree_get_text (start, end, FALSE, TRUE);
}

/**
 * bobgui_text_iter_get_visible_text:
 * @start: iterator at start of range
 * @end: iterator at end of range
 *
 * Returns visible text in the given range.
 *
 * Like [method@Bobgui.TextIter.get_text], but invisible text
 * is not included. Invisible text is usually invisible because
 * a `BobguiTextTag` with the “invisible” attribute turned on has
 * been applied to it.
 *
 ...
 */
char *
bobgui_text_iter_get_visible_text (const BobguiTextIter  *start,
                                const BobguiTextIter  *end)
{
  g_return_val_if_fail (start != NULL, NULL);
  g_return_val_if_fail (end != NULL, NULL);
  ...
  return _bobgui_text_btree_get_text (start, end, FALSE, FALSE);
}

/**
 * bobgui_text_iter_get_paintable:
 * @iter: an iterator
 *
 ...
 **/
GdkPaintable *
bobgui_text_iter_get_paintable (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  ...
  if (real->segment->type != &bobgui_text_paintable_type)
    return NULL;
  else
...
/**
 * bobgui_text_iter_get_child_anchor:
 * @iter: an iterator
 *
...
bobgui_text_iter_get_child_anchor (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  ...
  if (real->segment->type != &bobgui_text_child_type)
    return NULL;
  else
...
/**
 * bobgui_text_iter_get_marks:
 * @iter: an iterator
 *
 * Returns a list of all `BobguiTextMark` at this location.
 *
 ...
 *
 * Returns: (element-type BobguiTextMark) (transfer container):
 *   list of `BobguiTextMark`
 */
GSList*
bobgui_text_iter_get_marks (const BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  BobguiTextLineSegment *seg;
  GSList *retval;
  ...
  while (seg != real->segment)
    {
      if (seg->type == &bobgui_text_left_mark_type ||
...
/**
 * bobgui_text_iter_get_toggled_tags:
 * @iter: an iterator
 * @toggled_on: %TRUE to get toggled-on tags
 *
 * Returns a list of `BobguiTextTag` that are toggled on or off at this
 * point.
 *
 ...
 *
 * Returns: (element-type BobguiTextTag) (transfer container): tags
 *   toggled at this point
 */
GSList*
bobgui_text_iter_get_toggled_tags  (const BobguiTextIter  *iter,
                                 gboolean            toggled_on)
{
  BobguiTextRealIter *real;
  BobguiTextLineSegment *seg;
  ...
  while (seg != real->segment)
    {
      if (toggled_on)
        ...
        {
          if (seg->type == &bobgui_text_toggle_off_type)
            {
              retval = g_slist_prepend (retval, seg->body.toggle.info->tag);
            }
        }
...
/**
 * bobgui_text_iter_starts_tag:
 * @iter: an iterator
 * @tag: (nullable): a `BobguiTextTag`
 *
 ...
 * words, unlike [method@Bobgui.TextIter.ends_tag], if
 * this function returns %TRUE, [method@Bobgui.TextIter.has_tag]
 * will also return %TRUE for the same parameters.
 *
 * Returns: whether @iter is the start of a range tagged with @tag
 **/
gboolean
bobgui_text_iter_starts_tag (const BobguiTextIter *iter,
                          BobguiTextTag        *tag)
{
  BobguiTextRealIter *real;
  BobguiTextLineSegment *seg;
 ...
 * unlike [method@Bobgui.TextIter.starts_tag], if this function
 * returns %TRUE, [method@Bobgui.TextIter.has_tag] will return
 * %FALSE for the same parameters.
 *
 * Returns: whether @iter is the end of a range tagged with @tag
 */
gboolean
bobgui_text_iter_ends_tag   (const BobguiTextIter  *iter,
                          BobguiTextTag         *tag)
{
  BobguiTextRealIter *real;
  BobguiTextLineSegment *seg;
 ...
 * or ends at @iter.
 *
 * This is equivalent to (bobgui_text_iter_starts_tag() ||
 * bobgui_text_iter_ends_tag())
 *
 * Returns: whether @tag is toggled on or off at @iter
 */
gboolean
bobgui_text_iter_toggles_tag (const BobguiTextIter  *iter,
                           BobguiTextTag         *tag)
{
  BobguiTextRealIter *real;
  BobguiTextLineSegment *seg;
 ...
 *
 * See also [method@Bobgui.TextIter.starts_tag] and
 * [method@Bobgui.TextIter.ends_tag].
 *
 * Returns: whether @iter is tagged with @tag
 */
gboolean
bobgui_text_iter_has_tag (const BobguiTextIter   *iter,
                       BobguiTextTag          *tag)
{
  BobguiTextRealIter *real;
  ...
  if (real->line_byte_offset >= 0)
    {
      return _bobgui_text_line_byte_has_tag (real->line, real->tree,
                                          real->line_byte_offset, tag);
    }
  else
    {
      g_assert (real->line_char_offset >= 0);
      return _bobgui_text_line_char_has_tag (real->line, real->tree,
                                          real->line_char_offset, tag);
    }
}

/**
 * bobgui_text_iter_get_tags:
 * @iter: a `BobguiTextIter`
 *
 ...
 *
 * The `BobguiTextTag`s in the list don’t have a reference added,
 * but you have to free the list itself.
 *
 * Returns: (element-type BobguiTextTag) (transfer container): list of
 *   `BobguiTextTag`
 */
GSList*
bobgui_text_iter_get_tags (const BobguiTextIter *iter)
{
  GPtrArray *tags;
  GSList *retval;
 ...
 * user via `BobguiTextView`. If no tags applied to this text affect
 ...
 * editable range. Use [method@Bobgui.TextIter.can_insert] to handle this
 * case.
 *
 * Returns: whether @iter is inside an editable range
 */
gboolean
bobgui_text_iter_editable (const BobguiTextIter *iter,
                        gboolean           default_setting)
{
  ...
  values = bobgui_text_attributes_new ();

  values->editable = default_setting;

  bobgui_text_iter_get_attributes (iter, values);

  retval = values->editable;

  bobgui_text_attributes_unref (values);

  return retval;
}

/**
 * bobgui_text_iter_can_insert:
 * @iter: an iterator
 ...
 * [method@Bobgui.TextBuffer.insert_interactive] uses this function
 * to decide whether insertions are allowed at a given position.
 *
 * Returns: whether text inserted at @iter would be editable
 */
gboolean
bobgui_text_iter_can_insert (const BobguiTextIter *iter,
                          gboolean           default_editability)
{
  ...
  /* If at start/end of buffer, default editability is used */
  else if ((bobgui_text_iter_is_start (iter) ||
  ...
  else
    {
       ...
       */
      BobguiTextIter prev = *iter;
      bobgui_text_iter_backward_char (&prev);
      return bobgui_text_iter_editable (&prev, default_editability);
    }
}

gboolean
bobgui_text_iter_get_attributes (const BobguiTextIter  *iter,
                              BobguiTextAttributes  *values)
{
  GPtrArray *tags;
  ...
  _bobgui_text_attributes_fill_from_tags (values, tags);
...
/**
 * bobgui_text_iter_get_language:
 * @iter: an iterator
 *
 * Returns the language in effect at @iter.
 *
 * If no tags affecting language apply to @iter, the return
 * value is identical to that of [func@Bobgui.get_default_language].
 *
 * Returns: (transfer full): language in effect at @iter
 */
PangoLanguage *
bobgui_text_iter_get_language (const BobguiTextIter *iter)
{
  BobguiTextAttributes *values;
...
/**
 * bobgui_text_iter_starts_line:
 * @iter: an iterator
 *
 * Returns %TRUE if @iter begins a paragraph.
 *
 * This is the case if [method@Bobgui.TextIter.get_line_offset]
 * would return 0. However this function is potentially more
 * efficient than [method@Bobgui.TextIter.get_line_offset], because
 ...
 */
gboolean
bobgui_text_iter_starts_line (const BobguiTextIter   *iter)
{
  BobguiTextRealIter *real;
 ...
 */
gboolean
bobgui_text_iter_ends_line (const BobguiTextIter   *iter)
{
  gunichar wc;
...
#define PARAGRAPH_SEPARATOR 0x2029
  ...
  wc = bobgui_text_iter_get_char (iter);

  if (wc == '\r' || wc == PARAGRAPH_SEPARATOR || wc == 0) /* wc == 0 is end iterator */
    return TRUE;
  else if (wc == '\n')
    {
      BobguiTextIter tmp = *iter;
      ...
      if (bobgui_text_iter_get_line_offset (&tmp) == 0)
        return TRUE;

      if (!bobgui_text_iter_backward_char (&tmp))
        return TRUE;

      return bobgui_text_iter_get_char (&tmp) != '\r';
    }
  else
    return FALSE;
}
...
bobgui_text_iter_is_end (const BobguiTextIter *iter)
{
  ...
  if (!_bobgui_text_line_contains_end_iter (real->line, real->tree))
    return FALSE;
  ...
  return _bobgui_text_btree_is_end (real->tree, real->line,
                                 real->segment,
                                 real->segment_byte_offset,
 ...
 */
gboolean
bobgui_text_iter_is_start (const BobguiTextIter *iter)
{
  return bobgui_text_iter_get_offset (iter) == 0;
}

/**
 * bobgui_text_iter_get_chars_in_line:
 * @iter: an iterator
 *
 ...
 */
int
bobgui_text_iter_get_chars_in_line (const BobguiTextIter   *iter)
{
  BobguiTextRealIter *real;
  int count;
  BobguiTextLineSegment *seg;
...
/**
 * bobgui_text_iter_get_bytes_in_line:
 * @iter: an iterator
 *
 ...
 */
int
bobgui_text_iter_get_bytes_in_line (const BobguiTextIter   *iter)
{
  BobguiTextRealIter *real;
  int count;
  BobguiTextLineSegment *seg;
 ...
 */
static gboolean
forward_line_leaving_caches_unmodified (BobguiTextRealIter *real)
{
  if (!_bobgui_text_line_contains_end_iter (real->line, real->tree))
    {
      BobguiTextLine *new_line;

      new_line = _bobgui_text_line_next (real->line);
      g_assert (new_line);
      g_assert (new_line != real->line);
      g_assert (!_bobgui_text_line_is_last (new_line, real->tree));
 ...
 */
static gboolean
backward_line_leaving_caches_unmodified (BobguiTextRealIter *real)
{
  BobguiTextLine *new_line;

  new_line = _bobgui_text_line_previous (real->line);
 ...
 */
static gboolean
forward_char (BobguiTextRealIter *real)
{
  BobguiTextIter *iter = (BobguiTextIter*)real;

  check_invariants ((BobguiTextIter*)real);
  ...
  if ( (real->segment_char_offset + 1) == real->segment->char_count)
    {
      ...
      return _bobgui_text_iter_forward_indexable_segment (iter);
    }
  else
    {
      ...
      g_assert (real->segment->type == &bobgui_text_char_type);
      ...
      if (bobgui_text_iter_is_end ((BobguiTextIter*)real))
        return FALSE;
      else
        return TRUE;
    }
...
gboolean
_bobgui_text_iter_forward_indexable_segment (BobguiTextIter *iter)
{
  /* Need to move to the next segment; if no next segment,
     need to move to next line. */
  BobguiTextLineSegment *seg;
  BobguiTextLineSegment *any_seg;
  BobguiTextRealIter *real;
  int chars_skipped;
  int bytes_skipped;
  ...
  if (seg != NULL)
    {
      ...
      return !bobgui_text_iter_is_end (iter);
    }
  else
    {
      /* End of the line */
      if (forward_line_leaving_caches_unmodified (real))
        {
          ...
          g_assert (bobgui_text_iter_starts_line (iter));
      ...
      else
        {
          ...
          g_assert (!_bobgui_text_line_is_last (real->line, real->tree));
          g_assert (_bobgui_text_line_contains_end_iter (real->line, real->tree));

          bobgui_text_iter_forward_to_line_end (iter);

          g_assert (bobgui_text_iter_is_end (iter));
...
static gboolean
at_last_indexable_segment (BobguiTextRealIter *real)
{
  BobguiTextLineSegment *seg;
 ...
 */
gboolean
_bobgui_text_iter_backward_indexable_segment (BobguiTextIter *iter)
{
  ...
  BobguiTextLineSegment *seg;
  BobguiTextLineSegment *any_seg;
  BobguiTextLineSegment *prev_seg;
  BobguiTextLineSegment *prev_any_seg;
  int bytes_skipped;
  int chars_skipped;
  ...
  if (seg == real->segment)
    {
       ...
       */
      if (bobgui_text_iter_backward_line (iter))
        {
 ...
 */
gboolean
bobgui_text_iter_forward_char (BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
...
/**
 * bobgui_text_iter_backward_char:
 * @iter: an iterator
 *
 ...
 */
gboolean
bobgui_text_iter_backward_char (BobguiTextIter *iter)
{
  g_return_val_if_fail (iter != NULL, FALSE);

  check_invariants (iter);

  return bobgui_text_iter_backward_chars (iter, 1);
}
...
/**
 * bobgui_text_iter_forward_chars:
 * @iter: an iterator
 ...
 */
gboolean
bobgui_text_iter_forward_chars (BobguiTextIter *iter, int count)
{
  BobguiTextRealIter *real;
  ...
  if (real == NULL)
    ...
    return bobgui_text_iter_backward_chars (iter, 0 - count);
  ...
  else
    {
      ...
      current_char_index = bobgui_text_iter_get_offset (iter);

      if (current_char_index == _bobgui_text_btree_char_count (real->tree))
        return FALSE; /* can't move forward */

      new_char_index = current_char_index + count;
      bobgui_text_iter_set_offset (iter, new_char_index);
...
/**
 * bobgui_text_iter_backward_chars:
 * @iter: an iterator
 ...
 */
gboolean
bobgui_text_iter_backward_chars (BobguiTextIter *iter, int count)
{
  BobguiTextRealIter *real;
  ...
  if (real == NULL)
    ...
    return bobgui_text_iter_forward_chars (iter, 0 - count);
...
/**
 * bobgui_text_iter_forward_text_chars:
 * @iter: a `BobguiTextIter`
 ...
 *
 * Equivalent to moving through the results of bobgui_text_iter_get_text(),
 * rather than bobgui_text_iter_get_slice().
 *
 * Returns: whether @iter moved and is dereferenceable
 */
gboolean
bobgui_text_iter_forward_text_chars  (BobguiTextIter *iter,
                                   int          count)
{
...
/**
 * bobgui_text_iter_backward_text_chars:
 * @iter: a `BobguiTextIter`
 * @count: number of chars to move
 *
 * Moves backward by @count text characters (paintables, widgets,
 * etc. do not count as characters for this). Equivalent to moving
 * through the results of bobgui_text_iter_get_text(), rather than
 * bobgui_text_iter_get_slice().
 *
 * Returns: whether @iter moved and is dereferenceable
 **/
gboolean
bobgui_text_iter_backward_text_chars (BobguiTextIter *iter,
                                   int          count)
{
 ...
 */
gboolean
bobgui_text_iter_forward_line (BobguiTextIter *iter)
{
  BobguiTextRealIter *real;
  ...
  if (forward_line_leaving_caches_unmodified (real))
  ...
  else
    {
      ...
      if (!bobgui_text_iter_is_end (iter))
        bobgui_text_iter_forward_to_end (iter);
...
/**
 * bobgui_text_iter_backward_line:
 * @iter: an iterator
 *
 ...
 */
gboolean
bobgui_text_iter_backward_line (BobguiTextIter *iter)
{
  BobguiTextLine *new_line;
  BobguiTextRealIter *real;
  ...
  real->any_segment = real->line->segments;
  real->segment = _bobgui_text_line_byte_to_segment (real->line,
                                                  0, &offset);
...
/**
 * bobgui_text_iter_forward_lines:
 * @iter: a `BobguiTextIter`
 ...
 */
gboolean
bobgui_text_iter_forward_lines (BobguiTextIter *iter, int count)
{
  FIX_OVERFLOWS (count);

  if (count < 0)
    return bobgui_text_iter_backward_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      check_invariants (iter);
      return bobgui_text_iter_forward_line (iter);
    }
  else
    {
      int old_line;
      ...
      old_line = bobgui_text_iter_get_line (iter);

      bobgui_text_iter_set_line (iter, old_line + count);

      if ((bobgui_text_iter_get_line (iter) - old_line) < count)
        {
...
/**
 * bobgui_text_iter_backward_lines:
 * @iter: a `BobguiTextIter`
 ...
 */
gboolean
bobgui_text_iter_backward_lines (BobguiTextIter *iter, int count)
{
  FIX_OVERFLOWS (count);

  if (count < 0)
    return bobgui_text_iter_forward_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      return bobgui_text_iter_backward_line (iter);
    }
  else
    {
      int old_line;
      ...
      bobgui_text_iter_set_line (iter, MAX (old_line - count, 0));

      return (bobgui_text_iter_get_line (iter) != old_line);
    }
}

/**
 * bobgui_text_iter_forward_visible_line:
 * @iter: an iterator
 *
 ...
 */
gboolean
bobgui_text_iter_forward_visible_line (BobguiTextIter *iter)
{
  while (bobgui_text_iter_forward_line (iter))
    {
      if (!_bobgui_text_btree_char_is_invisible (iter))
      ...
      else
        {
          do
            {
              if (!bobgui_text_iter_forward_char (iter))
                return FALSE;

              if (!_bobgui_text_btree_char_is_invisible (iter))
                return TRUE;
            }
          while (!bobgui_text_iter_ends_line (iter));
        }
    }
...
/**
 * bobgui_text_iter_backward_visible_line:
 * @iter: an iterator
 *
 ...
 */
gboolean
bobgui_text_iter_backward_visible_line (BobguiTextIter *iter)
{
  while (bobgui_text_iter_backward_line (iter))
    {
      if (!_bobgui_text_btree_char_is_invisible (iter))
        return TRUE;
      else
        {
          do
            {
              if (!bobgui_text_iter_backward_char (iter))
                return FALSE;

              if (!_bobgui_text_btree_char_is_invisible (iter))
                return TRUE;
            }
          while (!bobgui_text_iter_starts_line (iter));
        }
    }
...
/**
 * bobgui_text_iter_forward_visible_lines:
 * @iter: a `BobguiTextIter`
 * @count: number of lines to move forward
 *
 ...
 */
gboolean
bobgui_text_iter_forward_visible_lines (BobguiTextIter *iter,
                                     int          count)
{
  FIX_OVERFLOWS (count);

  if (count < 0)
    return bobgui_text_iter_backward_visible_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      check_invariants (iter);
      return bobgui_text_iter_forward_visible_line (iter);
    }
  else
    {
      while (bobgui_text_iter_forward_visible_line (iter) && count > 0)
        count--;
      return count == 0;
    }
}
...
/**
 * bobgui_text_iter_backward_visible_lines:
 * @iter: a `BobguiTextIter`
 * @count: number of lines to move backward
 *
 ...
 */
gboolean
bobgui_text_iter_backward_visible_lines (BobguiTextIter *iter,
                                      int          count)
{
  FIX_OVERFLOWS (count);

  if (count < 0)
    return bobgui_text_iter_forward_visible_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      return bobgui_text_iter_backward_visible_line (iter);
    }
  else
    {
      while (bobgui_text_iter_backward_visible_line (iter) && count > 0)
        count--;
      return count == 0;
    }
}
...
static gboolean
test_log_attrs (const BobguiTextIter *iter,
                TestLogAttrFunc    func)
{
  ...
  attrs = _bobgui_text_buffer_get_line_log_attrs (bobgui_text_iter_get_buffer (iter),
                                               iter, &char_len);

  offset = bobgui_text_iter_get_line_offset (iter);
...
static gboolean
find_line_log_attrs (const BobguiTextIter *iter,
...
static gboolean
find_by_log_attrs (BobguiTextIter     *arg_iter,
                   FindLogAttrFunc  func,
                   gboolean         forward)
{
  BobguiTextIter iter;
      ...
      if (found)
        {
          ...
          bobgui_text_iter_set_line_offset (&iter, offset);

          moved = !bobgui_text_iter_equal (&iter, arg_iter);

          *arg_iter = iter;
          return moved && !bobgui_text_iter_is_end (arg_iter);
        }

      if (forward)
        {
          if (!bobgui_text_iter_forward_line (&iter))
            return FALSE;
...
static gboolean
find_visible_by_log_attrs (BobguiTextIter     *iter,
                           FindLogAttrFunc  func,
                           gboolean         forward)
{
  BobguiTextIter pos;
  ...
  while (TRUE)
    {
      BobguiTextIter pos_before = pos;

      find_by_log_attrs (&pos, func, forward);

      if (bobgui_text_iter_equal (&pos_before, &pos))
        break;
...
typedef gboolean (* OneStepFunc) (BobguiTextIter *iter);
typedef gboolean (* MultipleStepFunc) (BobguiTextIter *iter, int count);

static gboolean
move_multiple_steps (BobguiTextIter *iter,
		     int count,
...
/**
 * bobgui_text_iter_forward_word_end:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_forward_word_end (BobguiTextIter *iter)
{
  return find_by_log_attrs (iter, find_word_end_func, TRUE);
}

/**
 * bobgui_text_iter_backward_word_start:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_backward_word_start (BobguiTextIter *iter)
{
...
/**
 * bobgui_text_iter_forward_word_ends:
 * @iter: a `BobguiTextIter`
 * @count: number of times to move
 *
 * Calls [method@Bobgui.TextIter.forward_word_end] up to @count times.
 *
 * Returns: %TRUE if @iter moved and is not the end iterator
 */
gboolean
bobgui_text_iter_forward_word_ends (BobguiTextIter      *iter,
                                 int               count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_forward_word_end,
			      bobgui_text_iter_backward_word_starts);
}
 ...
 * @count: number of times to move
 *
 * Calls [method@Bobgui.TextIter.backward_word_start] up to @count times.
 *
 * Returns: %TRUE if @iter moved and is not the end iterator
 */
gboolean
bobgui_text_iter_backward_word_starts (BobguiTextIter      *iter,
                                    int                count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_backward_word_start,
...
/**
 * bobgui_text_iter_forward_visible_word_end:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_forward_visible_word_end (BobguiTextIter *iter)
{
  return find_visible_by_log_attrs (iter, find_word_end_func, TRUE);
}

/**
 * bobgui_text_iter_backward_visible_word_start:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_backward_visible_word_start (BobguiTextIter      *iter)
{
  return find_visible_by_log_attrs (iter, find_word_start_func, FALSE);
}

/**
 * bobgui_text_iter_forward_visible_word_ends:
 * @iter: a `BobguiTextIter`
 * @count: number of times to move
 *
 * Calls [method@Bobgui.TextIter.forward_visible_word_end] up to @count times.
 *
 * Returns: %TRUE if @iter moved and is not the end iterator
 */
gboolean
bobgui_text_iter_forward_visible_word_ends (BobguiTextIter *iter,
					 int          count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_forward_visible_word_end,
			      bobgui_text_iter_backward_visible_word_starts);
}
 ...
 * @count: number of times to move
 *
 * Calls [method@Bobgui.TextIter.backward_visible_word_start] up to @count times.
 *
 * Returns: %TRUE if @iter moved and is not the end iterator
 */
gboolean
bobgui_text_iter_backward_visible_word_starts (BobguiTextIter *iter,
					    int          count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_backward_visible_word_start,
 ...
 */
gboolean
bobgui_text_iter_starts_word (const BobguiTextIter *iter)
{
  return test_log_attrs (iter, is_word_start_func);
}

/**
 * bobgui_text_iter_ends_word:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_ends_word (const BobguiTextIter *iter)
{
  return test_log_attrs (iter, is_word_end_func);
}

/**
 * bobgui_text_iter_inside_word:
 * @iter: a `BobguiTextIter`
 *
 ...
 * for nearly any language.
 *
 * Note that if [method@Bobgui.TextIter.starts_word] returns %TRUE,
 ...
 */
gboolean
bobgui_text_iter_inside_word (const BobguiTextIter *iter)
{
  return test_log_attrs (iter, inside_word_func);
}

/**
 * bobgui_text_iter_starts_sentence:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_starts_sentence (const BobguiTextIter *iter)
{
  return test_log_attrs (iter, is_sentence_start_func);
}

/**
 * bobgui_text_iter_ends_sentence:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_ends_sentence (const BobguiTextIter *iter)
{
  return test_log_attrs (iter, is_sentence_end_func);
}

/**
 * bobgui_text_iter_inside_sentence:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_inside_sentence (const BobguiTextIter *iter)
{
  return test_log_attrs (iter, inside_sentence_func);
}

/**
 * bobgui_text_iter_forward_sentence_end:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_forward_sentence_end (BobguiTextIter *iter)
{
  return find_by_log_attrs (iter, find_sentence_end_func, TRUE);
}

/**
 * bobgui_text_iter_backward_sentence_start:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_backward_sentence_start (BobguiTextIter *iter)
{
 ...
 */
/**
 * bobgui_text_iter_forward_sentence_ends:
 * @iter: a `BobguiTextIter`
 * @count: number of sentences to move
 *
 * Calls [method@Bobgui.TextIter.forward_sentence_end] @count times.
 *
 ...
 */
gboolean
bobgui_text_iter_forward_sentence_ends (BobguiTextIter      *iter,
                                     int               count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_forward_sentence_end,
			      bobgui_text_iter_backward_sentence_starts);
}
 ...
 * @count: number of sentences to move
 *
 * Calls [method@Bobgui.TextIter.backward_sentence_start] up to @count times.
 *
 ...
 */
gboolean
bobgui_text_iter_backward_sentence_starts (BobguiTextIter *iter,
                                        int          count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_backward_sentence_start,
...
/**
 * bobgui_text_iter_forward_cursor_position:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_forward_cursor_position (BobguiTextIter *iter)
{
  return find_by_log_attrs (iter, find_forward_cursor_pos_func, TRUE);
}

/**
 * bobgui_text_iter_backward_cursor_position:
 * @iter: a `BobguiTextIter`
 *
 * Like [method@Bobgui.TextIter.forward_cursor_position], but moves backward.
 *
 * Returns: %TRUE if we moved
 */
gboolean
bobgui_text_iter_backward_cursor_position (BobguiTextIter *iter)
{
  return find_by_log_attrs (iter, find_backward_cursor_pos_func, FALSE);
}

/**
 * bobgui_text_iter_forward_cursor_positions:
 * @iter: a `BobguiTextIter`
 * @count: number of positions to move
 *
 * Moves up to @count cursor positions.
 *
 * See [method@Bobgui.TextIter.forward_cursor_position] for details.
 *
 * Returns: %TRUE if we moved and the new position is dereferenceable
 */
gboolean
bobgui_text_iter_forward_cursor_positions (BobguiTextIter *iter,
                                        int          count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_forward_cursor_position,
			      bobgui_text_iter_backward_cursor_positions);
}
 ...
 */
gboolean
bobgui_text_iter_backward_cursor_positions (BobguiTextIter *iter,
                                         int          count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_backward_cursor_position,
...
/**
 * bobgui_text_iter_forward_visible_cursor_position:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_forward_visible_cursor_position (BobguiTextIter *iter)
{
  return find_visible_by_log_attrs (iter, find_forward_cursor_pos_func, TRUE);
}

/**
 * bobgui_text_iter_backward_visible_cursor_position:
 * @iter: a `BobguiTextIter`
 *
 * Moves @iter backward to the previous visible cursor position.
 *
 * See [method@Bobgui.TextIter.backward_cursor_position] for details.
 *
 * Returns: %TRUE if we moved and the new position is dereferenceable
 */
gboolean
bobgui_text_iter_backward_visible_cursor_position (BobguiTextIter *iter)
{
  return find_visible_by_log_attrs (iter, find_backward_cursor_pos_func, FALSE);
}

/**
 * bobgui_text_iter_forward_visible_cursor_positions:
 * @iter: a `BobguiTextIter`
 * @count: number of positions to move
 *
 ...
 */
gboolean
bobgui_text_iter_forward_visible_cursor_positions (BobguiTextIter *iter,
						int          count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_forward_visible_cursor_position,
			      bobgui_text_iter_backward_visible_cursor_positions);
}
 ...
 */
gboolean
bobgui_text_iter_backward_visible_cursor_positions (BobguiTextIter *iter,
						 int          count)
{
  return move_multiple_steps (iter, count,
			      bobgui_text_iter_backward_visible_cursor_position,
...
/**
 * bobgui_text_iter_is_cursor_position:
 * @iter: a `BobguiTextIter`
 *
 * Determine if @iter is at a cursor position.
 *
 * See [method@Bobgui.TextIter.forward_cursor_position] or
 ...
 */
gboolean
bobgui_text_iter_is_cursor_position (const BobguiTextIter *iter)
{
  return test_log_attrs (iter, is_cursor_pos_func);
}

/**
 * bobgui_text_iter_set_line_offset:
 * @iter: a `BobguiTextIter`
 ...
 * next line. See [method@Bobgui.TextIter.set_line_index] if you have a byte
 * index rather than a character offset.
 */
void
bobgui_text_iter_set_line_offset (BobguiTextIter *iter,
                               int          char_on_line)
{
  BobguiTextRealIter *real;
  int chars_in_line;
  ...
  chars_in_line = bobgui_text_iter_get_chars_in_line (iter);
  ...
  if (char_on_line < chars_in_line)
    ...
    bobgui_text_iter_forward_line (iter); /* set to start of next line */

  check_invariants (iter);
}

/**
 * bobgui_text_iter_set_line_index:
 * @iter: a `BobguiTextIter`
 * @byte_on_line: a byte index relative to the start of @iter’s current line
 *
 * Same as [method@Bobgui.TextIter.set_line_offset], but works with a
 * byte index. The given byte index must be at
 * the start of a character, it can’t be in the middle of a UTF-8
 * encoded character.
 */
void
bobgui_text_iter_set_line_index (BobguiTextIter *iter,
                              int          byte_on_line)
{
  BobguiTextRealIter *real;
  int bytes_in_line;
  ...
  bytes_in_line = bobgui_text_iter_get_bytes_in_line (iter);
  ...
  if (byte_on_line < bytes_in_line)
    ...
    bobgui_text_iter_forward_line (iter);
...
/**
 * bobgui_text_iter_set_visible_line_offset:
 * @iter: a `BobguiTextIter`
 * @char_on_line: a character offset
 *
 * Like [method@Bobgui.TextIter.set_line_offset], but the offset is in visible
 * characters, i.e. text with a tag making it invisible is not
 * counted in the offset.
 */
void
bobgui_text_iter_set_visible_line_offset (BobguiTextIter *iter,
                                       int          char_on_line)
{
  int chars_seen = 0;
  BobguiTextIter pos;
  ...
  while (chars_seen < char_on_line)
    {
      ...
      if (!bobgui_text_iter_forward_char (&pos))
        break;
  ...
  if (_bobgui_text_iter_get_text_line (&pos) == _bobgui_text_iter_get_text_line (iter))
    *iter = pos;
  else
    bobgui_text_iter_forward_line (iter);
}
...
/**
 * bobgui_text_iter_set_visible_line_index:
 * @iter: a `BobguiTextIter`
 * @byte_on_line: a byte index
 *
 * Like [method@Bobgui.TextIter.set_line_index], but the index is in visible
 * bytes, i.e. text with a tag making it invisible is not counted
 * in the index.
 */
void
bobgui_text_iter_set_visible_line_index (BobguiTextIter *iter,
                                      int          byte_on_line)
{
  BobguiTextRealIter *real;
  int offset = 0;
  ...
  real = bobgui_text_iter_make_real (&pos);
 ...
 */
void
bobgui_text_iter_set_line (BobguiTextIter *iter,
                        int          line_number)
{
  BobguiTextLine *line;
  int real_line;
  ...
  line = _bobgui_text_btree_get_line_no_last (real->tree, line_number, &real_line);
 ...
 */
void
bobgui_text_iter_set_offset (BobguiTextIter *iter,
                          int          char_offset)
{
  BobguiTextLine *line;
  BobguiTextRealIter *real;
  ...
  line = _bobgui_text_btree_get_line_at_char (real->tree,
                                           char_offset,
                                           &line_start,
                                           &real_char_index);
...
bobgui_text_iter_forward_to_end  (BobguiTextIter *iter)
{
  ...
  buffer = _bobgui_text_btree_get_buffer (real->tree);

  bobgui_text_buffer_get_end_iter (buffer, iter);
}

/* FIXME this and bobgui_text_iter_forward_to_line_end() could be cleaned up
 ...
 */
static int
find_paragraph_delimiter_for_line (BobguiTextIter *iter)
{
  BobguiTextIter end;
  end = *iter;

  if (_bobgui_text_line_contains_end_iter (_bobgui_text_iter_get_text_line (&end),
  ...
  else
    {
      ...
      while (!bobgui_text_iter_ends_line (&end))
        bobgui_text_iter_backward_char (&end);
    }

  return bobgui_text_iter_get_line_offset (&end);
}

/**
 * bobgui_text_iter_forward_to_line_end:
 * @iter: a `BobguiTextIter`
 *
 ...
 */
gboolean
bobgui_text_iter_forward_to_line_end (BobguiTextIter *iter)
{
  int current_offset;
  int new_offset;
  ...
  current_offset = bobgui_text_iter_get_line_offset (iter);
  new_offset = find_paragraph_delimiter_for_line (iter);

  if (current_offset < new_offset)
    {
      /* Move to end of this line. */
      bobgui_text_iter_set_line_offset (iter, new_offset);
      return !bobgui_text_iter_is_end (iter);
    }
  else
    {
...
/**
 * bobgui_text_iter_forward_to_tag_toggle:
 * @iter: a `BobguiTextIter`
 * @tag: (nullable): a `BobguiTextTag`
 *
 ...
 */
gboolean
bobgui_text_iter_forward_to_tag_toggle (BobguiTextIter *iter,
                                     BobguiTextTag  *tag)
{
  BobguiTextLine *next_line;
  BobguiTextLine *current_line;
  BobguiTextRealIter *real;
  ...
  next_line = _bobgui_text_line_next_could_contain_tag (current_line,
                                                     real->tree, tag);

  while (_bobgui_text_iter_forward_indexable_segment (iter))
    {
          ...
          if (next_line == NULL)
            {
              ...
              _bobgui_text_btree_get_end_iter (real->tree, iter);
              return FALSE;
            }
      ...
      if (bobgui_text_iter_toggles_tag (iter, tag))
        {
...
/**
 * bobgui_text_iter_backward_to_tag_toggle:
 * @iter: a `BobguiTextIter`
 * @tag: (nullable): a `BobguiTextTag`
 *
 ...
 */
gboolean
bobgui_text_iter_backward_to_tag_toggle (BobguiTextIter *iter,
                                      BobguiTextTag  *tag)
{
  ...
  current_line = real->line;
  prev_line = _bobgui_text_line_previous_could_contain_tag (current_line,
                                                        real->tree, tag);
  ...
  if (is_segment_start (real))
    {
      if (!_bobgui_text_iter_backward_indexable_segment (iter))
        return FALSE;
    }
  else
    {
      ...
      if (!bobgui_text_iter_backward_chars (iter, real->segment_char_offset))
        return FALSE;
    }
          ...
          if (prev_line == NULL)
            {
              ...
              _bobgui_text_btree_get_iter_at_char (real->tree, iter, 0);
              return FALSE;
            }
  ...
  while (_bobgui_text_iter_backward_indexable_segment (iter));
...
static gboolean
matches_pred (BobguiTextIter *iter,
              BobguiTextCharPredicate pred,
              gpointer user_data)
{
  int ch;

  ch = bobgui_text_iter_get_char (iter);

  return (*pred) (ch, user_data);
}

/**
 * bobgui_text_iter_forward_find_char:
 * @iter: a `BobguiTextIter`
 ...
 */
gboolean
bobgui_text_iter_forward_find_char (BobguiTextIter         *iter,
                                 BobguiTextCharPredicate pred,
                                 gpointer             user_data,
                                 const BobguiTextIter   *limit)
{
  ...
  if (limit &&
      bobgui_text_iter_compare (iter, limit) >= 0)
    return FALSE;

  while ((limit == NULL ||
          !bobgui_text_iter_equal (limit, iter)) &&
 ...
 * @limit: (nullable): search limit
 *
 * Same as [method@Bobgui.TextIter.forward_find_char],
 * but goes backward from @iter.
 *
 * Returns: whether a match was found
 */
gboolean
bobgui_text_iter_backward_find_char (BobguiTextIter         *iter,
                                  BobguiTextCharPredicate pred,
                                  gpointer             user_data,
                                  const BobguiTextIter   *limit)
{
...
static void
forward_chars_with_skipping (BobguiTextIter *iter,
                             int          count,
  ...
  while (i > 0)
    {
      ...
      if (skip_nontext &&
          bobgui_text_iter_get_char (iter) == BOBGUI_TEXT_UNKNOWN_CHAR)
        ignored = TRUE;
      ...
      if (!ignored && skip_decomp)
        {
          ...
          buffer_len = g_unichar_to_utf8 (bobgui_text_iter_get_char (iter), buffer);
...
static gboolean
lines_match (const BobguiTextIter *start,
             const char **lines,
             ...
             BobguiTextIter *match_end)
{
  ...
  next = *start;
  bobgui_text_iter_forward_line (&next);

  /* No more text in buffer, but *lines is nonempty */
  if (bobgui_text_iter_equal (start, &next))
    {
      return FALSE;
    }
  ...
  if (slice)
    {
      if (visible_only)
        line_text = bobgui_text_iter_get_visible_slice (start, &next);
      else
        line_text = bobgui_text_iter_get_slice (start, &next);
    }
  else
    {
      if (visible_only)
        line_text = bobgui_text_iter_get_visible_text (start, &next);
      else
        line_text = bobgui_text_iter_get_text (start, &next);
    }
...
/**
 * bobgui_text_iter_forward_search:
 ...
 *
 * @match_start will never be set to a `BobguiTextIter` located before @iter,
 * even if there is a possible @match_end after or at @iter.
 *
 * Returns: whether a match was found
 */
gboolean
bobgui_text_iter_forward_search (const BobguiTextIter *iter,
                              const char        *str,
...
{
  char **lines = NULL;
  BobguiTextIter match;
  gboolean retval = FALSE;
  BobguiTextIter search;
  gboolean visible_only;
  gboolean slice;
  gboolean case_insensitive;
  ...
  if (*str == '\0')
    {
      ...
      if (bobgui_text_iter_forward_char (&match))
        {
          if (limit &&
              bobgui_text_iter_equal (&match, limit))
            return FALSE;
  ...
  visible_only = (flags & BOBGUI_TEXT_SEARCH_VISIBLE_ONLY) != 0;
  slice = (flags & BOBGUI_TEXT_SEARCH_TEXT_ONLY) == 0;
  case_insensitive = (flags & BOBGUI_TEXT_SEARCH_CASE_INSENSITIVE) != 0;
  ...
  do
    {
       ...
       * bobgui_text_iter_get_text() is called repeatedly on
       * a single line.
       */
      BobguiTextIter end;

      if (limit &&
          bobgui_text_iter_compare (&search, limit) >= 0)
        break;
      ...
      if (lines_match (&search, (const char **)lines,
                       visible_only, slice, case_insensitive, &match, &end))
        {
          if (limit == NULL ||
              (limit &&
               bobgui_text_iter_compare (&end, limit) <= 0))
            {
              retval = TRUE;
  ...
  while (bobgui_text_iter_forward_line (&search));
...
struct _LinesWindow
{
  ...
  BobguiTextIter first_line_start;
  BobguiTextIter first_line_end;
                   ...
                   const BobguiTextIter *start)
{
  ...
  bobgui_text_iter_set_line_offset (&line_start, 0);

  if (bobgui_text_iter_equal (&line_start, &line_end))
    {
      /* we were already at the start; so go back one line */
      bobgui_text_iter_backward_line (&line_start);
    }
  ...
  while (i >= 0)
    {
      ...
      if (win->slice)
        {
          if (win->visible_only)
            line_text = bobgui_text_iter_get_visible_slice (&line_start, &line_end);
          else
            line_text = bobgui_text_iter_get_slice (&line_start, &line_end);
        }
      else
        {
          if (win->visible_only)
            line_text = bobgui_text_iter_get_visible_text (&line_start, &line_end);
          else
            line_text = bobgui_text_iter_get_text (&line_start, &line_end);
        }
...
{
  BobguiTextIter new_start;
  char *line_text;

  new_start = win->first_line_start;

  if (!bobgui_text_iter_backward_line (&new_start))
    return FALSE;
  else
    {
      ...
      bobgui_text_iter_forward_line (&win->first_line_end);
    }

  if (win->slice)
    {
      if (win->visible_only)
        line_text = bobgui_text_iter_get_visible_slice (&win->first_line_start,
                                                     &win->first_line_end);
      else
        line_text = bobgui_text_iter_get_slice (&win->first_line_start,
                                             &win->first_line_end);
    }
  else
    {
      if (win->visible_only)
        line_text = bobgui_text_iter_get_visible_text (&win->first_line_start,
                                                    &win->first_line_end);
      else
        line_text = bobgui_text_iter_get_text (&win->first_line_start,
                                            &win->first_line_end);
    }
...
/**
 * bobgui_text_iter_backward_search:
 * @iter: a `BobguiTextIter` where the search begins
 * @str: search string
 ...
 *
 * Same as [method@Bobgui.TextIter.forward_search], but moves backward.
 *
 * @match_end will never be set to a `BobguiTextIter` located after @iter,
 * even if there is a possible @match_start before or at @iter.
 *
 * Returns: whether a match was found
 */
gboolean
bobgui_text_iter_backward_search (const BobguiTextIter *iter,
                               const char        *str,
                               BobguiTextSearchFlags flags,
                               BobguiTextIter       *match_start,
                               BobguiTextIter       *match_end,
                               const BobguiTextIter *limit)
{
  ...
  if (limit &&
      bobgui_text_iter_compare (limit, iter) > 0)
    return FALSE;

  if (*str == '\0')
    {
      /* If we can move one char, return the empty string there */
      BobguiTextIter match = *iter;

      if (limit && bobgui_text_iter_equal (limit, &match))
        return FALSE;

      if (bobgui_text_iter_backward_char (&match))
        {
          if (match_start)
            *match_start = match;
          if (match_end)
  ...
  do
    {
      ...
      if (limit &&
          bobgui_text_iter_compare (limit, &win.first_line_end) > 0)
        {
      ...
      if (first_line_match &&
                                           ...
                                           case_insensitive))
        {
          /* Match! */
          int offset;
          BobguiTextIter start_tmp;
          BobguiTextIter end_tmp;
          ...
          if (limit &&
              bobgui_text_iter_compare (limit, &start_tmp) > 0)
...
/**
 * bobgui_text_iter_equal:
 * @lhs: a `BobguiTextIter`
 * @rhs: another `BobguiTextIter`
 *
 ...
 * bit faster than [method@Bobgui.TextIter.compare].
 *
 * Returns: %TRUE if the iterators point to the same place in the buffer
 **/
gboolean
bobgui_text_iter_equal (const BobguiTextIter *lhs,
                     const BobguiTextIter *rhs)
{
  BobguiTextRealIter *real_lhs;
  BobguiTextRealIter *real_rhs;

  real_lhs = (BobguiTextRealIter*)lhs;
  real_rhs = (BobguiTextRealIter*)rhs;
...
gboolean
_bobgui_text_iter_same_line (const BobguiTextIter *lhs,
                          const BobguiTextIter *rhs)
{
  BobguiTextRealIter *real_lhs;
  BobguiTextRealIter *real_rhs;

  real_lhs = bobgui_text_iter_make_surreal (lhs);
  real_rhs = bobgui_text_iter_make_surreal (rhs);
 ...
 */
int
bobgui_text_iter_compare (const BobguiTextIter *lhs,
                       const BobguiTextIter *rhs)
{
  BobguiTextRealIter *real_lhs;
  BobguiTextRealIter *real_rhs;
  ...
  if (real_lhs->line == real_rhs->line)
  ...
  else
    {
      ...
      line1 = bobgui_text_iter_get_line (lhs);
      line2 = bobgui_text_iter_get_line (rhs);
      if (line1 < line2)
        return -1;
      else if (line1 > line2)
        return 1;
 ...
 */
gboolean
bobgui_text_iter_in_range (const BobguiTextIter *iter,
                        const BobguiTextIter *start,
                        const BobguiTextIter *end)
{
  g_return_val_if_fail (iter != NULL, FALSE);
  ...
  g_return_val_if_fail (bobgui_text_iter_compare (start, end) <= 0, FALSE);

  return bobgui_text_iter_compare (iter, start) >= 0 &&
    bobgui_text_iter_compare (iter, end) < 0;
}

/**
 * bobgui_text_iter_order:
 * @first: a `BobguiTextIter`
 * @second: another `BobguiTextIter`
 *
 ...
 * such as [method@Bobgui.TextIter.in_range], that expect a
 * pre-sorted range.
 */
void
bobgui_text_iter_order (BobguiTextIter *first,
                     BobguiTextIter *second)
{
  g_return_if_fail (first != NULL);
  g_return_if_fail (second != NULL);

  if (bobgui_text_iter_compare (first, second) > 0)
    {
      BobguiTextIter tmp;
...
void
_bobgui_text_btree_get_iter_at_char (BobguiTextBTree *tree,
                                  BobguiTextIter *iter,
                                  int char_index)
{
  BobguiTextRealIter *real = (BobguiTextRealIter*)iter;
  int real_char_index;
  int line_start;
  BobguiTextLine *line;
  ...
  line = _bobgui_text_btree_get_line_at_char (tree, char_index,
...
void
_bobgui_text_btree_get_iter_at_line_char (BobguiTextBTree *tree,
                                       BobguiTextIter  *iter,
                                       int           line_number,
                                       int           char_on_line)
{
  ...
  line = _bobgui_text_btree_get_line_no_last (tree, line_number, &real_line);
...
void
_bobgui_text_btree_get_iter_at_line_byte (BobguiTextBTree   *tree,
                                       BobguiTextIter    *iter,
                                       int             line_number,
                                       int             byte_index)
{
...
void
_bobgui_text_btree_get_iter_at_line      (BobguiTextBTree   *tree,
                                       BobguiTextIter    *iter,
                                       BobguiTextLine    *line,
                                       int             byte_offset)
{
...
void
_bobgui_text_btree_get_iter_at_line_ptr_char (BobguiTextBTree   *tree,
                                           BobguiTextIter    *iter,
                                           BobguiTextLine    *line,
                                           int             char_offset)
{
...
gboolean
_bobgui_text_btree_get_iter_at_first_toggle (BobguiTextBTree   *tree,
                                          BobguiTextIter    *iter,
                                          BobguiTextTag     *tag)
{
  BobguiTextLine *line;
  ...
  line = _bobgui_text_btree_first_could_contain_tag (tree, tag);

  if (line == NULL)
    {
      /* Set iter to last in tree */
      _bobgui_text_btree_get_end_iter (tree, iter);
      check_invariants (iter);
      return FALSE;
    }
  else
    {
      ...
      if (!bobgui_text_iter_toggles_tag (iter, tag))
        bobgui_text_iter_forward_to_tag_toggle (iter, tag);
...
gboolean
_bobgui_text_btree_get_iter_at_last_toggle  (BobguiTextBTree   *tree,
                                          BobguiTextIter    *iter,
                                          BobguiTextTag     *tag)
{
  gboolean found;
  ...
  if (bobgui_text_iter_toggles_tag (iter, tag))
    ...
    found = bobgui_text_iter_backward_to_tag_toggle (iter, tag);
...
gboolean
_bobgui_text_btree_get_iter_at_mark_name (BobguiTextBTree *tree,
                                       BobguiTextIter *iter,
                                       const char *mark_name)
{
  ...
  mark = _bobgui_text_btree_get_mark_by_name (tree, mark_name);

  if (mark == NULL)
    return FALSE;
  else
    {
      _bobgui_text_btree_get_iter_at_mark (tree, iter, mark);
      check_invariants (iter);
      return TRUE;
    }
}
...
void
_bobgui_text_btree_get_iter_at_paintable (BobguiTextBTree *tree,
                                       BobguiTextIter *iter,
                                       BobguiTextLineSegment *seg)
{
  g_return_if_fail (iter != NULL);
  ...
  g_assert (seg->body.paintable.line == _bobgui_text_iter_get_text_line (iter));
  check_invariants (iter);
}

void
_bobgui_text_btree_get_iter_at_mark (BobguiTextBTree *tree,
                                  BobguiTextIter *iter,
                                  BobguiTextMark *mark)
{
  BobguiTextLineSegment *seg;
  ...
  g_assert (seg->body.mark.line == _bobgui_text_iter_get_text_line (iter));
  check_invariants (iter);
}

void
_bobgui_text_btree_get_iter_at_child_anchor (BobguiTextBTree       *tree,
                                          BobguiTextIter        *iter,
                                          BobguiTextChildAnchor *anchor)
{
  ...
  g_assert (seg->body.child.line == _bobgui_text_iter_get_text_line (iter));
  check_invariants (iter);
}

void
_bobgui_text_btree_get_end_iter         (BobguiTextBTree   *tree,
                                      BobguiTextIter    *iter)
{
  g_return_if_fail (iter != NULL);
  g_return_if_fail (tree != NULL);

  _bobgui_text_btree_get_iter_at_char (tree,
                                   iter,
                                   _bobgui_text_btree_char_count (tree));
  check_invariants (iter);
}

void
_bobgui_text_iter_check (const BobguiTextIter *iter)
{
  const BobguiTextRealIter *real = (const BobguiTextRealIter*)iter;
  int line_char_offset, line_byte_offset, seg_char_offset, seg_byte_offset;
  BobguiTextLineSegment *byte_segment = NULL;
  BobguiTextLineSegment *byte_any_segment = NULL;
  BobguiTextLineSegment *char_segment = NULL;
  BobguiTextLineSegment *char_any_segment = NULL;
  gboolean segments_updated;

  /* This function checks our class invariants for the Iter class. */

  g_assert (sizeof (BobguiTextIter) == sizeof (BobguiTextRealIter));

  if (real->chars_changed_stamp !=
      _bobgui_text_btree_get_chars_changed_stamp (real->tree))
  ...
  segments_updated = (real->segments_changed_stamp ==
                      _bobgui_text_btree_get_segments_changed_stamp (real->tree));
  ...
  if (real->line_byte_offset >= 0)
    {
      _bobgui_text_line_byte_locate (real->line, real->line_byte_offset,
      ...
      if (segments_updated)
        {
          ...
          if (byte_segment->type == &bobgui_text_char_type)
            {
              const char *p;
              p = byte_segment->body.chars + seg_byte_offset;

              if (!bobgui_text_byte_begins_utf8_char (p))
  ...
  if (real->line_char_offset >= 0)
    {
      _bobgui_text_line_char_locate (real->line, real->line_char_offset,
      ...
      if (segments_updated)
        {
          ...
          if (char_segment->type == &bobgui_text_char_type)
            {
              const char *p;
      ...
      if (char_segment->type == &bobgui_text_char_type)
        {
          ...
          if (!bobgui_text_byte_begins_utf8_char (char_segment->body.chars + seg_byte_offset))
  ...
  if (real->cached_line_number >= 0)
    {
      ...
      should_be = _bobgui_text_line_get_number (real->line);
  ...
  if (real->cached_char_index >= 0)
    {
      if (real->line_char_offset >= 0) /* only way we can check it
                                          ...
                                          invariant. */
        {
          ...
          char_index = _bobgui_text_line_char_index (real->line);
  ...
  if (_bobgui_text_line_is_last (real->line, real->tree))
