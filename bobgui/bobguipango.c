/* bobguipango.c - pango-related utilities
 *
 ...
 */
PangoAttrList *
_bobgui_pango_attr_list_merge (PangoAttrList *into,
                            PangoAttrList *from)
{
  if (from)
    {
...
attribute_from_text (BobguiBuilder  *builder,
                     const char  *name,
                     const char  *value,
                     GError     **error)
{
  ...
  if (!bobgui_builder_value_from_string_type (builder, PANGO_TYPE_ATTR_TYPE, name, &val, error))
    return NULL;
  ...
  switch (type)
    ...
    case PANGO_ATTR_STYLE:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_STYLE, value, &val, error))
        attribute = pango_attr_style_new (g_value_get_enum (&val));
      break;
    case PANGO_ATTR_WEIGHT:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_WEIGHT, value, &val, error))
        attribute = pango_attr_weight_new (g_value_get_enum (&val));
      break;
    case PANGO_ATTR_VARIANT:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_VARIANT, value, &val, error))
        attribute = pango_attr_variant_new (g_value_get_enum (&val));
      break;
    case PANGO_ATTR_STRETCH:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_STRETCH, value, &val, error))
        attribute = pango_attr_stretch_new (g_value_get_enum (&val));
      break;
    case PANGO_ATTR_UNDERLINE:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_UNDERLINE, value, &val, NULL))
      ...
      else
        {
          ...
          g_value_unset (&val);
          if (bobgui_builder_value_from_string_type (builder, G_TYPE_BOOLEAN, value, &val, error))
    ...
    case PANGO_ATTR_GRAVITY:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_GRAVITY, value, &val, error))
        attribute = pango_attr_gravity_new (g_value_get_enum (&val));
      break;
    case PANGO_ATTR_GRAVITY_HINT:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_GRAVITY_HINT, value, &val, error))
    ...
    case PANGO_ATTR_SIZE:
      if (bobgui_builder_value_from_string_type (builder, G_TYPE_INT, value, &val, error))
    ...
    case PANGO_ATTR_FOREGROUND:
      if (bobgui_builder_value_from_string_type (builder, GDK_TYPE_RGBA, value, &val, error))
        {
    ...
    case PANGO_ATTR_SCALE:
      if (bobgui_builder_value_from_string_type (builder, G_TYPE_DOUBLE, value, &val, error))
    ...
    case PANGO_ATTR_SHOW:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_SHOW_FLAGS, value, &val, error))
    ...
    case PANGO_ATTR_OVERLINE:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_OVERLINE, value, &val, NULL))
    ...
    case PANGO_ATTR_TEXT_TRANSFORM:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_TEXT_TRANSFORM, value, &val, error))
    ...
    case PANGO_ATTR_BASELINE_SHIFT:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_BASELINE_SHIFT, value, &val, NULL))
        attribute = pango_attr_baseline_shift_new (g_value_get_enum (&val));
      else if (bobgui_builder_value_from_string_type (builder, G_TYPE_INT, value, &val, NULL))
        attribute = pango_attr_baseline_shift_new (g_value_get_enum (&val));
      else
        g_set_error (error,
    ...
    case PANGO_ATTR_FONT_SCALE:
      if (bobgui_builder_value_from_string_type (builder, PANGO_TYPE_FONT_SCALE, value, &val, error))
...
void
bobgui_pango_attribute_start_element (BobguiBuildableParseContext  *context,
  ...
  if (strcmp (element_name, "attribute") == 0)
    {
      ...
      if (start)
        {
          if (!bobgui_builder_value_from_string_type (data->builder, G_TYPE_UINT, start, &val, error))
            {
              _bobgui_builder_prefix_error (data->builder, context, error);
              return;
            }
      ...
      if (end)
        {
          if (!bobgui_builder_value_from_string_type (data->builder, G_TYPE_UINT, end, &val, error))
            {
              _bobgui_builder_prefix_error (data->builder, context, error);
              return;
            }
...
{
  /* Keep these in sync with bobgui_wrap_mode_to_string() */
  switch (mode)
    {
...
void
bobgui_pango_get_font_attributes (PangoFontDescription   *font,
...
/*
 * bobgui_pango_get_default_attributes:
 ...
 */
void
bobgui_pango_get_default_attributes (PangoLayout   *layout,
                                  char        ***attribute_names,
                                  char        ***attribute_values)
{
...
/*
 * bobgui_pango_get_run_attributes:
 ...
 */
void
bobgui_pango_get_run_attributes (PangoLayout     *layout,
                              unsigned int     offset,
                              char          ***attribute_names,
                              char          ***attribute_values,
 ...
 */
static int
bobgui_pango_move_chars (PangoLayout *layout,
                      int          offset,
                      int          count)
{
 ...
 */
static int
bobgui_pango_move_words (PangoLayout  *layout,
                      int           offset,
                      int           count)
{
  const PangoLogAttr *attrs;
 ...
 */
static int
bobgui_pango_move_sentences (PangoLayout  *layout,
                          int           offset,
                          int           count)
{
  const PangoLogAttr *attrs;
 ...
 */
static int
bobgui_pango_move_lines (PangoLayout *layout,
                      int          offset,
                      int          count)
{
  GSList *lines, *l;
 ...
 */
static gboolean
bobgui_pango_is_inside_word (PangoLayout  *layout,
                          int           offset)
{
  const PangoLogAttr *attrs;
  int n_attrs;
...
/*
 * bobgui_pango_is_inside_sentence:
 * @layout: a `PangoLayout`
 * @offset: a character offset in @layout
 *
 * Returns whether the given position is inside
 ...
 */
static gboolean
bobgui_pango_is_inside_sentence (PangoLayout  *layout,
                              int           offset)
{
  const PangoLogAttr *attrs;
  int n_attrs;
...
static void
bobgui_pango_get_line_start (PangoLayout *layout,
                          int          offset,
                          int         *start_offset,
                          int         *end_offset)
{
...
char *
bobgui_pango_get_string_at (PangoLayout                  *layout,
                         unsigned int                  offset,
                         ...
                         unsigned int                 *end_offset)
{
  ...
  switch (granularity)
    {
    case BOBGUI_ACCESSIBLE_TEXT_GRANULARITY_CHARACTER:
      end = bobgui_pango_move_chars (layout, end, 1);
      break;

    case BOBGUI_ACCESSIBLE_TEXT_GRANULARITY_WORD:
      if (!attrs[start].is_word_start)
        start = bobgui_pango_move_words (layout, start, -1);
      if (bobgui_pango_is_inside_word (layout, end))
        end = bobgui_pango_move_words (layout, end, 1);
      while (!attrs[end].is_word_start && end < n_attrs - 1)
        end = bobgui_pango_move_chars (layout, end, 1);
      break;

    case BOBGUI_ACCESSIBLE_TEXT_GRANULARITY_SENTENCE:
      if (!attrs[start].is_sentence_start)
        start = bobgui_pango_move_sentences (layout, start, -1);
      if (bobgui_pango_is_inside_sentence (layout, end))
        end = bobgui_pango_move_sentences (layout, end, 1);
      while (!attrs[end].is_sentence_start && end < n_attrs - 1)
        end = bobgui_pango_move_chars (layout, end, 1);
      break;

    case BOBGUI_ACCESSIBLE_TEXT_GRANULARITY_LINE:
      bobgui_pango_get_line_start (layout, offset, &start, &end);
      break;

    case BOBGUI_ACCESSIBLE_TEXT_GRANULARITY_PARAGRAPH:
...
gboolean
bobgui_pango_glyph_item_has_color_glyphs (PangoGlyphItem *run)
{
...
gboolean
bobgui_pango_layout_has_color_glyphs (PangoLayout *layout)
{
  gboolean ret = FALSE;
  PangoLayoutIter *iter;
  ...
  while (pango_layout_iter_next_run (iter) && !ret)
    {
      ...
      if (bobgui_pango_glyph_item_has_color_glyphs (run))
        {
          ret = TRUE;
          break;
        }
