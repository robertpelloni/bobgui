...
#include "bobguigesturestylusprivate.h"
...
} BobguiGestureStylusPrivate;

G_DEFINE_TYPE_WITH_PRIVATE (BobguiGestureStylus, bobgui_gesture_stylus, BOBGUI_TYPE_GESTURE_SINGLE)
...
static void bobgui_gesture_stylus_get_property (GObject    *object,
                                             guint       prop_id,
                                             GValue     *value,
                                             GParamSpec *pspec)
{
	BobguiGestureStylus *gesture = BOBGUI_GESTURE_STYLUS (object);

    switch (prop_id)
      {
      case PROP_STYLUS_ONLY:
        g_value_set_boolean (value, bobgui_gesture_stylus_get_stylus_only (gesture));
        break;
      default:
...
static void bobgui_gesture_stylus_set_property (GObject      *object,
                                             guint         prop_id,
                                             const GValue *value,
                                             GParamSpec   *pspec)
{
    ...
    switch (prop_id)
      {
      case PROP_STYLUS_ONLY:
        bobgui_gesture_stylus_set_stylus_only (gesture, g_value_get_boolean (value));
        break;
      default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      }
...
static gboolean
bobgui_gesture_stylus_handle_event (BobguiEventController *controller,
                                 GdkEvent           *event,
                                 double              x,
                                 double              y)
{
  BobguiGestureStylusPrivate* priv;
  GdkModifierType modifiers;
  guint n_signal;

  priv = bobgui_gesture_stylus_get_instance_private (BOBGUI_GESTURE_STYLUS (controller));
  BOBGUI_EVENT_CONTROLLER_CLASS (bobgui_gesture_stylus_parent_class)->handle_event (controller, event, x, y);
...
static void
bobgui_gesture_stylus_class_init (BobguiGestureStylusClass *klass)
{
  GObjectClass* object_class;
  BobguiEventControllerClass *event_controller_class;

  object_class = G_OBJECT_CLASS (klass);
  object_class->get_property = bobgui_gesture_stylus_get_property;
  object_class->set_property = bobgui_gesture_stylus_set_property;

  /**
   * BobguiGestureStylus:stylus-only:
   *
  ...
  event_controller_class = BOBGUI_EVENT_CONTROLLER_CLASS (klass);
  event_controller_class->handle_event = bobgui_gesture_stylus_handle_event;

  /**
   * BobguiGestureStylus::proximity:
   * @gesture: the `BobguiGestureStylus` that emitted the signal
  ...
  signals[PROXIMITY] =
    g_signal_new (I_("proximity"),
                  ...
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (BobguiGestureStylusClass, proximity),
                  NULL, NULL,
  ...
  signals[DOWN] =
    g_signal_new (I_("down"),
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (BobguiGestureStylusClass, down),
                  NULL, NULL,
                  _bobgui_marshal_VOID__DOUBLE_DOUBLE,
                  G_TYPE_NONE, 2, G_TYPE_DOUBLE, G_TYPE_DOUBLE);
  ...
  signals[MOTION] =
    g_signal_new (I_("motion"),
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (BobguiGestureStylusClass, motion),
                  NULL, NULL,
                  _bobgui_marshal_VOID__DOUBLE_DOUBLE,
                  G_TYPE_NONE, 2, G_TYPE_DOUBLE, G_TYPE_DOUBLE);
  ...
  signals[UP] =
    g_signal_new (I_("up"),
                  G_TYPE_FROM_CLASS (klass),
                  G_SIGNAL_RUN_LAST,
                  G_STRUCT_OFFSET (BobguiGestureStylusClass, up),
                  NULL, NULL,
                  _bobgui_marshal_VOID__DOUBLE_DOUBLE,
                  G_TYPE_NONE, 2, G_TYPE_DOUBLE, G_TYPE_DOUBLE);
...
static void
bobgui_gesture_stylus_init (BobguiGestureStylus *gesture)
{
  BobguiGestureStylusPrivate* priv = bobgui_gesture_stylus_get_instance_private(gesture);

  priv->stylus_only = TRUE;
}

/**
 * bobgui_gesture_stylus_new:
 *
 * Creates a new `BobguiGestureStylus`.
 *
 * Returns: a newly created stylus gesture
 **/
BobguiGesture *
bobgui_gesture_stylus_new (void)
{
  return g_object_new (BOBGUI_TYPE_GESTURE_STYLUS,
                       NULL);
}

/**
 * bobgui_gesture_stylus_get_stylus_only:
 * @gesture: the gesture
 *
 ...
 */
gboolean
bobgui_gesture_stylus_get_stylus_only (BobguiGestureStylus *gesture)
{
  BobguiGestureStylusPrivate *priv = bobgui_gesture_stylus_get_instance_private (gesture);

  g_return_val_if_fail (BOBGUI_IS_GESTURE_STYLUS (gesture), FALSE);

  return priv->stylus_only;
}

/**
 * bobgui_gesture_stylus_set_stylus_only:
 * @gesture: the gesture
 ...
 */
void
bobgui_gesture_stylus_set_stylus_only (BobguiGestureStylus *gesture, gboolean stylus_only)
{
  BobguiGestureStylusPrivate *priv = bobgui_gesture_stylus_get_instance_private (gesture);

  g_return_if_fail (BOBGUI_IS_GESTURE_STYLUS (gesture));
...
/**
 * bobgui_gesture_stylus_get_axis:
 * @gesture: a `BobguiGestureStylus`
 ...
 * [signal@Bobgui.GestureStylus::down], [signal@Bobgui.GestureStylus::motion],
 * [signal@Bobgui.GestureStylus::up] or [signal@Bobgui.GestureStylus::proximity]
 * signals.
 *
 * Returns: %TRUE if there is a current value for the axis
 **/
gboolean
bobgui_gesture_stylus_get_axis (BobguiGestureStylus *gesture,
			     GdkAxisUse        axis,
			     double           *value)
{
  GdkEvent *event;
...
/**
 * bobgui_gesture_stylus_get_axes:
 * @gesture: a `BobguiGestureStylus`
 ...
 */
gboolean
bobgui_gesture_stylus_get_axes (BobguiGestureStylus  *gesture,
			     GdkAxisUse         axes[],
			     double           **values)
{
  GdkEvent *event;
...
/**
 * bobgui_gesture_stylus_get_backlog:
 * @gesture: a `BobguiGestureStylus`
 ...
 *
 * By default, BOBGUI will limit rate of input events. On stylus input
 ...
 * current [Bobgui.GestureStylus::motion] signal.
 *
 * This function may only be called within a [signal@Bobgui.GestureStylus::motion]
 * signal handler, the state given in this signal and obtainable through
 * [method@Bobgui.GestureStylus.get_axis] express the latest (most up-to-date)
 ...
 */
gboolean
bobgui_gesture_stylus_get_backlog (BobguiGestureStylus  *gesture,
                                ...
                                guint             *n_elems)
{
  ...
  native = bobgui_widget_get_native (bobgui_get_event_widget (event));
  bobgui_native_get_surface_transform (native, &surf_x, &surf_y);

  backlog_array = g_array_new (FALSE, FALSE, sizeof (GdkTimeCoord));
  event_widget = bobgui_get_event_widget (event);
  controller_widget = bobgui_event_controller_get_widget (BOBGUI_EVENT_CONTROLLER (gesture));
  for (i = 0; i < n_coords; i++)
    {
      const GdkTimeCoord *time_coord = &history[i];
      graphene_point_t p;

      if (bobgui_widget_compute_point (event_widget, controller_widget,
...
/**
 * bobgui_gesture_stylus_get_device_tool:
 * @gesture: a `BobguiGestureStylus`
 *
 ...
 */
GdkDeviceTool *
bobgui_gesture_stylus_get_device_tool (BobguiGestureStylus *gesture)
{
  GdkEvent *event;
