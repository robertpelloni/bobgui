...
struct _BobguiTrashMonitorClass
{
  GObjectClass parent_class;

  void (* trash_state_changed) (BobguiTrashMonitor *monitor);
};
...
G_DEFINE_TYPE (BobguiTrashMonitor, _bobgui_trash_monitor, G_TYPE_OBJECT)

static BobguiTrashMonitor *the_trash_monitor;

#define ICON_NAME_TRASH_EMPTY "user-trash-symbolic"
#define ICON_NAME_TRASH_FULL  "user-trash-full-symbolic"

static void
bobgui_trash_monitor_dispose (GObject *object)
{
  BobguiTrashMonitor *monitor;

  monitor = BOBGUI_TRASH_MONITOR (object);
...
static void
_bobgui_trash_monitor_class_init (BobguiTrashMonitorClass *class)
{
  GObjectClass *gobject_class;

  gobject_class = (GObjectClass *) class;

  gobject_class->dispose = bobgui_trash_monitor_dispose;
...
trash_query_info_cb (GObject *source,
                     ...
                     gpointer user_data)
{
  BobguiTrashMonitor *monitor = BOBGUI_TRASH_MONITOR (user_data);
  GFileInfo *info;
  guint32 item_count;
...
static void
recompute_trash_state (BobguiTrashMonitor *monitor)
{
  GFile *file;
...
static void
_bobgui_trash_monitor_init (BobguiTrashMonitor *monitor)
{
  GFile *file;
...
/*
 * _bobgui_trash_monitor_get:
 *
 * Returns: (transfer full): a new reference to the singleton
 * BobguiTrashMonitor object.  Be sure to call g_object_unref() on it when you are
 * done with the trash monitor.
 */
BobguiTrashMonitor *
_bobgui_trash_monitor_get (void)
{
  if (the_trash_monitor != NULL)
    ...
    {
      the_trash_monitor = g_object_new (BOBGUI_TYPE_TRASH_MONITOR, NULL);
...
/*
 * _bobgui_trash_monitor_get_icon:
 * @monitor: a BobguiTrashMonitor
 *
 ...
 */
GIcon *
_bobgui_trash_monitor_get_icon (BobguiTrashMonitor *monitor)
{
  const char *icon_name;

  g_return_val_if_fail (BOBGUI_IS_TRASH_MONITOR (monitor), NULL);
...
/*
 * _bobgui_trash_monitor_get_has_trash:
 * @monitor: a BobguiTrashMonitor
 *
 * Returns: #TRUE if there is trash in the trash:/// folder, or #FALSE otherwise.
 */
gboolean
_bobgui_trash_monitor_get_has_trash (BobguiTrashMonitor *monitor)
{
  g_return_val_if_fail (BOBGUI_IS_TRASH_MONITOR (monitor), FALSE);
