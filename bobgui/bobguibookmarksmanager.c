...
#include "deprecated/bobguifilechooser.h" /* for the GError types */

G_GNUC_BEGIN_IGNORE_DEPRECATIONS

static void
_bobgui_bookmark_free (gpointer data)
{
  BobguiBookmark *bookmark = data;
...
get_legacy_bookmarks_file (void)
{
  ...
  filename = g_build_filename (g_get_home_dir (), ".bobgui-bookmarks", NULL);
...
get_bookmarks_file (void)
{
  ...
  /* Use bobgui-3.0's bookmarks file as the format didn't change.
   * Add the 3.0 file format to get_legacy_bookmarks_file() when
   * the format does change.
   */
  filename = g_build_filename (g_get_user_config_dir (), "bobgui-3.0", "bookmarks", NULL);
  file = g_file_new_for_path (filename);
  g_free (filename);
...
parse_bookmarks (const char *contents)
{
  ...
  for (i = 0; lines[i]; i++)
	...
	continue;
      ...
      bookmark = g_new0 (BobguiBookmark, 1);
...
static void
notify_changed (BobguiBookmarksManager *manager)
{
                       ...
                       gpointer      data)
{
  ...
  BobguiBookmarksManager *manager = data;
  char *contents = NULL;
  GError *error = NULL;
  ...
  g_slist_free_full (manager->bookmarks, _bobgui_bookmark_free);
		...
		GSList *bookmarks)
{
  ...
  for (l = bookmarks; l; l = l->next)
    {
      BobguiBookmark *bookmark = l->data;
      char *uri;
...
_bobgui_bookmarks_manager_new (BobguiBookmarksChangedFunc changed_func, gpointer changed_func_data)
{
  BobguiBookmarksManager *manager;
  GFile *bookmarks_file;
  GError *error;

  manager = g_new0 (BobguiBookmarksManager, 1);
...
void
_bobgui_bookmarks_manager_free (BobguiBookmarksManager *manager)
{
...
GSList *
_bobgui_bookmarks_manager_list_bookmarks (BobguiBookmarksManager *manager)
{
...
find_bookmark_link_for_file (GSList *bookmarks, GFile *file, int *position_ret)
{
  ...
  for (; bookmarks; bookmarks = bookmarks->next)
    {
      BobguiBookmark *bookmark = bookmarks->data;
...
gboolean
_bobgui_bookmarks_manager_has_bookmark (BobguiBookmarksManager *manager,
                                     GFile               *file)
{
  GSList *link;
...
gboolean
_bobgui_bookmarks_manager_insert_bookmark (BobguiBookmarksManager *manager,
					GFile               *file,
					int                  position,
					GError             **error)
{
...
gboolean
_bobgui_bookmarks_manager_remove_bookmark (BobguiBookmarksManager *manager,
					GFile               *file,
					GError             **error)
{
  GSList *link;
  ...
  if (link)
    {
      BobguiBookmark *bookmark = link->data;

      manager->bookmarks = g_slist_remove_link (manager->bookmarks, link);
      _bobgui_bookmark_free (bookmark);
      g_slist_free_1 (link);
    }
  else
    {
...
gboolean
_bobgui_bookmarks_manager_reorder_bookmark (BobguiBookmarksManager *manager,
					 GFile               *file,
					 int                  new_position,
					 GError             **error)
{
...
char *
_bobgui_bookmarks_manager_get_bookmark_label (BobguiBookmarksManager *manager,
					   GFile               *file)
{
  GSList *bookmarks;
  char *label = NULL;
...
gboolean
_bobgui_bookmarks_manager_set_bookmark_label (BobguiBookmarksManager *manager,
					   GFile               *file,
					   const char          *label,
					   GError             **error)
{
...
static gboolean
_bobgui_bookmarks_manager_get_xdg_type (BobguiBookmarksManager *manager,
                                     GFile               *file,
...
gboolean
_bobgui_bookmarks_manager_get_is_builtin (BobguiBookmarksManager *manager,
                                       GFile               *file)
{
  GUserDirectory xdg_type;

  /* if this is not an XDG dir, it's never builtin */
  if (!_bobgui_bookmarks_manager_get_xdg_type (manager, file, &xdg_type))
    return FALSE;

  /* exclude XDG locations we don't display by default */
  return _bobgui_bookmarks_manager_get_is_xdg_dir_builtin (xdg_type);
}

gboolean
_bobgui_bookmarks_manager_get_is_xdg_dir_builtin (GUserDirectory xdg_type)
{
