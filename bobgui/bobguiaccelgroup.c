 ...
 */
gboolean
bobgui_accelerator_valid (guint           keyval,
...
/**
 * bobgui_accelerator_parse_with_keycode:
 ...
 *
 * This is similar to [func@Bobgui.accelerator_parse] but handles keycodes as
 * well. This is only useful for system-level components, applications should
 * use [func@Bobgui.accelerator_parse] instead.
 *
 ...
 */
gboolean
bobgui_accelerator_parse_with_keycode (const char      *accelerator,
                                    GdkDisplay      *display,
 ...
 */
gboolean
bobgui_accelerator_parse (const char      *accelerator,
                       guint           *accelerator_key,
                       GdkModifierType *accelerator_mods)
{
  return bobgui_accelerator_parse_with_keycode (accelerator, NULL, accelerator_key, NULL, accelerator_mods);
}

/**
 * bobgui_accelerator_name_with_keycode:
 ...
 * into a string that can be parsed by [func@Bobgui.accelerator_parse_with_keycode].
 *
 * This is similar to [func@Bobgui.accelerator_name] but handling keycodes.
 * This is only useful for system-level components, applications
 * should use [func@Bobgui.accelerator_name] instead.
 *
 * Returns: a newly allocated accelerator name.
 */
char *
bobgui_accelerator_name_with_keycode (GdkDisplay      *display,
                                   guint            accelerator_key,
                                   guint            keycode,
                                   GdkModifierType  accelerator_mods)
{
  ...
  bobgui_name = bobgui_accelerator_name (accelerator_key, accelerator_mods);

  if (!accelerator_key)
    {
      char *name;
      name = g_strdup_printf ("%s0x%02x", bobgui_name, keycode);
      g_free (bobgui_name);
      return name;
    }
...
/**
 * bobgui_accelerator_name:
 * @accelerator_key: accelerator keyval
 * @accelerator_mods: accelerator modifier mask
 *
 * Converts an accelerator keyval and modifier mask into a string
 * that can be parsed by [func@Bobgui.accelerator_parse].
 *
 * For example, if you pass in `GDK_KEY_q` and `GDK_CONTROL_MASK`,
 * this function returns `<Control>q`.
 *
 * If you need to display accelerators in the user interface,
 * see [func@Bobgui.accelerator_get_label].
 *
 * Returns: (transfer full): a newly-allocated accelerator name
 */
char *
bobgui_accelerator_name (guint           accelerator_key,
                      GdkModifierType accelerator_mods)
{
...
/**
 * bobgui_accelerator_get_label_with_keycode:
 * @display: (nullable): a `GdkDisplay`
 * @accelerator_key: accelerator keyval
 * @keycode: accelerator keycode
 * @accelerator_mods: accelerator modifier mask
 ...
 *
 * This function is similar to [func@Bobgui.accelerator_get_label],
 * but handling keycodes. This is only useful for system-level
 * components, applications should use [func@Bobgui.accelerator_get_label]
 * instead.
 *
 * Returns: (transfer full): a newly-allocated string representing the accelerator
 */
char *
bobgui_accelerator_get_label_with_keycode (GdkDisplay      *display,
                                        guint            accelerator_key,
                                        guint            keycode,
                                        GdkModifierType  accelerator_mods)
{
  ...
  bobgui_label = bobgui_accelerator_get_label (accelerator_key, accelerator_mods);

  if (!accelerator_key)
    {
      char *label;
      label = g_strdup_printf ("%s0x%02x", bobgui_label, keycode);
      g_free (bobgui_label);
      return label;
    }
 ...
 */
char *
bobgui_accelerator_get_label (guint           accelerator_key,
                           GdkModifierType accelerator_mods)
{
  GString *gstring;
  ...
  bobgui_accelerator_print_label (gstring, accelerator_key, accelerator_mods);

  return g_string_free (gstring, FALSE);
}

void
bobgui_accelerator_print_label (GString        *gstring,
                             guint           accelerator_key,
                             GdkModifierType accelerator_mods)
{
...
/**
 * bobgui_accelerator_get_accessible_label:
 * @accelerator_key: accelerator keyval
 * @accelerator_mods: accelerator modifier mask
 *
 * Generates an accessible description of an accelerator.
 *
 * This function is similar to [func@Bobgui.accelerator_get_label] but it is meant
 * for accessibility layers labels rather than user-facing labels. The output
 * of this function is fit for [enum@Bobgui.AccessibleProperty.KEY_SHORTCUTS].
 *
 ...
 */
char *
bobgui_accelerator_get_accessible_label (guint           accelerator_key,
                                      GdkModifierType accelerator_mods)
{
  GString *gstring;
  gboolean seen_mod = FALSE;
...
/**
 * bobgui_accelerator_get_default_mod_mask:
 *
 ...
 */
GdkModifierType
bobgui_accelerator_get_default_mod_mask (void)
{
