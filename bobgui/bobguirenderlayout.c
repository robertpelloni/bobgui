...
void
bobgui_css_style_snapshot_layout (BobguiCssBoxes *boxes,
                               ...
                               PangoLayout *layout)
{
  ...
  bobgui_snapshot_push_debug (snapshot, "Layout");
  ...
  if (!bobgui_css_shadow_value_is_clear (style->used->text_shadow))
    {
      for (guint i = 0; i < bobgui_css_shadow_value_get_n_shadows (style->used->text_shadow); i++)
        {
          graphene_point_t offset;
          GdkColor color;
          double radius;

          bobgui_css_shadow_value_get_offset (style->used->text_shadow, i, &offset);
          bobgui_css_shadow_value_get_color (style->used->text_shadow, i, &color);
          radius = bobgui_css_shadow_value_get_radius (style->used->text_shadow, i);

          bobgui_snapshot_save (snapshot);
          bobgui_snapshot_translate (snapshot, &offset);

          if (radius != 0)
            bobgui_snapshot_push_blur (snapshot, radius);

          if (bobgui_pango_layout_has_color_glyphs (layout))
            {
              GdkColor black = GDK_COLOR_SRGB (0, 0, 0, 1);
              graphene_rect_t bounds;

              get_text_bounds (layout, &bounds);
              bobgui_snapshot_push_mask (snapshot, GSK_MASK_MODE_ALPHA);
              bobgui_snapshot_add_layout (snapshot, layout, &black);
              bobgui_snapshot_pop (snapshot);
              bobgui_snapshot_add_color (snapshot, &color, &bounds);
              bobgui_snapshot_pop (snapshot);
              gdk_color_finish (&black);
            }
          else
            {
              bobgui_snapshot_add_layout (snapshot, layout, &color);
            }
  ...
  bobgui_snapshot_add_layout (snapshot, layout, &text_color);
...
static void
snapshot_insertion_cursor (BobguiSnapshot     *snapshot,
                           BobguiCssStyle     *style,
                           double           width,
                           double           height,
                           double           aspect_ratio,
                           ...
                           gboolean         draw_arrow)
{
  ...
  bobgui_css_color_to_color (is_primary
                            ? bobgui_css_color_value_get_color (style->used->caret_color)
                            : bobgui_css_color_value_get_color (style->used->secondary_caret_color),
                          &color);
...
void
bobgui_css_style_snapshot_caret (BobguiCssBoxes    *boxes,
                              GdkDisplay     *display,
                              BobguiSnapshot    *snapshot,
                              int             x,
                              int             y,
                              ...
                              PangoDirection  direction)
{
  ...
  g_object_get (bobgui_settings_get_for_display (display),
                ...
                "bobgui-cursor-aspect-ratio", &aspect_ratio,
                NULL);
  ...
  bobgui_snapshot_save (snapshot);
  bobgui_snapshot_translate (snapshot, &GRAPHENE_POINT_INIT (x + MIN (cursor1->x, cursor1->x + cursor1->width), y + cursor1->y));
  ...
  if (direction2 != PANGO_DIRECTION_NEUTRAL)
    {
      bobgui_snapshot_save (snapshot);
      bobgui_snapshot_translate (snapshot, &GRAPHENE_POINT_INIT (x + MIN (cursor2->x, cursor2->x + cursor2->width), y + cursor2->y));
      snapshot_insertion_cursor (snapshot,
                                 boxes->style,
                                 cursor2->width,
                                 cursor2->height,
