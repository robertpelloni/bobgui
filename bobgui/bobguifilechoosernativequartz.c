 ...
 * bobguifilechoosernativequartz.c: Quartz Native File selector dialog
...
- (void)popUpButtonSelectionChanged:(id)sender
{
  ...
  GListModel *filters = bobgui_file_chooser_get_filters (BOBGUI_FILE_CHOOSER (data->self));
      ...
      if (data->self->current_filter)
        {
          ...
          if (current_filter_index != BOBGUI_INVALID_LIST_POSITION)
...
static gboolean
file_filter_to_quartz (BobguiFileFilter *file_filter,
                       NSMutableArray *filters,
		       NSMutableArray *filter_names)
{
  const char *name;
  ...
  pattern_nsstrings = _bobgui_file_filter_get_as_pattern_nsstrings (file_filter);
  if (pattern_nsstrings == NULL)
    return FALSE;

  name = bobgui_file_filter_get_name (file_filter);
...
gboolean
bobgui_file_chooser_native_quartz_show (BobguiFileChooserNative *self)
{
  FileChooserQuartzData *data;
  ...
  data->create_folders = bobgui_file_chooser_get_create_folders (BOBGUI_FILE_CHOOSER (self));
  ...
  if (transient_for)
    {
      BobguiNative *native = BOBGUI_NATIVE (transient_for);
...
void
bobgui_file_chooser_native_quartz_hide (BobguiFileChooserNative *self)
{
