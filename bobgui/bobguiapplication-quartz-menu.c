...
- (id)initWithTitle:(NSString *)title model:(GMenuModel *)model observable:(BobguiActionObservable *)observable;

- (id)initWithTitle:(NSString *)title trackerItem:(BobguiMenuTrackerItem *)trackerItem;

@end

@interface NSMenuItem (BobguiMenuTrackerItem)

+ (id)menuItemForTrackerItem:(BobguiMenuTrackerItem *)trackerItem;
...
- (id)initWithTrackerItem:(BobguiMenuTrackerItem *)aTrackerItem
{
  ...
  if (self != nil)
    {
      const char *action_name = bobgui_menu_tracker_item_get_action_name (aTrackerItem);
      const char *special = bobgui_menu_tracker_item_get_special (aTrackerItem);
      ...
      if (bobgui_menu_tracker_item_get_has_link (trackerItem, G_MENU_LINK_SUBMENU))
        {
...
- (void)didChangeLabel
{
  char *label = _bobgui_elide_underscores (bobgui_menu_tracker_item_get_label (trackerItem));
...
{
#if 0
  GIcon *icon = bobgui_menu_tracker_item_get_icon (trackerItem);
  ...
  if (icon != NULL)
    {
      ...
      icon = bobgui_icon_theme_lookup_by_gicon (theme, icon, ICON_SIZE, scale, 0);

      if (icon != NULL)
        {
          cancellable = g_cancellable_new ();
          bobgui_icon_load_symbolic_async (icon, &foreground, &success, &warning, &error,
...
{
  [self setHidden:bobgui_menu_tracker_item_get_is_visible (trackerItem) ? NO : YES];
}

- (void)didChangeToggled
{
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS
  [self setState:bobgui_menu_tracker_item_get_toggled (trackerItem) ? NSOnState : NSOffState];
  G_GNUC_END_IGNORE_DEPRECATIONS
}

- (void)didChangeAccel
{
  const char *accel = bobgui_menu_tracker_item_get_accel (trackerItem);
  ...
  if (accel != NULL)
    {
      ...
      bobgui_accelerator_parse (accel, &key, &mask);
...
- (void)didSelectItem:(id)sender
{
  ...
  BobguiWidget *focus_widget = [self findFocusWidget];
  const char *action_name = bobgui_menu_tracker_item_get_action_name (trackerItem);

  if (focus_widget != NULL && action_name != NULL)
    {
      GVariant *action_target = bobgui_menu_tracker_item_get_action_target (trackerItem);
      bobgui_widget_activate_action_variant (focus_widget, action_name, action_target);
      if (action_target)
        g_variant_unref (action_target);
    }
  else
    bobgui_menu_tracker_item_activated (trackerItem);
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
  /* Mimic macOS' behavior of traversing the reponder chain. */
  BobguiWidget *focus_widget = [self findFocusWidget];
  if (focus_widget != NULL && bobgui_widget_get_sensitive (focus_widget))
    {
      const char *action_name = bobgui_menu_tracker_item_get_action_name (trackerItem);
      gboolean enabled = FALSE;
      BobguiActionMuxer *muxer =  _bobgui_widget_get_action_muxer (focus_widget, FALSE);

      if (action_name == NULL || muxer == NULL)
        return bobgui_menu_tracker_item_get_sensitive (trackerItem) ? YES : NO;

      if (bobgui_action_muxer_query_action (muxer, action_name, &enabled, NULL, NULL, NULL, NULL))
        return enabled ? YES : NO;
    }
  return bobgui_menu_tracker_item_get_sensitive (trackerItem) ? YES : NO;
}

-(BobguiWidget *)findFocusWidget
{
  GApplication *app = g_application_get_default ();
  BobguiWindow *window;

  if (!BOBGUI_IS_APPLICATION (app))
    return NULL;

  window = bobgui_application_get_active_window (BOBGUI_APPLICATION (app));
  if (window != NULL)
    return bobgui_window_get_focus (window);
  return NULL;
}

@end

@implementation NSMenuItem (BobguiMenuTrackerItem)

+ (id)menuItemForTrackerItem:(BobguiMenuTrackerItem *)trackerItem
{
  if (bobgui_menu_tracker_item_get_is_separator (trackerItem))
...
static void
menu_item_inserted (BobguiMenuTrackerItem *item,
                    int                 position,
                    gpointer            user_data)
{
...
- (id)initWithTitle:(NSString *)title model:(GMenuModel *)model observable:(BobguiActionObservable *)observable
{
  ...
  if (self != nil)
    {
      tracker = bobgui_menu_tracker_new (observable,
                                      model,
                                      NO,
                                      YES,
                                      YES,
...
- (id)initWithTitle:(NSString *)title trackerItem:(BobguiMenuTrackerItem *)trackerItem
{
  ...
  if (self != nil)
    {
      tracker = bobgui_menu_tracker_new_for_item_link (trackerItem,
                                                       G_MENU_LINK_SUBMENU,
                                                       YES,
                                                       YES,
                                                       menu_item_inserted,
...
- (void)dealloc
{
  bobgui_menu_tracker_free (tracker);
...
void
bobgui_application_impl_quartz_setup_menu (GMenuModel     *model,
                                        BobguiActionMuxer *muxer)
{
  NSMenu *menu;

  if (model != NULL)
    menu = [[GNSMenu alloc] initWithTitle:@"Main Menu" model:model observable:BOBGUI_ACTION_OBSERVABLE (muxer)];
  else
