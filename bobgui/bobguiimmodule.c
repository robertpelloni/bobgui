...
#include "bobguiimcontextwaylandprivate.h"
#endif

#ifdef GDK_WINDOWING_BROADWAY
#include "broadway/gdkbroadway.h"
#include "bobguiimcontextbroadway.h"
#endif
...
#include "bobguiimcontextquartz.h"
#endif

#ifdef GDK_WINDOWING_ANDROID
#include "bobguiimcontextandroid.h"
#endif
...
#define SIMPLE_ID     "bobgui-im-context-simple"
#define ALT_SIMPLE_ID "simple"
#define NONE_ID       "bobgui-im-context-none"
#define ALT_NONE_ID   "none"

/**
 * _bobgui_im_module_create:
 ...
 *   if that could not be created, a newly created `BobguiIMContextSimple`
 */
BobguiIMContext *
_bobgui_im_module_create (const char *context_id)
{
  GIOExtensionPoint *ep;
  GIOExtension *ext;
  GType type;
  BobguiIMContext *context = NULL;

  if (strcmp (context_id, NONE_ID) == 0)
    return NULL;

  ep = g_io_extension_point_lookup (BOBGUI_IM_MODULE_EXTENSION_POINT_NAME);
...
/**
 * _bobgui_im_module_get_default_context_id:
 ...
 */
const char *
_bobgui_im_module_get_default_context_id (GdkDisplay *display)
{
  ...
  envvar = g_getenv ("BOBGUI_IM_MODULE");
  if (envvar)
    {
      char **immodules;
      ...
      if (context_id)
      ...
      else
        {
          ...
          if (!warned)
            {
              g_warning ("No IM module matching BOBGUI_IM_MODULE=%s found", envvar);
              warned = TRUE;
            }
        }
    }
  ...
  g_object_get (G_OBJECT (settings), "bobgui-im-module", &tmp, NULL);
  if (tmp)
    {
      char **immodules;
  ...
  g_error ("BOBGUI was run without any IM module being present. This must not happen.");

  return SIMPLE_ID;
}

void
bobgui_im_module_ensure_extension_point (void)
{
  GIOExtensionPoint *ep;
  ...
  BOBGUI_DEBUG (MODULES, "Registering extension point %s", BOBGUI_IM_MODULE_EXTENSION_POINT_NAME);

  ep = g_io_extension_point_register (BOBGUI_IM_MODULE_EXTENSION_POINT_NAME);
  g_io_extension_point_set_required_type (ep, BOBGUI_TYPE_IM_CONTEXT);

  registered = TRUE;
}

void
bobgui_im_modules_init (void)
{
  ...
  g_type_ensure (bobgui_im_context_simple_get_type ());
#ifdef GDK_WINDOWING_WAYLAND
  g_type_ensure (bobgui_im_context_wayland_get_type ());
#endif
#ifdef GDK_WINDOWING_BROADWAY
  g_type_ensure (bobgui_im_context_broadway_get_type ());
#endif
#ifdef GDK_WINDOWING_WIN32
  g_type_ensure (bobgui_im_context_ime_get_type ());
#endif
#ifdef GDK_WINDOWING_MACOS
  g_type_ensure (bobgui_im_context_quartz_get_type ());
#endif
#ifdef GDK_WINDOWING_ANDROID
  g_type_ensure (bobgui_im_context_android_get_type ());
#endif

  scope = g_io_module_scope_new (G_IO_MODULE_SCOPE_BLOCK_DUPLICATES);

  paths = _bobgui_get_module_path ("immodules");
  for (i = 0; paths[i]; i++)
    {
