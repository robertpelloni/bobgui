                 ...
                 gpointer      user_data)
{
  ...
  if (bobgui_show_uri_win32_finish (BOBGUI_WINDOW (source), result, &error))
...
static gboolean
bobgui_show_uri_android (const gchar         *uri,
                      ...
                      GError             **error)
{
  ...
  //       gdk_android_toplevel_get_bars_color which returns the default BOBGUI light/dark background color,
...
static void
bobgui_show_uri_data_free (BobguiShowUriData *data)
{
  if (data->parent && data->handle)
    bobgui_window_unexport_handle (data->parent, data->handle);
...
launch_uri_done (GObject      *source,
                 ...
                 gpointer      user_data)
{
  BobguiShowUriData *data = user_data;
  GError *error = NULL;
  ...
  bobgui_show_uri_data_free (data);
}
 ...
 *
 * Deprecated: 4.10: Use [method@Bobgui.FileLauncher.launch] or
 *   [method@Bobgui.UriLauncher.launch] instead
 */
void
bobgui_show_uri_full (BobguiWindow           *parent,
                   const char          *uri,
                   guint32              timestamp,
                   ...
                   gpointer             user_data)
{
  ...
  g_task_set_source_tag (task, bobgui_show_uri_full);
  bobgui_show_uri_win32 (parent, uri, FALSE, cancellable, show_win32_done, task);
  ...
  GError *err = NULL;
  if (bobgui_show_uri_android (uri, toplevel, &err))
    g_task_return_boolean (task, TRUE);
  else
    g_task_return_error (task, err);
  g_object_unref (task);
#else
  BobguiShowUriData *data;
  ...
  data = g_new0 (BobguiShowUriData, 1);
  ...
  g_task_set_source_tag (data->task, bobgui_show_uri_full);

  if (!parent || !bobgui_window_export_handle (parent, window_handle_exported, data))
    window_handle_exported (parent, NULL, data);
#endif
}

/**
 * bobgui_show_uri_full_finish:
 * @parent: the `BobguiWindow` passed to bobgui_show_uri()
 * @result: `GAsyncResult` that was passed to @callback
 * @error: return location for an error
 *
 * Finishes the bobgui_show_uri() call and returns the result
 * of the operation.
 *
 ...
 */
gboolean
bobgui_show_uri_full_finish (BobguiWindow     *parent,
                          GAsyncResult  *result,
                          GError       **error)
{
  g_return_val_if_fail (parent == NULL || BOBGUI_IS_WINDOW (parent), FALSE);
  g_return_val_if_fail (g_task_is_valid (result, parent), FALSE);
  g_return_val_if_fail (g_task_get_source_tag (G_TASK (result)) == bobgui_show_uri_full, FALSE);
...
show_uri_done (GObject      *object,
               ...
               gpointer      data)
{
  BobguiWindow *parent = BOBGUI_WINDOW (object);
  GError *error = NULL;
  ...
  if (!bobgui_show_uri_full_finish (parent, result, &error))
    {
      ...
      dialog = bobgui_alert_dialog_new ("%s", _("Could not show link"));
 ...
 */
void
bobgui_show_uri (BobguiWindow  *parent,
              const char *uri,
              guint32     timestamp)
{
  bobgui_show_uri_full (parent, uri, timestamp, NULL, show_uri_done, NULL);
}
