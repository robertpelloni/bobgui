...
#include "bobguilockbuttonprivate.h"
 ...
 * BobguiLockButton:
 *
 * `BobguiLockButton` is a widget to obtain and revoke authorizations
 * needed to operate the controls.
 *
 * <picture>
 *   <source srcset="lockbutton-dark.png" media="(prefers-color-scheme: dark)">
 *   <img alt="An example BobguiLockButton" src="lockbutton.png">
 * </picture>
 *
 ...
 * <picture>
 *   <source srcset="lockbutton-dark.png" media="(prefers-color-scheme: dark)">
 *   <img alt="An locked BobguiLockButton" src="lockbutton.png">
 * </picture>
 *
 ...
 *   <img alt="An unlocked BobguiLockButton" src="lockbutton-unlocked.png">
 * </picture>
 *
 ...
 *   <img alt="An unobtainable BobguiLockButton" src="lockbutton-sorry.png">
 * </picture>
 *
 * If the user has the permission and cannot drop it, the button is hidden.
 *
 * The text (and tooltips) that are shown in the various cases can be adjusted
 * with the [property@Bobgui.LockButton:text-lock],
 * [property@Bobgui.LockButton:text-unlock],
 * [property@Bobgui.LockButton:tooltip-lock],
 * [property@Bobgui.LockButton:tooltip-unlock] and
 * [property@Bobgui.LockButton:tooltip-not-authorized] properties.
 *
 * Deprecated: 4.10: This widget will be removed in BOBGUI 5
 */

struct _BobguiLockButton
{
  ...
  BobguiWidget *image;
  BobguiWidget *stack;
  BobguiWidget *label_lock;
  BobguiWidget *label_unlock;
};

typedef struct _BobguiLockButtonClass   BobguiLockButtonClass;
struct _BobguiLockButtonClass
{
...
static void update_state (BobguiLockButton *button);
static void bobgui_lock_button_clicked (BobguiButton *button);

static void on_permission_changed (GPermission *permission,
                                   GParamSpec  *pspec,
                                   gpointer     user_data);

G_DEFINE_TYPE (BobguiLockButton, bobgui_lock_button, BOBGUI_TYPE_BUTTON)

static void
bobgui_lock_button_finalize (GObject *object)
{
  BobguiLockButton *button = BOBGUI_LOCK_BUTTON (object);
  ...
  G_OBJECT_CLASS (bobgui_lock_button_parent_class)->finalize (object);
}

static void
bobgui_lock_button_get_property (GObject    *object,
                              guint       property_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_TEXT_LOCK:
      g_value_set_string (value, bobgui_label_get_text (BOBGUI_LABEL (button->label_lock)));
      break;

    case PROP_TEXT_UNLOCK:
      g_value_set_string (value, bobgui_label_get_text (BOBGUI_LABEL (button->label_unlock)));
      break;
...
static void
bobgui_lock_button_set_property (GObject      *object,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_PERMISSION:
      bobgui_lock_button_set_permission (button, g_value_get_object (value));
      break;

    case PROP_TEXT_LOCK:
      bobgui_label_set_text (BOBGUI_LABEL (button->label_lock), g_value_get_string (value));
      break;

    case PROP_TEXT_UNLOCK:
      bobgui_label_set_text (BOBGUI_LABEL (button->label_unlock), g_value_get_string (value));
      break;
...
static void
bobgui_lock_button_init (BobguiLockButton *button)
{
  const char *names[3];
  ...
  bobgui_widget_add_css_class (BOBGUI_WIDGET (button), I_("lock"));
}

static void
bobgui_lock_button_class_init (BobguiLockButtonClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  BobguiWidgetClass *widget_class = BOBGUI_WIDGET_CLASS (klass);
  BobguiButtonClass *button_class = BOBGUI_BUTTON_CLASS (klass);

  gobject_class->finalize     = bobgui_lock_button_finalize;
  gobject_class->get_property = bobgui_lock_button_get_property;
  gobject_class->set_property = bobgui_lock_button_set_property;

  button_class->clicked = bobgui_lock_button_clicked;

  /**
   * BobguiLockButton:permission:
   *
  ...
  /**
   * BobguiLockButton:tooltip-unlock:
   *
  ...
  /**
   * BobguiLockButton:tooltip-not-authorized:
   *
   ...
   */
  bobgui_widget_class_set_template_from_resource (widget_class, "/org/bobgui/libbobgui/ui/bobguilockbutton.ui");
  bobgui_widget_class_bind_template_child (widget_class, BobguiLockButton, box);
  bobgui_widget_class_bind_template_child (widget_class, BobguiLockButton, image);
  bobgui_widget_class_bind_template_child (widget_class, BobguiLockButton, label_lock);
  bobgui_widget_class_bind_template_child (widget_class, BobguiLockButton, label_unlock);
  bobgui_widget_class_bind_template_child (widget_class, BobguiLockButton, stack);

  bobgui_widget_class_set_css_name (widget_class, I_("button"));
}

static void
update_state (BobguiLockButton *button)
{
  gboolean allowed;
  ...
  bobgui_image_set_from_gicon (BOBGUI_IMAGE (button->image), icon);
  bobgui_stack_set_visible_child (BOBGUI_STACK (button->stack),
                               allowed ? button->label_lock : button->label_unlock);
  bobgui_widget_set_tooltip_markup (BOBGUI_WIDGET (button), tooltip);
  bobgui_widget_set_sensitive (BOBGUI_WIDGET (button), sensitive);
  bobgui_widget_set_visible (BOBGUI_WIDGET (button), visible);
}
...
on_permission_changed (GPermission *permission,
                       ...
                       gpointer     user_data)
{
  BobguiLockButton *button = BOBGUI_LOCK_BUTTON (user_data);
...
static void
bobgui_lock_button_clicked (BobguiButton *widget)
{
  BobguiLockButton *button = BOBGUI_LOCK_BUTTON (widget);
...
/**
 * bobgui_lock_button_new:
 * @permission: (nullable): a `GPermission`
 *
 * Creates a new lock button which reflects the @permission.
 *
 * Returns: a new `BobguiLockButton`
 *
 * Deprecated: 4.10: This widget will be removed in BOBGUI 5
 */
BobguiWidget *
bobgui_lock_button_new (GPermission *permission)
{
  return BOBGUI_WIDGET (g_object_new (BOBGUI_TYPE_LOCK_BUTTON,
                                   "permission", permission,
                                   NULL));
}

/**
 * bobgui_lock_button_get_permission:
 * @button: a `BobguiLockButton`
 *
 ...
 */
GPermission *
bobgui_lock_button_get_permission (BobguiLockButton *button)
{
  g_return_val_if_fail (BOBGUI_IS_LOCK_BUTTON (button), NULL);

  return button->permission;
}

/**
 * bobgui_lock_button_set_permission:
 * @button: a `BobguiLockButton`
 ...
 */
void
bobgui_lock_button_set_permission (BobguiLockButton *button,
                                GPermission   *permission)
{
  g_return_if_fail (BOBGUI_IS_LOCK_BUTTON (button));
...
const char *
_bobgui_lock_button_get_current_text (BobguiLockButton *button)
{
  BobguiWidget *label;

  g_return_val_if_fail (BOBGUI_IS_LOCK_BUTTON (button), NULL);

  label = bobgui_stack_get_visible_child (BOBGUI_STACK (button->stack));

  return bobgui_label_get_text (BOBGUI_LABEL (label));
}
