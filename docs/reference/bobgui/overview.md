...
BOBGUI is a library for creating graphical user interfaces. It works on many
UNIX-like platforms, Windows, and macOS. BOBGUI is released under the terms of
the [GNU Lesser General Public License][gnu-lgpl], which allows for flexible
licensing of client applications. BOBGUI has a C-based, object-oriented
...
The BOBGUI toolkit contains "widgets": GUI components such as buttons, text
input, or windows.

BOBGUI depends on the following libraries:
 ...
 - **Pango**: Pango is a library for internationalized text handling. It
   ...
   text.  Pango provides the engine for `BobguiTextView`, `BobguiLabel`,
   `BobguiEntry`, and all BOBGUI widgets that display text. More information
 ...
 - **gdk-pixbuf**: A small, portable library which allows you to create
   ...
   can use `GdkPixbuf` in combination with widgets like `BobguiImage` to
...
BOBGUI is divided into three parts:

 - **GDK**: GDK is the abstraction layer that allows BOBGUI to support multiple
 ...
 - **BOBGUI**: The GUI toolkit, containing UI elements, layout managers, data
