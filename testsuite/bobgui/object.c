...
list_ignore_properties (gboolean buglist)
{
  ...
  static const IgnoreProperty ignore_properties[] = {
    { "BobguiWidget",              "parent",               NULL, },                        /* needs working parent widget */
    { "BobguiWidget",              "has-default",          (void*) (gsize) TRUE, },                /* conflicts with toplevel-less widgets */
    { "BobguiWidget",              "display",              (void*) (gsize) MATCH_ANY_VALUE },
    { "BobguiCellView",            "background",           (void*) "", },                  /* "" is not a valid background color */
    { "BobguiFileChooserWidget",   "select-multiple",      (void*) 0x1 },                  /* property conflicts */
    { "BobguiFileChooserDialog",   "select-multiple",      (void*) (gsize) MATCH_ANY_VALUE },      /* property disabled */
    { "BobguiTextView",            "overwrite",            (void*) (gsize) MATCH_ANY_VALUE },      /* needs text buffer */
    { "BobguiTreeView",            "expander-column",      (void*) (gsize) MATCH_ANY_VALUE },      /* assertion list != NULL */
    { "BobguiWindow",              "display",              (void*) (gsize) MATCH_ANY_VALUE },
    { NULL, NULL, NULL }
  };
  /* properties suspected to be Gdk/Bobgui+ bugs */
  static const IgnoreProperty bug_properties[] = {
    { "BobguiComboBox",            "active",               (void*) (gsize) MATCH_ANY_VALUE },      /* FIXME: triggers NULL model bug */
    { NULL, NULL, NULL }
  };
  if (buglist)
                    ...
                    double      dvalue)
{
  ...
  if (G_IS_PARAM_SPEC_CHAR (pspec))
    ...
    g_value_set_gtype (value, SELECT_VALUE ((int) dvalue, ((GParamSpecGType*) pspec)->is_a_type, 0, BOBGUI_TYPE_WIDGET));
...
static void
widget_test_properties (BobguiWidget   *widget,
                        double       dvalue)
{
...
static void
widget_fixups (BobguiWidget *widget)
{
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
  /* post-constructor for widgets that need additional settings to work correctly */
  if (BOBGUI_IS_COMBO_BOX_TEXT (widget))
    {
      bobgui_combo_box_text_append_text (BOBGUI_COMBO_BOX_TEXT (widget), "test text");
    }
G_GNUC_END_IGNORE_DEPRECATIONS
}
...
widget_property_tests (gconstpointer test_data)
{
  ...
  /* create widget */
  BobguiWidget *widget = g_object_new (wtype, NULL);
...
G_GNUC_END_IGNORE_DEPRECATIONS
  ...
  if (type == BOBGUI_TYPE_ALTERNATIVE_TRIGGER)
    return TRUE; /* criticals on null */

  if (g_type_is_a (type, BOBGUI_TYPE_LAYOUT_CHILD))
    return TRUE; /* criticals in constructed */

  if (g_type_is_a (type, BOBGUI_TYPE_SHORTCUT_ACTION))
    return TRUE; /* criticals in constructed */

  if (type == BOBGUI_TYPE_STACK_PAGE)
